...
and introduce a generalised off-line training method using the \DIFdelbegin \DIFdel{parametric activation function}\DIFdelend \DIFaddbegin \DIFadd{Parametric Activation Function}\DIFaddend ~(PAF) to map the abstract numerical values of the ANN to concrete physical units, such as current and firing rate in the SNN.
...
\DIFaddbegin \DIFadd{Furthermore, we put forward solutions to reduce correlations between spike trains; thereby addressing the result of performance drop in on-line SNN training. 
}\DIFaddend The promising results of \DIFdelbegin \DIFdel{training }\DIFdelend spiking Autoencoders~(AEs) and Restricted Boltzmann Machines~(\DIFdelbegin \DIFdel{RBMs}\DIFdelend \DIFaddbegin \DIFadd{SRBMs}\DIFaddend ) exhibit equivalent, sometimes even superior, classification and reconstruction capabilities compared to their \DIFdelbegin \DIFdel{equivalent }\DIFdelend non-spiking \DIFdelbegin \DIFdel{methods}\DIFdelend \DIFaddbegin \DIFadd{counterparts}\DIFaddend .
