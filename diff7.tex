...
The simple off-line SNN training method has enabled and encouraged interesting applications to run in SNNs.
 ...
 \begin{itemize} 
	...
	\DIFaddbegin \DIFadd{We used spike count per frame per frequency channel as a pixel in a spectrum image, and trained CNNs based on these spectrum images.
	The trained weights were transferred directly to an equivalent convolutional SNN.
	}\DIFaddend The next step is to implement the model entirely on neuromorphic hardware including the cochlea and the SNN, thus to run it in real time.
