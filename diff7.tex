...
It involves }\DIFdelend \DIFaddbegin \DIFadd{between the cognitive capabilities of Spiking Neural Networks~(SNNs) and Artificial Neural Networks~(ANNs) on Artificial Intelligence~(AI) tasks.
...
\DIFadd{On one hand, this research contributes to the NE community a simple }\DIFaddend off-line \DIFdelbegin \DIFdel{deep }\DIFdelend SNN training method\DIFdelbegin \DIFdel{and a novel activation function, Noisy Softplus, and }\DIFdelend \DIFaddbegin \DIFadd{.
...
On the other hand, }\DIFaddend the \DIFdelbegin \DIFdel{achievements of successful training on an deep spiking ConvNet which has performed close to original ANN 's even surpassing the state-of-the-art classification accuracy, we confirm the hypothesis. 
}\DIFdelend \DIFaddbegin \DIFadd{method is generalised in terms of spiking neural models and hardware platforms, since it works on standard LIF neurons which are supported by most neuromorphic hardware.
...
\DIFdel{the calculation on Noisy Softplus and its derivative is no more than Softplus function, except for doubled computation on weighted sum of its input ($net$ and $\sigma$ in Equations~\ref{equ:mi_input} and ~\ref{equ:si_input}).
		They are yet much more simplified than Siergert function which saves training time and energy}\DIFdelend \DIFaddbegin \DIFadd{Among the existing approaches, this method is the first and only one that unifies the representation of neurons in ANNs and the spiking ones in SNNs using abstract activation functions, while some other approaches~\mbox{%DIFAUXCMD
...
, but also includes coloured noise where the Siegert formula~\mbox{%DIFAUXCMD
\citep{Jug_etal_2012}
}%DIFAUXCMD
only works on white noise.
Regarding simplicity, this training method performs effectively using Rectified Linear Units~(ReLUs), however, other studies~\mbox{%DIFAUXCMD
\citep{Jug_etal_2012,hunsberger2015spiking}
}%DIFAUXCMD
...
require an extra step to adapt the trained weights to the SNN.
...
These training methods have realised the on-line}\DIFdelend \DIFaddbegin \DIFadd{Spiking AEs and RBMs can be trained with SRM by sharing the synaptic weights between $ab$ and $cd$, and applying symmetric learning rates $\pm \eta_s$ on the weight changes respectively.
...
The current limitation prohibiting \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend off-line SNN training method from wide use lies in the lack of supporting tools.
...
Besides, we will look into the networks with feedback connections, the Recurrent Neural Networks~(RNNs), thus to adapt the off-line training method to most network architectures.
...
\DIFadd{The plan of developing software and libraries in the near future includes:
}
...
\DIFdelbegin \DIFdel{An other issue is the parameter calibration on the scaling factor of the combined activation, thus numerical analysis is considered for future work to express the factors with biological parameters of a LIF neuron}\DIFdelend \DIFaddbegin \paragraph{\DIFadd{Recurrent Neural Networks.}}
...
have been trained with NSP neurons and will be transferred onto the SpiNNaker system~\mbox{%DIFAUXCMD
...
}\paragraph{\DIFadd{Beyond rate coding.}}
...
\DIFadd{Since new problems will continue to arise before vision becomes a solved question, the NE dataset will evolve as research progresses. 
...
\DIFadd{Energy-efficient neuromorphic hardware platforms have been successfully used to support large-scale simulations of }\DIFaddend biologically-plausible \DIFdelbegin \DIFdel{spiking units.
}\DIFdelend \DIFaddbegin \DIFadd{SNNs for brain understanding, but they are still far from intelligent enough to achieve Neuromorphic Cognition.
...
\DIFadd{Last but not least, this work contributes to the NE community with a uniform dataset to evaluate SNNs' performance at both model and hardware level.
