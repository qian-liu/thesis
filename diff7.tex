...
Hence, AI engineers are capable of implementing their ANN models on NE platforms without knowledge of SNN or hardware-specific programming, \DIFdelbegin \DIFdel{and furthermore get benefit }\DIFdelend \DIFaddbegin \DIFadd{thereby benefiting }\DIFaddend from neuromorphic hardware in many ways: such as energy-efficiency, biological realism, low latency and real-time processing.
...
\paragraph{2. Unsupervised Deep Learning modules can be trained on-line on SNNs with biologically-plausible synaptic plasticity to demonstrate a learning capability \DIFdelbegin \DIFdel{as competent as }\DIFdelend \DIFaddbegin \DIFadd{equivalent to }\DIFaddend ANNs.\\}
...
Thanks to the \DIFdelbegin \DIFdel{formalisation of these proposed learning algorithms with numerical analysis}\DIFdelend \DIFaddbegin \DIFadd{accurate parameter estimation}\DIFaddend , the classification results (94.72\% for SAE and 94.35\% for SRBM) have outperformed \DIFdelbegin \DIFdel{other existing SAE and SRBM models.
The first on-line training algorithm proposed by~}\DIFdelend \DIFaddbegin \DIFadd{most existing on-line models.
}\DIFaddend \citet{neil2013online} ignored the mathematical analysis thus achieved its best \DIFdelbegin \DIFdel{classification }\DIFdelend performance only at 81.5\%.
\citet{neftci2013event} conducted training on a recurrent network which was more biologically plausible\DIFdelbegin \DIFdel{but required a global signal to control the direction of the synapses and resulted in }\DIFdelend \DIFaddbegin \DIFadd{, but with a different goal of neural sampling this method achieved }\DIFaddend a worse classification \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{at }\DIFaddend 91.9\%\DIFdelbegin \DIFdel{even with a bigger network.
...
Meanwhile, other }\DIFdelend \DIFaddbegin \DIFadd{.
Their extended work~\mbox{%DIFAUXCMD
\citep{neftci2016stochastic}
}%DIFAUXCMD
improved the performance to the state-of-the-art, 95.6}\%\DIFadd{, but requires extra Deep Learning technique, dropout~\mbox{%DIFAUXCMD
\citep{srivastava2014dropout}
}%DIFAUXCMD
, and more training epochs.
Nevertheless, \mbox{%DIFAUXCMD
\citet{neftci2016stochastic}
}%DIFAUXCMD
did not compare the trained SNN to the non-spiking counterpart, RBM-dropout; while our method closely reproduced the learning curves of the equivalent ANNs.
...
Therefore, this method enables the genuine `live and learn' in }\DIFaddend on-line \DIFdelbegin \DIFdel{training methods~\mbox{%DIFAUXCMD
\citep{neftci2016stochastic,neftci2017event}
}%DIFAUXCMD
suffer from the manual termination of the learning.
}\DIFdelend \DIFaddbegin \DIFadd{SNN training.
%DIF > meanwhile, other methods~\citep{neftci2016stochastic,neftci2017event} suffer from the manual termination of learning when the performance reaches the best.
}\DIFaddend Furthermore, in theory our method works on any spiking neuron \DIFaddbegin \DIFadd{model }\DIFaddend and requires a simple rectangular STDP, thus can be easily implemented on general neuromorphic hardware.
...
In Chapter~\ref{cha:bench} we presented a dataset \DIFdelbegin \DIFdel{which contains }\DIFdelend \DIFaddbegin \DIFadd{containing }\DIFaddend four different spike representations of the MNIST stationary hand-written digit database. % rate-based code, rank-order code, DVS recorded flashing and moving inputs. 
The Poissonian subset is intended for benchmarking existing rate-based recognition methods.
...
Fast recognition can be verified on the DVS recorded flashing input subset, since just 30~ms of useful spike trains are recorded for each image.
\DIFdelbegin \DIFdel{Looking forward, the continuous spike trains captured from the }\DIFdelend \DIFaddbegin \DIFadd{Another }\DIFaddend DVS recorded moving input can be used to test mobile neuromorphic robots.
\citet{orchard2015convert} presented a neuromorphic dataset using a similar approach \DIFaddbegin \DIFadd{to capture moving input}\DIFaddend , but the spike trains were obtained with micro-saccades.
...
%DIFDELCMD < %%%
\paragraph{\DIFdel{Supporting software tools.}}
%DIFAUXCMD
...
This requires the development of a set of software and libraries\DIFaddbegin \DIFadd{.
...
%DIF > The current limitation prohibiting the off-line SNN training method from wide use lies in the lack of supporting tools.
\DIFadd{The plan of developing software and libraries in the near future includes}\DIFaddend :
...
%	Furthermore, numerical analysis is considered to present coloured noise introduced by 1~ms time resolution and the synaptic time constant $\tau_{syn}$, refer to Section~\ref{sec:response_func}, thus to model $p$ with biological parameters of a LIF neuron instead of parameter calibration.
	...
	\item supporting libraries for SNN training in popular Deep Learning platforms, e.g. Tensorflow~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{tensorflow2015-whitepaper}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{abadi2016tensorflow}
}%DIFAUXCMD
}\DIFaddend , which will include the proposed activation functions NSP and PAF in the ANN models.
...
\paragraph{Recurrent Neural Networks\DIFdelbegin \DIFdel{~(RNNs)}\DIFdelend .}
...
\begin{itemize}
	...
	\item Speech recognition of simulated \DIFdelbegin \DIFdel{cochlea generated spikes }\DIFdelend \DIFaddbegin \DIFadd{spikes from a cochlea model }\DIFaddend has achieved a promising accuracy at the initial test-idea stage.
...
\DIFaddbegin \DIFadd{The proposed on-line learning method is limited to rate coding and greedy layer-wise training.
...
}\DIFaddend \paragraph{Beyond rate coding.}
...
The STDP rule usually works locally with a teaching signal in \DIFdelbegin \DIFdel{cognitive tasks}\DIFdelend \DIFaddbegin \DIFadd{supervised learning}\DIFaddend , however, error backpropagation~(BP) does not provide the teaching targets for all the hidden units of a network.
...
Initial work~\citep{samadi2017deep,neftci2017event} has shown that these random feedback weights work effectively to replace precise BP\DIFaddbegin \DIFadd{, but endures complex neuron/synaptic models and network architectures}\DIFaddend .
In the future, we will continue the investigation \DIFdelbegin \DIFdel{of the area of }\DIFdelend \DIFaddbegin \DIFadd{in BP alternatives within }\DIFaddend on-line \DIFdelbegin \DIFdel{training on deep SNNs}\DIFdelend \DIFaddbegin \DIFadd{SNN training}\DIFaddend .
...
\paragraph{\DIFaddbegin \DIFadd{Genuine Learning on }\DIFaddend Neuromorphic Hardware.}
...
\DIFaddbegin \DIFadd{Since new problems will continue to arise before vision becomes a solved problem, the NE dataset will evolve as research progresses. 
...
The initial plan is to use the DVS retina to convert the KTH\DIFdelbegin \DIFdel{and Weizmann }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep{schuldt2004recognizing}
}%DIFAUXCMD
and Weizmann~\mbox{%DIFAUXCMD
\citep{blank2005actions}
}%DIFAUXCMD
}\DIFaddend benchmarks to spiking versions.
Meanwhile, \DIFdelbegin \DIFdel{providing a }\DIFdelend \DIFaddbegin \DIFadd{using the proposed }\DIFaddend software DVS retina\DIFdelbegin \DIFdel{simulator }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep{7850249}
}%DIFAUXCMD
}\DIFaddend to transform frames into spike trains is also on the schedule.
...
The success of the generalised off-line method paves the way to energy-efficient AI \DIFdelbegin \DIFdel{on mobile devices and }\DIFdelend \DIFaddbegin \DIFadd{from mobile devices to }\DIFaddend huge computer clusters.
...
The proposed method tackles the problem of accurately translating the weight tuning \DIFdelbegin \DIFdel{of AEs and RBMs }\DIFdelend from numerical computations to event-based, biologically-plausible STDP rules in SNNs\DIFdelbegin \DIFdel{.
...
In doing so, it }\DIFaddend directs the future work to formalise event-based learning to closely mimic the numerical computations in conventional ANNs.
