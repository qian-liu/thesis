...
This proposed SNN training method is simpler and even more straight-forward than the other ANN-based training approach~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{cao2015spiking,diehl2015fast}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{cao2015spiking,diehl2015fast}
}%DIFAUXCMD
}\DIFaddend which requires an extra step of converting ANN-trained weights \DIFdelbegin \DIFdel{to SNN's}\DIFdelend \DIFaddbegin \DIFadd{for use at the SNN}\DIFaddend .
In addition, the normalisation algorithm~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{diehl2015fast}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{diehl2015fast}
}%DIFAUXCMD
}\DIFaddend , proposed for \DIFdelbegin \DIFdel{weights }\DIFdelend \DIFaddbegin \DIFadd{weight }\DIFaddend transformation, only works for simple integrate-and-fire neurons, and cannot be generalised for biologically-plausible neurons.
Moreover, the novel activation function, Noisy Softplus, tackles all the problems raised by the Siegert formula which \DIFaddbegin \DIFadd{has been }\DIFaddend used to model sigmoid-like spiking neurons~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Jug_etal_2012}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Jug_etal_2012}
...
%		\item easily applied to any training method, for example BP, thanks to its derivative function defined in Equation~\ref{equ:logist}.
		...
		\item as one of the ReLU-liked activation functions, the output firing rate seldom exceeds the working range of \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend LIF neuron, for example the firing rates were around 0-200~Hz in the ConvNet model.
...
\DIFdelbegin \DIFdel{This SRM method successfully transfers multiplication operations to possibility of some specific firing events of }\DIFdelend \DIFaddbegin \DIFadd{represents the product of numerical values used in these unsupervised Deep Learning techniques with rate multiplication.
...
Moreover, these firing }\DIFdelend \DIFaddbegin \DIFadd{, and the simultaneous }\DIFaddend events can be \DIFdelbegin \DIFdel{captured with the STDP rule and thus to update the weights accordingly in an on-line, event-based, and }\DIFdelend \DIFaddbegin \DIFadd{obtained by the }\DIFaddend biologically-plausible \DIFdelbegin \DIFdel{manner.
Such weight updates suit the conventional unsupervised deep learning modules, such as Autoencoders~(AE) and Restricted Boltzmann Machines~(RBM), where multiplication of the neural outputs is the main operation}\DIFdelend \DIFaddbegin \DIFadd{learning rule: Spike-Time-Dependant-Plasticity~(STDP)}\DIFaddend .
...
%DIF < \cite{orchard2015convert} have presented a neuromorphic dataset using a similar approach, but the spike trains were obtained with micro-saccades.
%DIF > \citep{orchard2015convert} have presented a neuromorphic dataset using a similar approach, but the spike trains were obtained with micro-saccades.
...
%DIF < In accordance with the suggestions from~\cite{tan2015bench}, the evaluation metrics highlight the strengths of spike-based vision tasks and the dataset design also promotes the research into rapid and low energy recognition (e.g. flashing digits).
%DIF > In accordance with the suggestions from~\citep{tan2015bench}, the evaluation metrics highlight the strengths of spike-based vision tasks and the dataset design also promotes the research into rapid and low energy recognition (e.g. flashing digits).
...
This requires the development of a supporting tool which enables SNN training in popular \DIFdelbegin \DIFdel{deep learning }\DIFdelend \DIFaddbegin \DIFadd{Deep Learning }\DIFaddend platforms e.g. Tensorflow~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{tensorflow2015-whitepaper}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{tensorflow2015-whitepaper}
}%DIFAUXCMD
}\DIFaddend , and an other automation tool that reads platform-dependant trained weights into \DIFdelbegin \DIFdel{PyNN~\mbox{%DIFAUXCMD
\cite{davison2008pynn}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{the PyNN~\mbox{%DIFAUXCMD
...
Ensemble models~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{rokach2010ensemble}
}%DIFAUXCMD
have been considered }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{rokach2010ensemble}
}%DIFAUXCMD
have been developed }\DIFaddend to train by this method and run their SNN models on SpiNNaker to take advantage of \DIFdelbegin \DIFdel{itss massive-parallel simulating }\DIFdelend \DIFaddbegin \DIFadd{its massively-parallel simulation }\DIFaddend ability.
Speech recognition of cochlea generated spikes has achieved a promising accuracy at the initial test-idea stage.
A further goal is to implement deep networks fit for ImageNet~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{deng2009imagenet}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{deng2009imagenet}
}%DIFAUXCMD
}\DIFaddend tasks, which will also \DIFdelbegin \DIFdel{requires }\DIFdelend \DIFaddbegin \DIFadd{require }\DIFaddend modelling various structures of \DIFdelbegin \DIFdel{deep learning}\DIFdelend \DIFaddbegin \DIFadd{Deep Learning}\DIFaddend , for example recurrent neural networks.
...
The modulation of STDP by a third factor such as dopamine has potentially interesting functional consequences that turn STDP from unsupervised learning into a reward-based learning paradigm~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{izhikevich2007solving}
}%DIFAUXCMD
.
It consists to  the hot research field of reinforcement learning}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{izhikevich2007solving}
}%DIFAUXCMD
which address to  Reinforcement Learning}\DIFaddend .
...
Synaptic Sampling Machines (S2Ms)~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{neftci2016stochastic}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{neftci2016stochastic}
}%DIFAUXCMD
}\DIFaddend employing a dropout~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{srivastava2014dropout}
}%DIFAUXCMD
mechanism }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{srivastava2014dropout}
}%DIFAUXCMD
mechanism which }\DIFaddend hugely improved the performance of its original model from 91.9\% to 95.6\%.
...
It is possible to measure the scan path or trajectory of the eyeball and those trajectories show particular interest in eyes, nose and mouth while viewing a human face~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{yarbus1967eye}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{yarbus1967eye}
...
To explore invariant object recognition, the dataset will include the NORB (NYU Object Recognition Benchmark) dataset~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{lecun2004learning}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{lecun2004learning}
}%DIFAUXCMD
}\DIFaddend , which contains images of objects that are first photographed in ideal conditions and then moved and placed in front of natural scene images.
