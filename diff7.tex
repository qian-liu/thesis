...
In terms of modelling accuracy, NSP not only takes the noise of the current influx \DIFaddbegin \DIFadd{brought by the random arrival of spikes }\DIFaddend into account, which is missing in soft LIF~\citep{hunsberger2015spiking}, but also includes coloured noise where the Siegert formula~\citep{Jug_etal_2012} only works on white noise.
...
Multiplication is the core operation in the unsupervised Deep Learning algorithms, Autoencoders~(AEs) and Restricted Boltzmann Machines~(RBMs)\DIFdelbegin \DIFdel{: $\Delta w \propto ab-cd$. }\DIFdelend \DIFaddbegin \DIFadd{. %DIF > : $\Delta w \propto ab-cd$.
}\DIFaddend In Chapter~\ref{cha:sdlm} we found that the product of two numerical values could be represented with rate multiplication of a pair of rate-coded spike trains;
...
\citet{neil2013online} ignored the \DIFdelbegin \DIFdel{mathematical analysis }\DIFdelend \DIFaddbegin \DIFadd{model formalisation and accurate parameter settings }\DIFaddend thus achieved its best performance only at 81.5\%.
...
In Chapter~\ref{cha:bench} we presented a dataset containing four different spike representations of the MNIST stationary hand-written digit database. %DIF <  rate-based code, rank-order code, DVS recorded flashing and moving inputs. 
%DIF >  rate-based code, rank-order cod                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     e, DVS recorded flashing and moving inputs. 
The Poissonian subset is intended for benchmarking existing rate-based recognition methods.
...
This dataset aims to convert static images into neuromorphic vision input, while the recordings of moving input in \DIFdelbegin \DIFdel{our paper }\DIFdelend \DIFaddbegin \DIFadd{this thesis }\DIFaddend are intended to promote position-invariant recognition.
Therefore, the datasets complement each other.
...
Moreover, some initial studies \DIFaddbegin \DIFadd{following our research }\DIFaddend have successfully applied the proposed method to various applications. 
...
\begin{itemize}
	\item Ensemble models~\citep{krogh1995neural} have been trained with NSP neurons and will be transferred onto the SpiNNaker system~\citep{furber2014spinnaker}\DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{.
	...
	Operating ensemble models on SNNs can }\DIFaddend take the advantages of energy-efficiency and massively-parallel processing \DIFaddbegin \DIFadd{on Neuromorphic Hardware}\DIFaddend .
	...
	\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{We used spike count per frame per frequency channel as a pixel in a spectrum image, and trained CNNs based on these spectrum images.
	The trained weights were transferred directly to an equivalent convolutional SNN.
	The }\DIFaddend next step is to implement the model entirely on neuromorphic hardware including the cochlea and the SNN, thus to run it in real time.
...
We have proposed a similar on-line learning algorithm for training precise-timing based spiking AEs and RBMs, which although still in the test-idea stage, \DIFaddbegin \DIFadd{result of }\DIFaddend the prototype has shown much faster learning speed than the rate-coding mechanism.
...
Last but not least, this work contributes to the NE community with a \DIFdelbegin \DIFdel{uniform }\DIFdelend \DIFaddbegin \DIFadd{unified }\DIFaddend dataset to evaluate SNNs' performance at both model and hardware level.
