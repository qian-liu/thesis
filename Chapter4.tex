...
An intuitive idea of bringing these Deep Learning techniques to spiking neural networks~(SNNs) is either transferring well-tuned deep ANN models into SNNs or translating numerical calculations of weight tuning to biologically-plausible synaptic learning rules.
...
Based on the former solution, this chapter proposes a generalised training method to train SNNs off-line on equivalent ANNs and transfer the tuned weights back to the SNNs;
...
Noisy Softplus~(NSP), which closely simulates the firing activity of Leaky Integrate-and-Fire~(LIF) neurons driven by noisy current influx.
The second problem is mapping the abstract numerical values of the ANN to physical variables, current (nA) and firing rate (Hz), in the SNN.
...
%\end{figure}
	...
	To verify the response function in practice, simulation tests were carried out using PyNN~\citep{davison2008pynn} to compare the reality with the analytical results~(the Siegert function).
...
%	Thus the experiments show that a longer $\tau_{syn}$ increases the level of noise and widens the variance of the output firing rate.
	Although the use of the Siegert function opened the door for modelling response activities of LIF neurons as the activation function used in ANNs~\citep{Jug_etal_2012}, there are several drawbacks of this method:
	\begin{itemize}
