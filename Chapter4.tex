...
SNN training, now, can be simplified to: firstly, estimate parameters for the PAF using NSP; secondly, use the PAF version of conventional activation functions to train an equivalent ANN; and finally transfer the tuned weights directly into the SNN without any conversion.
...
Therefore, most importantly, this research provides the Neuromorphic Engineering~(NE) community with a simple, but effective and generalised off-line SNN training method which notably simplifies the development of AI applications on neuromorphic hardware.
...
%	The impressive disparities in cognitive capabilities and energy consumption drives the research into biologically-plausible spiking neurons and brain inspired computers, known as neuromorphic engineering~\citep{furber2016large}.
	...
	We specified the differences between a conventional artificial neuron and a spiking neuron in Section~\ref{sec:spike}: a regular artificial neuron comprises a weighted summation of input data, $net_j=\sum x_i w_i$, and an activation function, $f$, applied to the sum, $net_j$.
...
\section{Siegert: Modelling the Response Function}
	...
	\begin{equation}
	\lambda_\mathit{out}=
	\left [ \tau_{\textit{\textrm{refrac}}}-\tau_m\log \left ( 1-\frac{V_{th}-V_\mathit{rest}}{IR_m}  \right )\right ]^{-1}, \textrm{~when~} IR_m>V_{th}-V_{rest},
	\label{equ:consI}
	\end{equation}
	otherwise the membrane potential cannot reach the threshold $V_{th}$ and the output firing rate is zero. 
	The absolute refractory period $\tau_{\textit{\textrm{refrac}}}$ is included, during which period synaptic inputs are ignored.
	...
	\begin{equation}
	\frac{\D V}{\D t}=\frac{V_{rest} - V}{\tau_m } + \frac{m_I}{C_m} + \frac{s_I}{C_m}~~.
	\label{equ:LIF_V2}
	\end{equation}
	...
	\end{aligned}
	\end{equation}	
	The last term $\D W_t$ is a Wiener process; $W_{t +\D t} - W_{t}$ obeys a Gaussian distribution with mean 0 and variance $\D t$; thus $r$ is a random number sampled in accordance with a Gaussian distribution of zero mean and unit variance.
...
%http://neuronaldynamics.epfl.ch/online/Ch1.S3.html about V tradractory need to write in Chapter 2.	
	...
	\begin{equation}
	\lambda_\mathit{out}=
	\left [ \tau_{\textit{\textrm{refrac}}}+\tau_m \int_{\frac{V_\mathit{rest}-\mu_v \tau_m }{\sigma_v \sqrt{\tau_m}}}^{\frac{V_{th}-\mu_v \tau_m }{\sigma_v \sqrt{\tau_m}}} \sqrt{\pi} \exp(V^{2}) (1+\mathit{erf}(V)) \D V \right ]^{-1} ~.
	\label{equ:siegert}
	\end{equation}
	...
	The bottom (zero noise) line in Figure~\ref{Fig:physics} illustrates the response function of such an LIF neuron injected with constant current, which inspired the proposal of ReLUs.
	As noise increases, the level of firing rates also rises.
	Thus the Softplus function approximates the response activity to noisy current, but only represents a single level of noise; for example, the top line in Figure~\ref{Fig:physics} shows the curve when $s_I=1$.
		\begin{table}[th]
			\centering
...
%	and the noise added to the mean current is not white noise, but pink noise.
	...
	Nevertheless, the analytical response function, the Siegert formula, cannot approximate either of the practical simulations (see Figure~\ref{Fig:current}).
...
%		This issue will be addressed in Section~\ref{subsec:practice}.
	...
	\begin{figure}[bht!]
		...
		%			\includegraphics[width=\textwidth]{pics_iconip/revise_siegert1.png}
		%			\caption{Response firing rate of an LIF neuron with a current offset, $I_{\textit{\textrm{offset}}}$ = 0.11 nA.}
		%		\end{subfigure}\\
		\begin{subfigure}[t]{0.7\textwidth}
		...
		\end{subfigure}
		%		\begin{subfigure}[t]{0.48\textwidth}
		%			\includegraphics[width=\textwidth]{pics_iconip/revise_5.pdf}
		%			\caption{NSP in 3D.}
		%		\end{subfigure}
		\caption[NSP models the LIF response function.]{
			...
			%			(b) The same response activities shifted by a constant current offset, $I_{\textit{\textrm{offset}}} = 0.11$ nA.
...
%	The noisy-current driven firing rates shown in Figure~\ref{fig:nsp}(b) are not different from the curves in Figure~\ref{fig:nsp}(a) except that a constant current $I_{\textit{\textrm{offset}}}$ is added to the current influx thereby shifting the curves left on the x-axis by $I_{\textit{\textrm{offset}}}$.  
%	However, the shift is essential to enable accurate fitting of the response firing rates of the LIF neuron by using NSP.
%	Note that, the current offset $I_{\textit{\textrm{offset}}}$ can be seen as a bias of the input $x$ in the NSP function, $f_{\textit{\textrm{NSP}}}(x-I_{\textit{\textrm{offset}}}, \sigma)$, thus the specific parameter of LIF neuron is not arbitrarily chosen, but, configured by precise estimation.
%	The parameter estimation methods will be illustrated in Section~\ref{sec:af_model}.
...
%	However, before we can replace an LIF neuron with an abstract activation function, it is necessary to map the numerical values of NSP to physical variables in SNNs.
	...
	Suitable calibrations of the noise scaling factor $k$, input bias $b$ and mapping scaling factor $S$ in Equation~\ref{equ:fit} enable NSP to closely match the practical response firing rates of LIF neurons given various biological parameters.
	...
	To keep the simple format of traditional activation functions, $y=f(x)$, which has no constant bias on the input, it is easy to pass the bias $b$ to the LIF parameter, the constant current offset, $I_{\textit{\textrm{offset}}}=b$.
...
%	\end{figure}
	...
	The transformed signal then forms the output of an artificial neuron, which can be denoted as $y_j=f(net_j)$, see Figure~\ref{Fig:tneuron_c}.
	
	\begin{figure}[bt]
		\centering
		\includegraphics[width=0.6\textwidth]{pics_iconip/neuron.pdf}
		\caption[A conventional artificial neuron.]{A copy of Figure~\ref{Fig:neuron_net} showing a general artificial neuron where an activation function transforms the weighted sum $net_j$  to its outcome $y_j$.}
		\label{Fig:tneuron_c}
	\end{figure}
	
	Equation~\ref{equ:distr0} illustrates the physical interpretation of the input of an NSP function, the noisy current influx, which has the mean of $m_I$, and the variance of $S_I^2$.
...
%	However, a spiking neuron modelled by NSP takes the firing rate as its input/output, thus in a neural network Equation~\ref{equ:distr0} can be written as:
	...
	\begin{equation}
	 x_i=\lambda_{i}\tau_{\textit{\textrm{syn}}}~.
	\label{equ:nsp_input}
	\end{equation}
	Figure~\ref{Fig:tneuron_a} illustrates the process that an NSP-modelled artificial spiking neuron takes the input vector $\mathbf{x}$ which is converted from the input firing rate $\bm{\lambda}$; transforms the weighted sum $net\_x_{j}$ and $net_\_\sigma^2_j$ to the abstract output $y_j$; and scales up $y_i$ with the factor $S$ to the output firing rate $\lambda_j$. 
	\begin{figure}[tbh!]
		\centering
		\includegraphics[width=0.9\textwidth]{pics_iconip/neuron_o.pdf}
		\caption[An artificial spiking neuron modelled by NSP.]{An artificial spiking neuron modelled by NSP. A spiking neuron takes firing rate $\lambda_i$ as its input, which then forms the abstract numerical equivalence by multiplying the synaptic constant: $x_i=\lambda_{i}\tau_{\textit{\textrm{syn}}}$. The NSP transforms the noisy current influx including both the mean ($net\_x_{j}$, solid lines) and the variance $net_\_\sigma^2_j$ (dashed lines) to the abstract firing rate $y_j$. Finally, the output of the NSP is mapped to the physical units, firing rates ($\lambda_j$) in Hz, by multiplying $S$.}
		\label{Fig:tneuron_a}
	\end{figure}

	
	If instead of multiplying every input firing rate $\lambda_i$ by $\tau_{\textit{\textrm{syn}}}$ (left of Fig.~\ref{Fig:tneuron_a}) we do it in every output firing rate after $\lambda_j$, (right of Fig.~\ref{Fig:tneuron_b}) we obtain the same neuron model and structure as a typical neuron in ANNs, see Figure~\ref{Fig:tneuron_c}, that neurons take $\mathbf{x}$ as input and output abstract value $y$.
	
	\begin{figure}[bt]
		\centering
		\includegraphics[width=0.85\textwidth]{pics_iconip/neuron_PAF.pdf}
		\caption[An artificial spiking neuron modelled by PAF-NSP.]{An artificial spiking neuron modelled by PAF-NSP, whose input and output are numerical values, equivalent to those of ANNs. PAF includes the scaling factors $S$ and the synaptic time constant $\tau_{\textit{\textrm{syn}}}$ in the combined activation function, which links the firing activity of a spiking neuron to the numerical value of ANNs.}
		\label{Fig:tneuron_b}
	\end{figure}
...
%	We will compare these ReLU-like functions in modelling LIF neurons in Section~\ref{subsec:compareAF}.
	Note that the calculation of noise level is not necessary for other activation functions, thus they only take the mean of the current influx as the input (the solid lines in Figure~\ref{Fig:tneuron_b}).
	So the noise level can be set to a constant for Softplus or considered as 0 for ReLU.
	
	It is also significant to transform numerical values of training and testing data to firing rates in the first/last layer of the SNN.
...
\label{sec:iconipResult}
	...
	% Training
	To train the ConvNet, firstly, we estimated parameter $p$ for PAFs given LIF configurations listed in Table~\ref{tbl:pynnConfig} and $\tau_{\textit{\textrm{syn}}}=0.005$~s, $p = S \times \tau_{\textit{\textrm{syn}}} = 1.085$, where $(k=0.31, b=0.1, S=217)$ were calibrated using NSP. 
...
%	To validate how well the Noisy Softplus activation fits to the response firing rate of LIF neurons in a real application, we simulated the model on NEST~\citep{gewaltig2007nest} using the Poisson MNIST dataset~\citep{liu2016bench} and the neurons of a convolutional map were observed.
	...
	Ten testing images were presented with spike trains whose firing rates were calculated as: $\bm{\lambda} = \bm{x}/ \tau_{\textit{\textrm{syn}}}$.
	The inputs were convolved with a trained $5\times5$ kernel, and the output firing rates of the spiking neurons were recorded, see Figure~\ref{fig:cnn}.
...
%	Hence, the firing rate driven by lower noise level is closer to ReLU curve than Softplus.
	...
	We trained the network with binary labels on the output layer, thus the expected firing rate of correct classification was $1 \times S=217$~Hz according to Equation~\ref{equ:nsp_input}.
	The firing rates of the recognition test fell to the valid range.
...
%	train an equivalent ANN and transfer the trained weights back to SNNs.
	This training procedure consists of three simple stages: first, estimate parameters for PAF using NSP; second, use a PAF version of conventional activation functions for ANN training; % can be generalised to activation units other than NSP.
