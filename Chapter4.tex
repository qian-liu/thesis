...
\end{figure}
	...
	Notice that the estimation of LIF response activity requires noisy current with white noise, however
...
%	and the noise added to the mean current is not white noise, but pink noise.
	...
	It is caused by the coarse resolution (1~ms) of the spikes, thus the standard deviation of the current signal is larger than 0.2, shown in Figure~\ref{Fig:lif_pois}(e).
	While Figure~\ref{Fig:lif_pois}(b) shows a close estimation of the firing rate driven by synaptic current by providing the result of the Gauss noisy current test, because that both current signals have similar time correlation.
...
%	Thus the experiments show that a longer $\tau_{syn}$ increases the level of noise and widens the variance of the output firing rate.
	...
	Therefore due to the limited time resolution of common SNN simulators and the time taken for neurotransmitter release, mismatches exist between the analytical response function and the practical neural dynamics.
