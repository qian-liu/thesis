...
%	The trained networks were scaled to SNNs and compared on recognition rates, 93.34\%, 96.43\% and 97.03\% with a conversion loss of 4.76\%, 0.91\% and 0.74\%.
	...
	From DNN to SNN, the classification error increases by 44.14\%, 56.94\% and 134.27\% for Noisy softplus, ReLU and Softplus, because of the mismatch between the activation models and the practical firing rates which is in accordance with the Euclidean distance result.
	However, the fine tuning's little effort decreases the error by 14.36\%, 8.54\% and 35.25\% respectively, which improves the recognition capability of the SNNs.
	The improvement of ReLU is not as much as the others, because there is no problem of layered-up accumulated offset.
	...
	Softplus benefits the most from fine tuning, since the mismatch is much corrected by Noisy Softplus. 
