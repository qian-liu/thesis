...
\section{Modelling the Response Function}
	...
	&=\frac{V_{rest} - V}{\tau_m }\D t + \mu \D t + \sigma y ~~. 
	\end{aligned}
	\end{equation}	
...
%http://neuronaldynamics.epfl.ch/online/Ch1.S3.html about V tradractory need to write in Chapter 2.	
	...
	\begin{equation}
	\lambda_\mathit{out}=
	\left [ t_\mathit{ref}+\tau_m \int_{\frac{V_\mathit{rest}-\mu \tau_m }{\sigma \sqrt{\tau_m}}}^{\frac{V_{th}-\mu \tau_m }{\sigma \sqrt{\tau_m}}} \sqrt{\pi} \exp(u^{2}) (1+\mathit{erf}(u)) \D u \right ]^{-1} ~.
	\label{equ:siegert}
	\end{equation}
	...
	Thus the Softplus function approximates the response activity to noisy current, but only represents a single level of noise; for example, the dotted line in Figure~\ref{Fig:physics} shows the curve when $s_I=1$.
		\begin{table}[th]
			...
			\begin{tabular}{c c l}
				...
				%\hline
%				$I_{\textit{\textrm{offset}}}$ & 0.1~nA & offset of current influx  \\
			\end{tabular}
			%\egroup
...
%		This issue will be addressed in Section~\ref{subsec:practice}.
	...
	\begin{figure}[tbh!]
		...
		\begin{subfigure}[t]{0.48\textwidth}
			...
			\caption{Response firing rate of an LIF neuron with a current offset, $I_{\textit{\textrm{offset}}}$.}
		\end{subfigure}
		\begin{subfigure}[t]{0.48\textwidth}
		...
		\caption[NSP models the LIF response function.]{
			...
			(b)
			(c) NSP activates the input $x$ according to different noise levels where $k=0.16$.}
		\label{fig:nsp}
	\end{figure}
...
%	However, such a derivative function of low complexity does not present in Siegert function.
	...
	\begin{equation}
	x = \tau_{\textit{\textrm{syn}}}\sum_i w_i\lambda_{i}+I_{\textit{\textrm{offset}}}~, ~\sigma^2=\frac{1}{2}\tau_{\textit{\textrm{syn}}}\sum_i w_i^2\lambda_{i}~,
	\label{equ:distr}
	\end{equation}
	where $\lambda_i$ indicates the firing rate of an input spike train.
...
%	\begin{equation}
%	\begin{aligned}
%		& {m_I}_j = \sum_i w_{ij}(\lambda_{i}\tau_{\textit{\textrm{syn}}})~, \textrm{  then}\\
...
%	\begin{equation}
%	\begin{aligned}
%		& {s_I^2}_j=\sum_i(\frac{1}{2} w_{ij}^2) (\lambda_{i}\tau_{\textit{\textrm{syn}}})~, \textrm{  then}\\
...
%	The result of fine tuning on a ConvNet will be demonstrated in Section~\ref{subsec:result_compare}.
	...
	\begin{equation}
	\lambda_{\textit{\textrm{out}}} \simeq f_{\textit{\textrm{NSP}}}(x, \sigma) \times S = k \sigma \log [1 + \exp(\frac{x}{k \sigma})] \times S~.
	\label{equ:fit}
	\end{equation}	
...
\label{sec:iconipResult}
	...
	% Training
	To train the ConvNet, firstly, we estimated parameter $p$ for PAFs given LIF configurations listed in Table~\ref{tbl:pynnConfig} and $\tau_{\textit{\textrm{syn}}}=5$~ms, $p = S \times \tau_{\textit{\textrm{syn}}} = 1.005$, where $(k=0.30, S=201)$ were calibrated using NSP. 
