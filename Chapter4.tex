...
Afterwards, a complete SNN training method is explained employing the Noisy Softplus activations and generalised to other activation functions.
...
\subsection{Neural Science Background}
	...
	\label{equ:noiseI}
	\end{equation}
	Figure~\ref{Fig:physics} shows the response curve (\ref{equ:noiseI}) of a LIF neuron driven by noisy currents with Gaussian noise of $\mu$ mean and $\sigma$ standard deviation.
	...
	The solid (zero noise) line in Figure~\ref{Fig:physics} illustrates the response function of an LIF neuron, which inspired the proposal of ReLUs.
	As noise increases, the level of firing rates also rises, and the Softplus activation function approximates the responding activity driven by some noise, for example the dotted line in Figure~\ref{Fig:physics}.
...
%	\end{itemize}  
	...
	Thus we observe the mismatch with the test of the injected noisy current. %todo rephrase
