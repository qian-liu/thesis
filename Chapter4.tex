...
An intuitive idea of bringing these Deep Learning techniques to Spiking Neural Networks~(SNNs) is either transferring well-tuned deep ANN models into SNNs or translating numerical calculations of weight modulations to biologically-plausible synaptic learning rules.
...
Based on the former solution, this chapter proposes a generalised method to train SNNs off-line on equivalent ANNs and transfer the tuned weights back to the SNNs;
...
Consequently we introduce the Parametric Activation Function~(PAF), $y = p \times f(x)$, which successfully includes physical units to conventional activation functions thus unifies the representations of neurons in ANNs and the ones in SNNs.
...
SNN training, now, can be simplified as: firstly, estimate parameter $p$ for the PAF using NSP; secondly, use the PAF version of conventional activation functions to train an equivalent ANN; and finally transfer the tuned weights directly into the SNN without any conversion.
...
Therefore, most importantly, this research provides the Neuromorphic Engineering~(NE) community a simple, but effective and generalised off-line SNN training method which notably simplifies the development of Artificial Intelligence~(AI) applications on neuromorphic hardware.
...
%	An intuitive idea is to train SNNs on an equivalent ANN and then transfer the trained weights to the SNN.
	...
	These activation functions used in ANNs without physical units are called `abstract' to differ from response functions of spiking neurons.
	We select them for LIF modelling and SNN modulations because of the simplicity and generalised use for training ANNs.
...
%	Thus, NSP~\citep{liu2016noisy} (Section~\ref{sec:NSP}), we proposed closely match the response activity of LIF neurons by including current noise as the second factor in the activation function and taking account of coloured noise driven by $\tau_{\textit{\textrm{syn}}}$.
	...
	Nevertheless, these simple IF neurons are usually difficult to implement on analogue circuits, thus only feasible on digital neuromorphic hardware, e.g. TrueNorth~\citep{merolla2014million}.
	...
	Thus, the training can be not only simplified by using conventional simple activation functions, such as Rectified Linear Units~(ReLUs), but also the method can be generalised to target standard LIF neurons which are supported by most neuromorphic hardware platforms.
...
%http://neuronaldynamics.epfl.ch/online/Ch1.S3.html about V tradractory need to write in Chapter 2.	
	...
	Figure~\ref{Fig:physics} shows the response curves (Equation~\ref{equ:siegert}) of an LIF neuron driven by noisy currents with Gaussian noise with mean $m_I$ and standard deviation $s_I$.
...
%	Thus the experiments show that a longer $\tau_{syn}$ increases the level of noise and widens the variance of the output firing rate.
		Although the use of the Siegert function opened the door for modelling LIF response function to work similarly to activation functions used in ANNs~\citep{Jug_etal_2012}, there are several drawbacks of this method:
	\begin{itemize}
...
%	The result of fine tuning on a ConvNet will be demonstrated in Section~\ref{subsec:result_compare}.
	...
	\begin{figure}[tbh!]
		...
		\begin{subfigure}[t]{\textwidth}
			...
			\caption{An artificial spiking neuron modelled by PAF-NSP, whose input and output are numerical values, equivalent to those of ANNs. PAF includes the scaling factors $S$ and the synaptic time constant $\tau_{syn}$ in the combined activation function.}
		\end{subfigure}\\\bigskip
		\begin{subfigure}[t]{\textwidth}
			\centering
		...
		\end{subfigure}
		\caption{Comparisons of three activation functions using in ANNs: (a) the NSP, (b) the PAF-NSP, and (c) a conventional ANN activation function. The PAF links the firing activity of a spiking neuron to the numerical value of ANNs.}
		\label{Fig:tneuron}
	\end{figure}
	...
	Because Softplus and NSP both will converge to ReLU, when the input increases, so they can share the scaling factor $p$.
...
%	However, the loss stabilised fastest, which means a possible shorter training time.
	...
	Since similar findings have shown that networks with added noise, e.g. dropout~\cite{srivastava2014dropout}, are much faster to train. 
...
\label{subsec:result_compare_rec}
	...
	Regarding the classification accuracy, it achieves the state-of-the-art performance of SNNs and wins all the other methods using LIF neurons.
...
\section{Summary}
%\section{Discussions}
	We presented a generalised off-line SNN training method to tackle the research problem of equipping SNNs with equivalent cognitive capability as ANNs.
%	train an equivalent ANN and transfer the trained weights back to SNNs.
	...
	Regarding the generalisation, the training not only uses popular activation functions in ANNs, but also targets standard LIF neurons for the purpose of widely use on neuromorphic hardware. 
