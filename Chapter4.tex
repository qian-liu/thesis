...
%http://neuronaldynamics.epfl.ch/online/Ch1.S3.html about V tradractory need to write in Chapter 2.	
	...
	\begin{equation}
	\lambda_\mathit{out}=
	\left [ t_\mathit{ref}+\tau_m \int_{\frac{V_\mathit{rest}-\mu_v \tau_m }{\sigma_v \sqrt{\tau_m}}}^{\frac{V_{th}-\mu_v \tau_m }{\sigma_v \sqrt{\tau_m}}} \sqrt{\pi} \exp(V^{2}) (1+\mathit{erf}(V)) \D V \right ]^{-1} ~.
	\label{equ:siegert}
	\end{equation}
...
%	\end{figure}
	...
	Equation~\ref{equ:distr0} illustrates the physical interpretation of the input of an NSP function, the noisy current influx, which has the mean of $m_I$, and the variance of $\sigma_2$.
