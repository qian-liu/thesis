...
In the last chapter, we have shown the promising deep learning research in the field of artificial neural networks~(ANNs). 
An intuitive idea of bringing these deep learning techniques to spiking neural networks~(SNNs) is to train SNNs on an equivalent ANN and then transfer the trained weights to the SNN.
We call this method `off-line' training, since it does not take place on SNNs directly, but rather on ANNs instead.
This chapter proposes a generalised, off-line, simple, SNN training method and aims to address the first research problem of the thesis: how to train SNNs on equivalent ANNs, thus to make trained SNNs as competent as ANNs in cognitive tasks. 
...
SNNs can be trained by first training an equivalent ANN and then transferring the trained weights to the SNN, but there are two significant problems to be solved.
...
Noisy Softplus~(NSP), which closely models the response activity of Leaky Integrate-and-Fire~(LIF) neurons.
...
%	The impressive disparities in cognitive capabilities and energy consumption drives the research into biologically-plausible spiking neurons and brain inspired computers, known as neuromorphic engineering~\cite{furber2016large}.
	...
	As mentioned in Section~\ref{sec:spike}, we have specified the differences of a conventional artificial neuron and a spiking neuron.
	Here we give a brief comparison again, that	a regular artificial neuron (Figure~\ref{Fig:compare_as4}(a)) comprises a weighted summation of input data, $\sum x_i w_i$, and an activation function, $f$, applied on the sum.
...
%	However, in this paper we exclude biases for both artificial and spiking neurons to simplify neural models and to reduce the number of parameters.
	Meanwhile the inputs of a spiking neuron (Figure~\ref{Fig:compare_as4}(b)) are spike trains, which generates current influx through neural synapses (connections).
	...
	These fundamental differences in input/output representation and neural computation raise the main research problem of how to operate and train biologically-plausible SNNs to make them as competent as ANNs in cognitive tasks.
	In this chapter, we focus on the specific research problem of off-line SNN training that SNNs are trained on equivalent ANNs and then the trained weights are transferred to the SNNs.
...
%	An intuitive idea is to train SNNs on an equivalent ANN and then transfer the trained weights to the SNN.
	...
	We will illustrate these problems in detail in Section~\ref{sec:siergert} and~\ref{subsec:practice}.
	...
	Therefore, the first problem of off-line SNN training is the accurate modelling of the neural response activity of LIF neurons using abstract activation functions, in the hope of (1) increasing the performance of SNN training and (2) reducing the computation complexity.
...
%\section{Discussions}
	...
	The best classification accuracy of 99.07\% using LIF neurons in a PyNN simulation outperforms state-of-the-art SNN models of LIF neurons and is very close to the result using IF neurons~\cite{diehl2015fast}.
