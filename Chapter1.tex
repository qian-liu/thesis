...
%in practice, there have been two significant problems prohibiting the SNN from becoming `intelligent' as its non-spiking counterpart, the traditional artificial neural networks~(ANN).
...
%With the neuromorphic hardware ready and the increasing knowledge of SNNs, it is time to take an extra step towards the ultimate goal of NE by equipping neuromorphic computers with equivalent performance as ANNs' to solve AI engineering tasks.
%
...
%These models are `reactive' since they are typically comprised of only two neural layers, which is difficult to perform sophisticated effective cognition.
