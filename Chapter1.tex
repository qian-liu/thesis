...
Computers conduct Boolean logic and deterministic digital operations based on synchronous clocks while nervous systems consist of parallel-distributed, event-driven, stochastic unreliable components~\cite{indiveri2009artificial}.
...
\begin{figure}[tbh!]
	...
	\caption{
		Neuromorphic engineering and machines learning has been to diverging directions.
		This thesis emerge the objectives to the ultimate goal of cognition on neuromorphic hardware.
		}
	\label{fig:intro}
\end{figure}
...
This is a bidirectional process that building a biologically inspired computer requires the knowledge of brain understanding, while simulating the brain activities in large scale and in real time can be only feasible on neuromorphic hardware.
...
Thus the breakdown of Dennard scaling and the problem of power dissipation drove the chip manufacturers to multi/many-core architecture.
However, the increased number of active transistors in a multi-core chip cost more power consumption that some fraction of the cores remains completely powered-off given the thermal design power constraint.
Dark silicon~\cite{esmaeilzadeh2011dark} addresses to the issue of inactive area of a multi-core chip, which could be up to 50\% nodes in 8~nm technology.
...
Therefore, learning from the biology may build up computers alternate conventional Von Neumann architecture and thus to solve the problem of microprocessor/memory performance gap~\cite{wulf1995hitting}, also known as Von Neumann bottleneck, where the system speed is determined and limited by the memory performance.

%SNN


%Although there are a number of neuromorphic platforms available for large-scale SNN simulations,
During the last decade, researchers are using the capabilities created by rapid developments in neuromorphic engineering to address the dual aims of understanding brain functions and building brain-like machines.
Spiking Neural Network (SNN) holds the key as the main approach for both of the goals.
...
The long-term ultimate goal of NE is to equip neuromorphic machines with genuinely intelligence~\cite{konar1999artificial}.
Today, in neuromorphic modelling we know the information processing in the brain is conducted by the basic computing units, spiking neurons, the information transmission takes place in the synapses and we can draw the connections among neurons from anatomical experiments.
Moreover, in neuromorphic computing, massive large-scale SNN hardware simulators are ready to use.
However, how to operate these brain-like machines to be competent in cognitive applications still remains unsolved.

While Deep Learning within the research of ANNs has dominated the state-of-the-art solutions on cognitive tasks, e.g. the exceeding human-level performance on image classification tasks~\cite{he2015delving}.
