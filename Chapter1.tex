...
The SNN has not achieved the intelligent performance of its non-spiking counterpart, the ANN, particularly when used in deep neural networks.
Deep Learning research in the field of ANNs has dominated state-of-the-art solutions for cognitive tasks, e.g. exceeding human-level performance on image classification tasks~\citep{he2015delving}, see Section~\ref{sec:dl_history} for more examples.
...
%TODO Current status
Designing and training SNNs for AI tasks started from building simple reactive systems of Winner-Take-All~(WTA) circuits and spike-based plasticity.
...
Researchers turn to Deep Learning for building intelligent SNNs.
Initial studies have shown that SNNs can be trained by first training an equivalent ANN and then transferring the trained weights to the SNN.
This method is called `off-line' training, since it does not take place on SNNs directly, but rather on ANNs instead.
However, previous work~\citep{Jug_etal_2012,hunsberger2015spiking,diehl2015fast} on such off-line training methods has demonstrated good performance on recognition tasks, but struggled in terms of poor modelling accuracy of the spiking neurons and high computational complexity.
In term of `On-line' training methods where Deep Learning modules are trained purely on SNNs in an event-driven manner, existing work~\citep{neil2013online,neftci2013event,burbank2015mirrored} has failed to provide SNNs with recognition accuracy equivalent to ANNs due to the lack of mathematical analysis.
The study of this thesis will continue the inspiring work on deep SNNs to overcome the shortcomings of existing methods to tackle the research problem of equipping SNNs with equivalent performance on AI tasks. 
...
%	Aim: to formalise a local learning rule based on synaptic plasticity for unsupervised, event-based, biologically-plausible training of deep SNNs in order to catch up with the recognition performances of ANNs.
	...
	\item 
	A new set of spike-based vision datasets can provide resources to support fair competition between researchers since new concerns relating to energy efficiency and recognition latency emerge in SNNs run on NE.
