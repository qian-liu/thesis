...
To provide meaningful comparisons between these proposed SNN models and other existing methods within the rapidly advancing field of NE, we propose a large dataset of spike-based visual stimuli \DIFaddbegin \DIFadd{to unify the data resources for objective comparisons;
...
%	It was tested on a convolutional network and showed a recognition capability a close to the conventional ANN. 
	...
	\DIFaddbegin \DIFadd{Moreover, we propose decorrelation methods to solve the problem of continuous performance drop caused by correlated spike trains.
	}\DIFaddend Thus, spiking AEs and RBMs can be trained with SRM and approach the same, sometimes even superior, classification and reconstruction capabilities compared to their equivalent non-spiking models.
...
\textbf{Chapter~\ref{cha:sdlm}} proposes the STDP-based learning algorithm for training spiking AEs and RBMs \DIFdelbegin \DIFdel{; and shows equivalent recognition }\DIFdelend \DIFaddbegin \DIFadd{on-line; and test the second hypothesis that on-line training is able to improve the cognitive }\DIFaddend capability of SNNs \DIFdelbegin \DIFdel{compared to }\DIFdelend \DIFaddbegin \DIFadd{to catch up with the }\DIFaddend ANNs.
