...
Advances in computing power and machine learning have \DIFdelbegin \DIFdel{benefited }\DIFdelend \DIFaddbegin \DIFadd{endowed }\DIFaddend computers with a \DIFdelbegin \DIFdel{rapid }\DIFdelend \DIFaddbegin \DIFadd{rapidly }\DIFaddend growing performance in cognitive tasks \DIFdelbegin \DIFdel{, }\DIFdelend such as recognising objects~\citep{deng2009imagenet} and playing GO~\citep{silver2016mastering}\DIFdelbegin \DIFdel{). 
...
In neuromorphic computing, massive }\DIFdelend \DIFaddbegin \DIFadd{As the considerable development of NE, massively }\DIFaddend large-scale \DIFdelbegin \DIFdel{SNN hardware simulators are ready to use}\DIFdelend \DIFaddbegin \DIFadd{SNN hardware simulators (indicated in Chapter~\ref{cha:bkg}) are ready to use, such as the High Input Count Analog Neural Network (HI-CANN)~\mbox{%DIFAUXCMD
...
Moreover, simulations of massive SNNs~\mbox{%DIFAUXCMD
...
However, \DIFdelbegin \DIFdel{due to the fundamental difference of data representation and neural computation between spiking and artificial neurons, how to operate these brain-like machines to be competent in cognitive applications still remains unsolved.
}\DIFdelend \DIFaddbegin \DIFadd{the SNN simulations only reconstruct the network behaviours and neural dynamics of some subsystem of the brain, `but without precisely functionally simulating that subsystem'~\mbox{%DIFAUXCMD
\citep{de2010world}
}%DIFAUXCMD
.
...
argued that the next challenge of NE is to make these brain-inspired computers effectively cognitive, also known as `Neuromorphic Cognition'.
...
\DIFadd{In SNNs, Spike-Timing-Dependent Plasticity (STDP) as a learning mechanism based on biological observations has been theoretically proved to be equivalent to a stochastic version of powerful machine learning algorithms, such as Expectation Maximization~\mbox{%DIFAUXCMD
...
Secondly, the fundamental differences in data representation and neural computation between spiking and artificial neurons make it difficult to transform ANN models to SNN algorithms, see Chapter~\ref{cha:bkg} for more detail.
...
Thus, recent SNN applications~\mbox{%DIFAUXCMD
\citep{bill2014compound,diehl2015unsupervised}
}%DIFAUXCMD
in AI tasks (summarised in Chapter~\ref{cha:bench}) typically comprise only two neural layers and exploit STDP learning rules and/or Winner-Take-All~(WTA) circuits on the synaptic connections.
...
\DIFadd{With neuromorphic hardware ready for massive SNN simulations, the main research problem arises: how to improve the cognitive performance of SNNs }\DIFaddend to catch up with \DIFdelbegin \DIFdel{ANNs' ability to beat human performance using SNNs , a shared ultimate goal has emerged.
...
Researchers turn to Deep Learning to build `smarter' SNNs.
...
has failed to provide SNNs with recognition accuracy equivalent to ANNs due to }\DIFaddend the \DIFdelbegin \DIFdel{cognition performance of SNNs and ANNs}\DIFdelend \DIFaddbegin \DIFadd{lack of mathematical analysis.
...
\DIFdelbegin \DIFdel{Aim: to formalise a local learning rule based on synaptic plasticity for unsupervised, event-based, biologically-plausible training of deep SNNs in order to catch up the recognition performances of ANNs.
	...
	\item 
	A new set of spike-based vision datasets can provide resources to support \DIFdelbegin \DIFdel{fair }\DIFdelend \DIFaddbegin \DIFadd{objective }\DIFaddend competition between researchers since new concerns relating to energy efficiency and recognition latency emerge in \DIFdelbegin \DIFdel{Neuromorphic Vision}\DIFdelend \DIFaddbegin \DIFadd{SNNs run on NE platforms}\DIFaddend .
...
%	This work comprises Chapter 3 and was published and presented to the International Conference on Artificial Neural Networks 2015.
	...
	\item 
	\DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{A }\DIFaddend generalised SNN training method to train an equivalent ANN and transfer the trained weights back to SNNs.
...
%	It was tested on a convolutional network and showed a recognition capability a close to the conventional ANN. 
	...
	\item 
	\DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{An }\DIFaddend on-line unsupervised learning algorithm working purely on event-based local STDP for training spiking \DIFdelbegin \DIFdel{Autoencoder }\DIFdelend \DIFaddbegin \DIFadd{Autoencoders }\DIFaddend and Restricted Boltzmann \DIFdelbegin \DIFdel{Machine}\DIFdelend \DIFaddbegin \DIFadd{Machines}\DIFaddend .
...
}\DIFdelend \DIFaddbegin \DIFadd{biologically-plausible learning rule: Spike-Time-Dependent-Plasticity~(STDP).
...
%DIF > 	The numerical analysis of the proposed algorithm accurately estimate the configurations of parameters thus to closely mimic the unsupervised learning on these modules and improves the recognition performance of SNNs to reach the ANNs'.
	...
	\item 
	\DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{A }\DIFaddend dataset and the corresponding evaluation methodology for comparisons of Neuromorphic Vision \DIFaddbegin \DIFadd{systems}\DIFaddend .
