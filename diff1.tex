...
\DIFdelbegin \DIFdel{Deep Learning research in the field of ANNs has dominated the state-of-the-art solutions on cognitive tasks, e.
g.
}\DIFdelend %DIF > Meanwhile, Deep Learning research in the field of artificial neural networks~(ANNs) has dominated state-of-the-art solutions for AI engineering tasks, e.g. exceeding human-level performance on image classification~\citep{he2015delving}, see Chapter~\ref{cha:dnn} for more examples.
%DIF > 
...
%DIF > in practice, there have been two significant problems prohibiting the SNN from becoming `intelligent' as its non-spiking counterpart, the traditional artificial neural networks~(ANN).
%DIF > Firstly, the fundamental difference of data representation and neural computation between spiking and artificial neurons makes it difficult to transform ANN models to SNN algorithms, see Chapter~\ref{cha:bkg} for more detail.
...
%DIF > Secondly, the computation cost for simulating large SNNs of size as big as commonly-used deep ANNs was considered to be infeasible, but has been gradually solved by NE.
%DIF > With the neuromorphic hardware ready and the increasing knowledge of SNNs, it is time to take an extra step towards the ultimate goal of NE by equipping neuromorphic computers with equivalent performance as ANNs' to solve AI engineering tasks.
%DIF > 
...
%DIF > Chapter~\ref{cha:bench} summarises the recent development of applying SNNs in AI tasks, which started from building simple `reactive' systems of Winner-Take-All~(WTA) circuits and spike-based plasticity~\citep{bill2014compound,diehl2015unsupervised}.
%DIF > These models are `reactive' since they are typically comprised of only two neural layers, which is difficult to perform sophisticated effective cognition.
...
%DIF > Therefore, researchers turn to Deep Learning for building `smarter' SNNs.
%DIF > Initial studies have shown that SNNs can be trained by first training an equivalent deep ANN and then transferring the tuned weights to the SNN.
%DIF > This method is called `off-line' training, since it does not take place on SNNs directly, but rather on ANNs instead.
%DIF > Chapter~\ref{cha:Conv} discusses these `off-line' training models in detail, and proposes a simple, generalised, off-line SNN training method to overcome the problems of poor modelling accuracy and high computational complexity of the existing methods~\citep{Jug_etal_2012,hunsberger2015spiking,diehl2015fast}.
%DIF > In terms of `On-line' training, see Chapter~\ref{cha:sdlm}, Deep Learning modules are trained purely on SNNs in an event-driven manner.
%DIF > Previous work~\citep{neil2013online,neftci2013event,burbank2015mirrored} has failed to provide SNNs with recognition accuracy equivalent to ANNs due to the lack of mathematical analysis.
%DIF > We continue the inspiring works on these biologically-plausible `on-line' training and propose a formalised method to train layered-up Deep Learning modules on SNNs.
%DIF > To provide meaningful comparisons between these proposed SNN models and other existing methods within the rapidly advancing field of NE, we propose a large dataset of spike-based visual stimuli and a corresponding evaluation methodology to estimate the overall performance of SNN models and their hardware implementations in Chapter~\ref{cha:bench}.
\DIFaddbegin 
...
\DIFadd{In brief, Figure~\ref{fig:intro} summarises the introduction and demonstrates the outline of }\DIFaddend the \DIFdelbegin \DIFdel{exceeding human-level performance on image classification tasks~\mbox{%DIFAUXCMD
\citep{he2015delving}
}%DIFAUXCMD
.
Figure~\ref{fig:intro} shows the diverging objectives }\DIFdelend \DIFaddbegin \DIFadd{thesis.
...
With the support of accumulated knowledge }\DIFaddend of SNNs and \DIFdelbegin \DIFdel{ANNs, where SNNs focus on mimicking the brain while ANNs are active in solving engineering AI problems.
%DIF < TODO need to modify
Despite remarkable computational success , ANNsignore the spiking nature of neural communication that is fundamental for biological neuronal networks.
At the same time, to catch up with ANNs' ability to beat human performance using SNNs, a shared ultimate goal has emerged.
The key problem of }\DIFdelend \DIFaddbegin \DIFadd{the massive neuromorphic SNN simulators, plus the huge success of Deep Learning in ANNs, the research of }\DIFaddend the \DIFdelbegin \DIFdel{research describe in this thesis is to enable SNNs to have cognitive capabilities, and the main challenge is to close the gap of the cognition }\DIFdelend \DIFaddbegin \DIFadd{thesis targets at enabling deep SNNs with equivalent performance of ANNs on AI tasks thus approaching Neuromorphic Cognition.
To achieve the thesis aim, we propose an on-line and an off-line SNN training methods to bring Deep Learning advantages to SNNs, and provide a spike-based dataset and its corresponding evaluation methodology to measure the }\DIFaddend performance of SNNs and \DIFdelbegin \DIFdel{ANNs}\DIFdelend \DIFaddbegin \DIFadd{their hardware implementations}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdelend \section{\DIFdelbegin \DIFdel{Statement of the Problem}\DIFdelend \DIFaddbegin \DIFadd{Motivation and Aims}\DIFaddend }
