...
For instance, AlphaGO~\citep{silver2016mastering} has a power consumption of 1~MW on its $1,920$ CPUs and 280 GPUs when playing the game with one of the best human players whose brain is rated at 20~W\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{Computers }\DIFdelend \DIFaddbegin \DIFadd{Typical computers }\DIFaddend employ Boolean logic and deterministic digital operations usually based on synchronous clocks while nervous systems employ parallel, distributed, event-driven, stochastically unreliable components~\citep{indiveri2009artificial}: neurons.
...
To provide meaningful comparisons between these proposed SNN models and other existing methods within the rapidly advancing field of NE, we propose a large dataset of spike-based visual stimuli \DIFaddbegin \DIFadd{to unify data resources for objective comparisons;
}\DIFaddend and a corresponding evaluation methodology to estimate the overall performance of SNN models and their hardware implementations in Chapter~\ref{cha:bench}.
...
Although there are a number of neuromorphic platforms available for large-scale \DIFdelbegin \DIFdel{Spiking Neural Network~(SNN ) }\DIFdelend \DIFaddbegin \DIFadd{SNN }\DIFaddend simulations, the problem of programming these brain-like machines to be competent in cognitive applications still remains unsolved.
On the other hand, Deep Learning has emerged in \DIFdelbegin \DIFdel{Artificial Neural Network (ANN ) }\DIFdelend \DIFaddbegin \DIFadd{ANN }\DIFaddend research to dominate state-of-the-art solutions for cognitive tasks.
...
%	Aim: to generalise a training method on conventional ANNs whose trained connections can be transferred to corresponding SNNs to deliver similar recognition performance.
	...
	\item 
	Unsupervised Deep Learning modules can be trained on-line on SNNs with biologically-plausible synaptic plasticity to demonstrate a learning capability \DIFdelbegin \DIFdel{as competent as }\DIFdelend \DIFaddbegin \DIFadd{equivalent to }\DIFaddend ANNs.
...
%	It was tested on a convolutional network and showed a recognition capability a close to the conventional ANN. 
	...
	\DIFdelbegin \DIFdel{Thus, spiking AEs and RBMs can be trained with SRM and approach the same, sometimes even superior, classification and reconstruction capabilities compared to their equivalent non-spiking models.
	}\DIFdelend In addition, the numerical analysis of the proposed algorithm accurately estimates the parameters, thus closely mimicking the learning behaviour of the AE and RBM modules, and improves the learning performance compared to existing methods.
	%DIF < 	The learning ability matched the non-spiking ANNs and verified the feasibility of unsupervised training on deep SNNs. 
	\DIFaddbegin \DIFadd{Moreover, we propose solutions to the problem of continuous performance drop caused by correlated spike trains.
	...
	\DIFdelbegin \DIFdel{This work contributes to (1) promoting meaningful comparisons between algorithms and neuromorphic platforms in the field of NE, (2) allowing comparison with conventional image recognition methods, (3) providing an assessment of the state of the art in spike-based visual recognition, and (4) helping researchers identify future directions and advance the field}\DIFdelend \DIFaddbegin \DIFadd{We also present a potential benchmark system which is evaluated using the Poissonian subset of the NE15-MNIST dataset.
	It provides a baseline for further comparisons with upcoming SNN models}\DIFaddend .
...
The author participated in workshops organised by the NE community, 1) to establish and contribute to collaborations on mutual interests; 2) to catch up with cutting-edge research and collect inspiration; \DIFaddbegin \DIFadd{and }\DIFaddend 3) to discuss the author's own findings with key researchers in the field.
\begin{itemize}
	\item 
...
\textbf{Chapter~\ref{cha:dnn}} gives an overview of popular architectures and models of Deep Learning and illustrates the mechanism of the \DIFdelbegin \DIFdel{convolutional network~(ConvNet}\DIFdelend \DIFaddbegin \DIFadd{Convolutional Networks~(ConvNets}\DIFaddend ), the \DIFdelbegin \DIFdel{Autoencoders~(AEs)}\DIFdelend \DIFaddbegin \DIFadd{AEs}\DIFaddend , and the \DIFdelbegin \DIFdel{Restricted Boltzmann Machine~(RBM) }\DIFdelend \DIFaddbegin \DIFadd{RBMs }\DIFaddend in detail.

\textbf{Chapter~\ref{cha:Conv}} demonstrates the generalised off-line SNN training method \DIFdelbegin \DIFdel{by using a novel biologically-inspired activation function, Noisy Softplus, which is well-matched to the response function of Leaky Integrate-and-Fire neurons; and validates the performance by testing on a convolutional network.
}\DIFdelend \DIFaddbegin \DIFadd{to confirm the first hypothesis that SNNs can be trained off-line and perform equivalently as ANNs in cognitive tasks.
%DIF > by using a novel biologically-inspired activation function, NSP, which is well-matched to the response function of Leaky Integrate-and-Fire neurons; and validates the performance by testing on a convolutional network.
}\DIFaddend 

\textbf{Chapter~\ref{cha:sdlm}} proposes \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend STDP-based learning algorithm for training spiking AEs and RBMs \DIFdelbegin \DIFdel{; and shows equivalent recognition capability of SNNs compared to }\DIFdelend \DIFaddbegin \DIFadd{on-line; and test the second hypothesis that on-line training is able to improve the cognitive capabilities of SNNs and catch up with }\DIFaddend ANNs.
...
\section{Summary}
	...
	With the support of accumulated knowledge of SNNs and the massive neuromorphic SNN simulators (both refer to Chapter~\ref{cha:bkg}), plus the huge success of Deep Learning in ANNs (see Chapter~\ref{cha:dnn}), the research aims at understanding how to operate and train biologically-plausible SNNs to close the gap in cognitive capabilities between SNNs and ANNs on AI tasks, \DIFdelbegin \DIFdel{thus }\DIFdelend \DIFaddbegin \DIFadd{thereby }\DIFaddend approaching Neuromorphic Cognition.

	To achieve the thesis aim, we propose an \DIFdelbegin \DIFdel{on-line }\DIFdelend \DIFaddbegin \DIFadd{off-line }\DIFaddend (Chapter~\ref{cha:Conv}) and an \DIFdelbegin \DIFdel{off-line }\DIFdelend \DIFaddbegin \DIFadd{on-line }\DIFaddend (Chapter~\ref{cha:sdlm}) SNN training method to bring Deep Learning advantages to SNNs, and provide a spike-based dataset and its corresponding evaluation methodology to measure the performance of SNN models and \DIFdelbegin \DIFdel{their hardware implementations }\DIFdelend \DIFaddbegin \DIFadd{the neuromorphic hardware platforms }\DIFaddend in Chapter~\ref{cha:bench}.
