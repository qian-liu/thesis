...
\DIFdelend \DIFaddbegin \chapter[Off-line SNN training]{\DIFaddend Generalised Off-line \DIFaddbegin \DIFadd{SNN }\DIFaddend Training\DIFdelbegin \DIFdel{of Spiking Neural Networks}\DIFdelend }
\label{cha:Conv}
%Paragraph One: LINK
%DIF < In the last chapter, we answered our first research question of validating the capability of real-time cognitive application built on neuromorphic platform by building an object recognition prototype running completely on hardware in an absolute spike-based fashion.
%DIF < It built up the basis for the later research that neuromorphic hardware has been capable of running large applications standing alone without a general computer.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend In the last chapter, we \DIFdelbegin \DIFdel{have illustrated }\DIFdelend \DIFaddbegin \DIFadd{described }\DIFaddend the promising Deep Learning research in the field of artificial neural networks~(ANNs). 
An intuitive idea of bringing these Deep Learning techniques to spiking neural networks~(SNNs) is \DIFdelbegin \DIFdel{to operate deep SNNs using the trained weights of the equivalent ANNs.
Hence SNNs can be trained by first training an equivalent ANN and then transferring the trained weights to the SNN.
We call this method `off-line', since the training does not take place on SNNsdirectly, but rather on ANNs instead.
This chapter proposes a generalised }\DIFdelend \DIFaddbegin \DIFadd{either transferring well-tuned deep ANN models into SNNs or translating numerical calculations of weight modulations to biologically-plausible synaptic learning rules.
%DIF > operate deep SNNs using the trained weights of the equivalent ANNs.
%DIF > Hence SNNs can be trained by first training an equivalent ANN and then transferring the trained weights to the SNN.
%DIF > We call this method `off-line', since the training does not take place on SNNs directly, but rather on ANNs instead.
Based on the former solution, this chapter proposes a generalised method to train SNNs }\DIFaddend off-line \DIFdelbegin \DIFdel{training of SNNs to address the first research problem of the thesis }\DIFdelend \DIFaddbegin \DIFadd{on equivalent ANNs and transfer the tuned weights back to the SNNs;
while next chapter explores the latter idea of converting Deep Learning algorithms directly to on-line synaptic plasticity rules.
Both chapters address the thesis question}\DIFaddend : how to \DIFdelbegin \DIFdel{train SNNs on equivalent ANNs, thus to make trained SNNs }\DIFdelend \DIFaddbegin \DIFadd{operate and train SNNs to make them }\DIFaddend as competent as ANNs \DIFdelbegin \DIFdel{in cognitive }\DIFdelend \DIFaddbegin \DIFadd{on Artificial Intelligence~(AI) }\DIFaddend tasks.
...
In this chapter, we propose \DIFdelbegin \DIFdel{an activation function }\DIFdelend \DIFaddbegin \DIFadd{a novel activation function used in ANNs}\DIFaddend , Noisy Softplus~(NSP), \DIFdelbegin \DIFdel{which closely models the response }\DIFdelend \DIFaddbegin \DIFadd{to closely simulate the }\DIFaddend firing activity of Leaky Integrate-and-Fire~(LIF) neurons \DIFaddbegin \DIFadd{driven by noisy current influx}\DIFaddend .
The second problem is \DIFdelbegin \DIFdel{mapping }\DIFdelend \DIFaddbegin \DIFadd{to map }\DIFaddend the abstract numerical values of the \DIFdelbegin \DIFdel{ANN to concrete physical units in the SNN, such as current }\DIFdelend \DIFaddbegin \DIFadd{ANNs to physical variables, current (nA) }\DIFaddend and firing rate \DIFdelbegin \DIFdel{.
Thus }\DIFdelend \DIFaddbegin \DIFadd{(Hz), in the SNNs.
Consequently }\DIFaddend we introduce the \DIFdelbegin \DIFdel{parametric activation function}\DIFdelend \DIFaddbegin \DIFadd{Parametric Activation Function}\DIFaddend ~(PAF\DIFdelbegin \DIFdel{: $y = p \times f(x)$), to tackle this problem.
With these problems solved, SNN training }\DIFdelend \DIFaddbegin \DIFadd{), $y = p \times f(x)$, which successfully includes physical units to conventional activation functions thus unifies the representations of neurons in ANNs and the ones in SNNs.
Therefore, an SNN is able to be modelled and trained on an equivalent ANN using the conventional training algorithms, such as Backpropagation~(BP).
%DIF > With these problems solved, SNN training can be simplified as: (1) estimate parameter $p$ for the PAF according to biological configurations of LIF neurons, and (2) use the PAF instead of conventional activation functions to train an equivalent ANN.
%DIF > The trained weights can be transferred directly into the spiking version of the same network without further conversion.
%DIF > In addition, we propose a fine tuning method as an option which helps the trained network to match the SNN closer.
%DIF > Based on this generalised training method, we achieve the best SNN accuracy on the MNIST task using LIF neurons, 99.07\%, on a 6-layer spiking convolutional neural network~(ConvNet).
}

\DIFadd{The significance lies in the simplicity and generalisation of the proposed method.
SNN training, now, }\DIFaddend can be simplified as: \DIFdelbegin \DIFdel{(1) }\DIFdelend \DIFaddbegin \DIFadd{firstly, }\DIFaddend estimate parameter $p$ for \DIFdelbegin \DIFdel{PAF according to biological configurations of LIF neurons, and (2) use PAF instead }\DIFdelend \DIFaddbegin \DIFadd{the PAF using NSP; secondly, use the PAF version }\DIFaddend of conventional activation functions to train an equivalent ANN\DIFdelbegin \DIFdel{.
The trained weights can be transferred }\DIFdelend \DIFaddbegin \DIFadd{; and finally transfer the tuned weights }\DIFaddend directly into the \DIFdelbegin \DIFdel{spiking version of the same network without further conversion.
In addition, we propose a fine tuning method as an option which helps the trained network to closer match the SNN.
Based on this generalised training method, we achieve the best SNN accuracy on the MNIST task using LIF neurons , 99.07}%DIFDELCMD < \%%%%
\DIFdel{, on a 6-layer spiking convolutional neural network~(ConvNet)}\DIFdelend \DIFaddbegin \DIFadd{SNN without any conversion.
...
%DIF > In addition, the proposed method also preserves state-of-the-art accuracy of MNIST task using standard LIF neurons.
}

\DIFadd{Therefore, most importantly, this research provides the Neuromorphic Engineering~(NE) community a simple, but effective and generalised off-line SNN training method which notably simplifies the development of Artificial Intelligence~(AI) applications on neuromorphic hardware.
In turn, it enables ANN users to implement their models on neuromorphic hardware without the knowledge of spiking neurons or programming on specific hardware;
however, at the same time, benefit from the advantages of neuromorphic computers: such as real-time processing, low latency, biological realism and energy efficiency.
Furthermore, the success of the proposed off-line training method paves the way to energy-efficient AI on neuromorphic machines scaling from mobile devices to huge computer clusters}\DIFaddend .


%DIF > to inform the ANN community that massively-parallel, energy-efficient, Neuromorphic hardware Platforms (NPs) not only contribute to understanding the brain in Neuroscience, but are also ready to run large AI models in real-time.
\DIFaddbegin 

\DIFaddend In this chapter, we begin by introducing existing solutions \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend off-line SNN training in Section~\ref{sec:intro_NSP}.
...
%	The impressive disparities in cognitive capabilities and energy consumption drives the research into biologically-plausible spiking neurons and brain inspired computers, known as neuromorphic engineering~\citep{furber2016large}.
	...
	We \DIFdelbegin \DIFdel{have }\DIFdelend specified the differences \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{between }\DIFaddend a conventional artificial neuron and a spiking neuron in Section~\ref{sec:spike}: \DIFaddbegin \DIFadd{a }\DIFaddend regular artificial neuron comprises a weighted summation of input data, $\sum x_i w_i$, and an activation function, $f$, applied \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend the sum.
...
%	However, in this thesis we exclude biases for both artificial and spiking neurons to simplify neural models and to reduce the number of parameters.
	However the inputs of a spiking neuron (Figure~\ref{Fig:compare_as4}) are spike trains, which \DIFdelbegin \DIFdel{generates }\DIFdelend \DIFaddbegin \DIFadd{generate }\DIFaddend current influx through neural synapses (connections).
	A single spike creates a current pulse with an amplitude of $w$, which is defined as the synaptic efficacy, and the current then decays exponentially with a \DIFdelbegin \DIFdel{decaying }\DIFdelend \DIFaddbegin \DIFadd{decay }\DIFaddend rate determined by the synaptic time constant, $\tau_{\textit{\textrm{syn}}}$.
	The current pulses \DIFdelbegin \DIFdel{then }\DIFdelend consequently produce post-synaptic potentials~(PSPs) on the neuron driving its membrane potential \DIFaddbegin \DIFadd{to }\DIFaddend change over time, and trigger spikes as outcomes when the neuron's membrane potential reaches some threshold.
	The dynamics of the current influxes, PSPs, membrane potentials, and spike trains are all time dependent, while \DIFaddbegin \DIFadd{the }\DIFaddend neurons of ANNs only cope with abstract numerical values representing spiking rate, without timing information.
%	It is the stochasticity, in contrast to the continuously differentiable functions used by ANNs, that is intrinsic to the event-based spiking process, making network training difficult.
	Therefore, these fundamental differences in input/output representation and neural computation \DIFdelbegin \DIFdel{raise }\DIFdelend \DIFaddbegin \DIFadd{form }\DIFaddend the main research problem of how to operate and train biologically-plausible SNNs to make them as competent as ANNs in cognitive tasks.
	In this chapter, we focus on the \DIFdelbegin \DIFdel{specific research problem }\DIFdelend \DIFaddbegin \DIFadd{solutions }\DIFaddend of off-line \DIFdelbegin \DIFdel{SNN }\DIFdelend training where SNNs are trained on equivalent ANNs and then the \DIFdelbegin \DIFdel{trained }\DIFdelend \DIFaddbegin \DIFadd{tuned }\DIFaddend weights are transferred to the SNNs.
...
%	An intuitive idea is to train SNNs on an equivalent ANN and then transfer the trained weights to the SNN.
	...
	Jug et al.~\citep{Jug_etal_2012} first proposed the \DIFdelbegin \DIFdel{use of the }\DIFdelend Siegert formula~\citep{siegert1951first} \DIFdelbegin \DIFdel{as the activation function in training }\DIFdelend \DIFaddbegin \DIFadd{to model the response function of a spiking neuron, which worked as a Sigmoid unit in training spiking }\DIFaddend Deep Belief Networks\DIFdelbegin \DIFdel{, which }\DIFdelend \DIFaddbegin \DIFadd{.
	The Siegert formula }\DIFaddend maps incoming currents driven by Poisson spike trains to the response firing rate of an LIF neuron\DIFaddbegin \DIFadd{, similar to the activation functions used in ANNs which transform the summed input to corresponding outcomes}\DIFaddend .
	The variables of the \DIFdelbegin \DIFdel{activation }\DIFdelend \DIFaddbegin \DIFadd{response }\DIFaddend function are in physical units, thus the trained weights can be transferred directly into SNNs.
	...
	%The ratio of the spiking rate to its maximum is equivalent to the output of a sigmoid neuron.
	%A spiking Deep Belief Network (DBN)~\citep{Stromatias2015scalable} of four layers of RBMs was implemented on neuromorphic hardware, SpiNNaker~\citep{furber2014spinnaker}, to recognise hand written digits in real time.
	However, the Siegert formula is inaccurate as it models the current noise as white~\citep{liu2016noisy}, $\tau_{\textit{\textrm{syn}}} \to 0$, \DIFdelbegin \DIFdel{but it }\DIFdelend \DIFaddbegin \DIFadd{which }\DIFaddend is not feasible in practice.
	...
	Moreover, the high complexity of the Siegert function and the computation of its derivative to obtain the error gradient cause much longer training \DIFdelbegin \DIFdel{time}\DIFdelend \DIFaddbegin \DIFadd{times}\DIFaddend , thus consuming more energy, when compared to regular ANN activation functions, \DIFdelbegin \DIFdel{such as }\DIFdelend \DIFaddbegin \DIFadd{e.g. }\DIFaddend Sigmoid.
	...
	A softened version of the response function of LIF neurons \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{has been }\DIFaddend proposed~\citep{hunsberger2015spiking} and is less computationally expensive than the Siegert function.
	However, the model \DIFdelbegin \DIFdel{ignored }\DIFdelend \DIFaddbegin \DIFadd{ignores }\DIFaddend the dynamic noise change introduced by input spikes, assuming \DIFaddbegin \DIFadd{a }\DIFaddend static noise level of the current influx.
	Therefore the training \DIFdelbegin \DIFdel{required }\DIFdelend \DIFaddbegin \DIFadd{requires }\DIFaddend additional noise on the response firing rate and on the training data\DIFdelbegin \DIFdel{, thus included }\DIFdelend \DIFaddbegin \DIFadd{;
	however, the manually-added noise is far from the actual activity of the network and includes }\DIFaddend hyper-parameters in the model.
	%	What's more, the activation function compromised the modelling accuracy for computational simplicity.

	
	Although \DIFdelbegin \DIFdel{both the }\DIFdelend \DIFaddbegin \DIFadd{the trained weights can be directly used in SNNs since both the above }\DIFaddend LIF response functions \DIFdelbegin \DIFdel{take }\DIFdelend \DIFaddbegin \DIFadd{accept }\DIFaddend and output variables in physical units\DIFdelbegin \DIFdel{such that the trained weights can be directly used in SNNs}\DIFdelend , they struggle in \DIFaddbegin \DIFadd{terms of }\DIFaddend poor modelling accuracy and high \DIFdelbegin \DIFdel{computation complexity.
	Besides, the disadvantage of response functions employing physical units is that }\DIFdelend \DIFaddbegin \DIFadd{computational complexity.
	Moreover, }\DIFaddend they lose the numerical abstraction of firing rates in ANNs, thus\DIFdelbegin \DIFdel{they can be only used for trainingSNNs, meanwhile }\DIFdelend \DIFaddbegin \DIFadd{, being constrained to SNN training.
	Meanwhile, }\DIFaddend other widely-used activation functions \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend ANNs cannot be transformed to \DIFdelbegin \DIFdel{fit in SNN training}\DIFdelend \DIFaddbegin \DIFadd{model SNNs}\DIFaddend .
	Therefore, the first problem \DIFdelbegin \DIFdel{of off-line SNN training }\DIFdelend is the accurate modelling of the neural response activity of LIF neurons	using abstract activation functions, in the hope of (1) increasing the \DIFdelbegin \DIFdel{performance of SNN training}\DIFdelend \DIFaddbegin \DIFadd{modelling accuracy}\DIFaddend , (2) reducing the computation complexity and (3) generalising off-line SNN training to \DIFdelbegin \DIFdel{the way of training ANNs.
	We call activation functions `abstract' referring to the ones }\DIFdelend \DIFaddbegin \DIFadd{commonly-used ANN activation functions.
	These activation functions used in ANNs }\DIFaddend without physical units \DIFdelbegin \DIFdel{which are used in the ANNs, and }\DIFdelend \DIFaddbegin \DIFadd{are called `abstract' to differ from response functions of spiking neurons.
	We }\DIFaddend select them for LIF modelling \DIFdelbegin \DIFdel{due to their }\DIFdelend \DIFaddbegin \DIFadd{and SNN modulations because of the }\DIFaddend simplicity and generalised use for training ANNs.
...
%	Thus, NSP~\citep{liu2016noisy} (Section~\ref{sec:NSP}), we proposed closely match the response activity of LIF neurons by including current noise as the second factor in the activation function and taking account of coloured noise driven by $\tau_{\textit{\textrm{syn}}}$.
	...
	Then the second problem is to map the abstract activation functions to physical units \DIFaddbegin \DIFadd{used in SNNs}\DIFaddend : current in nA and firing rates in Hz\DIFdelbegin \DIFdel{, hence the trained weights of ANNs with such mapped activation functions can be used in }\DIFdelend \DIFaddbegin \DIFadd{.
	In doing so, the neuronal activities of an SNN can be modelled with such scaled activation functions and the trained weights can be transferred into }\DIFaddend SNNs without conversion.
	...
	%trained with noisy input, the 4-layered spiking autoencoder reached 98.37\% accuracy on MNIST. 
	Instead of directly solving this problem, an alternative way is to train an ANN with abstract activation functions and then \DIFdelbegin \DIFdel{convert }\DIFdelend \DIFaddbegin \DIFadd{modulate }\DIFaddend the trained weights to fit in SNNs.
	...
	\DIFdelbegin \DIFdel{Normalising the Rectified Linear Units~(ReLU)-trained weights for use on }\DIFdelend \DIFaddbegin \DIFadd{Nevertheless, these simple IF neurons are usually difficult to implement on analogue circuits, thus only feasible on digital neuromorphic hardware, e.g. TrueNorth~\mbox{%DIFAUXCMD
\citep{merolla2014million}
}%DIFAUXCMD
.
	Tuning these trained ANN models to adapt to }\DIFaddend simplified IF neurons \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend relatively straightforward, so this method sets the state-of-the-art performance.
	However, this chapter (in Section~\ref{sec:genrial_SNN}) aims to address the second problem of mapping abstract activation functions to the response firing activity of biologically-plausible LIF neurons\DIFdelbegin \DIFdel{, thus to complete a formalised SNN training mechanism and to generalise the method to commonly-used }\DIFdelend \DIFaddbegin \DIFadd{.
	Thus, the training can be not only simplified by using conventional }\DIFaddend simple activation functions, such as \DIFdelbegin \DIFdel{ReLU.
	}\DIFdelend \DIFaddbegin \DIFadd{Rectified Linear Units~(ReLUs), but also the method can be generalised to target standard LIF neurons which are supported by most neuromorphic hardware platforms.
	}

	
	
\DIFaddend \section{Modelling the \DIFaddbegin \DIFadd{LIF }\DIFaddend Response Function}
	\label{sec:response_func}
	\DIFaddbegin \DIFadd{The response function, in the context of this thesis, indicates the firing rate of a spiking neuron in the presence of input current.
	In Section~\ref{sec:siergert}, we introduce the first use of Siegert formula to model the response function of an LIF neuron.
	Although, Siegert formula enables the off-line SNN training, it has several drawbacks, see Section~\ref{subsec:practice}.
	...
	We then multiply \DIFdelbegin \DIFdel{the }\DIFdelend both sides of Equation~\ref{equ:LIF_V2} by a short time step $\D t$, the stochastic differential equation of the membrane potential is: % satisfies an Ornstein-Uhlenbeck process:
	\begin{equation}
	\begin{aligned}
	...
	\end{aligned}
	\end{equation}	
	The last term $\D W_t$ is a Wiener process\DIFdelbegin \DIFdel{, that }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend $W_{t +\D t} - W_{t}$ obeys \DIFaddbegin \DIFadd{a }\DIFaddend Gaussian distribution with mean 0 and variance $\D t$.
...
%http://neuronaldynamics.epfl.ch/online/Ch1.S3.html about V tradractory need to write in Chapter 2.	
	The response function~\citep{rauch2003neocortical,la2008response} of the LIF neuron to a noisy current, also known as \DIFaddbegin \DIFadd{the }\DIFaddend Siegert formula, is \DIFdelbegin \DIFdel{driven by the }\DIFdelend \DIFaddbegin \DIFadd{a function of }\DIFaddend $\mu$ and $\sigma$:
	\begin{equation}
	\lambda_\mathit{out}=
	...
	Figure~\ref{Fig:physics} shows the response curves (Equation~\ref{equ:siegert}) of \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend LIF neuron driven by noisy currents with Gaussian noise \DIFdelbegin \DIFdel{of $m_I$ mean and $s_I$ standard deviation }\DIFdelend \DIFaddbegin \DIFadd{with mean $m_I$ and standard deviation $s_I$}\DIFaddend .
	...
	Softplus units only represent a single level of noise\DIFdelbegin \DIFdel{that, }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend for example, the dotted line in Figure~\ref{Fig:physics} \DIFdelbegin \DIFdel{is drawn }\DIFdelend \DIFaddbegin \DIFadd{shows the curve }\DIFaddend when $s_I=1$.
...
%	\end{itemize}  
	...
	\subsection{Mismatch of \DIFaddbegin \DIFadd{The }\DIFaddend Siegert Function to Practice}
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Current sampled at $dt$=1~ms.}}
		%DIFAUXCMD
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Current sampled at $dt$=10~ms.}}
		%DIFAUXCMD
%DIFDELCMD < \end{subfigure}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Recorded response firing rate of a LIF neuron driven by }\textit{\DIFdel{NoisyCurrentSource}} %DIFAUXCMD
\DIFdel{sampled at every (a) 1~ms and (b) 10~ms. Averaged firing rates of 10 simulation trails are shown in bold lines, and the grey colour fills the range between the minimum to maximum of the firing rates. The analytical LIF response function, Siegert formula (Equation~\ref{equ:siegert}), is drawn in thin lines (shown in Figure~\ref{Fig:physics}) to compare with the practical simulation.}}
		%DIFAUXCMD
%DIFDELCMD < \label{Fig:current}
...
%DIFDELCMD < 		\centering
%DIFDELCMD < 		%%%
\DIFdel{\makebox[0.43\textwidth]{$dt$=1~ms} \makebox[0.43\textwidth]{$dt$=10~ms} }%DIFDELCMD < \par
%DIFDELCMD < 		\begin{subfigure}[t]{0.43\textwidth}
%DIFDELCMD < 			\includegraphics[width=\textwidth]{pics_iconip/curr_dt1.pdf}
%DIFDELCMD < 			%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Current sampled at $dt$=1~ms.}}
...
%DIFDELCMD < \end{subfigure}
%DIFDELCMD < 		\begin{subfigure}[t]{0.43\textwidth}
%DIFDELCMD < 			\includegraphics[width=\textwidth]{pics_iconip/curr_dt10.pdf}
%DIFDELCMD < 			%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Current sampled at $dt$=10~ms.}}
...
%DIFDELCMD < \end{subfigure}\\
%DIFDELCMD < 		\begin{subfigure}[t]{0.43\textwidth}
%DIFDELCMD < 			\includegraphics[width=\textwidth]{pics_iconip/psd_dt1.pdf}
%DIFDELCMD < 			%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Spectrum analysis of (a).}}
		%DIFAUXCMD
%DIFDELCMD < \end{subfigure}
%DIFDELCMD < 		\begin{subfigure}[t]{0.43\textwidth}
%DIFDELCMD < 			\includegraphics[width=\textwidth]{pics_iconip/psd_dt10.pdf}
%DIFDELCMD < 			%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Spectrum analysis of (b).}}
		%DIFAUXCMD
%DIFDELCMD < \end{subfigure}\\
%DIFDELCMD < 		\begin{subfigure}[t]{0.43\textwidth}
%DIFDELCMD < 			\includegraphics[width=\textwidth]{pics_iconip/autocorr_dt1.pdf}
%DIFDELCMD < 			%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Autocorrelation of (a).}}
		%DIFAUXCMD
%DIFDELCMD < \end{subfigure}
%DIFDELCMD < 		\begin{subfigure}[t]{0.43\textwidth}
%DIFDELCMD < 			\includegraphics[width=\textwidth]{pics_iconip/autocorr_dt10.pdf}
%DIFDELCMD < 			%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Autocorrelation of (b).}}
		%DIFAUXCMD
%DIFDELCMD < \end{subfigure}\\
%DIFDELCMD < 		\begin{subfigure}[t]{0.43\textwidth}
%DIFDELCMD < 			\includegraphics[width=\textwidth]{pics_iconip/distr_dt1.pdf}
%DIFDELCMD < 			%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Distribution of samples of (a).}}
		%DIFAUXCMD
%DIFDELCMD < \end{subfigure}
%DIFDELCMD < 		\begin{subfigure}[t]{0.43\textwidth}
%DIFDELCMD < 			\includegraphics[width=\textwidth]{pics_iconip/distr_dt10.pdf}
%DIFDELCMD < 			%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Distribution of samples of (b).}}
		%DIFAUXCMD
%DIFDELCMD < \end{subfigure}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textit{\DIFdel{NoisyCurrentSource}} %DIFAUXCMD
\DIFdel{samples noisy currents from Gauss distribution at every 1~ms (left) and 10~ms (right). The signals are shown in time domain in (a) and (b), and spectrum domain in (c) and (d). The autocorrelation of both current signals are shown in (e) and (f). The distribution of the discrete samples are plotted in bar chart to compare with PDF of the original Gauss distribution, shown in (g) and (h).}}
		%DIFAUXCMD
%DIFDELCMD < \label{Fig:lif_curr}
%DIFDELCMD < 	\end{figure}
%DIFDELCMD < %%%
...
%\end{figure}
	...
	To verify the \DIFdelbegin \DIFdel{response }\DIFdelend \DIFaddbegin \DIFadd{Siegert }\DIFaddend function in practice, simulation tests were carried out using PyNN~\citep{davison2008pynn} to compare \DIFaddbegin \DIFadd{the reality }\DIFaddend with the analytical results~(the Siegert function).
	...
	\DIFaddbegin \begin{figure}[tbp!]
		...
		\begin{subfigure}[t]{0.49\textwidth}
			...
			\caption{\DIFaddFL{Current sampled at $dt$=1~ms.}}
		\end{subfigure}
		\begin{subfigure}[t]{0.49\textwidth}
			\includegraphics[width=\textwidth]{pics_iconip/2-10.png}
			\caption{\DIFaddFL{Current sampled at $dt$=10~ms.}}
		\end{subfigure}
		\caption{\DIFaddFL{Recorded response firing rate of an LIF neuron driven by }\textit{\DIFaddFL{NoisyCurrentSource}} \DIFaddFL{sampled at every (a) 1~ms and (b) 10~ms. Averaged firing rates of 10 simulation trails are shown in bold lines, and the grey colour fills the range between the minimum to maximum of the firing rates. The analytical LIF response function, the Siegert formula (Equation~\ref{equ:siegert}), is drawn in thin lines (shown in Figure~\ref{Fig:physics}) to compare with the practical simulation.}}
		\label{Fig:current}
	\end{figure}
	...
	\begin{figure}[tbp!]
		\centering
		\DIFaddFL{\makebox[0.43\textwidth]{$dt$=1~ms} \makebox[0.43\textwidth]{$dt$=10~ms} }\par
		...
		\end{subfigure}
		\caption{\textit{\DIFaddFL{NoisyCurrentSource}} \DIFaddFL{samples noisy currents from a Gauss distribution every 1~ms (left) and 10~ms (right). The signals are shown in the time domain in (a) and (b), and in the spectrum domain in (c) and (d). The autocorrelation of both current signals are shown in (e) and (f). The distribution of the discrete samples are plotted in bar charts to compare with the PDF of the original Gauss distribution, shown in (g) and (h).}}
		\label{Fig:lif_curr}
	\end{figure}

	\DIFaddend The curves in Figures~\ref{Fig:current} illustrate \DIFaddbegin \DIFadd{the }\DIFaddend output firing rate driven by different \DIFdelbegin \DIFdel{level }\DIFdelend \DIFaddbegin \DIFadd{levels }\DIFaddend of noise.
	...
	Although both current signals \DIFdelbegin \DIFdel{followed }\DIFdelend \DIFaddbegin \DIFadd{follow }\DIFaddend the same Gaussian distribution, see Figures~\ref{Fig:lif_curr}~(g) and (h), the current is a white noise when $dt=1$~ms, but a coloured noise, e.g. increased Power Spectral Density~(PSD) at lower frequency, when $dt=10$~ms, see Figures~\ref{Fig:lif_curr}~(c) and (d).
...
%\end{figure}
	...
	\begin{figure}[tbp!]
		...
		\begin{subfigure}[t]{0.49\textwidth}
			...
			\caption{$\tau_{syn}$=10~ms.}
		\end{subfigure}
		\caption{Recorded response firing rate of \DIFdelbeginFL \DIFdelFL{a }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{an }\DIFaddendFL LIF neuron driven by \DIFaddbeginFL \DIFaddFL{a }\DIFaddendFL synaptic current with two synaptic time constants: (a) $\tau_{syn}$=10~ms and (b) $\tau_{syn}$=10~ms. \DIFdelbeginFL \DIFdelFL{Averaged }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Average }\DIFaddendFL firing rates of 10 simulation trails are shown in bold lines, and the grey colour fills the range between the minimum to maximum of the firing rates. The firing rates recorded driven by \textit{NoisyCurrentSource} \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL are drawn in thin lines (shown in Figure~\ref{Fig:lif_curr}) \DIFdelbeginFL \DIFdelFL{to compare with}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for comparison}\DIFaddendFL .}
	...
	\begin{figure}[tbp!]
		...
		\end{subfigure}
		\caption{Noisy currents generated by 100 Poinsson spike trains to \DIFdelbeginFL \DIFdelFL{a }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{an }\DIFaddendFL LIF neuron with synaptic time constant $\tau_{syn}$=1~ms (left) and $\tau_{syn}$=10~ms (right). The currents are shown in \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL time domain in (a) and (b), and \DIFaddbeginFL \DIFaddFL{in the }\DIFaddendFL spectrum domain in (c) and (d). The autocorrelation of both current signals are shown in (e) and (f). The distribution of the generated samples are plotted in bar chart \DIFaddbeginFL \DIFaddFL{form }\DIFaddendFL to compare to the expected Gauss distribution, shown in (g) and (h).}
		\label{Fig:lif_pois}
	\end{figure}
	...
	\DIFdelbegin \DIFdel{Figures~\ref{Fig:spike_curr} illustrate }\DIFdelend \DIFaddbegin \DIFadd{Figure~\ref{Fig:spike_curr} illustrates }\DIFaddend the recorded firing rates responding to the Poissoin spike trains compared to the mean firing rate driven by \textit{NoisyCurrentSource} in Figure~\ref{Fig:current}.
...
%	The use of noisy currents assumes that the post-synaptic potential (PSP) is a delta function, e.g. $\tau_{syn}$ tends to the limits of 0.
	...
	Therefore, the current at any time $t$ during \DIFdelbegin \DIFdel{decaying period is dependant to }\DIFdelend \DIFaddbegin \DIFadd{the decay period is dependent on }\DIFaddend the value at \DIFaddbegin \DIFadd{the }\DIFaddend previous time step, which makes the synaptic current a coloured noise, see Figures~\ref{Fig:lif_pois}~(c) and (d).
%	and the noise added to the mean current is not white noise, but pink noise.
	...
	and the $\tau_{syn}$, even \DIFaddbegin \DIFadd{when as }\DIFaddend short as 1~ms, adds coloured noise instead of white noise to the current.
	However, Figure~\DIFdelbegin \DIFdel{\ref{Fig:lif_pois}~(h}\DIFdelend \DIFaddbegin \DIFadd{\ref{Fig:spike_curr}~(b}\DIFaddend ) shows a similar firing rate of both the synaptic driven current and the \textit{NoisyCurrentSource}, since both of the current signals have similar distribution \DIFaddbegin \DIFadd{(Figure~\ref{Fig:lif_pois}~(h)) }\DIFaddend and time correlation \DIFaddbegin \DIFadd{(Figure~\ref{Fig:lif_pois}~(f))}\DIFaddend .
	Nevertheless, the analytical response function, \DIFaddbegin \DIFadd{the }\DIFaddend Siegert formula, cannot approximate either of the practical simulations.
...
%	Thus the experiments show that a longer $\tau_{syn}$ increases the level of noise and widens the variance of the output firing rate.
		Although the use of \DIFaddbegin \DIFadd{the }\DIFaddend Siegert function opened the door for modelling \DIFdelbegin \DIFdel{response activities of LIF neurons as activation function }\DIFdelend \DIFaddbegin \DIFadd{LIF response function to work similarly to activation functions }\DIFaddend used in ANNs~\citep{Jug_etal_2012}, there are several drawbacks of this method:
	\begin{itemize}
		\item most importantly, the numerical analysis \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend an LIF response function is far from accurate in practice. `Practice' in the chapter means SNN simulations using LIF neurons.
		Thus the inaccurate model generates \DIFdelbegin \DIFdel{error }\DIFdelend \DIFaddbegin \DIFadd{errors }\DIFaddend between the estimation and the practical response firing rate.
%		This issue will be addressed in Section~\ref{subsec:practice}.

		
		\item the high complexity of the Siegert function causes much longer training \DIFdelbegin \DIFdel{time }\DIFdelend \DIFaddbegin \DIFadd{times }\DIFaddend and more energy, let alone the high-cost computation on its derivative.
		...
		\item \DIFaddbegin \DIFadd{The }\DIFaddend Siegert function is used to replace sigmoid functions for training spiking Restricted Boltzmann Machines~\citep{Jug_etal_2012}\DIFdelbegin \DIFdel{, thus }\DIFdelend \DIFaddbegin \DIFadd{.
		Therefore }\DIFaddend neurons have to fire at high frequency (higher than half of the maximum firing rate) to represent the activation of a sigmoid unit\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{; thus }\DIFaddend the network activity results in high power dissipation.
		\item better learning performance has been reported using ReLU \DIFaddbegin \DIFadd{than Sigmoid units}\DIFaddend , so modelling \DIFaddbegin \DIFadd{a }\DIFaddend ReLU-like activation function for spiking neurons is needed.  
	\end{itemize}

	Therefore, we propose the NSP function which provides solutions to the drawbacks of \DIFaddbegin \DIFadd{the }\DIFaddend Siegert unit.
...
%	However, such a derivative function of low complexity does not present in Siegert function.
	Due to the limited time resolution of common SNN simulators and the time taken for neurotransmitter release, $\tau_{syn}$, mismatches exist between the analytical response function, the Siegert formula, and practical neural activities.
	Consequently to model the practical \DIFdelbegin \DIFdel{responses of LIF neurons }\DIFdelend \DIFaddbegin \DIFadd{LIF response function }\DIFaddend (see Figure~\ref{fig:nsp}(a)) whose output firing rates are determined by \DIFaddbegin \DIFadd{both }\DIFaddend the mean and variance of the noisy input currents, the NSP was proposed \DIFaddbegin \DIFadd{as follows}\DIFaddend :
	\begin{equation}
	y = f_{ns}(x, \sigma) = k \sigma \log [1 + \exp(\frac{x}{k \sigma})]~,
	\label{equ:nsp}
	\end{equation}
	...
	\DIFdelbegin %DIFDELCMD < \begin{figure}[thb!]
%DIFDELCMD < 		%%%
\DIFdelend \DIFaddbegin \begin{figure}[tbh!]
		...
		\caption{
			...
			(a) Firing rates measured by simulations \DIFdelbeginFL \DIFdelFL{on }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{of }\DIFaddendFL an LIF neuron driven by different input currents and discrete noise levels.
	...
	Since the NSP takes two variables as inputs, the activation function can be plotted in 3D, see Figure\DIFaddbegin \DIFadd{~\ref{Fig:NSP3D}.
	}\DIFaddend 
...
\label{sec:genrial_SNN}
	%DIF < 	We have discussed modelling response firing activity of a LIF neuron with a unified activation function, Noisy Softplus.
%DIF < 	However the demonstration of mapping the physical activity to numerical ANN calculations is still required to train the layered-up deep network.
\DIFaddbegin \DIFadd{We have discussed modelling the response firing activity of a LIF neuron with an abstract activation function, NSP.
	However, before we can replace an LIF neuron with an abstract activation function, it still requires to map the numerical values of NSP to physical variables in SNNs.
...
%	The result of fine tuning on a ConvNet will be demonstrated in Section~\ref{subsec:result_compare}.
	...
	Suitable calibrations of $k$ and $S$ in Equation~\ref{equ:fit} \DIFdelbegin \DIFdel{enables }\DIFdelend \DIFaddbegin \DIFadd{enable }\DIFaddend NSP to closely match the practical response firing rates of LIF neurons given various biological parameters.
	...
	The output firing rate $\lambda_{\textit{\textrm{out}}}$ is measured from SNN simulations where an LIF neuron is driven by synaptic input \DIFdelbegin \DIFdel{current }\DIFdelend \DIFaddbegin \DIFadd{currents }\DIFaddend of Poisson spike trains.
	...
	\DIFaddbegin \label{subsec:PAF}
	\DIFaddend \begin{figure}[tbh!]
		\centering
		\DIFdelbeginFL %DIFDELCMD < \begin{subfigure}[t]{0.7\textwidth}
%DIFDELCMD < 			\includegraphics[width=\textwidth]{pics_iconip/neuron_o.pdf}
%DIFDELCMD < 			%%%
\DIFdelendFL \DIFaddbeginFL \begin{subfigure}[t]{\textwidth}
			\centering
			\includegraphics[width=0.8\textwidth]{pics_iconip/neuron_o.pdf}
			\DIFaddendFL \caption{An artificial spiking neuron modelled by NSP\DIFaddbeginFL \DIFaddFL{, where firing rates are the input and output of the neuron. NSP transforms the noisy current influx including both the mean ($net_j$, solid lines) and the variance $\sigma^2_j$ (dashed lines) to the abstract firing rate $y_j$}\DIFaddendFL .}
		\end{subfigure}\\\DIFdelbeginFL %DIFDELCMD < \begin{subfigure}[t]{0.7\textwidth}
%DIFDELCMD < 			\includegraphics[width=\textwidth]{pics_iconip/neuron_PAF.pdf}
%DIFDELCMD < 			%%%
\DIFdelendFL \DIFaddbeginFL \bigskip
		\begin{subfigure}[t]{\textwidth}
			\centering
			\includegraphics[width=0.7\textwidth]{pics_iconip/neuron_PAF.pdf}
			\DIFaddendFL \caption{An artificial spiking neuron modelled by PAF-NSP\DIFaddbeginFL \DIFaddFL{, whose input and output are numerical values, equivalent to those of ANNs. PAF includes the scaling factors $S$ and the synaptic time constant $\tau_{syn}$ in the combined activation function}\DIFaddendFL .}
		\end{subfigure}\DIFaddbeginFL \\\bigskip
		\begin{subfigure}[t]{\textwidth}
			\centering
			\includegraphics[width=0.55\textwidth]{pics_iconip/neuron.pdf}
			\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{The PAF links the firing activity }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{A copy }\DIFaddendFL of \DIFaddbeginFL \DIFaddFL{Figure~\ref{Fig:neuron_net} showing }\DIFaddendFL a \DIFdelbeginFL \DIFdelFL{spiking }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{general artificial }\DIFaddendFL neuron \DIFdelbeginFL \DIFdelFL{to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{where an activation function transforms }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{numerical value of ANNs}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{weighted sum $net_j$  to its outcome $y_j$}\DIFaddendFL .}
		\DIFaddbeginFL \end{subfigure}
		\caption{\DIFaddFL{Comparisons of three activation functions using in ANNs: (a) the NSP, (b) the PAF-NSP, and (c) a conventional ANN activation function. The PAF links the firing activity of a spiking neuron to the numerical value of ANNs.}}
		\DIFaddendFL \label{Fig:tneuron}
	\end{figure}

	Neurons in ANNs take inputs from their previous layer, and feed the weighted sum of their input, $net_j = \sum_i w_{ij}x_i$, to the activation function.
	The transformed signal then forms the output of an artificial neuron, which can be denoted as $y_j=f(net_j)$, see Figure~\DIFdelbegin \DIFdel{\ref{Fig:compare_as}(a}\DIFdelend \DIFaddbegin \DIFadd{\ref{Fig:tneuron}(c}\DIFaddend ).
	...
	If instead of multiplying every input by $\tau_{\textit{\textrm{syn}}}$ (left of Fig.~\ref{Fig:tneuron}(a)) we do it in every output after $\lambda_j$, (right of Fig.~\ref{Fig:tneuron}(b)) we obtain the same neuron model and structure as a typical neuron in ANNs, \DIFdelbegin \DIFdel{See Figure~\ref{Fig:compare_as}(a}\DIFdelend \DIFaddbegin \DIFadd{see Figure~\ref{Fig:tneuron}(c}\DIFaddend ), that neurons take $x$ as input and output $y$.
	...
	Excitingly, PAF not only allows NSP to model spiking LIF neurons on ANNs\DIFdelbegin \DIFdel{, once }\DIFdelend \DIFaddbegin \DIFadd{.
	Once }\DIFaddend the parameter $p$ is acquired the PAF can be generalised to other \DIFaddbegin \DIFadd{Relu-like }\DIFaddend activation functions.
	\DIFaddbegin \DIFadd{Because Softplus and NSP both will converge to ReLU, when the input increases, so they can share the scaling factor $p$.
	We will compare these ReLU-like functions in modelling LIF neurons in Section~\ref{subsec:compareAF}.
	}\DIFaddend Note that the calculation of noise level is not necessary for other activation functions, \DIFdelbegin \DIFdel{for example, it }\DIFdelend \DIFaddbegin \DIFadd{thus they only take the mean of the current influx as the input (the solid lines in Figure~\ref{Fig:tneuron}).
	So the noise level }\DIFaddend can be set to a constant for Softplus or \DIFaddbegin \DIFadd{considered as }\DIFaddend 0 for ReLU.
	\DIFaddbegin 

	\DIFaddend \subsection{\DIFdelbegin \DIFdel{Generalised SNN }\DIFdelend Training \DIFaddbegin \DIFadd{Method}\DIFaddend }
	\label{subsec:ns_train}

	The simple \DIFdelbegin \DIFdel{change on activation functions }\DIFdelend \DIFaddbegin \DIFadd{idea of PAF }\DIFaddend presented in the previous section allows the use of common ANN training methods to obtain SNN-compatible weights.
	Consequently, training SNNs can be done in three simple steps: 
	\begin{enumerate}
		\item Calibrate the parameters $(k, S)$ for Noisy Softplus which models \DIFaddbegin \DIFadd{the }\DIFaddend response firing rates of LIF neurons, thus to estimate \DIFaddbegin \DIFadd{the }\DIFaddend parameter $p=S \times \tau_{\textit{\textrm{syn}}}$ for PAFs. Since $(k, S)$ are solely dependent on the biological configurations of an LIF neuron, the same $p$ can be \DIFdelbegin \DIFdel{used for }\DIFdelend \DIFaddbegin \DIFadd{shared with }\DIFaddend different activation functions and \DIFdelbegin \DIFdel{network architectures}\DIFdelend \DIFaddbegin \DIFadd{repeatedly used for various network architectures and applications}\DIFaddend .
		\item Train any feed-forward ANN with a PAF version of \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{a ReLU-like }\DIFaddend activation function.
	...
	\subsection{Fine Tuning}
	\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \label{subsec:finetune}
	\DIFadd{As stated above, we }\DIFaddend can train the network with any PAF \DIFdelbegin \DIFdel{as stated above}\DIFdelend \DIFaddbegin \DIFadd{version of conventional ReLU-like activation functions}\DIFaddend , and then fine-tune it with PAF-NSP in the hope of improving the performance of the equivalent SNN by closely modelling the spiking neurons with NSP.
	...
	Although binary labels \DIFdelbegin \DIFdel{enlarges }\DIFdelend \DIFaddbegin \DIFadd{enlarge }\DIFaddend the disparities between the correct recognition label and the rest for better classification capability, 
	spiking neurons seldom stay silent even with negative current influx, thus setting labels to 0 \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend not practical for training SNNs.
	...
	Firstly, using the NSP activation functions causes every single neuron to run \DIFdelbegin \DIFdel{as }\DIFdelend \DIFaddbegin \DIFadd{at }\DIFaddend a similar noise level as in SNNs, thus the weights trained by other activation functions will be tuned to fit closer to SNNs.
	Secondly, the output firing rate of any LIF neuron is greater than zero as long as noise exists in their synaptic input.
	Thus adding \DIFdelbegin \DIFdel{up }\DIFdelend a small offset on the labels directs the model to approximate practical SNNs. 
	The result of fine tuning on a ConvNet will be demonstrated in Section~\ref{subsec:result_compare}.

\section{Results}
\label{sec:iconipResult}
	\DIFaddbegin \DIFadd{Finally, the proposed generalised SNN training method is put into practice.
	...
	\DIFaddend % Architecture
	A spiking ConvNet was trained on \DIFaddbegin \DIFadd{the }\DIFaddend MNIST~\citep{lecun1998gradient} dataset, 
...
%	\end{figure}
	...
	\subsection{\DIFdelbegin \DIFdel{Neural }\DIFdelend \DIFaddbegin \DIFadd{Single Neuronal }\DIFaddend Activity}
	\DIFaddbegin \label{subsec:compareAF}
\DIFaddend %	To validate how well the Noisy Softplus activation fits to the response firing rate of LIF neurons in a real application, we simulated the model on NEST~\citep{gewaltig2007nest} using the Poisson MNIST dataset~\citep{liu2016bench} and the neurons of a convolutional map were observed.
	To validate how well the NSP activation fits \DIFdelbegin \DIFdel{to }\DIFdelend the response firing rate of LIF neurons in SNNs, we simulated one of the PAF-NSP trained ConvNets on NEST.
	...
	Ten testing images were convolved with a trained $5\times5$ kernel, and the output firing rates of the spiking neurons were recorded, see Figure~\ref{fig:cnn}\DIFaddbegin \DIFadd{, }\DIFaddend and compared to the predictions of these PAFs: $\lambda' = S \times f(x) = y/\tau_{\textit{\textrm{syn}}}$, see Figure~\ref{fig:af_compare}.
		...
		\begin{subfigure}[t]{0.3\textwidth}
			...
			\label{Fig:64}
		\end{subfigure}\\
		\DIFdelbeginFL %DIFDELCMD < \begin{subfigure}[t]{0.5\textwidth}
%DIFDELCMD < 			\includegraphics[width=\textwidth]{pics_iconip/7.pdf}
%DIFDELCMD < 			%%%
\DIFdelendFL \DIFaddbeginFL \begin{subfigure}[t]{\textwidth}
			\centering
			\includegraphics[width=0.5\textwidth]{pics_iconip/7.pdf}
			\DIFaddendFL \caption{Output firing rates of classification neurons.}
		\end{subfigure}
...
%	Hence, the firing rate driven by lower noise level is closer to ReLU curve than Softplus.
	...
	The Euclidean distances, $\sqrt{\sum_{j}(\lambda_j' - \lambda_j)^2}$, between the spike counts and the \DIFdelbegin \DIFdel{predicted firing rates }\DIFdelend \DIFaddbegin \DIFadd{firing rates predicted }\DIFaddend by NSP, ReLU and Softplus were 184.57, 361.64 and 1102.76 respectively.
	We manually selected a static noise level of 0.45 for Softplus, whose estimated firing rates located roughly on the top slope of the real response activity.
	...
	Hence, the firing rate driven by the lower noise level is closer to \DIFaddbegin \DIFadd{the }\DIFaddend ReLU curve than \DIFaddbegin \DIFadd{to }\DIFaddend Softplus.
...
%	However, the loss stabilised fastest, which means a possible shorter training time.
	...
	ReLU learned fastest with the lowest loss, thanks to its steepest derivative.
	In comparison, Softplus accumulated spontaneous `firing rates' layer-by-layer and its derivative may experience gradual or even vanishing gradients during back propagation, which result in \DIFdelbegin \DIFdel{a }\DIFdelend more difficult training.
	The performance of NSP \DIFdelbegin \DIFdel{lied }\DIFdelend \DIFaddbegin \DIFadd{lay }\DIFaddend between these two in terms of loss and learning speed.
	The loss stabilised to the same level \DIFdelbegin \DIFdel{of the Softplus'}\DIFdelend \DIFaddbegin \DIFadd{as Softplus}\DIFaddend , because of the same problem of gradual gradients.
	\DIFdelbegin \DIFdel{However, it stabilised faster thanks to }\DIFdelend \DIFaddbegin 

	\DIFadd{However, the learning stabilised the fastest using NSP which may be caused by }\DIFaddend the accurate modelling of the noise\DIFdelbegin \DIFdel{, which means a possibly shorter }\DIFdelend \DIFaddbegin \DIFadd{.
	Since similar findings have shown that networks with added noise, e.g. dropout~\mbox{%DIFAUXCMD
\cite{srivastava2014dropout}
}%DIFAUXCMD
, are much faster to train. 
	The result is promising that NSP may shorten the }\DIFaddend training time.
	\begin{figure}[tbp!]
		\centering
...
\DIFaddbegin \label{subsec:result_compare_rec}
	\DIFaddend \begin{figure}[tbp!]
		...
		\caption{Classification accuracy.
			The trained weights were tested using the same activation function as training (DNN\_Orig), then transferred to \DIFaddbeginFL \DIFaddFL{an }\DIFaddendFL SNN and tested on NEST simulation (SNN\_Orig), and finally fine-tuned to be tested on SNN (SNN\_FT) as well.  }
	...
	The best classification accuracy achieved by a larger spiking ConvNet~(\DIFdelbegin \DIFdel{784-16c-16p-64c-64c-10fc}\DIFdelend \DIFaddbegin \DIFadd{784-16c-16p-64c-64p-10fc}\DIFaddend ) was 99.07\% after fine tuning, a 0.14\% drop from the ANN test (99.21\%).
	...
	\DIFaddbegin \subsubsection{\DIFadd{Comparisons in Literature}}
	\DIFaddend It is useful to compare with existing SNN training methods shown in Table~\ref{tbl:compare_paf} where we order them on the computation complexity (descending).
	...
	The generalised training method presented \DIFdelbegin \DIFdel{in this paper has a simple activation function, }\DIFdelend \DIFaddbegin \DIFadd{here uses simple abstract activation functions, e.g. PAF-ReLU;
	}\DIFaddend it requires no \DIFdelbegin \DIFdel{conversions }\DIFdelend \DIFaddbegin \DIFadd{modulations }\DIFaddend of trained weights \DIFdelbegin \DIFdel{, which are well fitted to biologically-plausible LIF neurons, and has }\DIFdelend \DIFaddbegin \DIFadd{to adapt to SNNs, but uses }\DIFaddend a single \emph{optional} additional processing of fine tuning\DIFaddbegin \DIFadd{;
	...
	Regarding the classification accuracy, it achieves the state-of-the-art performance of SNNs and wins all the other methods using LIF neurons}\DIFaddend .
...
\end{sidewaystable*}
	...
	\DIFaddbegin \subsubsection{\DIFadd{Recognition Time}}
...
\section{Summary}
%\section{Discussions}
	We presented a generalised \DIFaddbegin \DIFadd{off-line }\DIFaddend SNN training method to \DIFdelbegin \DIFdel{train an equivalent ANN and transfer the trained weights back to SNNs .
	}\DIFdelend \DIFaddbegin \DIFadd{tackle the research problem of equipping SNNs with equivalent cognitive capability as ANNs.
%DIF > 	train an equivalent ANN and transfer the trained weights back to SNNs.
	}\DIFaddend This training procedure consists of \DIFdelbegin \DIFdel{two }\DIFdelend \DIFaddbegin \DIFadd{three }\DIFaddend simple stages: first, estimate parameter $p$ for PAF using NSP\DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend second, use a PAF version of conventional activation functions for ANN training\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend % can be generalised to activation units other than NSP.
	%The training of a SNN model is exactly the same as ANN training, and 
	\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{third, the }\DIFaddend trained weights can be directly \DIFdelbegin \DIFdel{used in }\DIFdelend \DIFaddbegin \DIFadd{transferred to the }\DIFaddend SNN without any further transformation.
	\DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{Regarding the generalisation, the training not only uses popular activation functions in ANNs, but also targets standard LIF neurons for the purpose of widely use on neuromorphic hardware. 
	Furthermore, this }\DIFaddend method requires the least computation complexity while performing most effectively among existing algorithms.
