...
In the last chapter, we \DIFdelbegin \DIFdel{have illustrated }\DIFdelend \DIFaddbegin \DIFadd{described }\DIFaddend the promising Deep Learning research in the field of artificial neural networks~(ANNs). 
...
Noisy Softplus~(NSP), which closely models the \DIFdelbegin \DIFdel{response }\DIFdelend firing activity of Leaky Integrate-and-Fire~(LIF) neurons.
...
With these problems solved, SNN training can be simplified as: (1) estimate parameter $p$ for \DIFaddbegin \DIFadd{the }\DIFaddend PAF according to biological configurations of LIF neurons, and (2) use \DIFaddbegin \DIFadd{the }\DIFaddend PAF instead of conventional activation functions to train an equivalent ANN.
The trained weights can be transferred directly into the spiking version of the same network without further conversion.
In addition, we propose a fine tuning method as an option which helps the trained network to \DIFdelbegin \DIFdel{closer }\DIFdelend match the SNN \DIFaddbegin \DIFadd{closer}\DIFaddend .
...
In this chapter, we begin by introducing existing solutions \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend off-line SNN training in Section~\ref{sec:intro_NSP}.
...
%DIF < Jug et al.~\cite{Jug_etal_2012} first proposed the use of the Siegert function to replace the sigmoid activation function in training Restricted Boltzmann Machine (RBM).
%DIF > Jug et al.~\citep{Jug_etal_2012} first proposed the use of the Siegert function to replace the sigmoid activation function in training Restricted Boltzmann Machine (RBM).
...
%DIF < A spiking Deep Belief Network (DBN)~\cite{Stromatias2015scalable} of four layers of RBMs was implemented on neuromorphic hardware, SpiNNaker~\cite{furber2014spinnaker}, to recognise hand written digits in real time.
%DIF > A spiking Deep Belief Network (DBN)~\citep{Stromatias2015scalable} of four layers of RBMs was implemented on neuromorphic hardware, SpiNNaker~\citep{furber2014spinnaker}, to recognise hand written digits in real time.
%
%However, cortical neurons seldom saturate their firing rate as sigmoid neurons.
%DIF < Thus ReLU were proposed to replace sigmoid neurons and surpassed the performance of other popular activation units thanks to their advantage of sparsity~\cite{glorot2011deep} and robustness towards the vanishing gradient problem. % which will be described in Section~\ref{sec:activation}.
%DIF > Thus ReLU were proposed to replace sigmoid neurons and surpassed the performance of other popular activation units thanks to their advantage of sparsity~\citep{glorot2011deep} and robustness towards the vanishing gradient problem. % which will be described in Section~\ref{sec:activation}.
%%TODO Chapter2 about relu and vanishing gradient problem.
%DIF < %	~\cite{dugas2001incorporating}.
%DIF > %	~\citep{dugas2001incorporating}.
...
%DIF < Better performance~\cite{cao2015spiking,diehl2015fast} than Siegert-trained RBM has been demonstrated in Spiking ConvNets, but this method is designed for simple integrate and fire (IF) neurons without leakage and requires extra step for converting ANN-trained weights for SNN use.
%DIF < The training used only ReLUs and zero bias to avoid negative outputs, and applied a deep learning technique, dropout~\cite{srivastava2014dropout}, to increase the classification accuracy.
%DIF > Better performance~\citep{cao2015spiking,diehl2015fast} than Siegert-trained RBM has been demonstrated in Spiking ConvNets, but this method is designed for simple integrate and fire (IF) neurons without leakage and requires extra step for converting ANN-trained weights for SNN use.
%DIF > The training used only ReLUs and zero bias to avoid negative outputs, and applied a deep learning technique, dropout~\citep{srivastava2014dropout}, to increase the classification accuracy.
...
%DIF < This work was extended to a Recursive Neural Network (RNN)~\cite{diehl2016conversion} and run on the TrueNorth~\cite{merolla2014million} neuromorphic hardware platform.
%DIF > This work was extended to a Recursive Neural Network (RNN)~\citep{diehl2016conversion} and run on the TrueNorth~\citep{merolla2014million} neuromorphic hardware platform.
%
...
%DIF < Recent work~\cite{hunsberger2015spiking} proposed the Soft LIF response function for training SNNs, which is equivalent to Softplus activation of ANNs.
%DIF > Recent work~\citep{hunsberger2015spiking} proposed the Soft LIF response function for training SNNs, which is equivalent to Softplus activation of ANNs.
%	
%\section{Modelling The Activation Function}
%\label{sec:af_model}
%DIF < We have introduced the existing work of modelling the response firing activity of LIF neurons using Siegert~\cite{Jug_etal_2012} function.
%DIF > We have introduced the existing work of modelling the response firing activity of LIF neurons using Siegert~\citep{Jug_etal_2012} function.
...
%DIF < 	Advances in computing power and deep learning have benefited computers with a rapidly growing performance in cognitive tasks, such as recognising objects~\cite{deng2009imagenet} and playing GO~\cite{silver2016mastering}. 
%DIF > 	Advances in computing power and deep learning have benefited computers with a rapidly growing performance in cognitive tasks, such as recognising objects~\citep{deng2009imagenet} and playing GO~\citep{silver2016mastering}. 
...
%DIF < 	For instance, AlphaGO~\cite{silver2016mastering} consumed 1~MW of power on its 1920 CPUs and 280 GPUs when playing the game with one of the best human players whose brain merely consumed about 20~W.
%DIF > 	For instance, AlphaGO~\citep{silver2016mastering} consumed 1~MW of power on its 1920 CPUs and 280 GPUs when playing the game with one of the best human players whose brain merely consumed about 20~W.
%	
...
%DIF < 	Computers employ Boolean logic and deterministic digital operations based usually on synchronous clocks while nervous systems employ parallel-distributed, event-driven, stochastically unreliable components~\cite{indiveri2009artificial}, neurons.
%DIF < 	The impressive disparities in cognitive capabilities and energy consumption drives the research into biologically-plausible spiking neurons and brain inspired computers, known as neuromorphic engineering~\cite{furber2016large}.
	%DIF > 	Computers employ Boolean logic and deterministic digital operations based usually on synchronous clocks while nervous systems employ parallel-distributed, event-driven, stochastically unreliable components~\citep{indiveri2009artificial}, neurons.
%DIF > 	The impressive disparities in cognitive capabilities and energy consumption drives the research into biologically-plausible spiking neurons and brain inspired computers, known as neuromorphic engineering~\citep{furber2016large}.
...
%	An intuitive idea is to train SNNs on an equivalent ANN and then transfer the trained weights to the SNN.
	...
	Jug et al.~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Jug_etal_2012}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Jug_etal_2012}
}%DIFAUXCMD
}\DIFaddend first proposed the use of the Siegert formula~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{siegert1951first}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{siegert1951first}
}%DIFAUXCMD
}\DIFaddend as the activation function in training Deep Belief Networks, which maps incoming currents driven by Poisson spike trains to the response firing rate of an LIF neuron.
	...
	%DIF < Jug et al\cite{Jug_etal_2012} first proposed the use of the Siegert function to replace the sigmoid activation function in training Restricted Boltzmann Machines (RBMs).
	%DIF > Jug et al\citep{Jug_etal_2012} first proposed the use of the Siegert function to replace the sigmoid activation function in training Restricted Boltzmann Machines (RBMs).
	%The Siegert units map incoming currents driven by Poisson spike trains to the response firing rate of an LIF neuron.
	%The ratio of the spiking rate to its maximum is equivalent to the output of a sigmoid neuron.
	%DIF < A spiking Deep Belief Network (DBN)~\cite{Stromatias2015scalable} of four layers of RBMs was implemented on neuromorphic hardware, SpiNNaker~\cite{furber2014spinnaker}, to recognise hand written digits in real time.
	%DIF > A spiking Deep Belief Network (DBN)~\citep{Stromatias2015scalable} of four layers of RBMs was implemented on neuromorphic hardware, SpiNNaker~\citep{furber2014spinnaker}, to recognise hand written digits in real time.
	However, the Siegert formula is inaccurate as it models the current noise as white~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{liu2016noisy}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{liu2016noisy}
}%DIFAUXCMD
}\DIFaddend , $\tau_{\textit{\textrm{syn}}} \to 0$, \DIFdelbegin \DIFdel{but it }\DIFdelend \DIFaddbegin \DIFadd{which }\DIFaddend is not feasible in practice.
	...
	%DIF < 	A similar activation function of Soft LIF~\cite{hunsberger2015spiking} was introduced to simplify the computation complexity.
	%DIF > 	A similar activation function of Soft LIF~\citep{hunsberger2015spiking} was introduced to simplify the computation complexity.
	We will illustrate these problems in detail in Section~\ref{sec:response_func}. %{sec:siergert} and~\ref{subsec:practice}.

	A softened version of the response function of LIF neurons \DIFdelbegin \DIFdel{is proposed~\mbox{%DIFAUXCMD
\cite{hunsberger2015spiking}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{has been proposed~\mbox{%DIFAUXCMD
\citep{hunsberger2015spiking}
}%DIFAUXCMD
}\DIFaddend and is less computationally expensive than the Siegert function.
	...
	Therefore the training required additional noise on the response firing rate and on the training data, thus \DIFdelbegin \DIFdel{included }\DIFdelend \DIFaddbegin \DIFadd{requiring }\DIFaddend hyper-parameters in the model.
	%	What's more, the activation function compromised the modelling accuracy for computational simplicity.

	
	Although both the \DIFaddbegin \DIFadd{above }\DIFaddend LIF response functions \DIFdelbegin \DIFdel{take }\DIFdelend \DIFaddbegin \DIFadd{accept }\DIFaddend and output variables in physical units such that the trained weights can be directly used in SNNs, they struggle in \DIFaddbegin \DIFadd{terms of }\DIFaddend poor modelling accuracy and high computation complexity.
	Besides, the disadvantage of response functions employing physical units is that they lose the numerical abstraction of firing rates in ANNs, thus they can be only used for training SNNs, meanwhile other widely-used \DIFdelbegin \DIFdel{activation functions of ANNs }\DIFdelend \DIFaddbegin \DIFadd{ANN activation functions }\DIFaddend cannot be transformed \DIFdelbegin \DIFdel{to fit in }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend SNN training. 
	Therefore, the first problem of off-line SNN training is the accurate modelling of the neural response activity of LIF neurons using abstract activation functions, in the hope of (1) increasing the performance of SNN training, (2) reducing the computation complexity and (3) generalising off-line SNN training to \DIFdelbegin \DIFdel{the way of training ANNs}\DIFdelend \DIFaddbegin \DIFadd{commonly-used ANN activation functions}\DIFaddend .
	We call activation functions  \DIFdelbegin \DIFdel{`abstract' referring to the ones }\DIFdelend without physical units \DIFdelbegin \DIFdel{which are used in the ANNs}\DIFdelend \DIFaddbegin \DIFadd{`abstract'}\DIFaddend , and select them for LIF modelling due to their simplicity and generalised use for training ANNs.
	Thus we propose the activation function, NSP~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{liu2016noisy}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{liu2016noisy}
}%DIFAUXCMD
}\DIFaddend , in Section~\ref{sec:NSP} to address this problem.
%DIF < 	Thus, NSP~\cite{liu2016noisy} (Section~\ref{sec:NSP}), we proposed closely match the response activity of LIF neurons by including current noise as the second factor in the activation function and taking account of coloured noise driven by $\tau_{\textit{\textrm{syn}}}$.
	%DIF > 	Thus, NSP~\citep{liu2016noisy} (Section~\ref{sec:NSP}), we proposed closely match the response activity of LIF neurons by including current noise as the second factor in the activation function and taking account of coloured noise driven by $\tau_{\textit{\textrm{syn}}}$.
	...
	%DIF < Based on the fact that cortical neurons seldom saturate their firing rate as sigmoid neurons,ReLU~\cite{glorot2011deep} were proposed to replace sigmoid neurons and surpassed the performance of other popular activation units.
	%DIF > Based on the fact that cortical neurons seldom saturate their firing rate as sigmoid neurons,ReLU~\citep{glorot2011deep} were proposed to replace sigmoid neurons and surpassed the performance of other popular activation units.
	%trained with noisy input, the 4-layered spiking autoencoder reached 98.37\% accuracy on MNIST. 
	...
	Researchers~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{cao2015spiking,diehl2015fast}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{cao2015spiking,diehl2015fast}
}%DIFAUXCMD
}\DIFaddend successfully applied this method on less biologically-realistic and simplified integrate-and-fire (IF) neurons.
...
%http://neuronaldynamics.epfl.ch/online/Ch1.S3.html about V tradractory need to write in Chapter 2.	
	The response function~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{rauch2003neocortical,la2008response}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{rauch2003neocortical,la2008response}
}%DIFAUXCMD
}\DIFaddend of the LIF neuron to a noisy current, also known as \DIFaddbegin \DIFadd{the }\DIFaddend Siegert formula, is \DIFdelbegin \DIFdel{driven by the }\DIFdelend \DIFaddbegin \DIFadd{a function of }\DIFaddend $\mu$ and $\sigma$:
	\begin{equation}
	\lambda_\mathit{out}=
...
%DIF < 	Nair and Hinton~\cite{nair2010rectified} stated that ReLU preserve information about relative intensities(intensity equivarance) provided they have zero biases and are noise-free, but binary units do not.
%DIF > 	Nair and Hinton~\citep{nair2010rectified} stated that ReLU preserve information about relative intensities(intensity equivarance) provided they have zero biases and are noise-free, but binary units do not.
...
%	\end{figure}
%	\begin{itemize}
%DIF < 		\item sum of an infinite number of binary units with each having a bias less than the previous one~~\cite{nair2010rectified}.
%DIF > 		\item sum of an infinite number of binary units with each having a bias less than the previous one~~\citep{nair2010rectified}.
...
%	\end{itemize}  
	...
	\begin{figure}[tbp!]
		...
		\begin{subfigure}[t]{0.49\textwidth}
			...
			\caption{Current sampled at $dt$=10~ms.}
		\end{subfigure}
		\caption{Recorded response firing rate of \DIFdelbeginFL \DIFdelFL{a }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{an }\DIFaddendFL LIF neuron driven by \textit{NoisyCurrentSource} sampled at every (a) 1~ms and (b) 10~ms. Averaged firing rates of 10 simulation trails are shown in bold lines, and the grey colour fills the range between the minimum to maximum of the firing rates. The analytical LIF response function, \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL Siegert formula (Equation~\ref{equ:siegert}), is drawn in thin lines (shown in Figure~\ref{Fig:physics}) to compare with the practical simulation.}
		\label{Fig:current}
	\end{figure}
	...
	\begin{figure}[tbp!]
		...
		\end{subfigure}
		\caption{\textit{NoisyCurrentSource} samples noisy currents from \DIFaddbeginFL \DIFaddFL{a }\DIFaddendFL Gauss distribution \DIFdelbeginFL \DIFdelFL{at }\DIFdelendFL every 1~ms (left) and 10~ms (right). The signals are shown in \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL time domain in (a) and (b), and \DIFaddbeginFL \DIFaddFL{in the }\DIFaddendFL spectrum domain in (c) and (d). The autocorrelation of both current signals are shown in (e) and (f). The distribution of the discrete samples are plotted in bar \DIFdelbeginFL \DIFdelFL{chart }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{charts }\DIFaddendFL to compare with \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL PDF of the original Gauss distribution, shown in (g) and (h).}
		\label{Fig:lif_curr}
	\end{figure}
%\begin{figure}
%	\centering
...
%\end{figure}
	...
	To verify the response function in practice, simulation tests were carried out using PyNN~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{davison2008pynn}
}%DIFAUXCMD
to compare }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{davison2008pynn}
}%DIFAUXCMD
to compare the reality }\DIFaddend with the analytical results~(the Siegert function).
	...
	The curves in Figures~\ref{Fig:current} illustrate \DIFaddbegin \DIFadd{the }\DIFaddend output firing rate driven by different \DIFdelbegin \DIFdel{level }\DIFdelend \DIFaddbegin \DIFadd{levels }\DIFaddend of noise.
...
%\end{figure}
	...
	where the mean and variance of the current are given by~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{la2008response}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{la2008response}
}%DIFAUXCMD
}\DIFaddend :
	\begin{equation}
...
%	Thus the experiments show that a longer $\tau_{syn}$ increases the level of noise and widens the variance of the output firing rate.
	Although the use of \DIFaddbegin \DIFadd{the }\DIFaddend Siegert function opened the door for modelling response activities of LIF neurons as \DIFaddbegin \DIFadd{the }\DIFaddend activation function used in ANNs~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Jug_etal_2012}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Jug_etal_2012}
}%DIFAUXCMD
}\DIFaddend , there are several drawbacks of this method:
	\begin{itemize}
...
%		This issue will be addressed in Section~\ref{subsec:practice}.
		...
		\item \DIFaddbegin \DIFadd{The }\DIFaddend Siegert function is used to replace sigmoid functions for training spiking Restricted Boltzmann Machines~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Jug_etal_2012}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Jug_etal_2012}
}%DIFAUXCMD
}\DIFaddend , thus neurons have to fire at high frequency (higher than half of the maximum firing rate) to represent the activation of a sigmoid unit and the network activity results in high power dissipation.
		\item better learning performance has been reported using ReLU, so modelling \DIFaddbegin \DIFadd{a }\DIFaddend ReLU-like activation function for spiking neurons is needed.  
	\end{itemize}
...
%	However, such a derivative function of low complexity does not present in Siegert function.
	Due to the limited time resolution of common SNN simulators and the time taken for neurotransmitter release, $\tau_{syn}$, mismatches exist between the analytical response function, the Siegert formula, and practical neural activities.
	Consequently to model the practical responses of LIF neurons (see Figure~\ref{fig:nsp}(a)) whose output firing rates are determined by the mean and variance of the noisy input currents, the NSP was \DIFdelbegin \DIFdel{proposed}\DIFdelend \DIFaddbegin \DIFadd{propose}\DIFaddend :
	\begin{equation}
	y = f_{ns}(x, \sigma) = k \sigma \log [1 + \exp(\frac{x}{k \sigma})]~,
	\label{equ:nsp}
	\end{equation}
	where $x$ and $\sigma$ refer to the mean and standard deviation of the input current, $y$ indicates the intensity of the output firing rate, and $k$, determined by the biological configurations on the LIF neurons~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{liu2016noisy}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{liu2016noisy}
}%DIFAUXCMD
}\DIFaddend ~(listed in Table~\ref{tbl:pynnConfig}), controls the shape of the curves.
...
%	The result of fine tuning on a ConvNet will be demonstrated in Section~\ref{subsec:result_compare}.
	...
	If instead of multiplying every input by $\tau_{\textit{\textrm{syn}}}$ (left of Fig.~\ref{Fig:tneuron}(a)) we do it in every output after $\lambda_j$, (right of Fig.~\ref{Fig:tneuron}(b)) we obtain the same neuron model and structure as a typical neuron in ANNs, \DIFdelbegin \DIFdel{See }\DIFdelend \DIFaddbegin \DIFadd{see }\DIFaddend Figure~\ref{Fig:compare_as}(a), that neurons take $x$ as input and output $y$.
	...
	Once }\DIFaddend the parameter $p$ is acquired the PAF can be generalised to other activation functions.
	...
	The simple change \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend activation functions presented in the previous section allows the use of common ANN training methods to obtain SNN-compatible weights.
...
\label{sec:iconipResult}
	...
	% Architecture
	A spiking ConvNet was trained on \DIFdelbegin \DIFdel{MNIST~\mbox{%DIFAUXCMD
\cite{lecun1998gradient}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{the MNIST~\mbox{%DIFAUXCMD
\citep{lecun1998gradient}
	...
	Finally, the trained weights were then directly transferred to the corresponding spiking ConvNets for recognition test on the SNN simulator, NEST~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
\citep{gewaltig2007nest}
	...
	At the testing stage, the input images were converted to Poisson spike trains~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{liu2016bench}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{liu2016bench}
}%DIFAUXCMD
}\DIFaddend and presented for 1~s each.
...
%DIF < 	%	~\cite{lecun1998gradient}
%DIF > 	%	~\citep{lecun1998gradient}
...
%DIF < 	To validate how well the Noisy Softplus activation fits to the response firing rate of LIF neurons in a real application, we simulated the model on NEST~\cite{gewaltig2007nest} using the Poisson MNIST dataset~\cite{liu2016bench} and the neurons of a convolutional map were observed.
	%DIF > 	To validate how well the Noisy Softplus activation fits to the response firing rate of LIF neurons in a real application, we simulated the model on NEST~\citep{gewaltig2007nest} using the Poisson MNIST dataset~\citep{liu2016bench} and the neurons of a convolutional map were observed.
...
\subsection{Recognition Performance}	
	...
	The generalised training method presented \DIFdelbegin \DIFdel{in this paper }\DIFdelend \DIFaddbegin \DIFadd{here }\DIFaddend has a simple activation function, it requires no conversions of trained weights, which are well fitted to biologically-plausible LIF neurons, and has a single \emph{optional} additional processing of fine tuning.
	...
	\begin{sidewaystable*}[htbp] \small
		...
		\begin{center}
			...
			\begin{tabular}{l c c c c c c}
				...
				\\
				\hline
				\begin{mycell}{3cm} \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Jug_etal_2012}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Jug_etal_2012}
				...
				\begin{mycell}{1.8cm} \textbf{No} \end{mycell} & 
				\begin{mycell}{3cm} 94.94\\~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
\end{sidewaystable*}
	...
	%DIF < The network structure was the same with the state-of-the-art model which reported the best classification accuracy of 99.1\%~\cite{diehl2015fast} in ANN-trained SNNs: 12c5-2s-64c5-2s-10fc.
	%DIF > The network structure was the same with the state-of-the-art model which reported the best classification accuracy of 99.1\%~\citep{diehl2015fast} in ANN-trained SNNs: 12c5-2s-64c5-2s-10fc.
	...
	\label{equ:energy}
	\end{equation}
	where $\lambda_j$ is the output firing rate, $N_j$ is the number of post-synaptic neurons it connects to, $T$ is the testing time, and $E_{syn}$ is the energy cost for a synaptic event of some specific neuromorphic hardware, for example, about 8~nJ on SpiNNaker~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
%\section{Discussions}
	...
	The trained weights can be directly used in \DIFaddbegin \DIFadd{the }\DIFaddend SNN without any further transformation.
	...
	The best classification accuracy of 99.07\% using LIF neurons in a PyNN simulation outperforms state-of-the-art SNN models of LIF neurons and is equivalent to the best result achieved using IF neurons~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{diehl2015fast}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{diehl2015fast}
