...
This }\DIFdelend \DIFaddbegin \DIFadd{either transferring well-tuned deep ANN models into SNNs or translating numerical calculations of weight tuning to biologically-plausible synaptic learning rules.
...
Based on the former solution, this }\DIFaddend chapter proposes a generalised \DIFdelbegin \DIFdel{off-line training of SNNs to address the first research problem of }\DIFdelend \DIFaddbegin \DIFadd{training method to train SNNs off-line on equivalent ANNs and transfer the tuned weights back to }\DIFaddend the \DIFdelbegin \DIFdel{thesis}\DIFdelend \DIFaddbegin \DIFadd{SNNs;
...
Noisy Softplus~(NSP), which closely \DIFdelbegin \DIFdel{models the response }\DIFdelend \DIFaddbegin \DIFadd{simulates the }\DIFaddend firing activity of Leaky Integrate-and-Fire~(LIF) neurons \DIFaddbegin \DIFadd{driven by noisy current influx}\DIFaddend .
The second problem is mapping the abstract numerical values of the ANN to \DIFdelbegin \DIFdel{concrete physical units in the SNN, such as current }\DIFdelend \DIFaddbegin \DIFadd{physical variables, current (nA) }\DIFaddend and firing rate \DIFdelbegin \DIFdel{.
Thus }\DIFdelend \DIFaddbegin \DIFadd{(Hz), in the SNN.
Consequently }\DIFaddend we introduce the \DIFdelbegin \DIFdel{parametric activation function}\DIFdelend \DIFaddbegin \DIFadd{Parametric Activation Function}\DIFaddend ~(PAF: $y = p \times f(x)$), \DIFdelbegin \DIFdel{to tackle this problem.
With these problems solved, SNN training }\DIFdelend \DIFaddbegin \DIFadd{which successfully includes physical units to conventional activation functions thus unifies the representations of neurons in ANNs and the ones in SNNs.
Therefore, an SNN is able to be modelled and trained on an ANN using the same conventional training algorithms, such as Backpropagation~(BP).
...
Based on this generalised training method, we achieve the best SNN accuracy on the }\DIFdelend \DIFaddbegin \DIFadd{SNN without any conversion.
...
In addition, the proposed method also preserves state-of-the-art accuracy of }\DIFaddend MNIST task using \DIFdelbegin \DIFdel{LIF neurons, 99.07}%DIFDELCMD < \%%%%
\DIFdel{, on a 6-layer spiking convolutional neural network~(ConvNet) }\DIFdelend \DIFaddbegin \DIFadd{standard LIF neurons}\DIFaddend .

\DIFaddbegin \DIFadd{Therefore, most importantly, this research provides the Neuromorphic Engineering~(NE) community a simple, but effective and generalised off-line SNN training method which notably simplifies the development of Artificial Intelligence~(AI) applications on neuromorphic hardware.
In turn, it enables ANN users to implement their models on neuromorphic hardware without the knowledge of spiking networks or programming on specific hardware;
...
%\end{figure}
	...
	To verify the response function in practice, simulation tests were carried out using PyNN~\citep{davison2008pynn} to compare \DIFaddbegin \DIFadd{the reality }\DIFaddend with the analytical results~(the Siegert function).
...
%	Thus the experiments show that a longer $\tau_{syn}$ increases the level of noise and widens the variance of the output firing rate.
	Although the use of \DIFaddbegin \DIFadd{the }\DIFaddend Siegert function opened the door for modelling response activities of LIF neurons as \DIFaddbegin \DIFadd{the }\DIFaddend activation function used in ANNs~\citep{Jug_etal_2012}, there are several drawbacks of this method:
	\begin{itemize}
