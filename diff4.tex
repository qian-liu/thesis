\chapter[Off-line SNN training]{Generalised Off-line SNN Training}
\label{cha:Conv}
%Paragraph One: LINK
...
In Section~\ref{sec:iconipResult} we demonstrate the training of a spiking \DIFdelbegin \DIFdel{ConvNet}\DIFdelend \DIFaddbegin \DIFadd{Convolutional Network~(ConvNet)}\DIFaddend , and compare the proposed method to existing training algorithms.

\section{\DIFdelbegin \DIFdel{Introduction}\DIFdelend \DIFaddbegin \DIFadd{Related Work}\DIFaddend }
\DIFdelbegin %DIFDELCMD < \label{sec:intro_NSP}
%DIFDELCMD < %%%
...
\DIFdelendFL \DIFaddbeginFL \caption[A spiking neuron.]{\DIFaddendFL A spiking neuron. Spike trains flow into a spiking neuron as current influx, trigger linearly summed PSPs through synapses with different synaptic efficacy \textbf{w}, and the post-synaptic neuron generates output spikes when the membrane potential reaches some threshold.}
...
%	Thus, NSP~\citep{liu2016noisy} (Section~\ref{sec:NSP}), we proposed closely match the response activity of LIF neurons by including current noise as the second factor in the activation function and taking account of coloured noise driven by $\tau_{\textit{\textrm{syn}}}$.
	...
	Thus, the training can be not only simplified by using conventional simple activation functions, such as Rectified Linear Units~(ReLUs), but also the method can be generalised to target standard LIF neurons which are supported by most neuromorphic hardware\DIFdelbegin \DIFdel{platforms}\DIFdelend .

	
	
\section{Modelling the \DIFdelbegin \DIFdel{LIF }\DIFdelend Response Function}
	...
	\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{In Section~\ref{subsec:neuron_model}, we have demonstrated the  }\DIFaddend LIF neuron model \DIFdelbegin \DIFdel{follows the following membrane potential dynamics}\DIFdelend \DIFaddbegin \DIFadd{as follows}\DIFaddend :
	\begin{equation}
...
\DIFdelendFL \DIFaddbeginFL \caption[Response function of the LIF neuron.]{\DIFaddendFL Response function of the LIF neuron with noisy input currents with different standard deviations.}
	...
	Figure~\ref{Fig:physics} shows the response curves (Equation~\ref{equ:siegert}) of an LIF neuron driven by noisy currents \DIFdelbegin \DIFdel{with Gaussian noise with }\DIFdelend \DIFaddbegin \DIFadd{where the Gaussian noise is of }\DIFaddend mean $m_I$ and standard deviation $s_I$.
	...
	As noise increases, the level of firing rates also rises\DIFdelbegin \DIFdel{, and the Softplus activation }\DIFdelend \DIFaddbegin \DIFadd{.
	Thus the Softplus }\DIFaddend function approximates the response \DIFdelbegin \DIFdel{firing activity driven by currentwith Gaussian white noise added.
	Softplus units only represent }\DIFdelend \DIFaddbegin \DIFadd{activity to noisy current, but only represents }\DIFaddend a single level of noise; for example, the dotted line in Figure~\ref{Fig:physics} shows the curve when $s_I=1$.

		\begin{table}[bt]
			\centering
			\DIFdelbeginFL %DIFDELCMD < \caption{\label{tbl:pynnConfig}%%%
\DIFdelendFL \DIFaddbeginFL \caption[LIF parameter settings.]{\DIFaddendFL Parameter setting for the current-based LIF neurons using PyNN.}
			\bgroup
			...
			\end{tabular}
			\egroup
			\DIFaddbeginFL \label{tbl:pynnConfig}
...
%\end{figure}
	...
	\begin{figure}[tbp!]
		...
		\end{subfigure}
		\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Recorded response firing rate driven by \textit{NoisyCurrentSource}.]{\DIFaddendFL Recorded response firing rate of an LIF neuron driven by \textit{NoisyCurrentSource} \DIFaddbeginFL \DIFaddFL{in PyNN, }\DIFaddendFL sampled at every (a) 1~ms and (b) 10~ms. Averaged firing rates of 10 simulation trails are shown in bold lines, and the grey colour fills the range between the minimum to maximum of the firing rates. The analytical LIF response function, the Siegert formula (Equation~\ref{equ:siegert}), is drawn in thin lines (shown in Figure~\ref{Fig:physics}) to compare with the practical simulation.}
		\label{Fig:current}
	\end{figure}
...
%DIF < 		\caption{$\tau_{syn}$=1~ms}
%DIF > 		\caption{$\tau_{\textit{\textrm{syn}}}$=1~ms}
%	\end{subfigure}%
%	\hspace{1em}
%	\begin{subfigure}[c]{0.45\textwidth}\raggedleft
%		\raisebox{-.5\height}{\includegraphics[width=.9\textwidth]
...
%DIF < 		\caption{$\tau_{syn}$=10~ms}
%DIF > 		\caption{$\tau_{\textit{\textrm{syn}}}$=10~ms}
%	\end{subfigure}%
%DIF < 	\caption{Recorded response firing rate of a LIF neuron driven by synaptic current (bold lines) comparing to the noisy current source (thin lines). The synaptic current is shown in time domain and spectrum domain, so as its autocorrelation, and the distribution. (a) is tested when $\tau_{syn}$=1~ms, and (b) for $\tau_{syn}$=10~ms.}
...
%\end{figure}
	...
	\begin{figure}[tbp!]
		...
		\begin{subfigure}[t]{0.49\textwidth}
			...
			\caption{\DIFdelbeginFL \DIFdelFL{$\tau_{syn}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\tau_{\textit{\textrm{syn}}}$}\DIFaddendFL =1~ms.}
		\end{subfigure}
		\begin{subfigure}[t]{0.49\textwidth}
			\includegraphics[width=\textwidth]{pics_iconip/spiked_curve_10.png}
			\caption{\DIFdelbeginFL \DIFdelFL{$\tau_{syn}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\tau_{\textit{\textrm{syn}}}$}\DIFaddendFL =10~ms.}
		\end{subfigure}
		\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Recorded response firing rate driven by a synaptic current.]{\DIFaddendFL Recorded response firing rate of an LIF neuron driven by a synaptic current with two synaptic time constants: (a) \DIFdelbeginFL \DIFdelFL{$\tau_{syn}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\tau_{\textit{\textrm{syn}}}$}\DIFaddendFL =10~ms and (b) \DIFdelbeginFL \DIFdelFL{$\tau_{syn}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\tau_{\textit{\textrm{syn}}}$}\DIFaddendFL =10~ms. Average firing rates of 10 simulation trails are shown in bold lines, and the grey colour fills the range between the minimum to maximum of the firing rates. The firing rates recorded driven by \textit{NoisyCurrentSource} are drawn in thin lines (shown in Figure~\ref{Fig:lif_curr}) for comparison.}
		\label{Fig:spike_curr}
	\end{figure}

	\begin{figure}[tbp!]
		\centering
		\makebox[0.43\textwidth]{\DIFdelbeginFL \DIFdelFL{$\tau_{syn}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\tau_{\textit{\textrm{syn}}}$}\DIFaddendFL =1~ms} \makebox[0.43\textwidth]{\DIFdelbeginFL \DIFdelFL{$\tau_{syn}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\tau_{\textit{\textrm{syn}}}$}\DIFaddendFL =10~ms} \par
		\begin{subfigure}[t]{0.43\textwidth}
		...
		\end{subfigure}
		\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Noisy currents generated by Poisson spike trains.]{\DIFaddendFL Noisy currents generated by 100 Poisson spike trains to an LIF neuron with synaptic time constant \DIFdelbeginFL \DIFdelFL{$\tau_{syn}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\tau_{\textit{\textrm{syn}}}$}\DIFaddendFL =1~ms (left) and \DIFdelbeginFL \DIFdelFL{$\tau_{syn}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\tau_{\textit{\textrm{syn}}}$}\DIFaddendFL =10~ms (right). The currents are shown in the time domain in (a) and (b), and in the spectrum domain in (c) and (d). The autocorrelation of both current signals are shown in (e) and (f). The distribution of the generated samples are plotted in bar chart form to compare to the expected Gaussian distribution, shown in (g) and (h).}
		\label{Fig:lif_pois}
	\end{figure}
	...
	\label{equ:distr0}
	\end{equation}
	where \DIFdelbegin \DIFdel{$\tau_{syn}$ }\DIFdelend \DIFaddbegin \DIFadd{$\tau_{\textit{\textrm{syn}}}$ }\DIFaddend is the synaptic time constant, and each Poisson spike train connects to the neuron with a strength of $w_i$ and a firing rate of $\lambda_i$.
...
%DIF < 	The use of noisy currents assumes that the post-synaptic potential (PSP) is a delta function, e.g. $\tau_{syn}$ tends to the limits of 0.
	%DIF > 	The use of noisy currents assumes that the post-synaptic potential (PSP) is a delta function, e.g. $\tau_{\textit{\textrm{syn}}}$ tends to the limits of 0.
	in practice the release of neurotransmitter takes time (\DIFdelbegin \DIFdel{$\tau_{syn} >> 0$}\DIFdelend \DIFaddbegin \DIFadd{$\tau_{\textit{\textrm{syn}}} >> 0$}\DIFaddend ) and the synaptic current decays exponentially \DIFdelbegin \DIFdel{$I_{syn} = I_0 e^{\frac{-t}{\tau_{syn}}}$}\DIFdelend \DIFaddbegin \DIFadd{$I_{syn} = I_0 e^{-\frac{t}{\tau_{\textit{\textrm{syn}}}}}$}\DIFaddend .
...
%	and the noise added to the mean current is not white noise, but pink noise.
	...
	and the \DIFdelbegin \DIFdel{$\tau_{syn}$}\DIFdelend \DIFaddbegin \DIFadd{$\tau_{\textit{\textrm{syn}}}$}\DIFaddend , even when as short as 1~ms, adds coloured noise instead of white noise to the current.
...
%DIF < 	Thus the experiments show that a longer $\tau_{syn}$ increases the level of noise and widens the variance of the output firing rate.
		%DIF > 	Thus the experiments show that a longer $\tau_{\textit{\textrm{syn}}}$ increases the level of noise and widens the variance of the output firing rate.
...
%		This issue will be addressed in Section~\ref{subsec:practice}.
	...
	\label{sec:NSP}
%DIF < 	Due to the limited time resolution of common SNN simulators and the time taken for neurotransmitter release, $\tau_{syn}$, mismatches exist between the analytical response function, the Siegert formula, and practical neural activities.
...
%	However, such a derivative function of low complexity does not present in Siegert function.
	Due to the limited time resolution of common SNN simulators and the time taken for neurotransmitter release, \DIFdelbegin \DIFdel{$\tau_{syn}$}\DIFdelend \DIFaddbegin \DIFadd{$\tau_{\textit{\textrm{syn}}}$}\DIFaddend , mismatches exist between the analytical response function, the Siegert formula, and practical neural activities.
	...
	\begin{figure}[tbh!]
		...
		\end{subfigure}
		\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[NSP models the LIF response function.]{\DIFaddendFL 
...
%	\begin{equation}
%	\begin{aligned}
%DIF < 		& {s_I^2}_j=\sum_i(\frac{1}{2} w_{ij}^2) (\lambda_{i}\tau_{syn})~, \textrm{  then}\\
...
%DIF < 	Referred to Figure~\ref{Fig:sneuron}, if we move the left end process of $\times \tau_{syn}$ to the right end after $\lambda_j$, Figure~\ref{Fig:sneuron} forms the same model structure as artificial neurons shown in Figure~\ref{Fig:neuron}: neurons take $x$ as input and outputs $y$, and this conversion is illustrated in Figure~\ref{Fig:tneuron}.
%DIF < 	The process within such an artificial neuron is divided into weighted summation and activation, which also applies to SNN modelling by combining the scaling factor $S$ and the synaptic time constant $\tau_{syn}$ to activation functions.
...
%	The result of fine tuning on a ConvNet will be demonstrated in Section~\ref{subsec:result_compare}.
	...
	\begin{figure}
		...
		\end{subfigure}
		\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[NSP fits to the response firing rates.]{\DIFaddendFL NSP fits to the response firing rates of LIF neurons in concrete physical units.
	...
	\begin{figure}[tbh!]
		...
		\begin{subfigure}[t]{\textwidth}
			...
			\caption{An artificial spiking neuron modelled by PAF-NSP, whose input and output are numerical values, equivalent to those of ANNs. PAF includes the scaling factors $S$ and the synaptic time constant \DIFdelbeginFL \DIFdelFL{$\tau_{syn}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\tau_{\textit{\textrm{syn}}}$ }\DIFaddendFL in the combined activation function.}
		\end{subfigure}\\\bigskip
		\begin{subfigure}[t]{\textwidth}
			\centering
...
\DIFdelendFL \DIFaddbeginFL \caption[Comparisons of three activation functions]{\DIFaddendFL Comparisons of three activation functions used in ANNs: (a) the NSP, (b) the PAF-NSP, and (c) a conventional ANN activation function. The PAF links the firing activity of a spiking neuron to the numerical value of ANNs.}
	...
	%DIF < 	The PAF version of the activation function (Eq.~\ref{equ:PAF}) will be linearly-scaled by the conversion parameter $S$ and the synaptic time constant $\tau_{syn}$:
	%DIF > 	The PAF version of the activation function (Eq.~\ref{equ:PAF}) will be linearly-scaled by the conversion parameter $S$ and the synaptic time constant $\tau_{\textit{\textrm{syn}}}$:
...
%DIF < 	The parameters of the activation function were calibrated as, $(k=0.30, S=201)$,  for LIF neurons~(see~Tablel~\ref{tbl:pynnConfig}) of $\tau_{syn}=5$~ms.
%DIF > 	The parameters of the activation function were calibrated as, $(k=0.30, S=201)$,  for LIF neurons~(see~Tablel~\ref{tbl:pynnConfig}) of $\tau_{\textit{\textrm{syn}}}=5$~ms.
...
%	
%DIF < 	The input $x$ of the network was calculated as Equation~\ref{equ:mi_input}: $x_i=\lambda_i\tau_{syn}$, and so as the weighted sum of the synaptic current (see Equation~\ref{equ:mi_input}), $net_j$ and its variance (see Equation~\ref{equ:si_input}), $\sigma^2_j$.
%DIF > 	The input $x$ of the network was calculated as Equation~\ref{equ:mi_input}: $x_i=\lambda_i\tau_{\textit{\textrm{syn}}}$, and so as the weighted sum of the synaptic current (see Equation~\ref{equ:mi_input}), $net_j$ and its variance (see Equation~\ref{equ:si_input}), $\sigma^2_j$.
%	With three combined activation functions as Equation~\ref{equ:full_act}:
%	\begin{equation}
%	\begin{aligned}
%DIF < 	&\textrm{(1) Noisy Softplus:~~}  y_j=k \sigma_j \log [1 + \exp(\frac{net_j}{k \sigma_j})] \times S \times \tau_{syn}~~,  \\
%DIF < 	&\textrm{(2) ReLU:~~ } y_j=max(0, net_j) \times S \times \tau_{syn}~~, \\
%DIF < 	&\textrm{(3) Softplus:~~ } y_j=k \sigma \log [1 + \exp(\frac{net_j}{k \sigma})] \times S \times \tau_{syn}~~, ~~~\sigma=0.45,  
%DIF > 	&\textrm{(1) Noisy Softplus:~~}  y_j=k \sigma_j \log [1 + \exp(\frac{net_j}{k \sigma_j})] \times S \times \tau_{\textit{\textrm{syn}}}~~,  \\
%DIF > 	&\textrm{(2) ReLU:~~ } y_j=max(0, net_j) \times S \times \tau_{\textit{\textrm{syn}}}~~, \\
%DIF > 	&\textrm{(3) Softplus:~~ } y_j=k \sigma \log [1 + \exp(\frac{net_j}{k \sigma})] \times S \times \tau_{\textit{\textrm{syn}}}~~, ~~~\sigma=0.45,  
% 	\end{aligned}
%	\end{equation}	
...
%DIF < 	The Euclidean distance, $\sqrt{\sum_{j}(y_j/\tau_{syn} - \lambda_j)}$, between the spike counts and the predicted firing rates by Noisy Softplus, ReLU and Softplus was 184.57, 361.64 and 1102.76 respectively.
%DIF > 	The Euclidean distance, $\sqrt{\sum_{j}(y_j/\tau_{\textit{\textrm{syn}}} - \lambda_j)}$, between the spike counts and the predicted firing rates by Noisy Softplus, ReLU and Softplus was 184.57, 361.64 and 1102.76 respectively.
...
\DIFdelendFL \DIFaddbeginFL \caption[The recorded firing rate of the convolution outcomes.]{\DIFaddendFL 
	...
	We trained the network with binary labels on the output layer, thus the expected firing rate of correct classification was \DIFdelbegin \DIFdel{$1/\tau_{syn}=200$}\DIFdelend \DIFaddbegin \DIFadd{$1/\tau_{\textit{\textrm{syn}}}=200$}\DIFaddend ~Hz according to Equation~\ref{Fig:tneuron}.
...
%	However, the loss stabilised fastest, which means a possible shorter training time.
	\DIFaddbegin \begin{figure}[tbp!]
		...
		\caption[Comparisons of loss during training.]{\DIFaddFL{Comparisons of loss during training using Noisy Softplus, ReLU and Softplus activation functions. Bold lines show the average of three training trials, and the grey colour illustrates the range between the minimum and the maximum values of the trials.  }}
		\label{Fig:loss_ns}
	\end{figure}
	...
	Similar findings have shown that networks with added noise, e.g. dropout~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{srivastava2014dropout}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{srivastava2014dropout}
}%DIFAUXCMD
}\DIFaddend , are much faster to train. 
	The result is promising that NSP may shorten the training time.
\DIFdelbegin %DIFDELCMD < \begin{figure}[tbp!]
%DIFDELCMD < 		\centering
%DIFDELCMD < 		\includegraphics[width=0.7\textwidth]{pics_iconip/8.png}
%DIFDELCMD < 		%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Comparisons of Loss during training using Noisy Softplus, ReLU and Softplus activation functions. Bold lines show the average of three training trials, and the grey colour illustrates the range between the minimum and the maximum values of the trials.  }}
		%DIFAUXCMD
%DIFDELCMD < \label{Fig:loss_ns}
%DIFDELCMD < 	\end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin 

\DIFaddend %	The trained networks were scaled to SNNs and compared on recognition rates, 93.34\%, 96.43\% and 97.03\% with a conversion loss of 4.76\%, 0.91\% and 0.74\%.
...
\DIFdelendFL \DIFaddbeginFL \caption[Classification accuracy.]{\DIFaddendFL Classification accuracy.
	...
	The accuracy loss \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend caused by the mismatch between the activations and the practical response firing rates, see examples in Figure~\ref{fig:af_compare}, and the strict binary labels for NSP and Softplus activations.
	Fortunately, the problem is alleviated by fine tuning which \DIFdelbegin \DIFdel{increased }\DIFdelend \DIFaddbegin \DIFadd{increases }\DIFaddend the classification accuracy by 0.38\%, 0.19\% and 2.06\%, and \DIFdelbegin \DIFdel{resulted }\DIFdelend \DIFaddbegin \DIFadd{results }\DIFaddend in the total loss of 0.43\%, 0.61\%, and 1.06\% respectively.
	...
	Softplus \DIFdelbegin \DIFdel{benefits }\DIFdelend \DIFaddbegin \DIFadd{benefited }\DIFaddend the most from fine tuning, since the huge mismatch~(Figure~\ref{fig:af_compare}(c)) of response firing rate is greatly corrected.
	The improvement of NSP \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend obtained from the offset on the labels which helps the network to fit practical SNNs.
	As the recognition performance of ReLU is already high, there \DIFdelbegin \DIFdel{'s }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend little room for improvement.
...
\DIFdelend \DIFaddbegin \begin{mycell}{2.2cm} \DIFaddend \textbf{No} \end{mycell} & 
				...
				\DIFdelbegin %DIFDELCMD < \begin{mycell}{1.8cm} %%%
\DIFdelend \DIFaddbegin \begin{mycell}{2.5cm} \DIFaddend \textbf{Yes} \end{mycell} &  
				\begin{mycell}{2.2cm} Noisy inputs\\ \DIFdelbegin %DIFDELCMD < \&%%%
...
\DIFdelend \DIFaddbegin \begin{mycell}{2.5cm} \DIFaddend No \end{mycell} &  
				\DIFdelbegin %DIFDELCMD < \begin{mycell}{1.8cm} %%%
\DIFdelend \DIFaddbegin \begin{mycell}{2.2cm} \DIFaddend Dropout  \end{mycell} & %\&\\Conversion
...
\end{sidewaystable*}
	...
	\begin{figure}[htb!]
		...
		\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[The classification accuracy after fine tuning.]{\DIFaddendFL The classification accuracy of 3 trials (averaged in bold lines, grey shading shows the range between minimum to maximum) over short response times, with (a) trained weights before fine tuning, and (b) after fine tuning.}
		\label{fig:ca_time}	
	\end{figure}
	...
	&= \DIFdelbegin %DIFDELCMD < \dfrac{y_j N_j T E_{syn}}{\tau_{syn}}%%%
\DIFdelend \DIFaddbegin \dfrac{y_j N_j T E_{syn}}{\tau_{\textit{\textrm{syn}}}}\DIFaddend ~~,
	...
	\begin{equation}
	\sum_j E_{j} =  \DIFdelbegin %DIFDELCMD < \dfrac{T E_{syn}}{\tau_{syn}} %%%
...
%	train an equivalent ANN and transfer the trained weights back to SNNs.
	...
	\DIFaddbegin \DIFadd{Another important feature of accurately modelling LIF neurons in ANNs is the acquisition of spiking neuron firing rates. These will aid deployment of SNNs in neuromorphic hardware by providing power and communication estimates, which would allow better usage or customisation of the hardware platforms.
	}

	
	\DIFaddend Regarding the generalisation, the training not only uses popular activation functions in ANNs, \DIFaddbegin \DIFadd{e.g. ReLU, }\DIFaddend but also targets standard LIF neurons which are widely used on neuromorphic hardware.
	\DIFaddbegin \DIFadd{Therefore, the proposed method remarkably simplifies the training of AI applications for neuromorphic hardware;
...
}\DIFaddend An important feature of accurately modelling LIF neurons in ANNs is the acquisition of spiking neuron firing rates. These will aid deployment of SNNs in neuromorphic hardware by providing power and communication estimates, which would allow better usage or customisation of the platforms.