...
\section{Modelling the LIF Response Function}
...
\DIFdelendFL \DIFaddbeginFL \caption[Response function of the LIF neuron.]{\DIFaddendFL Response function of the LIF neuron with noisy input currents with different standard deviations.}
	...
	Thus Softplus }\DIFaddend function approximates the response \DIFdelbegin \DIFdel{firing activity driven by currentwith Gaussian white noise added}\DIFdelend \DIFaddbegin \DIFadd{activity to noisy current}\DIFaddend .
...
\DIFdelendFL \DIFaddbeginFL \caption[Recorded response firing rate driven by \textit{NoisyCurrentSource}.]{\DIFaddendFL Recorded response firing rate of an LIF neuron driven by \textit{NoisyCurrentSource} \DIFaddbeginFL \DIFaddFL{in PyNN, }\DIFaddendFL sampled at every (a) 1~ms and (b) 10~ms. Averaged firing rates of 10 simulation trails are shown in bold lines, and the grey colour fills the range between the minimum to maximum of the firing rates. The analytical LIF response function, the Siegert formula (Equation~\ref{equ:siegert}), is drawn in thin lines (shown in Figure~\ref{Fig:physics}) to compare with the practical simulation.}
	...
	\begin{figure}[tbp!]
		...
		\end{subfigure}
		\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[\textit{NoisyCurrentSource} samples noisy currents from a Gaussian distribution.]{\DIFaddendFL \textit{NoisyCurrentSource} samples noisy currents from a Gaussian distribution every 1~ms (left) and 10~ms (right). The signals are shown in the time domain in (a) and (b), and in the spectrum domain in (c) and (d). The autocorrelation of both current signals are shown in (e) and (f). The distribution of the discrete samples are plotted in bar charts to compare with the PDF of the original Gaussian distribution, shown in (g) and (h).}
		\label{Fig:lif_curr}
	\end{figure}
...
%	However, the loss stabilised fastest, which means a possible shorter training time.
	...
	Similar findings have shown that networks with added noise, e.g. dropout~\cite{srivastava2014dropout}, are much faster to train. 
...
\DIFdelendFL \DIFaddbeginFL \caption[Classification accuracy.]{\DIFaddendFL Classification accuracy.
	...
	Fortunately, the problem \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend alleviated by fine tuning which increased the classification accuracy by 0.38\%, 0.19\% and 2.06\%, and resulted in the total loss of 0.43\%, 0.61\%, and 1.06\% respectively.
	...
	Softplus \DIFdelbegin \DIFdel{benefits }\DIFdelend \DIFaddbegin \DIFadd{benefited }\DIFaddend the most from fine tuning, since the huge mismatch~(Figure~\ref{fig:af_compare}(c)) of response firing rate \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend greatly corrected.
	The improvement of NSP was obtained from the offset on the labels which \DIFdelbegin \DIFdel{helps }\DIFdelend \DIFaddbegin \DIFadd{helped }\DIFaddend the network to fit practical SNNs.
	As the recognition performance of ReLU \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend already high, there's little room for improvement.
	Even though the fine-tuning procedure \DIFdelbegin \DIFdel{does }\DIFdelend \DIFaddbegin \DIFadd{did }\DIFaddend its job, the gain in accuracy \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend the smallest for this activation function.
	...
	The most efficient training in terms of both classification accuracy and algorithm complexity, \DIFdelbegin \DIFdel{takes }\DIFdelend \DIFaddbegin \DIFadd{took }\DIFaddend PAF-ReLU for ANN training and PAF-NSP for fine tuning.
