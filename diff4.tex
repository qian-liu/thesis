...
Therefore, most importantly, this research provides the Neuromorphic Engineering~(NE) community with a simple, but effective and generalised off-line SNN training method which notably simplifies the development of Artificial Intelligence~(AI) applications on neuromorphic hardware.
...
%	The impressive disparities in cognitive capabilities and energy consumption drives the research into biologically-plausible spiking neurons and brain inspired computers, known as neuromorphic engineering~\citep{furber2016large}.
	...
	We specified the differences between a conventional artificial neuron and a spiking neuron in Section~\ref{sec:spike}: a regular artificial neuron comprises a weighted summation of input data, $\sum x_i w_i$, and an activation function, $f$, applied to the sum.
...
\DIFdelend \DIFaddbegin \begin{aligned}
	...
	The last term $\D W_t$ is a Wiener process; $W_{t +\D t} - W_{t}$ obeys a Gaussian distribution with mean 0 and variance $\D t$\DIFaddbegin \DIFadd{; thus $y$ is a random number sampled in accordance with a Gaussian distribution of zero mean and unit variance}\DIFaddend .
...
%	and the noise added to the mean current is not white noise, but pink noise.
	...
	Nevertheless, the analytical response function, the Siegert formula, cannot approximate either of the practical simulations \DIFaddbegin \DIFadd{(see Figure~\ref{Fig:current} also)}\DIFaddend .
...
%DIFDELCMD < 	\end{figure}
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \DIFadd{To keep the simple format of traditional activation functions, $y=f(x)$, which has no bias on the input, it is easy to pass the bias $b$ to the LIF parameter, the constant current offset, $I_{\textit{\textrm{offset}}}=b$.
	...
	\begin{figure}[tbh!]
		...
		\begin{subfigure}[t]{\textwidth}
			\centering
			\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.8\textwidth]{pics_iconip/neuron_o.pdf}
%DIFDELCMD < 			%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.9\textwidth]{pics_iconip/neuron_o.pdf}
			\bigskip
			\DIFaddendFL \caption{An artificial spiking neuron modelled by NSP, where firing rates are the input and output of the neuron. NSP transforms the noisy current influx including both the mean (\DIFdelbeginFL \DIFdelFL{$net_j$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$net\_x_{j}$}\DIFaddendFL , solid lines) and the variance \DIFdelbeginFL \DIFdelFL{$\sigma^2_j$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$net_\_\sigma^2_j$ }\DIFaddendFL (dashed lines) to the abstract firing rate $y_j$.}
		\end{subfigure}\\\bigskip\DIFaddbeginFL \bigskip
		\DIFaddendFL \begin{subfigure}[t]{\textwidth}
			\centering
			\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.7\textwidth]{pics_iconip/neuron_PAF.pdf}
%DIFDELCMD < 			%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.85\textwidth]{pics_iconip/neuron_PAF.pdf}
		...
		\DIFaddendFL \begin{subfigure}[t]{\textwidth}
			\centering
			\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.55\textwidth]{pics_iconip/neuron.pdf}
%DIFDELCMD < 			%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.6\textwidth]{pics_iconip/neuron.pdf}
		...
		\end{subfigure}
		\DIFdelbeginFL %DIFDELCMD < \caption[Comparisons of three activation functions]{%%%
\DIFdelFL{Comparisons of three }\DIFdelendFL \DIFaddbeginFL \caption[PAF models spiking neuron as an conventional activation function.]{\DIFaddFL{PAF models spiking neuron as an conventional }\DIFaddendFL activation \DIFdelbeginFL \DIFdelFL{functions used in ANNs: }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{function.
		}\DIFaddendFL (a) \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{An artificial spiking neuron modelled by }\DIFaddendFL NSP, \DIFaddbeginFL \DIFaddFL{where the inputs and outputs are spiking rates instead of abstract numerical values of conventional activation functions.
		}\DIFaddendFL (b) the PAF-NSP, \DIFdelbeginFL \DIFdelFL{and (c) a conventional ANN activation function. The PAF }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{which }\DIFaddendFL links the firing activity of a spiking neuron to the numerical value of ANNs.
		\DIFaddbeginFL \DIFaddFL{(c) a conventional ANN activation function.}\DIFaddendFL }
		\label{Fig:tneuron}
	\end{figure}
	...
	However, a spiking neuron modelled by NSP takes the firing rate as its input/output, thus \DIFdelbegin \DIFdel{Equation~\ref{equ:distr} }\DIFdelend \DIFaddbegin \DIFadd{in a neural network Equation~\ref{equ:distr0} }\DIFaddend can be written as:
	\begin{equation}
	...
	}\DIFaddend % \textrm{~~and~~} x_i = \lambda_{i}\tau_{\textit{\textrm{syn}}}~.
	\label{equ:mi_input}
	\end{equation}
	\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{In accordance with $net_j = \sum_i w_{ij}x_i$, the }\DIFaddend input $ x_i $ of an artificial \DIFaddbegin \DIFadd{spiking }\DIFaddend neuron can be seen as $x_i=\lambda_{i}\tau_{\textit{\textrm{syn}}}$\DIFdelbegin \DIFdel{, see }\DIFdelend \DIFaddbegin \DIFadd{.
	}\DIFaddend Figure~\ref{Fig:tneuron}(a) \DIFaddbegin \DIFadd{illustrates the process that a NSP-modelled artificial spiking neuron takes the input vector $\mathbf{x}$ which is converted from the input firing rate $\bm{\lambda}$; transforms the weighted sum $net\_x_{j}$ and $net_\_\sigma^2_j$ to the abstract output $y_j$; and scales up $y_i$ with the factor $S$ to the output firing rate $\lambda_j$}\DIFaddend . 

	
	If instead of multiplying every input \DIFaddbegin \DIFadd{firing rate $\lambda_i$ }\DIFaddend by $\tau_{\textit{\textrm{syn}}}$ (left of Fig.~\ref{Fig:tneuron}(a)) we do it in every output \DIFaddbegin \DIFadd{firing rate }\DIFaddend after $\lambda_j$, (right of Fig.~\ref{Fig:tneuron}(b)) we obtain the same neuron model and structure as a typical neuron in ANNs, see Figure~\ref{Fig:tneuron}(c), that neurons take \DIFdelbegin \DIFdel{$x$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{x}$ }\DIFaddend as input and output \DIFaddbegin \DIFadd{abstract value }\DIFaddend $y$.
...
%	train an equivalent ANN and transfer the trained weights back to SNNs.
	This training procedure consists of three simple stages: first, estimate parameter $p$ for PAF using NSP; second, use a PAF version of conventional activation functions for ANN training; % can be generalised to activation units other than NSP.
