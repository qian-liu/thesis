...
In this chapter, we will confirm the hypotheses proposed in Chapter~\ref{cha:intro} to answer the thesis question: how to close the gap of the cognitive capabilities between Spiking Neural Networks~(SNNs) and Artificial Neural Network~(ANN) on Artificial Intelligence~(AI) tasks.
...
Moreover, the training of PAF-NSP neurons can be generalised to a PAF version of conventional activation functions, which greatly reduces the computational complexity of the activation functions.
...
Regarding simplicity, this training method performs effectively using Rectified Linear Unit~(ReLU), however, \citep{Jug_etal_2012,hunsberger2015spiking} suffer from high computational complexity.
In addition, thanks to the accurate modelling of SNNs on ANNs, the trained weights are ready to use on SNNs, while previous approaches ~\citep{cao2015spiking,diehl2015fast} require an extra step of converting the trained weights for use at the SNN.
Moreover, comparing to the requirements of specific neural models~\citep{cao2015spiking,diehl2015fast} and the hardware-specific training methods~\citep{diehl2016conversion,diehl2016truehappiness,esser2015backpropagation}, our approach is generalised to target standard LIF neuron and neuromorphic hardware platforms.
...
Moreover, the proposed method is the first to exploit novel techniques to decorrelate the spike trains since the correlation brings down the learning performance rapidly after it reaches the best.
...
\begin{itemize}
	\item Ensemble models~\citep{rokach2010ensemble} have been trained with NSP neurons and will be transferred onto SpiNNaker~\citep{furber2014spinnaker} system to take the advantages of energy-efficiency and massively-parallel simulation ability.
...
Thus, to equip these powerful brain-inspired neuromorphic computers with cognitive capabilities, this work contributes to close the gap of the performance between SNNs and ANNs on AI tasks.
...
The success of neuromorphic AI paves the way to energy-efficient AI on mobile devices and huge computational clusters.
...
The proposed method tackles the problem of accurately translating the weight change of AEs and RBMs from numerical computations to event-based, biologically-plausible STDP rules in SNNs.
...
The last but not least outcome contributes to the NE community with a uniform dataset to evaluate SNNs' performance at both model and hardware level to promote meaningful comparisons between these proposed SNN models and other existing methods within this rapidly advancing field.
Moreover, we hope to provide objective comparisons with conventional image recognition in ANNs, especially in terms of latency and energy consumption.
