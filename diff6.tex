...
The last two chapters have answered the main research question of this thesis showing that Spiking Neural Networks (SNNs) can be trained both on-line and off-line to achieve \DIFdelbegin \DIFdel{the equivalent cognitive capabilities of }\DIFdelend \DIFaddbegin \DIFadd{equivalent recognition performance of the }\DIFaddend Artificial Neural Networks~(ANNs).
To provide meaningful comparisons between \DIFdelbegin \DIFdel{theses }\DIFdelend \DIFaddbegin \DIFadd{these }\DIFaddend proposed SNN models and other existing methods within this rapidly advancing field of Neuromorphic Engineering~(NE), we propose that a large dataset of \DIFdelbegin \DIFdel{spike-based visual stimuli }\DIFdelend \DIFaddbegin \DIFadd{spike-encoded images/videos }\DIFaddend is needed and a corresponding evaluation methodology is also required to estimate the overall performance of SNN models and their hardware implementations.
...
Similarly, a real-time gesture recognition model~\citep{liu2014real} was implemented \DIFaddbegin \DIFadd{by us }\DIFaddend on a neuromorphic system with a DVS as a front-end and a SpiNNaker~\citep{furber2014spinnaker} machine as the back-end, where LIF neurons built up the ConvNet configured with biological parameters.
...
A similar model trained with STDP using a \DIFdelbegin \DIFdel{winner-take-all~(WTA ) }\DIFdelend \DIFaddbegin \DIFadd{WTA }\DIFaddend circuit was tested on a Poissonian spike presentation of the MNIST dataset achieving a performance of 95.0\%~\citep{diehl2015unsupervised}.
...
Adding synaptic internal state enabled STDP to work on bistable synapses, and this method achieved the best performance of 96.5\% among similar network architectures\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep{brader2007learning}}%DIFAUXCMD
}\DIFaddend . 
...
The NE database we propose here is a developing and evolving dataset\DIFdelbegin \DIFdel{consisting of }\DIFdelend \DIFaddbegin \DIFadd{.
It contains }\DIFaddend various spike-based representations of images\DIFdelbegin \DIFdel{and videos }\DIFdelend \DIFaddbegin \DIFadd{/videos and will also expand to include increasing numbers of encoding methods and commonly used datasets of Computer Vision}\DIFaddend .
...
\begin{itemize}
	...
	The development of the dataset and its evaluation methodology will introduce new challenges for the \DIFdelbegin \DIFdel{neuromorphic engineering }\DIFdelend \DIFaddbegin \DIFadd{NE }\DIFaddend community.
...
\begin{itemize}
	...
	\DIFaddbegin \DIFadd{This subset is built by my colleague Garibaldi~Pineda-Garc\'ia.
	}\DIFaddend \item \textit{DVS recorded flashing input},
	...
	\DIFaddbegin \DIFadd{This subset is contributed by our cooperator Teresa Serrano-Gotarredona.
...
\begin{figure}[bht!]
	...
	\caption[Snapshots of the jAER software.]{
		...
		The same image of digit `0' is transformed into spikes by \DIFaddbeginFL \DIFaddFL{DVS recording and Poisson generation, where }\DIFaddendFL (a) \DIFdelbeginFL \DIFdelFL{DVS recording }\DIFdelendFL and (b) \DIFdelbeginFL \DIFdelFL{Poisson generation}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{show the snapshots of the jAER software}\DIFaddendFL .
		\DIFaddbeginFL \DIFaddFL{They visualise the spike counts for each pixel of an image.
		}\DIFaddendFL (c) A raster plot of the Poisson spike trains.}
	\label{fig:zero}
\end{figure}
...
%\end{table}
%
Rank \DIFdelbegin \DIFdel{order coding }\DIFdelend \DIFaddbegin \DIFadd{Order Coding }\DIFaddend (ROC) can be performed using an algorithm known as the
...
Sharing the \DIFdelbegin \DIFdel{designs }\DIFdelend \DIFaddbegin \DIFadd{network details }\DIFaddend not only makes the model reproducible but also inspires fellow scientists to \DIFdelbegin \DIFdel{bring new points of view to the problem, }\DIFdelend \DIFaddbegin \DIFadd{build up new solutions based on existing work, thereby }\DIFaddend generating a positive feedback loop where everybody wins.
...
\begin{table}[th]
	...
	\begin{tabular}{c c l}
		...
		%\hline
		$\tau_{\textit{\textrm{refrac}}}$ & 2.0 ms & refractory period\\
		%\hline
...
\end{table}
%DIF > \begin{table}[hbbp]
%DIF > 	\centering
%DIF > 	\caption[LIF parameter setting using PyNN.]{Parameter setting for the current-based LIF neurons using PyNN.}
%DIF > 	\bgroup
...
%DIF > 		tau\_m & 20.0 & ms\\
%DIF > 		%\hline
%DIF > 		tau\_refrac & 2.0 & ms\\
%DIF > 		%\hline
%DIF > 		%  tau\_syn\_E & 1.0 & ms\\
%DIF > 		%  %\hline
%DIF > 		%  tau\_syn\_I & 1.0 & ms\\
%DIF > 		%  %\hline
%DIF > 		v\_reset & -70.0 & mV\\
%DIF > 		%\hline
%DIF > 		v\_rest & -65.0 & mV\\
%DIF > 		%\hline
%DIF > 		v\_thresh & -50.0 & mV\\
%DIF > 		%\hline
%DIF > 		i\_offset & 0.0 & mV\\
%DIF > 	\end{tabular}
%DIF > 	\egroup
%DIF > 	\label{tbl:pynnSetting}
%DIF > \end{table}
...
%DIF > We will set K to various numbers later to estimate the performance difference.
...
Thus the strategy of replacing weak weights \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend the same negative values was used throughout the case study.
...
In our experiment, two sets of weights were applied: the original STDP trained weights, and the \DIFdelbegin \DIFdel{scaled-up weights which are }\DIFdelend \DIFaddbegin \DIFadd{same weights strengthened }\DIFaddend 10 times \DIFdelbegin \DIFdel{stronger}\DIFdelend \DIFaddbegin \DIFadd{(which we call scaled-up weights)}\DIFaddend .
...
\DIFaddbegin \DIFadd{Building up this dataset is a team work, I claim my own contributions are two subsets of the dataset out of four, the complementary evaluation methodology and the on-line learning case study.
}\DIFaddend %It contains spike-based versions of existing widely-used databases in the vision recognition field.
