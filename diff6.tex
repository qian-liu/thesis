...
\DIFaddbegin \DIFadd{This chapter put forward the NE dataset as a unified resource to measure progress quantitatively within the field of neuromorphic vision.
}\DIFaddend The NE dataset \DIFdelbegin \DIFdel{proposed in this chapter makes the comparison }\DIFdelend \DIFaddbegin \DIFadd{and the complementary evaluation methodology make it possible to (1) provide objective comparisons of vision based SNN models and their implementation on neuromorphic platforms;
(2) compare }\DIFaddend of SNNs with conventional recognition methods \DIFdelbegin \DIFdel{possible }\DIFdelend by using converted spike representations of the same vision databases\DIFdelbegin \DIFdel{.
As far as we know, this is the first attempt at benchmarking neuromorphic vision recognition by providing public a }\DIFdelend \DIFaddbegin \DIFadd{;
(3) measure the state-of-the-art performance in }\DIFaddend spike-based \DIFdelbegin \DIFdel{dataset and evaluation metrics.
}\DIFdelend \DIFaddbegin \DIFadd{visual recognition;
and (4) promote future research into rapid and low energy recognition.
%DIF > As far as we know, this is the first attempt at benchmarking neuromorphic vision recognition by providing public a spike-based dataset and evaluation metrics.
}\DIFaddend In accordance with the suggestions from~\cite{tan2015bench}, the evaluation metrics highlight the strengths of spike-based vision tasks\DIFdelbegin \DIFdel{and the dataset design also promotes the research into rapid and low energy recognition (e.g. flashing digits).
}\DIFdelend \DIFaddbegin \DIFadd{: low latency and energy efficiency.
%DIF >  and the dataset design also promotes the research into rapid and low energy recognition (e.g. flashing digits).
}\DIFaddend 
...
The case study provides a baseline for robust comparisons \DIFdelbegin \DIFdel{between }\DIFdelend \DIFaddbegin \DIFadd{of other }\DIFaddend SNN models and their hardware implementations.
