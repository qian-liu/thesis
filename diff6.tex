...
\DIFadd{Today, increasing attention is being paid to research into spike-based neural computation both to gain a better understanding of the brain and to explore biologically-inspired computation.
Within this field, the primate visual pathway and its hierarchical organisation have been extensively studied.
...
\DIFaddbegin \DIFadd{A new set of vision benchmarks for spike-based neural processing are now needed to measure progress quantitatively within this rapidly advancing field.
%DIF > We propose that a large dataset of spike-based visual stimuli is needed to provide meaningful comparisons between different systems, and a corresponding evaluation methodology is also required to measure the performance of SNN models and their hardware implementations.
%DIF > 
}\DIFaddend In this chapter we first propose an initial NE dataset based on standard computer vision benchmarks and that uses digits from the MNIST database.
...
In Section~\DIFdelbegin \DIFdel{\ref{sec:chapt6_intro}}\DIFdelend \DIFaddbegin \DIFadd{\ref{sec:chapt6_relate}}\DIFaddend , diverse neuromorphic vision tasks are introduced to illustrate the requirements for a unified spike-based dataset.
...
\DIFdelbegin %DIFDELCMD < \label{sec:chapt6_intro}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{sec:chapt6_relate}
\DIFaddend Researchers are using the capabilities created by rapid developments in neuromorphic engineering to address the dual aims of understanding brain functions and building brain-like machines~\citep{furber2007neural}.
...
Recently~\citet{fu2012spiking} reported that their SNN implementation was capable of recognising facial expressions with a \DIFdelbegin \DIFdel{classification accuracy }\DIFdelend \DIFaddbegin \DIFadd{Classification Accuracy }\DIFaddend (CA) of 97.35\% on the JAFFE dataset~\citep{lyons1998coding} which contains 213 images of 7 facial expressions posed by 10 individuals.
...
Deep \DIFdelbegin \DIFdel{Neural Networks (DNNs) }\DIFdelend \DIFaddbegin \DIFadd{Learning }\DIFaddend have exceeded human-level performance on image classification tasks~\citep{he2015delving}, but mainstream \DIFdelbegin \DIFdel{DNN }\DIFdelend \DIFaddbegin \DIFadd{Deep Learning }\DIFaddend research is focussed on continuous rather than spiking neural networks.
...
The current trend towards training \DIFdelbegin \DIFdel{Spiking DNNs on line }\DIFdelend \DIFaddbegin \DIFadd{deep SNNs on-line }\DIFaddend using biologically-plausible learning methods is also promising.
...
\DIFaddbegin \DIFadd{More related work and detailed description of current research on deep SNNs can be found in Chapter~\ref{cha:Conv} and Chapter~\ref{cha:sdlm}.
}\DIFaddend 
...
In consequence, a new set of spike-based vision datasets is now needed to quantitatively measure progress within the rapidly advancing field of spike-based visual recognition and to provide resources to support \DIFdelbegin \DIFdel{fair }\DIFdelend \DIFaddbegin \DIFadd{objective }\DIFaddend competition between researchers.
...
Therefore a set of common metrics for performance evaluation in spike-based vision is \DIFaddbegin \DIFadd{also }\DIFaddend required to assess SNN models and their hardware implementations.
...
\DIFdelendFL \DIFaddbeginFL \caption[Snapshots of the jAER software.]{\DIFaddendFL 
...
The detailed description of the algorithm was demonstrated by Garibaldi~Pineda-Garc\'ia\, \DIFaddbegin \DIFadd{in the published paper}\DIFaddend ~\citep{liu2016bench} and the source Python scripts to transform images to ROC spike trains, and to convert the results into AER and PyNN spike source arrays, can be found in the dataset website.
...
\begin{figure}[tbh!]
	...
	\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[DVS sensor with flashing input.]{\DIFaddendFL DVS sensor with flashing input.
...
\label{sec:eval}
\DIFdelbegin \DIFdel{As a result of the spike-based processing used in SNN models, new }\DIFdelend \DIFaddbegin \DIFadd{New }\DIFaddend concerns about the latency and energy cost arise over performance assessment \DIFaddbegin \DIFadd{on SNNs, thereby highlighting the advantages of spike-based processing~\mbox{%DIFAUXCMD
\citep{tan2015bench}
}%DIFAUXCMD
}\DIFaddend .
Therefore we propose corresponding evaluation metrics and suggest a sufficient description of SNN models in this section.
Once a model is implemented on a neuromorphic platform, the hardware performance can \DIFaddbegin \DIFadd{also }\DIFaddend be evaluated by running the particular model.
...
\DIFdelbegin \DIFdel{A }\DIFdelend \DIFaddbegin \DIFadd{Consequently, a }\DIFaddend complementary evaluation methodology is essential to provide common metrics and assess both the model-level and hardware-level performance.


\subsection{\DIFdelbegin \DIFdel{Model-Level }\DIFdelend \DIFaddbegin \DIFadd{Model-level }\DIFaddend Evaluation}
\label{subsec:model}
...
\subsection{\DIFdelbegin \DIFdel{Hardware-Level }\DIFdelend \DIFaddbegin \DIFadd{Hardware-level }\DIFaddend Evaluation}
\label{subsec:hw}
...
\begin{table}[hbbp]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \caption{\label{tbl:pynnSetting}%%%
\DIFdelendFL \DIFaddbeginFL \caption[LIF parameter setting using PyNN.]{\DIFaddendFL Parameter setting for the current-based LIF neurons using PyNN.}
	\bgroup
	\def\arraystretch{1.4}
	\begin{tabular}{c c c}
		%\hline
	...
	\end{tabular}
	\egroup
	\DIFaddbeginFL \label{tbl:pynnSetting}
...
\begin{figure}[thb!]
	...
	%	}
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.8\textwidth]{pics_bench/fig6.jpg}
%DIFDELCMD < 	\caption{%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.7\textwidth]{pics_bench/fig6.jpg}
	\caption[The proposed SNN model.]{\DIFaddendFL The (a) training and (b) testing model of the \DIFdelbeginFL \DIFdelFL{two-layered spiking neural network}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{proposed case study SNN: STDP learning with supervised teaching signal}\DIFaddendFL .}
	\label{fig:model}
\end{figure} 
...
\DIFaddend The feed-forward testing network is shown in Figure~\ref{fig:model}(b) where Poisson spike trains are generated the same way as in the training with a total firing rate of 2K~Hz per image.
...
\begin{figure}[htb!]
	...
	\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Comparisons on different test time and input firing rate.]{\DIFaddendFL Accuracy, response time (latency) and synaptic event rate (Sopbs) change over test time and input firing rate per test image.
...
Thus the synaptic events happening in the inter-layer connections are 200\DIFaddbegin \DIFadd{~(100$\times$2) }\DIFaddend times the total input firing rate.
...
\DIFdelend \DIFaddbegin \caption[Comparisons of NEST and SpiNNaker performance.]{\DIFaddend Comparisons of NEST (N) on a PC and SpiNNaker (S) performance averaged over 10 trials.}
	\begin{center}
		\bgroup
		\def\arraystretch{1.4}
		\begin{tabular} {l| c  c c c c c}
...
\begin{figure}[hbt!]
	...
	\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Energy usages of different network size.]{\DIFaddendFL Energy usages of different network size both using NEST (blue) on a PC and SpiNNaker (black).}
	\label{fig:energy}
\end{figure}
...
\section{Summary}
\label{sec:summ}
%DIF < This chapter puts forward the NE dataset as a baseline for comparisons of vision based SNN models and neuromorphic platforms.
\DIFdelbegin \DIFdel{The NE dataset proposed in this chapter makes the comparison of SNNs with conventional recognition methods possible by using converted spike representations of the same vision databases.
As far as we know, this is the first attempt at benchmarking neuromorphic vision recognitionby providing public a }\DIFdelend \DIFaddbegin \DIFadd{This chapter put forward the NE dataset as a unified resource to quantitatively measure progress within the field of neuromorphic vision.
It enables not only objective comparisons among SNNs, but also between conventional ANNs and these }\DIFaddend spike-based \DIFdelbegin \DIFdel{dataset and evaluation metrics.
In accordance with the suggestions from~\mbox{%DIFAUXCMD
\cite{tan2015bench}
}%DIFAUXCMD
, the }\DIFdelend \DIFaddbegin \DIFadd{models.
...
\DIFadd{The current dataset NE-15 contains four spike-based subsets aiming at different purposes: (1) the Poissonian subset is intended for testing rate-based recognition methods;
...
\DIFadd{The proposed complementary evaluation methodology is the first to analytically assess SNN models and the neuromorphic hardware simulators.
...
These special }\DIFaddend evaluation metrics highlight the strengths of spike-based \DIFdelbegin \DIFdel{vision tasksand the dataset design also promotes the research into rapid and low energy recognition (e.g. flashing digits).
}\DIFdelend \DIFaddbegin \DIFadd{AI tasks.
%DIF >  suggested by~\citet{tan2015bench}
}\DIFaddend 

A \DIFaddbegin \DIFadd{potential }\DIFaddend benchmark system is evaluated using the Poissonian subset of the NE15-MNIST dataset.
...
This example \DIFdelbegin \DIFdel{benchmarking system }\DIFdelend demonstrates a recommended way of using the dataset, describing the SNN models and evaluating the system performance.
%They provide a baseline for comparisons and encourage improved algorithms and models to make use of the dataset.
The case study provides a baseline for robust comparisons \DIFdelbegin \DIFdel{between }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend SNN models and their hardware implementations\DIFaddbegin \DIFadd{, and also  validates the feasibility of the database and its evaluation methodology}\DIFaddend .
