...
With this dataset we hope to (1) promote meaningful \DIFdelbegin \DIFdel{comparison between algorithms }\DIFdelend \DIFaddbegin \DIFadd{comparisons between algorithms and neuromorphic platforms }\DIFaddend in the field of \DIFdelbegin \DIFdel{neural computation}\DIFdelend \DIFaddbegin \DIFadd{NE}\DIFaddend , (2) allow comparison with conventional \DIFdelbegin \DIFdel{image recognition methods}\DIFdelend \DIFaddbegin \DIFadd{non-spiking methods on image recognition}\DIFaddend , (3) provide an assessment of the state of the art in spike-based visual recognition, and (4) help researchers identify future directions and advance the field.
...
The rest of this chapter is structured as follows: Section~\ref{sec:method} elaborates the purpose and protocols of the proposed dataset and describes the sub-datasets and the methods employed to generate them; \DIFdelbegin \DIFdel{it also }\DIFdelend \DIFaddbegin \DIFadd{Section~\ref{sec:eval} }\DIFaddend demonstrates the suggested evaluation methodology for use with the dataset.
...
Achieving the brain's energy efficiency motivates the development of neuromorphic hardware, since the human brain \DIFdelbegin \DIFdel{consumes }\DIFdelend \DIFaddbegin \DIFadd{has a power consumption of }\DIFaddend only about 20~W\DIFdelbegin \DIFdel{of power}\DIFdelend ~\citep{drubach2000brain}.
...
\citep{riesenhuber1999hierarchical}
}%DIFAUXCMD
...
}\DIFaddend reported that their SNN implementation was capable of recognising facial expressions with a classification accuracy (CA) of 97.35\% on the JAFFE dataset~\citep{lyons1998coding} which contains 213 images of 7 facial expressions posed by 10 individuals.
%DIF < They employed simple integrate-and-fire neurons with rank order coding (ROC) where the earliest pre-synaptic spikes have the strongest impact on the post-synaptic potentials.
%DIF > They employed simple integrate-and-fire~(IF) neurons with rank order coding (ROC) where the earliest pre-synaptic spikes have the strongest impact on the post-synaptic potentials.
According to~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{vanrullen2002surfing}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{Using }\DIFdelend \DIFaddbegin \DIFadd{Therefore, using }\DIFaddend one spike per neuron, similar to the first spiking wave in biology,~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{delorme2001face}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
Convolutional Neural \DIFdelbegin \DIFdel{Network (ConvNet}\DIFdelend \DIFaddbegin \DIFadd{Networks (ConvNets}\DIFaddend ), inspired \DIFdelbegin \DIFdel{from the receptive field of vision cortex, has }\DIFdelend \DIFaddbegin \DIFadd{by the receptive fields of the visual cortex, have }\DIFaddend been implemented on spiking neurons.
...
\citep{bichler2012extraction}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{As mentioned in Chapter~\ref{cha:intro}, these models typically comprise only two neural layers and exploit STDP and/or Winner-Take-All~(WTA) circuits on the synaptic connections.
\mbox{%DIFAUXCMD
...
A similar model trained with STDP \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{using a }\DIFaddend winner-take-all~(WTA) circuit was tested on a Poissonian spike presentation of the MNIST dataset achieving a performance of 95.0\%~\citep{diehl2015unsupervised}.
...
\section{\DIFdelbegin \DIFdel{Materials and Methods}\DIFdelend \DIFaddbegin \DIFadd{NE Dataset}\DIFaddend }
\label{sec:method}
...
\begin{itemize}
	\item \textit{to promote meaningful comparisons \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{between }\DIFaddend algorithms \DIFaddbegin \DIFadd{and neuromorphic platforms }\DIFaddend in the field of \DIFdelbegin \DIFdel{spiking neural computation}\DIFdelend \DIFaddbegin \DIFadd{NE}\DIFaddend .}
	...
	\item \textit{to allow comparison with conventional \DIFaddbegin \DIFadd{non-spiking methods on }\DIFaddend image recognition\DIFdelbegin \DIFdel{methods}\DIFdelend .}
	...
	\item \textit{to provide an assessment of the state of the art in spike-based visual recognition\DIFdelbegin \DIFdel{on neuromorphic hardware}\DIFdelend .}
...
NE15-MNIST is the spiking version of an original non-spiking dataset which was downloaded from the MNIST Database of Handwritten Digits~\citep{lecun1998gradient}  website\footnote{\DIFdelbegin \DIFdel{http://yann.lecun.com/exdb/mnist/}\DIFdelend \DIFaddbegin \url{http://yann.lecun.com/exdb/mnist/}\DIFaddend }.
...
The detailed description of the algorithm \DIFdelbegin \DIFdel{is demonstrated in~\mbox{%DIFAUXCMD
\citep{liu2016bench}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{was demonstrated }\DIFaddend by Garibaldi~Pineda-Garc\'ia\,\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep{liu2016bench}
}%DIFAUXCMD
...
\label{subsec_flash}
The purpose of including the subset with DVS-recorded flashing digits is to promote research into rapid and accurate `core recognition', thus to encourage \DIFdelbegin \DIFdel{applying }\DIFdelend \DIFaddbegin \DIFadd{research into }\DIFaddend non-rate-based algorithms \DIFdelbegin \DIFdel{, for example ROC, to short DVS output spike trains}\DIFdelend \DIFaddbegin \DIFadd{to shorten the recognition time}\DIFaddend .

%DIF > applying non-rate-based algorithms, for example ROC, to short the DVS output spike trains.
\DIFaddbegin 
...
The subset \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend DVS recorded moving digits is provided by \DIFdelbegin \DIFdel{a collaborator, }\DIFdelend Teresa~Serrano-Gotarredona, and is  presented to address the challenges of position- and scale- invariance in computer vision.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend MNIST digits were scaled to three different sizes, using smooth interpolation algorithms to increase their size from the original \DIFdelbegin \DIFdel{28x28 }\DIFdelend \DIFaddbegin \DIFadd{28$\times$28 }\DIFaddend pixels, and displayed on the monitor with slow motion. 
...
\DIFdelend \DIFaddbegin \section{\DIFadd{Performance Evaluation}}
...
\DIFdelbegin \subsubsection{\DIFdel{Model-Level Evaluation}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Model-Level Evaluation}}
...
\DIFdelbegin \DIFdel{A small number of total synaptic events generated by a recognition task indicates the efficiency of the SNN model.
}\DIFdelend \DIFaddbegin 

%DIF > A small number of total synaptic events generated by a recognition task indicates the efficiency of the SNN model.
\DIFaddend A spike event is a synaptic operation evoked when one action potential is transmitted through one synapse~\citep{sharp2012power}.
...
%			\begin{mycell}{3cm} 91.6\% \\ 300~ms per test \end{mycell} \\% recognition
			...
			\begin{mycell}{3cm} 99.07\%, \\1~s per test\\\DIFaddbeginFL \DIFaddFL{35.24~ms}\\\DIFaddendFL $53.4$M Sopbs\end{mycell}\\ % recognition    
			...
			\begin{mycell}{3cm} 94.72\%, \\1~s per test\\\DIFaddbeginFL \DIFaddFL{21.68~ms}\\\DIFaddendFL $5.26$M Sopbs\end{mycell}\\ % recognition    
			%
...
\DIFdelbegin \subsubsection{\DIFdel{Hardware-Level Evaluation}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Hardware-Level Evaluation}}
...
The LIF neuron model follows the membrane potential dynamics\DIFaddbegin \DIFadd{~(Equation~\ref{equ:LIF_V})}\DIFaddend :
...
where $\tau_m$ is the membrane time constant, $ V_{rest} $ is the resting potential, $ R_{m} $ is the membrane resistance and \DIFdelbegin \DIFdel{$ I_{syn} $ }\DIFdelend \DIFaddbegin \DIFadd{$ I $ }\DIFaddend is the synaptic input current.
...
In terms of this case study, the performance of the model was evaluated with both software simulation (on NEST~\citep{gewaltig2007nest}) and \DIFaddbegin \DIFadd{on a }\DIFaddend hardware implementation (on SpiNNaker).
...
For simplicity of describing the system, \DIFdelbegin \DIFdel{one }\DIFdelend \DIFaddbegin \DIFadd{a set of }\DIFaddend standard configuration is \DIFdelbegin \DIFdel{set }\DIFdelend \DIFaddbegin \DIFadd{used }\DIFaddend as the example in the following sections.
...
\section{Summary}
\label{sec:summ}
\DIFdelbegin \DIFdel{This chapter puts forward the NE dataset as a baseline for comparisons of vision based SNN models and neuromorphic platforms.
It contains }\DIFdelend %DIF > This chapter puts forward the NE dataset as a baseline for comparisons of vision based SNN models and neuromorphic platforms.
\DIFaddbegin \DIFadd{The NE dataset proposed in this chapter makes the comparison of SNNs with conventional recognition methods possible by using converted spike representations of the same vision databases.
As far as we know, this is the first attempt at benchmarking neuromorphic vision recognition by providing public a }\DIFaddend spike-based \DIFdelbegin \DIFdel{versions of existing widely-used databases in the vision recognition field.
...
The datasetaims to provide unified }\DIFdelend \DIFaddbegin \DIFadd{dataset and evaluation metrics.
In accordance with the suggestions from~\mbox{%DIFAUXCMD
\cite{tan2015bench}
}%DIFAUXCMD
, the evaluation metrics highlight the strengths of }\DIFaddend spike-based \DIFdelbegin \DIFdel{vision benchmarks and complementary evaluation methodologies to assess the performance of SNN algorithms.
}\DIFdelend \DIFaddbegin \DIFadd{vision tasks and the dataset design also promotes the research into rapid and low energy recognition (e.g. flashing digits).
}

\DIFadd{A benchmark system is evaluated using the Poissonian subset of the NE15-MNIST dataset.
%DIF > The models were described and their performance on accuracy, network latency, simulation time and energy usage were presented.
...
%DIF > They provide a baseline for comparisons and encourage improved algorithms and models to make use of the dataset.
The case study provides a baseline for robust comparisons between SNN models and their hardware implementations.
}


%DIF > It contains spike-based versions of existing widely-used databases in the vision recognition field.
%DIF > Since new problems will continue to arise before vision becomes a solved question, the dataset will evolve as research progresses. 
%DIF > The conversion methods for transforming images and videos into spike trains will advance. The number of vision datasets will increase and the corresponding evaluation methodologies will evolve.
%DIF > The dataset aims to provide unified spike-based vision benchmarks and complementary evaluation methodologies to assess the performance of SNN algorithms.
\DIFaddend 
