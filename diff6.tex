...
The last two chapters have answered the main research question of this thesis \DIFaddbegin \DIFadd{showing }\DIFaddend that Spiking Neural Networks (SNNs) can be trained both on-line and off-line to achieve the equivalent cognitive capabilities of Artificial Neural Networks~(ANNs).
To provide meaningful comparisons between theses proposed SNN models and other existing methods within this rapidly advancing field of Neuromorphic Engineering~(NE), we propose that a large dataset of spike-based visual stimuli is needed and a corresponding evaluation methodology is also required to \DIFdelbegin \DIFdel{overall estimate the }\DIFdelend \DIFaddbegin \DIFadd{estimate the overall }\DIFaddend performance of SNN models and their hardware implementations.
...
Researchers are using the capabilities created by rapid developments in neuromorphic engineering to address the dual aims of understanding brain functions and building brain-like machines~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{furber2007neural}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{furber2007neural}
}%DIFAUXCMD
}\DIFaddend .
Neuromorphic engineering has delivered biologically-inspired sensors such as DVS~(Dynamic Vision Sensor) silicon retinas~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{serrano2013128, delbruck2008frame, yang2015dynamic, posch2014retinomorphic}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{serrano2013128, delbruck2008frame, yang2015dynamic, posch2014retinomorphic}
}%DIFAUXCMD
}\DIFaddend , which offer the prospect of low-cost visual processing thanks to their event-driven and redundancy-reducing style of information representation.
Moreover, SNN simulation tools~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{davison2008pynn, gewaltig2007nest, goodman2008brian}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
\cite{furber2014spinnaker,  schemmel2010wafer,benjamin2014neurogrid,merolla2014million}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
Achieving the brain's energy efficiency motivates the development of neuromorphic hardware, since the human brain consumes only about 20~W of power~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{drubach2000brain}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{drubach2000brain}
}%DIFAUXCMD
}\DIFaddend .
In the case of visual processing, the brain can accurately recognise objects remarkably quickly, e.g. in 200~ms in monkeys~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{fabre1998rapid}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{fabre1998rapid}
}%DIFAUXCMD
}\DIFaddend , even with short presentations (less than 100~ms) of the target objects~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{keysers2001speed}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{keysers2001speed}
}%DIFAUXCMD
}\DIFaddend .
Such rapid and highly accurate recognition is the target of modelling spike-based visual recognition.


%DIF < In the case of visual processing, the central visual system consists of several cortical areas which, according to anatomical experiments, are placed in a hierarchical structure ~\cite{felleman1991distributed}.
%DIF > In the case of visual processing, the central visual system consists of several cortical areas which, according to anatomical experiments, are placed in a hierarchical structure ~\citep{felleman1991distributed}.
...
%DIF < Biological studies have revealed that the information is unfolded along the ventral stream to the IT (Inferior Temporal) cortex~\cite{dicarlo2012does}.
%DIF > Biological studies have revealed that the information is unfolded along the ventral stream to the IT (Inferior Temporal) cortex~\citep{dicarlo2012does}.
%Inspired by these results, SNN models have successfully been adapted to computer vision tasks.  

%Bioligical inspiration
Inspired by biological studies of the visual ventral pathway, SNN models have successfully been adapted to visual recognition. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{riesenhuber1999hierarchical}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{riesenhuber1999hierarchical}
}%DIFAUXCMD
}\DIFaddend proposed a quantitative modelling framework for object recognition with position-, scale- and view-invariance.
Their cortex-like model has been analysed on several datasets~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
Recently~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{fu2012spiking}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{fu2012spiking}
}%DIFAUXCMD
}\DIFaddend reported that their SNN implementation was capable of recognising facial expressions with a classification accuracy (CA) of 97.35\% on the JAFFE dataset~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{lyons1998coding}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{lyons1998coding}
}%DIFAUXCMD
}\DIFaddend which contains 213 images of 7 facial expressions posed by 10 individuals.
...
\cite{vanrullen2002surfing}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{vanrullen2002surfing}
}%DIFAUXCMD
}\DIFaddend , the first wave of spikes carry explicit information through the ventral stream and in each stage meaningful information is extracted and spikes are regenerated. 
Using one spike per neuron, similar to the first spiking wave in biology,~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{delorme2001face}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{delorme2001face}
}%DIFAUXCMD
}\DIFaddend reported 100\% and 97.5\% accuracies on the face identification task over
...
%DIF < In terms of vision recognition on neuromorphic hardware, TrueNorth~\cite{merolla2014million} has been used to detect and recognise five classes of object in the video clips of the Neovision2 Tower dataset.
%DIF < \cite{Qiao2015re} presented an analogue on-line learning neuromorphic classification system which responds selectively to images of cars and motorbikes selected from the Caltech 101 dataset.
%DIF < Generic training algorithms for pattern recognition~\cite{schmuker2014neuromorphic} and regression/classification tasks~\cite{thakur2016low} have been implemented on neuromorphic platforms.
%DIF > In terms of vision recognition on neuromorphic hardware, TrueNorth~\citep{merolla2014million} has been used to detect and recognise five classes of object in the video clips of the Neovision2 Tower dataset.
%DIF > \citep{Qiao2015re} presented an analogue on-line learning neuromorphic classification system which responds selectively to images of cars and motorbikes selected from the Caltech 101 dataset.
%DIF > Generic training algorithms for pattern recognition~\citep{schmuker2014neuromorphic} and regression/classification tasks~\citep{thakur2016low} have been implemented on neuromorphic platforms.
...
%DIF < \cite{hubel1962receptive} first discovered the model of orientation selectivity (simple cells) and pooling mechanism (complex cells) in the primary cortex in cats, which lay the foundation of the Convolutional Neural Network (CNN)~\cite{lecun1998gradient}.
%DIF > \citep{hubel1962receptive} first discovered the model of orientation selectivity (simple cells) and pooling mechanism (complex cells) in the primary cortex in cats, which lay the foundation of the Convolutional Neural Network (CNN)~\citep{lecun1998gradient}.

Convolutional Neural \DIFdelbegin \DIFdel{Network (ConvNet}\DIFdelend \DIFaddbegin \DIFadd{Networks (ConvNets}\DIFaddend ), inspired from the receptive field of vision cortex, \DIFdelbegin \DIFdel{has }\DIFdelend \DIFaddbegin \DIFadd{have }\DIFaddend been implemented on spiking neurons.
An early Spiking ConvNet model identified the faces of 35 persons with a CA of 98.3\% exploiting simple integrate and fire neurons~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{matsugu2002convolutional}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{matsugu2002convolutional}
}%DIFAUXCMD
}\DIFaddend .
Another Spiking ConvNet model~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{zhao2014feedforward}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{zhao2014feedforward}
}%DIFAUXCMD
}\DIFaddend was trained and tested both with DVS raw data and Leaky Integrate-and-Fire (LIF) neurons.
...
In a further step forward,~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{camunas2012event}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{camunas2012event}
}%DIFAUXCMD
}\DIFaddend implemented a convolution processor module in hardware which could be combined with a DVS for high-speed recognition tasks.
...
Similarly, a real-time gesture recognition model~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{liu2014real}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{liu2014real}
}%DIFAUXCMD
}\DIFaddend was implemented on a neuromorphic system with a DVS as a front-end and a SpiNNaker~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{furber2014spinnaker}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{furber2014spinnaker}
}%DIFAUXCMD
}\DIFaddend machine as the back-end, where LIF neurons built up the ConvNet configured with biological parameters.
...
\cite{bichler2012extraction}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{bichler2012extraction}
}%DIFAUXCMD
}\DIFaddend demonstrated an unsupervised STDP learning model to classify car trajectories captured with a DVS retina. 
A similar model trained with STDP on winner-take-all~(WTA) circuit was tested on a Poissonian spike presentation of the MNIST dataset achieving a performance of 95.0\%~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{diehl2015unsupervised}
}%DIFAUXCMD
.
%DIF < Theoretical analysis~\cite{nessler2013bayesian} showed that unsupervised STDP was able to approximate a stochastic version of Expectation Maximisation, a powerful learning algorithm in machine learning.
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{diehl2015unsupervised}
}%DIFAUXCMD
.
%DIF > Theoretical analysis~\citep{nessler2013bayesian} showed that unsupervised STDP was able to approximate a stochastic version of Expectation Maximisation, a powerful learning algorithm in machine learning.
}\DIFaddend Another STDP trained model tested on computer simulation reached a 93.3\% CA on MNIST and had the potential to be implemented using memristors~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{bill2014compound}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{bill2014compound}
}%DIFAUXCMD
}\DIFaddend . 
Adding synaptic internal state enabled STDP to work on \DIFdelbegin \DIFdel{bistatble }\DIFdelend \DIFaddbegin \DIFadd{bistable }\DIFaddend synapses, and this method achieved the best performance of 96.5\% among similar network architectures. 

Deep Neural Networks (DNNs) have exceeded human-level performance on image classification tasks~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{he2015delving}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{he2015delving}
}%DIFAUXCMD
}\DIFaddend , but mainstream DNN research is focussed on continuous rather than spiking neural networks.
...
Early research into spiking deep networks focussed on converting off-line trained deep network into SNNs~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{o2013real}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{o2013real}
...
The network was initially implemented on an FPGA and achieved a CA of 92.0\%~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{neil2014minitaur}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{neil2014minitaur}
}%DIFAUXCMD
}\DIFaddend , while a later implementation on SpiNNaker scored 95.0\%~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
Recent advances have contributed to better translation by using modified units in a ConvNet~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{cao2015spiking}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{cao2015spiking}
}%DIFAUXCMD
}\DIFaddend and tuning the weights and thresholds~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{diehl2015fast}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{diehl2015fast}
...
An event-driven Contrastive Divergence (CD) training algorithm for Restricted Boltzmann Machines (RBMs) was proposed for Deep Belief Networks (DBNs) using LIF neurons with STDP synapses and verified on MNIST with a CA of 91.9\%~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{neftci2013event}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{neftci2013event}
}%DIFAUXCMD
}\DIFaddend .
The work extended to use stochastic synapses, and the recognition performance increased to 95.8\%~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{neftci2016stochastic}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{neftci2016stochastic}
}%DIFAUXCMD
}\DIFaddend .
...
However, in the field of conventional non-spiking computer vision, there are a number of datasets playing important roles at different times and with various objectives~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{lecun1998gradient,deng2009imagenet,blank2005actions,liu2009recognizing}
}%DIFAUXCMD
.
%DIF < The MNIST~\cite{lecun1998gradient} dataset is a subset of the NIST hand-written digits dataset; due to its straightforward target of classifying real-world images, the plain format of the binary data and simple patterns, MNIST has been one of the most popular datasets in computer vision for over 20 years.
%DIF < ImageNet~\cite{deng2009imagenet} was put forward to provide researchers with a large-scale image database that currently contains 14,197,122 images;
%DIF < this dataset is a well-recognised benchmark test for the deep learning community, and many attempts have been made to improve the performance of machine learning algorithms on this dataset, as was done, for example, by \cite{krizhevsky2012imagenet}.
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{lecun1998gradient,deng2009imagenet,blank2005actions,liu2009recognizing}
}%DIFAUXCMD
.
%DIF > The MNIST~\citep{lecun1998gradient} dataset is a subset of the NIST hand-written digits dataset; due to its straightforward target of classifying real-world images, the plain format of the binary data and simple patterns, MNIST has been one of the most popular datasets in computer vision for over 20 years.
%DIF > ImageNet~\citep{deng2009imagenet} was put forward to provide researchers with a large-scale image database that currently contains 14,197,122 images;
%DIF > this dataset is a well-recognised benchmark test for the deep learning community, and many attempts have been made to improve the performance of machine learning algorithms on this dataset, as was done, for example, by \citep{krizhevsky2012imagenet}.
}\DIFaddend %As a good example of a database catching up with state-of-the-art technologies, Microsoft COCO aims to solve three problems - objects categorisation, context understanding and spatial labelling - in scene understanding by providing large-scale datasets (300,000+ images).
%Similar examples can be found in video datasets.
%DIF < Two early benchmarks, the KTH ~\cite{schuldt2004recognizing} and Weizmann~\cite{blank2005actions} datasets, have been used extensively in the past decade. 
%DIF > Two early benchmarks, the KTH ~\citep{schuldt2004recognizing} and Weizmann~\citep{blank2005actions} datasets, have been used extensively in the past decade. 
%These videos were produced with scripted behaviours in a controlled environment (`in the lab').
%DIF < The YouTube Action Dataset~\cite{liu2009recognizing} targets recognising realistic actions from videos `in the wild';
%DIF > The YouTube Action Dataset~\citep{liu2009recognizing} targets recognising realistic actions from videos `in the wild';
...
%\subsection{MNIST}
%DIF < The MNIST~\cite{lecun1998gradient} dataset is a subset of the NIST hand written digits dataset.
%DIF > The MNIST~\citep{lecun1998gradient} dataset is a subset of the NIST hand written digits dataset.
...
%
%DIF < \subsection{ImageNet~\cite{deng2009imagenet}}
%DIF > \subsection{ImageNet~\citep{deng2009imagenet}}
...
%DIF < Synsets are meaningful concepts described with a few words or phrases, and they are organised in a hierarchy as in WordNet.%~\cite{deng2009imagenet}.
%DIF > Synsets are meaningful concepts described with a few words or phrases, and they are organised in a hierarchy as in WordNet.%~\citep{deng2009imagenet}.
...
%DIF < The dataset is a well-recognised benchmark test for the deep learning community, and many attempts have been made to improve the performance of machine learning algorithms on this dataset, for example~\cite{krizhevsky2012imagenet}.
%DIF > The dataset is a well-recognised benchmark test for the deep learning community, and many attempts have been made to improve the performance of machine learning algorithms on this dataset, for example~\citep{krizhevsky2012imagenet}.
%
%DIF < \subsection{Microsoft COCO~\cite{lin2014microsoft}}
%DIF > \subsection{Microsoft COCO~\citep{lin2014microsoft}}
...
The spike trains are in the format of Address-Event Representation~(AER)~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{mahowald1992vlsi}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{mahowald1992vlsi}
}%DIFAUXCMD
}\DIFaddend data, which are suitable for both event-driven computer simulations and neuromorphic systems.
...
NE15-MNIST is the spiking version of an original non-spiking dataset which was downloaded from the MNIST Database of Handwritten Digits~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{lecun1998gradient}
}%DIFAUXCMD
website}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{lecun1998gradient}
}%DIFAUXCMD
website}\DIFaddend \footnote{\DIFdelbegin \DIFdel{http://yann.lecun.com/exdb/mnist/}\DIFdelend \DIFaddbegin \url{http://yann.lecun.com/exdb/mnist/}\DIFaddend }.
...
MNIST is a popular task among the neuromorphic vision research community\DIFdelbegin \DIFdel{as stated in Section~\ref{sec:intro}}\DIFdelend .
...
\begin{itemize}
	...
	to encourage research into fast recognition methods to mimic the rapid and accurate `core recognition' in the primate ventral visual pathway~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{dicarlo2012does}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
\end{itemize}
The dataset \DIFdelbegin \DIFdel{can be found }\DIFdelend \DIFaddbegin \DIFadd{is published }\DIFaddend in the GitHub\DIFdelbegin \DIFdel{repository at: https://github.com/NEvision/NE15}\DIFdelend \DIFaddbegin \DIFadd{: }\url{https://github.com/NEvision/NE15}\DIFaddend .
\subsection{Data Description}
Two file formats are supported in the dataset: the jAER format~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{delbruck2008frame}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{delbruck2008frame}
}%DIFAUXCMD
}\DIFaddend (.dat or .aedat), and binary files in NumPy~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{numpyPython}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{numpyPython}
}%DIFAUXCMD
}\DIFaddend (.npy) format.
...
Figure~\ref{fig:zero}(a) is a snapshot of the software displaying a .aedat file which was recorded from a DVS retina~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{serrano2013128}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{serrano2013128}
}%DIFAUXCMD
}\DIFaddend .
The resolution of the DVS recorded data is 128$\times$128.
The second spike-based format used is a list of spike source arrays in PyNN~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{davison2008pynn}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{davison2008pynn}
}%DIFAUXCMD
}\DIFaddend , a description language for building spiking neuronal network models.
...
The timing of spikes in the cortex is highly irregular~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{squire1998findings}
}%DIFAUXCMD
.
%DIF < In the cortex, the timing of spikes is highly irregular~\cite{squire1998findings}.
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{squire1998findings}
}%DIFAUXCMD
.
%DIF > In the cortex, the timing of spikes is highly irregular~\citep{squire1998findings}.
}\DIFaddend An interpretation is that the inter-spike interval reflects a random process driven by the instantaneous firing rate.
...
A different way \DIFdelbegin \DIFdel{ofencoding }\DIFdelend \DIFaddbegin \DIFadd{of encoding }\DIFaddend spikes is to use a rank order code; this means
keeping just the order in which the spikes fired and disregarding their exact timing.
Rank-ordered spike trains have been used in vision tasks under a biological plausibility constraint, making them a viable way of encoding images for neural applications~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{van2001rate,sen2009evaluating,masmoudi2010novel}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{van2001rate,sen2009evaluating,masmoudi2010novel}
}%DIFAUXCMD
}\DIFaddend .
%
%\begin{table}[tbh!]
...
{FoCal algorithm~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{sen2009evaluating}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{sen2009evaluating}
}%DIFAUXCMD
}\DIFaddend }.
This algorithm models the foveola, the highest resolution area of the retina, with four ganglion cell layers each with a different scale of centre-surround receptive field~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{kolb2003retina}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{kolb2003retina}
}%DIFAUXCMD
}\DIFaddend .
The detailed description of the algorithm is demonstrated \DIFdelbegin \DIFdel{in~\mbox{%DIFAUXCMD
\cite{liu2016bench}
}%DIFAUXCMD
}\DIFdelend by Garibaldi~Pineda-Garc\'ia\,\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep{liu2016bench}
}%DIFAUXCMD
}\DIFaddend and the source Python scripts to transform images to ROC spike trains, and to convert the results into AER and PyNN spike source arrays, can be found in the dataset website.
...
%DIF < shows the parameters (described in~\cite{sen2009evaluating}) used to compute the convolution kernels at each 
%DIF > shows the parameters (described in~\citep{sen2009evaluating}) used to compute the convolution kernels at each 
%scale $w$.
%
%
%
...
The digits were displayed on an LCD monitor in front of the DVS retina~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{serrano2013128}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{serrano2013128}
}%DIFAUXCMD
}\DIFaddend and were placed in the centre of the visual field of the camera.
...
In total, this subset of the database contains 2\DIFdelbegin \DIFdel{$\times\begin{displaymath}60,000$ \end{displaymath}\DIFdelend \DIFaddbegin \DIFadd{$\times$60K }\DIFaddend recordings for training and 2\DIFdelbegin \DIFdel{$\times}10,000$ }\DIFdelend \DIFaddbegin \DIFadd{$\times$10K }\DIFaddend for testing.
...
\begin{figure}[tbh!]
	...
	\caption{DVS sensor with flashing input.
		...
		(b) The raster plot shows \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL ordered spike sequence over time.
		The total number of spikes \DIFdelbeginFL \DIFdelFL{are }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{is }\DIFaddendFL around \DIFdelbeginFL \DIFdelFL{$7,000$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{7K }\DIFaddendFL for both `ON' and `OFF' events.
...
The same DVS~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{serrano2013128}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{serrano2013128}
}%DIFAUXCMD
}\DIFaddend used in Section~\ref{subsec_flash} captured the movements of the digits and generated spike trains for each pixel in its 128$\times$128 resolution.
A total of \DIFdelbegin \DIFdel{$30,000$ }\DIFdelend \DIFaddbegin \DIFadd{30K }\DIFaddend recordings were made: 10 digits, at 3 different scales, \DIFdelbegin \DIFdel{$1,000$ }\DIFdelend \DIFaddbegin \DIFadd{1K }\DIFaddend different handwritten samples for each.
...
considers }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{dicarlo2012does}
}%DIFAUXCMD
considered }\DIFaddend the rapid ($<$200~ms) and accurate vision recognition in the brain as the essential problem of object recognition.
...
A spike event is a synaptic operation evoked when one action potential is transmitted through one synapse~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{sharp2012power}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{sharp2012power}
}%DIFAUXCMD
}\DIFaddend .
...
Following \DIFdelbegin \DIFdel{to }\DIFdelend the formatted evaluation as in Table~\ref{tb:model_eval}, Table~\ref{tb:software_comparison} lists a few SNN models \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend MNIST classification, although some details are missing. 
...
%		\bgroup
		...
		\begin{tabular}{ l c c c c }
			...
			%
			\begin{mycell}{0.5cm}~\DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
\cite{brader2007learning}
}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\citep{brader2007learning}
}%DIFAUXCMD
}\DIFaddendFL \end{mycell} & 
...
\citep{diehl2015unsupervised}
			...
			%
%DIF < 			\begin{mycell}{0.5cm}~\cite{beyeler2013categorization} \end{mycell} & 
%DIF > 			\begin{mycell}{0.5cm}~\citep{beyeler2013categorization} \end{mycell} & 
...
\citep{neftci2016stochastic}
			...
			%
			\begin{mycell}{0.5cm} %DIF <  %\cite{Stromatias2015scalable} \\ 
				\DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
\cite{stromatias2015robustness}
}%DIFAUXCMD
}\DIFdelendFL %DIF >  %\citep{Stromatias2015scalable} \\ 
...
\citep{zhao2014feedforward}
			...
			\begin{mycell}{3cm} 99.07\%, \\1~s per test\\$53.4$M Sopbs\end{mycell}\\ % recognition    

			\begin{mycell}{0.5cm}Chpt5\end{mycell}& 
			\begin{mycell}{2cm}Poisson\end{mycell} & % preprocessing
			\begin{mycell}{3cm} Two layers,\\ Autoencoders~(AE), \\LIF neurons\end{mycell}& % network
			\begin{mycell}{3cm} Event-driven, spike-based AE\\\DIFdelbeginFL \DIFdelFL{$18,000$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{18K}\DIFaddendFL ~s training, unsupervised\end{mycell}&   % training
			\begin{mycell}{3cm} 94.72\%, \\1~s per test\\$5.26$M Sopbs\end{mycell}\\ % recognition    
			%
			\begin{mycell}{0.5cm} Case Study \end{mycell}  & 
			\begin{mycell}{2cm} Poisson \end{mycell}& % preprocessing 
			\begin{mycell}{3cm} Fully connected decision layer, \\ LIF neurons \end{mycell}& % network
			\begin{mycell}{3cm} K-means clusters,\\Supervised STDP\\\DIFdelbeginFL \DIFdelFL{$18,000$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{18K}\DIFaddendFL ~s of training \end{mycell}& % training
		...
		\begin{minipage}{\textwidth}
			...
			\begin{savenotes}
				...
				\begin{tabular}{l c c c c c c}
					...
					\begin{mycell}{1.8cm} SpiNNaker \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{stromatias2013power}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{stromatias2013power}
}%DIFAUXCMD
					...
					%
					\begin{mycell}{1.8cm} TrueNorth \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{merolla2014million}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{merolla2014million}
}%DIFAUXCMD
}\DIFaddend \end{mycell} & \begin{mycell}{2.0cm}Digital, \\Scalable \end{mycell}& 
					...
					%
					\begin{mycell}{1.8cm} Neurogrid \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{benjamin2014neurogrid}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{benjamin2014neurogrid}
					...
					%
					\begin{mycell}{1.8cm} HI-CANN \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{schemmel2010wafer}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{schemmel2010wafer}
}%DIFAUXCMD
}\DIFaddend \end{mycell} & \begin{mycell}{2.0cm}Mixed-mode,\\Scalable\end{mycell} &
					\begin{mycell}{4.0cm}Fixed models,\\Config params\end{mycell}& 
					\begin{mycell}{2.0cm}Fixed rule\end{mycell}& 
					...
					%
					\begin{mycell}{1.8cm} HiAER-IFAT \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{yu201265k}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{yu201265k}
					...
					Real-time&
					\begin{mycell}{2.0cm}22-pJ/SE\\\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{park201465k}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{park201465k}
...
%DIF < Finer time resolution plays an important role in precision-sensitive neural models or in sub-millisecond tasks~\cite{lagorce2015breaking}.
%DIF > Finer time resolution plays an important role in precision-sensitive neural models or in sub-millisecond tasks~\citep{lagorce2015breaking}.
%Qian Liu done
...
Some researchers have suggested the use of energy per synaptic event (J/SE)~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{sharp2012power,stromatias2013power}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{sharp2012power,stromatias2013power}
}%DIFAUXCMD
}\DIFaddend as an energy metric because the large fan in and out of a neuron means that synaptic processing tends to dominate the total energy dissipation during a simulation.
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{merolla2014million}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{merolla2014million}
}%DIFAUXCMD
}\DIFaddend proposed the number of synaptic operations per second per Watt (Sops/W).
These two measures are equivalent, since J/SE$\times$Sops/W = 1.
%DIF < To investigate the power usage in detail, \cite{Diamond2016comparing} included the power drawn by the attached workstations.
%DIF > To investigate the power usage in detail, \citep{Diamond2016comparing} included the power drawn by the attached workstations.
...
The neuron with \DIFaddbegin \DIFadd{the }\DIFaddend highest output firing rate classifies a test image as the digit it represents.
...
In PyNN, $ R_{m} $ is \DIFdelbegin \DIFdel{presented }\DIFdelend \DIFaddbegin \DIFadd{represented }\DIFaddend by $ R_{m}=\tau_m/C_{m} $, where $C_{m} $ is the membrane capacitance.
...
The connections between the input neurons and the decision neurons are plastic, so the connection weights can be modulated during training with a multiplicative STDP learning rule, \DIFdelbegin \DIFdel{referring }\DIFdelend \DIFaddbegin \DIFadd{refer }\DIFaddend to Section~\ref{subsec:STDP} for more detail.
...
In terms of this case study, the performance of the model was evaluated with both software simulation (on NEST~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{gewaltig2007nest}
}%DIFAUXCMD
) and }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{gewaltig2007nest}
}%DIFAUXCMD
) and on a }\DIFaddend hardware implementation (on SpiNNaker).
...
The training set of \DIFdelbegin \DIFdel{$60,000$ }\DIFdelend \DIFaddbegin \DIFadd{60K }\DIFaddend hand written digits are firstly classified into 100 classes, 10 subclasses per digit, using K-means clusters.
...
The firing rates of the input neurons are assigned linearly according to their intensities and the total firing rate of all the 28$\times$28 input neurons is normalised to \DIFdelbegin \DIFdel{$2,000$}\DIFdelend \DIFaddbegin \DIFadd{2K}\DIFaddend ~Hz, that is, the sum of the firing rates of all of the input neurons is \DIFdelbegin \DIFdel{$2,000$}\DIFdelend \DIFaddbegin \DIFadd{2K}\DIFaddend ~Hz.
All the images together are presented for \DIFdelbegin \DIFdel{$18,000$}\DIFdelend \DIFaddbegin \DIFadd{18K}\DIFaddend ~s (about 300~ms per image) during training and at the same time a teaching signal of 50~Hz is conveyed to the decision neuron to trigger STDP learning.
...
Thus the strategy of replacing weak weights to \DIFaddbegin \DIFadd{the }\DIFaddend same negative values was used throughout the case study.
The feed-forward testing network is shown in Figure~\ref{fig:model}(b) where Poisson spike trains are generated the same way as in the training with a total firing rate of \DIFdelbegin \DIFdel{$2,000$}\DIFdelend \DIFaddbegin \DIFadd{2K}\DIFaddend ~Hz per image.
The input neurons convey the same spike trains to every decision neuron through its \DIFdelbegin \DIFdel{responding }\DIFdelend \DIFaddbegin \DIFadd{corresponding }\DIFaddend trained synaptic weights. 
One test trial contains \DIFdelbegin \DIFdel{$10,000$ }\DIFdelend \DIFaddbegin \DIFadd{10K }\DIFaddend images in total and each image is presented once and lasts 1~s with a 0.2~s blank period between consecutive images.
...
In our experiment, two sets of weights were applied: the original STDP trained weights, and \DIFaddbegin \DIFadd{the }\DIFaddend scaled-up weights which are 10 times stronger.
The spike rates of the test samples were also modified, ranging from 10 to \DIFdelbegin \DIFdel{$5,000$}\DIFdelend \DIFaddbegin \DIFadd{5K}\DIFaddend ~Hz.
\begin{figure}[htb!]
	\centering
...
With stronger weights, the accuracy is much higher when the input firing rate is less than \DIFdelbegin \DIFdel{$2,000$}\DIFdelend \DIFaddbegin \DIFadd{2K}\DIFaddend ~Hz.
...
When the spiking rate is less than \DIFdelbegin \DIFdel{$2,000$}\DIFdelend \DIFaddbegin \DIFadd{2K}\DIFaddend ~Hz, the network with stronger weights has a much shorter latency.
...
Figure~\ref{fig:assess}(e) shows the stable Sopbs of the entire network when the input firing rate is held at \DIFdelbegin \DIFdel{2000}\DIFdelend \DIFaddbegin \DIFadd{2K}\DIFaddend ~Hz and the test time increases.
...
Although the variations in the Sopbs \DIFdelbegin \DIFdel{climbs }\DIFdelend \DIFaddbegin \DIFadd{climb }\DIFaddend to around 8~kHz, it is not obvious in the figure because the output firing rates are relatively low and therefore so are the differences.
...
For the purpose of comparing the accuracy, simulation time, number of synaptic events and energy usage, different configurations have been tested on NEST (working on a PC with CPU: i5-4570 and 8G \DIFaddbegin \DIFadd{bytes }\DIFaddend memory) and on SpiNNaker.
...
The input rates in all of the tests are \DIFdelbegin \DIFdel{$5,000$}\DIFdelend \DIFaddbegin \DIFadd{5K}\DIFaddend ~Hz, and each image is presented for 1~s with a 0.2~s blank period between consecutive images during which the model receives no input.
...
\begin{sidewaystable}[ph!] 
	...
	\begin{center}
		...
		\begin{tabular} {l| c  c c c c c}
			...
			time (s)
			& S
			& \DIFdelbegin %DIFDELCMD < \multicolumn{5}{c}{$12,000$}
%DIFDELCMD < 			%%%
...
%DIF < \cite{Diamond2016comparing} compared the power usage and simulation time on three hardware platforms in detail regarding to all the stages while this paper focuses on the simulation part only.
%DIF > \citep{Diamond2016comparing} compared the power usage and simulation time on three hardware platforms in detail regarding to all the stages while this paper focuses on the simulation part only.
...
On the other hand, SpiNNaker works in (biologically) real time and the simulation time becomes shorter than the NEST simulation when \DIFdelbegin \DIFdel{$1,000$ }\DIFdelend \DIFaddbegin \DIFadd{1K }\DIFaddend patterns per digit (\DIFdelbegin \DIFdel{$1,000$ }\DIFdelend \DIFaddbegin \DIFadd{1K }\DIFaddend decision neurons per digit) are used.
...
%A complete SNN simulation contains three main stages: pre-processing (such as network building and input data generation), simulation and post-processing (for example, extracting recorded data).
%DIF < \cite{Diamond2016comparing} compared the power usage on three hardware platform in detail of all the stages while this paper focuses on the simulation part.
%DIF > \citep{Diamond2016comparing} compared the power usage on three hardware platform in detail of all the stages while this paper focuses on the simulation part.
...
The NEST simulation costs 767.67~s on average for the entire \DIFdelbegin \DIFdel{$12,000$}\DIFdelend \DIFaddbegin \DIFadd{12K}\DIFaddend ~s biological-time test, 20~W in power use on the PC and $15.35$~KJ of energy, while SpiNNaker works in real time using $4.92$~KJ of energy at a power of 0.41~W (see Table~\ref{tbl:compare}).
...
This result provides a baseline for comparison with other SNN models and neuromorphic \DIFdelbegin \DIFdel{hardwares}\DIFdelend \DIFaddbegin \DIFadd{hardware}\DIFaddend , and no optimisation is applied.
...
%DIF < This section aims to review and reinterpret results from previously published studies  \cite{Stromatias2015LiveDemo,Stromatias2015scalable,stromatias2015robustness}, which utilised the identical off-line trained\footnote{\url{https://github.com/dannyneil/edbn/}} spiking DBN as presented by \cite{o2013real}.
%DIF > This section aims to review and reinterpret results from previously published studies  \citep{Stromatias2015LiveDemo,Stromatias2015scalable,stromatias2015robustness}, which utilised the identical off-line trained\footnote{\url{https://github.com/dannyneil/edbn/}} spiking DBN as presented by \citep{o2013real}.
%
%DIF < Deep learning architectures and, in particular, Convolutional Networks~\cite{lecun1998gradient} and Deep Belief Networks (DBNs)~\cite{hinton2006fast} have been characterised as one of the breakthrough technologies of the decade~\cite{MIT_TechReview}. One of the advantages of these type of network is that their performance can be increased by adding more layers~\cite{hinton2006fast}.
...
%DIF < These parameters were chosen by~\cite{o2013real} to train this spiking DBN network. 
%DIF < %Later,~\cite{neil2014minitaur} used the identical offline trained network to map it on an event-driven FPGA implementation called Minitaur.
%DIF > These parameters were chosen by~\citep{o2013real} to train this spiking DBN network. 
%DIF > %Later,~\citep{neil2014minitaur} used the identical offline trained network to map it on an event-driven FPGA implementation called Minitaur.
...
%\end{table}
%
%DIF < The pixels of each MNIST digit from the testing set are converted into Poisson spike trains as described in Section~\ref{sec:poissonian}. %with a rate proportional to the intensity of their pixel, while their firing rates are scaled so that the total firing rate of the input population is constant~\cite{o2013real}.
...
%DIF < Using the same weight precision of Q3.8, SpiNNaker achieved a CA of 94.94\% when $1,500$~Hz was used for the input population~\cite{Stromatias2015scalable}. With the same firing rates and weight precision, Brian achieved a CA of 94.955\%. Results are summarised in Table~\ref{tab:casimulators}.
%DIF > Using the same weight precision of Q3.8, SpiNNaker achieved a CA of 94.94\% when $1,500$~Hz was used for the input population~\citep{Stromatias2015scalable}. With the same firing rates and weight precision, Brian achieved a CA of 94.955\%. Results are summarised in Table~\ref{tab:casimulators}.
...
%DIF < \cite{stromatias2015robustness} showed that spiking DBNs are capable of maintaining a high CA even for weight precisions down to Q3.3, while they are also remarkably robust to high levels of input noise regardless of the weight precision. 
%DIF > \citep{stromatias2015robustness} showed that spiking DBNs are capable of maintaining a high CA even for weight precisions down to Q3.3, while they are also remarkably robust to high levels of input noise regardless of the weight precision. 
%\begin{figure}[hbt!]
%	\centering
%	\includegraphics[width=0.6\textwidth]{pics_bench/fig9.jpg}
%DIF < 	\caption{DBN classification accuracy (CA) as a function of the weight bit precision for different input firing rates \cite{stromatias2015robustness}.}
%DIF > 	\caption{DBN classification accuracy (CA) as a function of the weight bit precision for different input firing rates \citep{stromatias2015robustness}.}
...
%DIF < 	\caption{Mean classification latency (black) and classification accuracy (blue) as a function of the input firing rate for the spiking DBN. Results are averaged over 4 trials, error bars show standard deviations \cite{Stromatias2015scalable}.}
%DIF > 	\caption{Mean classification latency (black) and classification accuracy (blue) as a function of the input firing rate for the spiking DBN. Results are averaged over 4 trials, error bars show standard deviations \citep{Stromatias2015scalable}.}
...
%DIF < 	\caption{Histogram of the classification latencies for the MNIST digits of the testing set when the input rates are set to $1,500$~Hz. The mean classification latency of the spiking DBN on SpiNNaker is 16 ms \cite{Stromatias2015scalable}.}
%DIF > 	\caption{Histogram of the classification latencies for the MNIST digits of the testing set when the input rates are set to $1,500$~Hz. The mean classification latency of the spiking DBN on SpiNNaker is 16 ms \citep{Stromatias2015scalable}.}
