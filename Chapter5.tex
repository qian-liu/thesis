...
Hence, the firing rate ($\lambda_x$) of the Poisson generator is $K$ times of the numerical value x, and can be approximated by the averaging spike count, $N_T(s_x)$, of the generated spike train $s_x$ over time length of $T$~ms:
\begin{equation}
\lambda_x = Kx \approx N_T(s_x) = \frac{1000}{T} \sum_{t=1}^{T} s_x(t) ,
\end{equation} 
and 1000 is the scale factor to transfer frequency per millisecond to frequency per second.
...
The weight rise/drop according to a rectangular STDP curve can detect the spike events of a pair of neurons when post-synaptic spike occurs no later than $\tau_{win}$, see Figure~\ref{fig:rtg_stdp}.
...
There are two set-ups of the same network architecture where ten visible neurons connect to the layer of ten hidden neurons symmetrically with all-to-all weights: Exp1, the values of all the input data are 1, $input_1 = [1, 1, 1,...,1]$; and Exp2, the input data range from 0.1 to 1 linearly with step of 0.1, $input_2 = [0.1, 0.2, 0.3,...,1]$.
These two experiments provide a close observation on the weight updates given the constant input values.
And Exp1 shows how the network responds to a same input value and reconstructs it; while Exp2 can demonstrate the influence of ranging input values, more importantly how different firing rate may affect the corresponding SNN.
...
The reason of the slower training comes to the weaker input which also results in lower output rate of the hidden units comparing to Exp1, see Figure~\ref{fig:ae_orig}(f), so that the positive part of the weight change, $\eta h_i v_j$, is much weakened. 
...
The spike count $N_{\tau_{dur}}$ of a generated Poisson spike train then transformed to the noisy input, $N_{\tau_{dur}}/(\tau_{dur} * K)$.
...
On the contrary, if either $v_j$ or $v'_j$ spikes outside the effective STDP window, the weight will remain unchanged.
...
\begin{figure}
	...
	\label{fig:SAE_orig}
\end{figure}
In order to reproduce the experiments of the AEs, we select the parameters (see Table~\ref{tbl:srm}) for training SAEs and also apply the same parameter for SRBM experiments.
Each input vector (image) presents as spiking trains with the length of 100~ms, and the STDP window is set to 10~ms.
The input values are scales by $K=100$ times, thus are used as firing rates of the spike trains.
Therefore, according to Equation~\ref{equ:eta_s}, the learning rate of SAEs and SRBMs is $10^{-4}$.

To compare with the AE experiments on noisy input data, Figure~\ref{fig:SAE_orig} records the weight change during training and the testing results generated by the test of each image presenting 1~s long.
...
The training is slower than the AE experiment and the loss takes longer time to decrease to a same level.
...
In addition, the loss performance drops comparing to the original SAE test because of the longer STDP window $\tau_{win}$ makers SRM more accurate reflecting to the first property of SRM algorithm in Section~\ref{sec:SRM}.
...
Comparing to the other solutions, it requires doubled Poisson spike trains: the input spikes work the same to activate the network, and the teaching spikes do not impact on the neural dynamics but only trigger learning on the synapses between the input layer and the hidden layer.

In terms of SAE training, teaching signals  alleviate the problem of correlations similar as the noisy threshold does, and the variance of the loss shrinks (Figure~\ref{fig:sols_ae} Solution~3).
...
And the over all count of spikes for each pixel is used as noisy input of convectional training to provide an equal comparison to SNNs.
...
We randomly select 49 hidden neurons out of 500 to display the weighs between the hidden and visible units (only the 784 neurons representing the images), see Figures~\ref{fig:weights_ae} and~\ref{fig:weights_rbm}.
The grey scales from white to black representing the values between -1 to 1.
Unlike the diverging weights in the previous section, the trained weights of all the methods keep within the similar range thanks to the big dataset which restricts the weight searching to a certain scope.  

The trained weights of original AE and AE with Poisson noisy input are of little difference.
...
The shorter period each image presents to the network, less energy is required when implemented in hardware.
...
Thus, about 400~ms is needed for SRBM with teaching signal to perform close to the best CA.
...
Thus, it may only cost SpiNNaker 0.021~W of power running for $2,000$~s, 42~J, for the whole synaptic events of the entire SAE testing on MNIST.
...
It is surprising to see that the original spike based training performs the best in both SAE and SRBM models.
A possible explanation could be overfitting, that it is easy for the models to reconstruct the images have been shown but difficult to generalise the models to unseen data.
...
SAE with noisy membrane threshold converges slowly to the loss level of noisy AE, while in SRBM training, the noise makes the loss oscillated and converges earlier than the traditional training.
...
whereas in SRBMs, teaching signal smooths the loss performance, and both curves converge blow the nRBM loss.

Therefore, it depends on the requirements to choose a proper spiking model for reconstruction.
...
On the contrary, SAE of noisy threshold wins in energy-efficient training and testing on classification, and is stable in reconstruction with similar performance of noisy AE.
...
This is the first attempt of formalising the biological-plausible learning rule to perform approximately to the unsupervised deep learning models.
The promising results exhibit equivalent and even surpassing classification and reconstruction capabilities of SAEs and SRBMs comparing to their conventional methods.
