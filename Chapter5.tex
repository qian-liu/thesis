...
For Exp~I the reconstruction error (loss or squared mean error) reduces exponentially to the limit of the computer's float precision (Figure~\ref{fig:ae_orig}(g)), the training (with size of $10\times10$) reach the performance of the loss of 0.001 using about 500 steps, and from that point the weights~(Figure~\ref{fig:ae_orig}(a)), visible reconstruction~(Figure~\ref{fig:ae_orig}(c)), and the hidden units~(Figure~\ref{fig:ae_orig}(e)) nearly stabilise.
