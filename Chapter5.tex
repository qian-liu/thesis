...
\begin{equation}
\lambda_a \lambda_b \approx \textit{\textrm{SRM}}(s_a, s_b, \tau_\textit{\textrm{dur}}, \tau_\textit{\textrm{win}}) = \frac{10^6}{\tau_{\textit{\textrm{dur}}} \tau_{\textit{\textrm{win}}}} \sum_{t=1}^{\tau_{\textit{\textrm{dur}}}} [s_a(t) \sum_{t_b=t}^{t+\tau_{\textit{\textrm{win}}}} s_b(t_b)]~.
\end{equation} 
...
\end{equation}
where $ \eta_s$ presents the learning rate of the STDP rule.
...
Thus, SRM can be described with weight updates caused by STDP:
\begin{equation}
...
If we separate the weight update of Equation~\ref{equ:two_sep} to a positive $\Delta w_+$ and negative part $\Delta w_-$:
\begin{equation}
\left\{
\begin{aligned} 
\Delta w_+ &= \eta ab\\
...
\right~,
\end{equation}
it is straight forward to estimate the parameter $\eta_s$ to precisely transform the weight update from numerical calculations to spike-based learning rules:
\begin{equation}
\Delta w_+ = ab=\frac{\lambda_a \lambda_b}{K^2} \approx 
\frac{\textit{\textrm{SRM}}(s_a, s_b, \tau_\textit{\textrm{dur}}, \tau_\textit{\textrm{win}})}{K^2} =
\frac{10^6}{\tau_{\textit{\textrm{dur}}}
\tau_{\textit{\textrm{win}}}
\eta_s K^2}
\Delta w_+~.
...
In addition, the default parameters of SNNs were the same as listed in Table~\ref{tbl:srm} except that in the improved solutions of SNN training the STDP window $tau_\textit{\textrm{win}}$ was doubled to 20~ms and the learning rate $\eta_s$ was set to $5 \times 10^{-5}$ accordingly;
