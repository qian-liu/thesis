...
The first comparison was conducted directly on the trained weights after three epochs of the entire training images, which demonstrated a quantitative analysis of the conventional methods and the SNN training solutions stated above.
...
We will start from the default configurations of the experiments, then take a closer look into the energy considerations: testing time and spiking rates.
...
\paragraph{Default configurations\\}
...
Here we highlight the effectiveness of the proposed method that (1) the learning curves of the SNNs are close to the conventional ANNs which proves the accurate estimation of the parameters used in SRMs; and (2) the decorrelated spike trains enables the genuine `live and learn' in on-line SNN training.
%\begin{figure}
%	\centering
%	\begin{subfigure}[t]{0.48\textwidth}
...
\section{Summary}
This chapter answers the research problem that SAEs and SRBMs can be trained with biologically-plausible synaptic plasticity using SRM and demonstrate learning capability as competent as Deep Learning modules: AEs and RBMs.
The proposed on-line training method and its improving solutions formalise a local learning algorithm based on STDP, the on-line, event-based learning rule, and have been applied to train spiking AEs and RBMs.
