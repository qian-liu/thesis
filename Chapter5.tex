\chapter[On-line SNN training with SRM]{On-line SNN training with Spike-Based Rate Multiplication}
\label{cha:sdlm}
%Paragraph One: LINK
...
Instead of transforming off-line trained ANN models into SNNs, we explore on-line approaches which directly modulate the plastic synapses between spiking neurons in biologically-plausible manner.
...
\label{sec:SRM_intro}
Before we investigate into the proposed method, it is helpful to clarify what on-line learning addresses in the thesis context and what special features exist in the SNN training.
Firstly, on-line systems `learn through play' that there is no separation between a training and a testing phase.
Typically, the systems learn and improve their capability as more data feeding to them continuously.
Secondly, they `live and learn' which means on-line learning never stops.
Thus, it is easy for an on-line system to transfer a learning task, by simply providing different data.
However, once a model is trained off-line, it remains fixed and stops learning.
The brain is a natural on-line system, thus brining its learning techniques to SNNs will equip neuromorphic computers with genuine learning capability towards Neuromorphic Cognition.
...
Therefore, the modulation of the synaptic efficacy is event-driven by the spikes and operates in biologically real time.

%Towards Neuromorphic Cognition, it is significant to endow these brain-like machines with on-line learning ability.

The main difficulty of proposing effective on-line methods is the lack of knowledge about learning in the brain.
...
However, the most common training algorithm of ANN, Backpropagation~(BP), is hardly transformed into STDP, since it only works locally with a teaching signal; but, BP does not provide the teaching targets for all the hidden units of a network.
...
Another problem is to accurately transform numerical calculations of weights modulation to spike-based synaptic learning rules.
Existing methods suffer from the performance loss due to the imprecise translation.
Moreover, the correlation across spike trains brings down the learning performance drastically after it peaks, thus making `live and learn' far from practice.

To address the problem of imprecise translation, we propose the Spike-based Rate Multiplication~(SRM) method.
This algorithm is the first to accurately transform numerical calculations of weight tuning to precise parameter configurations of equivalent STDP rules.
We select multiplication to be translated into SNN since it is the core operation in the greedy layer-wise training of Autoencoders~(AEs, Equation~\ref{equ:ae_widrow_hoff}) and Restricted Boltzmann Machines~(RBMs, Equation~\ref{equ:rbm_train}).
Regarding the performance loss caused by correlated spike trains, we put forward four methods to reduce the correlated spikes while learning, which is the only attempt to tackle this problem as far as we know.
Therefore, the on-line learning can keep going without worrying a manual stop.
...
in addition, the learning performance achieves the same, sometimes even superior, than the conventional ANNs and stabilises around the peak performance without decay thanks to the decorrelation methods.
...
Therefore the best classification performance achieved was only about 81.5\% on MNIST task, using purely spiking neurons.
%1. Neftci
...
The inspiration of the proposed method is to provide an equivalent learning rule as ReSuMe on rate-encoded SNNs.
...
If we see \textit{pre}, \textit{post} and \textit{teach} as vectors individually, and \textit{w} as a weight matrix of the all-to-all connections between \textit{pre} and \textit{post} in Figure~\ref{fig:resume}, it will form the ADALINE (Adaptive Linear Element)~\citep{widrow1960adaptive} network, see Figure~\ref{fig:adaline}.
...
\label{equ:two_sep}
\end{equation}
Especially for the training of AEs, the weight updates are the same as Equation~\ref{equ:widrow-hoff} if using Rectified Linear Unit~(ReLU) as the activation function.

Therefore, the multiplications are the core operations in the training of these rate-based ANN models.
Thus we propose the SRM to accurately transform the product of rates to weight tuning of event-based, biologically-plausible learning in SNNs.
There are a few steps to be followed: (1) present rate multiplication with simultaneous spikes generated from a pair of connected spiking neurons;
...
\label{equ:srm}
\end{equation}
So far we have accurately transformed numerical calculations of weight tuning to precise parameters of the SRM, thereby to the configurations on the STDP rules.
...
The initial weights were randomly generated with unified distribution from 0 to 0.01 and the learning rate of conventional models $\eta$, was set to 0.001, and was set to 0.0001 for spike-based training.
...
As a 'live and learn' system, there was no stop for learning, however for the purpose of observing the reconstruction performance, the weights were frozen every 10 steps of training and were validated on the testing data. 
...
Hence, shown in Figure~\ref{fig:noise_input}, the noisy training input added to the clean data a Gaussian noise with -0.05 mean and 0.29 variance for Exp1, and -0.02 mean and 0.22 variance for Exp2;
...
(e) and (f) draw the output of the hidden units during testing and assist the observation on weight change and the reconstruction;
...
The reconstruction loss reduced exponentially to the limit of the computer's float precision (Figure~\ref{fig:ae_orig}(g)), and reached $10^{-4}$ using about 600 steps, and from that point the weights, visible reconstruction, and the output of the hidden units nearly stabilised, see Figure~\ref{fig:ae_orig}(a,c,e).
...
The same experiments were repeated with noisy training and testing data to provide a fair comparison with the spike-based Deep Learning modules.
...
Secondly, the reconstruction and the output of the hidden units were noisy compared to the training and testing on clean data.
However, notably, the noise was much reduced from the noisy input data compared to Figures~\ref{fig:noise_input}~(c) and (d), indicating the robustness of AEs. 
...
Equation~\ref{equ:ae_widrow_hoff} states the learning rule of AE, which can be approximated by adding a positive STDP and a negative STDP in SNN training:
\begin{equation}
\label{equ:sae}
\begin{aligned}
...
The diverging weights not only made the loss increase, for example as shown in Figure~\ref{fig:srbm_orig}(g), but also led the weights too far from the expected effective range of the conventional AE or RBM training.
...
This section proposes four solutions to reduce the correlation between spike trains, thereby approximates AE and RBM training with equivalent spike-based on-line learning of SAEs and SRBMs.
...
To make the comparisons of all the proposed solutions more straightforward, we list the weights and loss change from Exp2 in Figure~\ref{fig:sols_ae} (SAE) and Figure~\ref{fig:sols_rbm}.
...
In terms of SAE training, teaching signals alleviated the problem of correlations, as for the noisy threshold, and the variance of the loss decreased (Figure~\ref{fig:sols_ae}[S3]).
We could observe a higher level of loss compared to the previous solutions, since a constant negative bias existed in the reconstructions.
It was caused by the stronger correlation on the negative weight update, therefore, to weaken the weight decrease the reconstruction had to be lower than the expected.  
...
Having discussed the problem of training SAEs and SRBMs, we finally applied the proposed methods to the MNIST task.
...
The training was conducted using a one-layer AE or RBM, see Figure~\ref{fig:MNSIT}, which consisted of 794 visible units including 784 neurons representing the images, 10 label units marking the classification, and 500 neurons are in the hidden layer.
...
the initial weights were identical for all the experiments as well as the input spike trains.
...
The first comparison was conducted directly on the trained weights after three epochs of the entire training images, which qualitatively demonstrated the difference of learning performance between conventional methods and the SNN training solutions stated above.
...
The other improved SAE trainings demonstrated a closer feature extraction to the conventional AEs.
...
In brief, from the qualitative comparisons on the trained features, the original SAE was the only exception which produce `substandard' features among the SAE methods.
However, in RBMs, SRBM-S3 generated the most qualitative features of all the SRBM methods and only SRBM-S3 and SRBM-S4 achieved similar learning performance as did the conventional nRBM-NI.
...
We start from the default configurations of the experiments, then take a closer look into the energy considerations.
%: testing time and spiking rates.
...
As the default configuration, the testing time was set to 1~s and $K$=100~Hz.
...
The test with noisy input (NI) on conventional Deep Learning modules was the important baseline for comparisons of conventional ANN training to the equivalent on-line biologically-plausible learning in SNNs.
Because, the NI test exploited the same training and testing data used in SNNs: the spike count of the Poisson spike trains fed into SNNs.
Thus, the evaluations on CA quantitatively measured how well the spike-based training fit to the conventional Deep Learning.


The results of conventional and spiking AE were shown in Figure~\ref{fig:sdlm_ca}(a).
Most significantly, in accordance with qualitative comparisons in Section~\ref{subsec:MNIST_weight}, all the SAE training, except for the original SAE, outperformed the AE-NI (93.29\%) baseline and achieved equivalent, even better CA than conventional AE with clean data (93.76\%).
The training on SAEs was slower than on AE at the beginning, due to the reason (explained in Section~\ref{subsec:exp_SAE}) of low spiking rates.
The classification performance of SAEs reached the AEs' after about 50K training steps, and started to diverge:
the original SAE kept a CA close to that of AEs until it peaked at 92.71\% at 87K steps, and then the CA began to decay because of the correlation problem;
...
Thus, decorrelated SAEs were proved to be as competent as AEs in classification tasks and their learning speed closely fit to the conventional AEs.

For the RBM testing, nRBM was more sensitive to the noise of the training images than AE so that the CA of nRBM-NI slowly dropped and oscillated more and more strongly as training proceeded, see Figure~\ref{fig:sdlm_ca}(b).
The peak performances of nRBM and nRBM-NI appeared on 146K and 97K steps at 94.85\% and 93.16\% respectively. 
The quantitative testing results of SRBMs drew a consistent conclusion with the trained weights described qualitatively above in Section~\ref{subsec:MNIST_weight}: the original SRBM worked poorly because of the highly correlated spikes; SRBM-S2 performed slightly better since the noisy membrane threshold reduced the correlations; SRBM-S3 operated the best which overtook the nRBM-NI baseline and fit the same learning curve of nRBM; and finally the performance of the combined method was between the S2 and S3 solutions, moreover, exceeded the nRBM-NI baseline.
SRBM-S3 achieved the best CA of 94.35\% among the SRBM models.
In short, although most of the decorrelation solutions performed worse than those of AEs, teaching signals of the S3 solution worked extremely well for training SRBM which achieved the same CA and reproduced the learning speed of conventional nRBM.

Therefore, we highlight the effectiveness of the proposed method taht (1) the learning curves of the SNNs are close to the conventional ANNs which proves the accurate estimation of the STDP parameter configurations using SRM; (2) the proposed solutions decorrelated the spike trains and enables the genuine `live and learn' in on-line SNN training; thereby (3) SAEs and SRBMs achieve the equivalent cognitive capabilities as the corresponding Deep Learning modules.
%\begin{figure}
%	\centering
%	\begin{subfigure}[t]{0.48\textwidth}
...
\begin{figure}
	...
	\caption{Reconstruct of the same digit '2' with trained weights using conventional AEs and RBMs to compare with their spike-based models.
		...
		All the reconstructions are taken on the steps of 1K, 10K, 60K (1~epoch), 120K (2~epoch), and 180K (3~epoch) during training.}
...
Figure~\ref{fig:sdlm_recon} showed how reconstruction improved over increasing training steps for each model.
Conventional RBMs demonstrated a better reconstruction capability than AEs in general, see Figure~\ref{fig:sdlm_loss}, thus RBMs were usually used for pre-training AEs or Deep Belief Networks.

It was surprising to see that the original SAE and SRBM trainings performed best in both SAE and SRBM models.
A possible explanation could be overfitting.
It was easy for the models to reconstruct the images which had been shown, but difficult to generalise the models to unseen data.
Classification, here, was equivalent to image in-painting in AEs and RBMs.
Therefore, the original SAE and SRBM worked well on reconstruction but poorly on classification.
Qualitatively shown in Figure~\ref{fig:sdlm_recon}, SRBMs reconstructed the image closely to the original input, while SAEs reproduced the image more closely to its `concept', a general digit.
Moreover, the original SAE and SRBM, see Figures~\ref{fig:sdlm_recon}(c) and (i), reconstructed the input image more closely than their improved decorrelated solutions respectively.

For the other decorrelation solutions in SAE, 
SAE-S2 converged slowly to the reconstruction loss level of AE-NI, 
while SAE-S3/S4 were sensitive to the extra teaching signals when trained for reconstruction tasks, thus their performance oscillated roughly during training.
In brief, the decorrelated SAE models converged to the same level of AE-NI and proved to be as equivalent as the conventional RBMs.

Regarding SRBM training, SRBM-S2 reached its best loss level the earliest, however vibrated around the same level of conventional nRBM-NI as training proceeded.
Meanwhile, the teaching signal smoothed the loss curve, and both SRBM-S3 and SRBM-S4 converged to lower levels of loss than the conventional nRBMs.
...
\section{Summary}
This chapter answers the research question that SNNs can be trained on-line with biologically-plausible learning rules and demonstrate  competent cognitive capabilities as conventional Deep Learning modules.
...
(1) present rate multiplication with simultaneous spikes generated from a pair of connected spiking neurons;
(2) capture the simultaneous events by the weight change of the synaptic connection using the STDP learning rule;
(3) precisely transform the learning rate $\eta$ to parameters used in SRM, thereby to make sure the weight change raised by STDP is equivalent to that of original ANN training.
Therefore, the SRM is the first to mathematically estimate the precise parameter configurations of the on-line STDP rules.

%%not limited to IF
%Although, existing methods suffer from the performance loss due to the imprecise translation, the proposed method proved to be effective where the learning curves of the SNNs are close to the conventional ANNs.
%Moreover, it is the first to achieve the genuine `live and learn' in on-line SNN training by decorrelating spike trains.
The other contribution to the spike-based on-line learning lies in the solutions to decorrelate spike trains, thereby to stop the performance from continuous decay after it peaks.
...
%In addition, the simple spike-based learning algorithm can be easily implemented on standard neuromorphic systems
