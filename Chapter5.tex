...
The first on-line training algorithm for unsupervised learning is event-based Contrastive Divergence (evtCD)~\cite{neil2013online} for SRBMs, which established the feasibility of applying the STDP rule to an approximate CD algorithm.
...
%1. Neftci
Neftci et al.~\cite{neftci2013event} derived the division of potentiation and depression to their event-based CD algorithm, but conducted both parts on the same neural populations which was more biologically plausible.
...
Another similar work on SRBM training~\cite{burbank2015mirrored} applied the STDP rule to train SAEs in a very much biologically-plausible manner.
...
%4. remote (ReSuMe)
The work we propose was mostly inspired from the supervised time-based spiking learning rule, ReSuMe~\cite{ponulak2010supervised}, where the output spikes of a post-synaptic neuron were trained to fire at the same times as the teaching spikes, see Figure~\ref{fig:resume}.
...
\begin{equation}
\lambda_x = Kx \approx N_T(s_x) = \frac{1000}{T} \sum_{t=0}^{T} s_x(t)~,
\end{equation} 
...
\begin{equation}
\lambda_a \lambda_b \approx \frac{10^6}{\tau_{\textit{\textrm{dur}}} \tau_{\textit{\textrm{win}}}} \sum_{t=0}^{\tau_{\textit{\textrm{dur}}}} [s_a(t) \sum_{t_b=t}^{t+\tau_{\textit{\textrm{win}}}} s_b(t_b)]~,
\end{equation} 
...
\begin{equation}
\begin{aligned}
\Delta w = SRM(s_a, s_b) &= \eta_s \sum_{t=0}^{\tau_{\textit{\textrm{dur}}}} [s_a(t) \sum_{t_b=t}^{t+\tau_{\textit{\textrm{win}}}} s_b(t_b)] \text{~,~and}\\
c=ab=\frac{\lambda_a \lambda_b}{K^2} &\approx \frac{10^6}{K^2 \tau_{\textit{\textrm{dur}}} \tau_{\textit{\textrm{win}}}} \sum_{t=0}^{\tau_{\textit{\textrm{dur}}}} [s_a(t) \sum_{t_b=t}^{t+\tau_{\textit{\textrm{win}}}} s_b(t_b)] \\
...
Instead of using binary units and sigmoid activations the RBM, we employed noisy ReLU (NReLU) units to construct an nRBM which was closer to biology and performed better in classification tasks~\cite{nair2010rectified}.
...
This section proposes three solutions to reduce the correlation between layers of neurons thus approximating numerical multiplications by SRM with correlated spike trains.
...
In addition, the loss dropped to a lower level compared to the original SAE test because the longer STDP window $\tau_{\textit{\textrm{win}}}$ also made SRM more accurate which reflected to the first property of SRM algorithm in Section~\ref{sec:SRM}.
