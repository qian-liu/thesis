...
In the previous chapter we have argued that deep SNNs are equally capable of classifying digits as are deep ANNs, and can be trained simply off-line on an equivalent ANN.
This chapter continues the discussion of training deep SNNs and takes an extra step towards Neuromorphic Cognition by using biologically-plausible synaptic plasticity  rules.


the second research problem of biologically inspired, on-line, spike-based SNN training.
...
Spiking AEs and RBMs can be trained with SRM by sharing the synaptic weights between $ab$ and $cd$, and applying symmetric learning rate $\pm \eta_s$ on the weight change respectively.
