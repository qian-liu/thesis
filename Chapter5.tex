...
\begin{equation}
    ...
    \right.
\end{equation}
In Section~\ref{sec:AE}, we demonstrate how Widrow-Hoff algorithm successfully applies to training AEs and in the next section we describe the implementation of SRM algorithm to deep learning models of spiking AEs and RBMs.
...
\label{sec:dSNN}
The Section conducts full experiments to compare conventional deep learning modules to spiking versions of them.
To start with, AEs and RBMs are trained with clean numerical values, and then with noisy values which are generated by presenting original values with Poisson spike trains and transferring the spike counts back to the noisy values.
The experiments run as a baseline to compare with spike based training in the section.
