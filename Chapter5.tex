...
The diverging weights are caused by the correlations between spike trains.
...
However, any pair of spike trains in training SAEs and SRBMs are correlated because of the synaptic connections between them.
In other words, the spike trains generated by a layer of neurons are strongly correlated to the spikes the lower layer feed to them; and also influence the spike firing of the upper layer, such as $\bf{v}$ and $\bf{h}$, and $\bf{h}$ and $\bf{v'}$ in SAE training.

In terms of SAE training, the strength of some synapses continuously increases because they have a relatively strong weights to trigger a hidden units to fire, but their transposed weights only have a weak impact on the firing of the reconstruction neuron.
On the contrary, the decreasing weights appear in the opposite situation.
Regarding to SRBM training, the diverging weights are more likely to happen.
