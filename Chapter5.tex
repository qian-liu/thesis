...
To present multiplication $c=ab$ with rate-based spike trains, $s_a$ and $s_b$, we need to introduce the firing rates, $\lambda_a$ and $\lambda_b$, and their possibilities of generating a spike at 1~ms resolution, $P_a/ms$ and $P_b/ms$.
The firing rate is the original value scaled by $\alpha Hz$, and equals to the possibility to fire per ms times 1 second: $\lambda_a = \alpha Hz\times a = 1000 P_a/s$, and  $\lambda_b = \alpha Hz\times b = 1000 P_b/s$.
The multiplication operation can be written as:
\begin{equation}
c=ab=\frac{\lambda_a \lambda_b}{\alpha^2} = \frac{10^6 P_a P_b}{\alpha^2}
\end{equation}

Then we investigated the way to present $P_a P_b$ in rate-based spike trains.
It can be easily converted to the average count of coincident spike events over the duration, $\tau_{dur}$, of the spike trains.
However, it is rare for two neurons to fire at exactly the same millisecond slot when the firing rates are low, we bring in the term, window period $\tau_{win}$:
\begin{equation}
<P_a P_b> = \frac{count_{\tau_{win}}(s_a, s_b)}{\tau_{win}\tau_{dur}}
\end{equation}
The mean $<P_a P_b>$ times equals to the count of firing events which are less than $\tau_{win}$ apart.
