...
The first on-line training algorithm \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend unsupervised learning is event-based \DIFdelbegin \DIFdel{contrastive divergence }\DIFdelend \DIFaddbegin \DIFadd{Contrastive Divergence }\DIFaddend (evtCD)~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{neil2013online}
}%DIFAUXCMD
for SRBM}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{neil2013online}
}%DIFAUXCMD
for SRBMs}\DIFaddend , which established the feasibility of applying \DIFaddbegin \DIFadd{the }\DIFaddend STDP rule to \DIFaddbegin \DIFadd{an }\DIFaddend approximate CD algorithm.
...
}\DIFaddend derived the division of \DIFdelbegin \DIFdel{potentiating }\DIFdelend \DIFaddbegin \DIFadd{potentiation }\DIFaddend and depression to their event-based CD algorithm, but conducted both parts on the same neural populations which was more biologically plausible.
...
And the rhythmic oscillation of these processes generated the particular form of neural sampling~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{petrovici2013stochastic}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{petrovici2013stochastic}
}%DIFAUXCMD
}\DIFaddend .
The work focused on the statistical aspect of the event-driven sampling, and was extended to the later work on Synaptic Sampling Machines (S2Ms)~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{neftci2016stochastic}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{neftci2016stochastic}
}%DIFAUXCMD
}\DIFaddend with stochastic synapses.
The classification accuracy of MNIST was 91.9\%, 1.7\% lower than the conventional RBM.
Another similar work on SRBM training~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{burbank2015mirrored}
}%DIFAUXCMD
applied }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{burbank2015mirrored}
}%DIFAUXCMD
applied the }\DIFaddend STDP rule to train SAEs in a very much biologically-plausible manner.
...
%4. remote (ReSuMe)
The work we propose was mostly inspired from the supervised time-based spiking learning rule, ReSuMe~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{ponulak2010supervised}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{ponulak2010supervised}
}%DIFAUXCMD
}\DIFaddend , where the output spikes of a post-synaptic neuron were trained to fire at \DIFdelbegin \DIFdel{expected time points }\DIFdelend \DIFaddbegin \DIFadd{the same times }\DIFaddend as the teaching spikes\DIFdelbegin \DIFdel{do}\DIFdelend , see Figure~\ref{fig:resume}.
...
%	\end{minipage}
	...
	\end{subfigure}
		...
		\end{subfigure}
	\caption{A pair of pre- and post-synaptic neurons trained by ReSuMe~\DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
\cite{ponulak2010supervised}
}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\citep{ponulak2010supervised}
}%DIFAUXCMD
}\DIFaddendFL with a teaching signal.}
	\label{fig:resume}
\end{figure}
...
Instead of using binary units and sigmoid activations \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend RBM, we employed noisy ReLU (NReLU) units to construct \DIFaddbegin \DIFadd{an }\DIFaddend nRBM which was closer to biology and performed better in classification tasks~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{nair2010rectified}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{nair2010rectified}
}%DIFAUXCMD
}\DIFaddend .
...
%DIF < There are various ways to introduce noise in formal spiking neuron models~\cite{gerstner2014neuronal}.
%DIF > There are various ways to introduce noise in formal spiking neuron models~\citep{gerstner2014neuronal}.
We introduce a method of `noisy threshold' (also called escape or hazard model)~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{gerstner2002spiking}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{gerstner2002spiking}
}%DIFAUXCMD
}\DIFaddend to generate noise in the output of spikes.
...
Hence, we can estimate their energy cost given the energy cost of a single synaptic event of some specific neuromorphic hardware, for example, about 8~nJ on SpiNNaker~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
Reconstruction is another essential function of the unsupervised learning models, such as applications of data de-noising~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{xie2012image}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{xie2012image}
}%DIFAUXCMD
}\DIFaddend and dimension reduction~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{hinton2006fast}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
