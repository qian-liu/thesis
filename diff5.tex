...
\DIFdelbegin \DIFdel{Having argued in }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend the previous chapter \DIFaddbegin \DIFadd{we have argued }\DIFaddend that deep SNNs are equally capable of classifying digits as \DIFdelbegin \DIFdel{deep ANNs do}\DIFdelend \DIFaddbegin \DIFadd{are deep ANNs}\DIFaddend , and can be trained simply \DIFdelbegin \DIFdel{just like ANNs}\DIFdelend \DIFaddbegin \DIFadd{off-line on an equivalent ANN}\DIFaddend .
This chapter continues the discussion of training deep SNNs and takes \DIFdelbegin \DIFdel{extra step forward to }\DIFdelend \DIFaddbegin \DIFadd{an extra step towards Neuromorphic Cognition by using biologically-plausible synaptic plasticity  rules.
}
...
\DIFdelbegin \DIFdel{In this chapter we will present the }\DIFdelend %DIF > In this chapter we present an on-line unsupervised learning algorithm, Spike-based Rate Multiplication~(SRM), working purely on event-based local STDP for training spiking Autoencoders and Restricted Boltzmann Machines.
...
This algorithm successfully transfers multiplication operations to possibility of some specific firing events of }\DIFdelend \DIFaddbegin \DIFadd{method precisely transformed the product of rates to the number of simultaneous spikes generated from }\DIFaddend a pair of \DIFdelbegin \DIFdel{rate-coded spike trains.
Moreover, these firing events can be captured with Synaptic-Time-Dependent }\DIFdelend \DIFaddbegin \DIFadd{connected spiking neurons.
Most importantly, the simultaneous events were captured by the weight change of the synaptic connection using the Spike-Timing-Dependent }\DIFaddend Plasticity~(STDP) \DIFdelbegin \DIFdel{rule thus to update the weights accordingly in an on-line, }\DIFdelend \DIFaddbegin \DIFadd{learning rule.
Therefore, the SRM tackles the problem of translating the weight tuning from numerical computations to }\DIFaddend event-based, \DIFdelbegin \DIFdel{and }\DIFdelend biologically-plausible \DIFdelbegin \DIFdel{manner.
...
It is crucial to provide deep SNNs with effective on-line training algorithms not only for building successful spike-based object recognition applications, but also for better power efficiency and scalability when training on neuromorphic hardware}\DIFdelend \DIFaddbegin \DIFadd{learning rules in SNNs.
}

\DIFadd{Spiking AEs and RBMs can be trained with SRM by sharing the synaptic weights between $ab$ and $cd$, and applying symmetric learning rate $\pm \eta_s$ on the weight change respectively.
...
\DIFdelbegin \DIFdel{This is the first attempt of formalising the biological-plausible learning rule to compete with the unsupervised deep learning models.
}\DIFdelend %DIF > This is the first attempt to formalise the biological-plausible learning rule to compete with unsupervised Deep Learning models.
