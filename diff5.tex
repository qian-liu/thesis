...
\DIFaddbegin \DIFadd{Researchers in neuromorphic engineering seem to take the term `on-line training' for granted~\mbox{%DIFAUXCMD
\citep{neil2013online,neftci2013event} }%DIFAUXCMD
without describing it clearly.
...
\DIFadd{Therefore, some special features appear in the on-line systems.
}\DIFaddend Firstly, on-line systems `learn through play' in that there is no separation between a training and a testing phase.
...
\DIFdelbegin \DIFdel{Hence, thirdly, }\DIFdelend \DIFaddbegin 

\DIFadd{One of the practical problems, }\DIFaddend the \DIFaddbegin \DIFadd{stability-plasticity dilemma~\mbox{%DIFAUXCMD
\citep{grossberg1987competitive}}%DIFAUXCMD
, is a typical example which only exists in artificial systems but not in the brain.
Off-line trained systems cannot learn anything new, whereas }\DIFaddend on-line \DIFdelbegin \DIFdel{approach exploits biologically-plausible learning rules, e.
g. Spike-Timing-Dependent Plasticity~(STDP).
Therefore, the modulation of the synaptic efficacy is event-driven by the spikes and operates in biological real time.
}\DIFdelend \DIFaddbegin \DIFadd{learning systems easily lose their previous knowledge.
...
%DIF > Hence, thirdly, the on-line approach exploits biologically-plausible learning rules, e.g. Spike-Timing-Dependent Plasticity~(STDP).
%DIF > Therefore, the modulation of the synaptic weights is event-driven by the spikes and operates in biological real time.
\DIFaddbegin 

\DIFaddend %Towards Neuromorphic Cognition, it is significant to endow these brain-like machines with on-line learning ability.
...
%Thus, the learning has to be manually stopped to avoid performance loss.
...
The \DIFdelbegin \DIFdel{`Post' }\DIFdelend \DIFaddbegin \textit{\DIFadd{`post'}} \DIFaddend neurons perform a weighted sum on the input data, and the error between their output and the teaching data is propagated to update the weights, thus to train the network to generate the same output as the teacher. 
...
%DIFDELCMD < \Delta w_+ = \eta ab &=\frac{\eta \lambda_a \lambda_b}{K^2}\\
%DIFDELCMD < &\approx \frac{\eta \textit{\textrm{SRM}}(s_a, s_b)}{K^2}\\ &=
%DIFDELCMD < \frac{\eta 10^6}{\tau_{\textit{\textrm{dur}}}
%DIFDELCMD < \tau_{\textit{\textrm{win}}}
%DIFDELCMD < \eta_s K^2}
%DIFDELCMD < \Delta w_+~,\\
%DIFDELCMD < \textrm{thereby,~~}
%DIFDELCMD < \eta_{s+} &=  \frac{\eta 10^6}{K^2 \tau_{\textit{\textrm{dur}}} \tau_{\textit{\textrm{win}}}}~,\\
%DIFDELCMD < \textrm{and similarly,~~}
%DIFDELCMD < \eta_{s-} &=  -\frac{\eta 10^6}{K^2 \tau_{\textit{\textrm{dur}}} \tau_{\textit{\textrm{win}}}}~.
%DIFDELCMD < %&\approx  \frac{10^6 SRM(s_a, s_b)}{K^2 \tau_{\textit{\textrm{dur}}} \tau_{\textit{\textrm{win}}}  \eta_s}~.
...
\DIFaddbegin \DIFadd{In another word, the final learning algorithm is implemented by constructing the STDP rule and setting proper parameters: the time window $\tau_{\textit{\textrm{dur}}}$, the learning rates $\eta_{s+}$ and $\eta_{s-}$; and during training, the weights between any two connected neuron will be continually modified when synchronous spikes occur. 
}\DIFaddend 
...
\label{sec:dSNN}
We have theoretically deduced the \DIFdelbegin \DIFdel{equivalence }\DIFdelend \DIFaddbegin \DIFadd{approximation }\DIFaddend of the numerical weight update to the one-line spike-based STDP rules.
...
\DIFaddbegin \DIFadd{We record  the reconstruction performance of the models, and carefully observe the dynamics of weights modifications, the activities of the hidden and output neurons and the reconstruction loss.
}\DIFaddend The experimental setup is described in Section~\ref{subsec:SNN_setup}, and the same experiments are carried out on all the training models for objective comparisons: AEs in Section~\ref{subsec:exp_AE}, RBMs in Section~\ref{subsec:exp_RBM}, SAEs in Section~\ref{subsec:exp_SAE} and SRBMs in Section~\ref{subsec:exp_SRBM}.
...
There were two set-ups of the same network architecture~(Figure~\ref{fig:sym_conn}) where ten visible neurons connected to a layer of ten hidden neurons symmetrically with all-to-all weights: in Experiment~1 (Exp1), \DIFdelbegin \DIFdel{the values of all }\DIFdelend \DIFaddbegin \DIFadd{each dimension (out of 10) of }\DIFaddend the input data \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend 1, $input_1 = [1, 1, 1,...,1]$; and in Experiment~2 (Exp2), the \DIFdelbegin \DIFdel{input data }\DIFdelend \DIFaddbegin \DIFadd{values }\DIFaddend ranged from 0.1 to 1 linearly with steps of 0.1, $input_2 = [0.1, 0.2, 0.3,...,1]$.
\DIFaddbegin \DIFadd{For both experiments, the same input data, either $input_1$ or $input_2$, is presented $5,000$ times for every trial of training.
}\DIFaddend These two experiments provided a close observation of the dynamic weight change given constant inputs.
...
The input \DIFdelbegin \DIFdel{vector}\DIFdelend \DIFaddbegin \DIFadd{data vector of ten dimensions}\DIFaddend , seen as an image, repeatedly fed into the network $5,000$ times during training.
\DIFaddbegin \DIFadd{Representing a data vector with the term `image' helps us to better demonstrate the reconstruction task, and to have a unified expression as images in the MNIST dataset for later use.
}\DIFaddend As a 'live and learn' system, there was no end to the learning, however for the purpose of observing the reconstruction performance, the weights were frozen every 10 steps of training and were validated on the testing data. 
...
The \DIFdelbegin \DIFdel{firing rates of the }\DIFdelend input values were scaled up by the factor \DIFaddbegin \DIFadd{of }\DIFaddend $K = 100$\DIFdelbegin \DIFdel{, thus }\DIFdelend \DIFaddbegin \DIFadd{~Hz, thus the firing rates of the poisson spike trains are }\DIFaddend $\lambda_1 = [100, 100, 100, ..., 100]$ Hz and $\lambda_2 = [10, 20, 30, ..., 100]$ Hz.
The spike count $N_{\tau_{\textit{\textrm{dur}}}}$ of the generated Poisson spike train then transformed to the noisy input~(NI) for use of conventional models: \DIFdelbegin \DIFdel{$N_{\tau_{\textit{\textrm{dur}}}}*1000/(\tau_{\textit{\textrm{dur}}} * K)$}\DIFdelend \DIFaddbegin \DIFadd{$N_{\tau_{\textit{\textrm{dur}}}} \times 1000/(\tau_{\textit{\textrm{dur}}} \times K)$}\DIFaddend .
...
\DIFaddbegin \DIFadd{Figure~\ref{fig:noise_input} shows the NI of Exp1 and Exp2; Every subfigure demonstrates $5,000$ data vectors, each data vector is shown as a column of coloured points, and each dimension of the input data is drawn with the same colour;
every single data vector represents the normalised firing rate of a trial of poisson spike trains, whose actual firing rates can be calculated by multiplying the values of the vector and the scaling factor $K$~(100 Hz).
The actual firing rates are recorded by randomly generated poisson spike trains given the expected firing rates of $\lambda_1 = [100, 100, 100, ..., 100]$ Hz and $\lambda_2 = [10, 20, 30, ..., 100]$ Hz.
The longer a spike train presents, the more spikes it has, and the actual firing rate is more likely to be close to the expectation.
The training poisson spike trains are 100~ms long, and the testing spike trains are 10 times longer, thus the actual firing rates are much more noisy in training data than in the testing data.
}\DIFaddend Hence, as shown in Figure~\ref{fig:noise_input}, \DIFdelbegin \DIFdel{the noisy training input }\DIFdelend \DIFaddbegin \DIFadd{NI }\DIFaddend added a Gaussian noise with -0.05 mean and 0.29 variance for Exp1\DIFdelbegin \DIFdel{to the clean data}\DIFdelend , and -0.02 mean and 0.22 variance for Exp2;
...
\begin{figure}
	...
	\begin{subfigure}[t]{0.48\textwidth}
		...
		\caption{Output of hidden units in Exp2}
	\end{subfigure}
	\caption[nRBM-NI training of the reconstruction tests.]{Changes of weights, output of visible and hidden units, and mean squared error (loss) during the nRBM training of the noisy reconstruction tests. 
...
\subsection[Noisy RBMs]{Training Noisy RBMs}
...
\subsection[Spiking AEs]{Training Spiking AEs}
...
\end{equation}
and an IF neuron fires when the membrane potential $V$ surpasses the membrane threshold \DIFdelbegin \DIFdel{$V_{thresh}$}\DIFdelend \DIFaddbegin \DIFadd{$V_{th}$}\DIFaddend , and $V$ resets to $V_{rest}$ after firing or when it reduces below $V_{rest}$.
...
\subsection[Spiking RBMs]{Training Spiking RBMs}
...
The diverging weights \DIFaddbegin \DIFadd{(see Figures~\ref{fig:SAE_orig}(a, b) and \ref{fig:srbm_orig}(a, b) ) }\DIFaddend not only make the loss increase, for example as shown in Figure~\ref{fig:srbm_orig}(g), but also lead the weights too far from the expected \DIFdelbegin \DIFdel{effective }\DIFdelend range of the conventional AE or RBM training\DIFaddbegin \DIFadd{, refer to Figures~\ref{fig:ae_noise}(a, b) and \ref{fig:rbm_noise}(a, b)}\DIFaddend .


\DIFdelbegin \DIFdel{The diverging weight values are caused by correlations between spike trains.
}\DIFdelend Note that the rate multiplication of two spike trains (stated in Equation~\ref{equ:mul}) works under the condition of the independence of the spike trains.
...
\DIFaddbegin \DIFadd{Thus, the unbalanced spike correlations between pairs of spike trains cause the strength of the weights diverge.
}\DIFaddend 
...
To make the comparisons of all the proposed solutions more straightforward, we list the weights and loss change from Exp2 in Figure~\ref{fig:sols_ae} (SAE) and Figure~\ref{fig:sols_rbm} (SRBM).
...
The Gaussian noise added to the membrane potential is randomly generated by $\mathcal{N}(0, \sigma)$~mV, where $\sigma$ is set to 0.2, 20\% of \DIFdelbegin \DIFdel{$V_{thresh} - V_{rest}.$
}\DIFdelend \DIFaddbegin \DIFadd{$V_{th} - V_{rest}.$
}\DIFaddend An appropriate $\sigma$ is chosen for introducing enough noise to decrease the correlation of the spike trains and also maintain a good loss performance.
...
We call these spike trains teaching signals \DIFdelbegin \DIFdel{$\bf{s}$}\DIFdelend \DIFaddbegin \DIFadd{$\bf{ts}$}\DIFaddend , because they are only used for weight updates but not conducted into the network.
...
Then the teaching signals are used in \DIFdelbegin \DIFdel{$\Delta w_{ij} = \textit{\textrm{STDP}}(s_j,h_i)$ }\DIFdelend \DIFaddbegin \DIFadd{$\Delta w_{ij} = \textit{\textrm{STDP}}(ts_j,h_i)$ }\DIFaddend to replace $\textit{\textrm{STDP}}(v_j,h_i)$ on the positive part of the weight change.
...
The average reconstruction loss is also worse than S1 and S2, similar to SAE test, because of the \DIFaddbegin \DIFadd{reconstruction }\DIFaddend bias. 

\subsection{Combined Solutions~(S4)}
...
\DIFdel{Results}\DIFdelend \DIFaddbegin \section[Case Study]{\DIFadd{Case Study}\DIFaddend : MNIST Test}
...
During testing, only the 784 neurons representing image pixels remained as visible units and the other 10 label units worked as the top layer of an \DIFdelbegin \DIFdel{MLP }\DIFdelend \DIFaddbegin \DIFadd{Multi-Layer Perceptron~(MLP) }\DIFaddend network for recognising digits.
...
In brief, from the qualitative comparisons on the trained features, the original SAE is the only exception which produce `substandard' features\DIFaddbegin \DIFadd{, noisy reconstructions, }\DIFaddend among the SAE methods.
...
For the RBM testing, nRBM is more sensitive to the noise of the training images than AE so that the CA of nRBM-NI slowly drops and oscillated more and more strongly as training proceeded, see Figure~\ref{fig:sdlm_ca}(b).
...
Although, \citet{neftci2016stochastic} keep the state-of-the art accuracy of 95.6\%, they exploited the \DIFdelbegin \DIFdel{edge }\DIFdelend \DIFaddbegin \DIFadd{cutting-edge }\DIFaddend tool, dropout and more training epochs, but did not report the comparison to the corresponding non-spiking RBM-dropout.
...
\begin{figure}
	...
	\begin{subfigure}[t]{0.48\textwidth}
		...
		\caption{SRBM-S3}
	\end{subfigure}
	\caption[Classification accuracy with various input firing rates.]{Classification accuracy during the entire testing time period with increasing input spike firing rate.}
	\label{fig:sdlm_freq}
\end{figure}
...
% and proved to be equivalent to the conventional RBMs.
...
%by showing that SNNs can be trained on-line with biologically-plausible learning rules and demonstrate cognitive capabilities as competent as conventional Deep Learning modules.
...
The other contribution to the spike-based on-line learning lies in the solutions to decorrelate spike trains, thus to prevent the learning performance from continuous decay after it peaks.
