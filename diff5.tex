...
\label{cha:sdlm}
Having argued in the previous chapter that deep SNNs are equally capable of classifying digits as \protect\TLSins{are} deep \protect\TLSdel{ANNs do,} \protect\TLSins{ANNs,} and can be trained simply just like \protect\TLSdel{ANNs.
This} \protect\TLSins{ANNs, this} chapter continues the discussion of training deep SNNs and takes \protect\TLSins{an} extra step forward to the second research problem of biologically inspired, on-line, spike-based SNN training.

In this chapter we \protect\TLSdel{will} present \protect\TLSdel{the} \protect\TLSins{an on-line unsupervised learning algorithm,} Spike-based Rate \protect\TLSdel{Multiplication method~(SRM).
This algorithm successfully transfers} \protect\TLSins{Multiplication~(SRM), working purely on event-based local STDP for training spiking Autoencoders and Restricted Boltzmann Machines.
...
Moreover, these firing} \protect\TLSins{trains, and the simultaneous} events can be \protect\TLSdel{captured with Synaptic-Time-Dependent Plasticity~(STDP) rule thus to update} \protect\TLSins{obtained by} the \protect\TLSdel{weights accordingly in an on-line, event-based, and} biologically-plausible \protect\TLSdel{manner.
Such weight updates suit the conventional unsupervised deep} learning \protect\TLSdel{modules, such as Autoencoders (AE)} \protect\TLSins{rule: Spike-Time-Dependant-Plasticity~(STDP).
This on-line training method achieves better performance than existing algorithms} and \protect\TLSdel{Restricted Boltzmann Machine~(RBMs), where multiplication of} \protect\TLSins{approaches} the \protect\TLSdel{neural outputs is} \protect\TLSins{same, sometimes better performance of} the \protect\TLSdel{main operation.} \protect\TLSins{equivalent non-spiking methods.}
It is crucial to provide deep SNNs with effective on-line training algorithms not only for building successful spike-based object recognition applications, but also for better power efficiency and scalability \protect\TLSdel{when} \protect\TLSins{provided by the biologically-plausible SNN} training on neuromorphic hardware.
...
\protect\TLSins{To begin with Section~\ref{sec:SRM_related}, we} explore the research question of on-line, event-based deep SNN training in the \protect\TLSdel{literature,} \protect\TLSins{literature.
In Section~\ref{sec:SRM} we} describe SRM \protect\TLSdel{in mathematical expression,} \protect\TLSins{mathematically,} and explain the factors influencing the accuracy of the \protect\TLSdel{method and the factors not.} \protect\TLSins{method.}
We \protect\TLSins{then} argue why the learning algorithm is suited to train spiking Autoencoders (SAEs) and Spiking Restricted Boltzmann Machines \protect\TLSdel{(SRBM).} \protect\TLSins{(SRBMs) in Section~\ref{sec:dSNN}.}
During the research we encountered the problem \protect\TLSdel{introduced by} \protect\TLSins{of} correlated spikes, and propose solutions to decorrelate spike trains in Section~\ref{sec:problem}.
Finally, \protect\TLSdel{the online, spike-based learning} \protect\TLSins{experiments in Section~\ref{sec:SRM_result} show that this on-line training method achieves better performance than existing} algorithms \protect\TLSdel{demonstrated similar even surpassing recognition} and \protect\TLSdel{reconstruction capabilities over conventional DNN training} \protect\TLSins{approaches the same, sometimes superior performance of the equivalent non-spiking methods} on the detailed comparison on the MNIST dataset.



\section{Related Works}
\protect\TLSins{\label{sec:SRM_related}}
The current trend towards training deep SNNs on-line using biologically-plausible learning methods is promising due to its potential benefit on low-energy hardware \protect\TLSdel{implementation.} \protect\TLSins{implementations.}
The first on-line training algorithm \protect\TLSdel{of} \protect\TLSins{for} unsupervised learning is event-based \protect\TLSdel{contrastive divergence} \protect\TLSins{Contrastive Divergence} (evtCD)~\citep{neil2013online} for \protect\TLSdel{SRBM,} \protect\TLSins{SRBMs,} which established the feasibility of applying \protect\TLSins{the} STDP rule to \protect\TLSins{an} approximate CD algorithm.
A review of training \protect\TLSins{methods for} RBM algorithms is given in Section~\ref{sec:rbm}.
The evtCD method divided the learning process into \protect\TLSdel{potentiating} \protect\TLSins{potentiation} and depression parts, which \protect\TLSdel{formed} \protect\TLSins{forms} the basic design of related research.
\protect\TLSdel{As}
\protect\TLSins{In} an initial \protect\TLSdel{attempt the} \protect\TLSins{attempts} mathematical analysis was mainly ignored, thus the best classification performance achieved was only about 81.5\% using purely spiking neurons.
Neftci et al.~\citep{neftci2013event} derived the division of \protect\TLSdel{potentiating} \protect\TLSins{potentiation} and depression to their event-based CD algorithm, but conducted both parts on the same neural populations which was more biologically plausible.
Therefore, a global signal was needed to differentiate the \protect\TLSdel{potentiating} \protect\TLSins{potentiation} and depression process.
...
Another similar work on SRBM training~\citep{burbank2015mirrored} applied \protect\TLSins{the} STDP rule to train SAEs in a very much biologically-plausible manner.
This work aimed at constructing artificial machine learning algorithms \protect\TLSdel{closely} \protect\TLSins{close} to biology, whereas the other works above focused on training spiking \protect\TLSdel{deep learning} \protect\TLSins{Deep Learning} modules with equivalent recognition capability as alternative \protect\TLSdel{deep learning} \protect\TLSins{Deep Learning} methods.

The work we propose was mostly inspired from the supervised time-based spiking learning rule, ReSuMe~\citep{ponulak2010supervised}, where the output spikes of a post-synaptic neuron were trained to fire at \protect\TLSdel{expected time points} \protect\TLSins{the same times} as the teaching \protect\TLSdel{spikes do,} \protect\TLSins{spikes,} see Figure~\ref{fig:resume}.
...
Therefore, as illustrated in Figure~\ref{fig:resume}, at the beginning a teaching spike fires earlier than a post-synaptic spike within an STDP window which makes the weight \protect\TLSdel{potentiates} \protect\TLSins{potentiat} more than it depresses;
thus driven by a stronger synaptic weight, the post-synaptic neuron fires earlier than it \protect\TLSins{is} supposed \protect\TLSdel{to (the teaching spike),} \protect\TLSins{to,} making the weight depresses a bit more than its potentiation;
finally, the weight stays unchanged because the post-synaptic neuron fires coincidently with the teaching \protect\TLSdel{spike} \protect\TLSins{spike,} thus the weight's potentiation cancels out the depression.
\begin{figure}
	\centering
...
The basic idea of the proposed method is to provide \protect\TLSins{an} equivalent learning rule as ReSuMe on rate-encoded SNNs.
...
The synaptic strength stops changing as the output neuron fires at the same frequency \protect\TLSdel{of} \protect\TLSins{as} the teaching signal.
In the following section, we will firstly \protect\TLSdel{demonstrate how} \protect\TLSins{describe} this idea \protect\TLSdel{was derived} \protect\TLSins{mathematically using the proposed term SRM, and equip learning} to \protect\TLSdel{mathematical descriptions of a} \protect\TLSins{the method by} biological-plausible \protect\TLSdel{learning rule, STDP.} \protect\TLSins{STDP rule.}
...
The `Post' neurons perform \protect\TLSins{a} weighted sum on the input data, and the error between their output and the teaching data \protect\TLSdel{are} \protect\TLSins{is} propagated to update the weights, thus to train the network to generate \protect\TLSins{the} same output as the \protect\TLSdel{teachers'.} \protect\TLSins{teacher's.} 
The learning algorithm was named after the researcher Widrow-Hoff:
\begin{equation}
\Delta w = \eta (teach - post)pre~.
\label{equ:widrow-hoff}
\end{equation}
The right hand \protect\TLSins{side} of the equation can be seen as a subtraction of two multiplication operations, $teach \times pre$ and $post \times pre$, times a learning rate, $\eta$.

SRM provides the solution to present numeric multiplication $c=ab$ with rate-based spike trains.
Firstly, the multiplier $a$ and the multiplicand $b$ are encoded into Poisson spike trains $s_a(t)=\{s_a(1),s_a(2),...,s_a(T)\}$ and $s_b(t)=\{s_b(1),s_b(2),...,s_b(T)\}$ \protect\TLSdel{in the} \protect\TLSins{with} 1~ms resolution, where $s(t)=1$ indicates a spike in the $t$th~ms and $s(t)=0$ means \protect\TLSdel{none.} \protect\TLSins{no spike.}
Secondly, a Poisson generator fires a sequence of \protect\TLSdel{spike train} \protect\TLSins{spikes} according to its firing rate, $\lambda$~Hz, which is assigned linearly to the original multiplier/multiplicand by a factor of $K$.
Hence, the firing rate ($\lambda_x$) of the Poisson generator is $K$ times the numerical value x, and can be approximated by the \protect\TLSdel{averaging} \protect\TLSins{average} spike count, $N_T(s_x)$, of the generated spike train $s_x(t)$ over time \protect\TLSdel{length of} $T$~ms:
\begin{equation}
...
Significantly, the approximation is more accurate as the observing time ($T$) grows since more spikes are generated over time and the \protect\TLSdel{averaging} \protect\TLSins{average} spike count becomes more reliable.
...
Finally, if we combine the concept of the STDP learning window \protect\TLSdel{to} \protect\TLSins{with} equation~\ref{equ:mul}, the length of the observing time on $s_b$ shrinks to a fixed-length windowing period, \protect\TLSdel{$\tau_{win}$.} \protect\TLSins{$\tau_{\textit{\textrm{win}}}$.}
Consequently, the rate multiplication can be conducted within only the local events in \protect\TLSdel{term} \protect\TLSins{terms} of time, although the accuracy may drop.
...
\end{equation} 
where \protect\TLSdel{$\tau_{dur}$} \protect\TLSins{$\tau_{\textit{\textrm{dur}}}$} replaces $T$ to represent the length of generated spike trains during SNN simulation.
The weight rise/drop according to a rectangular STDP curve can detect the spike events of a pair of neurons when a post-synaptic spike occurs \protect\TLSdel{no} \protect\TLSins{coincidently (no} later than \protect\TLSdel{$\tau_{win}$,} \protect\TLSins{$\tau_{\textit{\textrm{win}}}$),} see Figure~\ref{fig:rtg_stdp}.
Therefore, the overall weight change during time \protect\TLSdel{$\tau_{dur}$ indicates} \protect\TLSins{$\tau_{\textit{\textrm{dur}}}$ is determined by the number of coincident spikes of the pair of neurons, indicating} the rate multiplication, and is \protect\TLSdel{descried} \protect\TLSins{described} by SRM:
\begin{equation}
\begin{aligned}
...
\begin{figure}
	...
	\caption{Rectangular STDP curve.
		If the time difference \protect\TLSdel{of} \protect\TLSins{between} the post-synaptic spike and the pre-synaptic spike lies in the window \protect\TLSdel{on $\tau_{win}$,} \protect\TLSins{$\tau_{\textit{\textrm{win}}}$,} then the synaptic weight will increase or decrease by $\eta_s$ according to the sign of the SRM.}
	\label{fig:rtg_stdp}
\end{figure}

We can adjust $\eta_s$ to $ K^2 \protect\TLSdel{\tau_{dur} \tau_{win}10^{-6}$} \protect\TLSins{\tau_{\textit{\textrm{dur}}} \tau_{\textit{\textrm{win}}}10^{-6}$} to make $ab = \Delta w$.
In terms of the Widrow-Hoff learning algorithm, $\eta_s$ is set to be:
\begin{equation}
	\eta_s=
    \left\{
    \begin{aligned} 
    \eta K^2 \protect\TLSdel{\tau_{dur} \tau_{win}10^{-6},} \protect\TLSins{\tau_{\textit{\textrm{dur}}} \tau_{\textit{\textrm{win}}}10^{-6},} &\text{ when calculating } \eta teach \times pre\\
    - \eta K^2 \protect\TLSdel{\tau_{dur} \tau_{win}10^{-6},} \protect\TLSins{\tau_{\textit{\textrm{dur}}} \tau_{\textit{\textrm{win}}}10^{-6},} &\text{ when calculating } \eta post \times pre
    \end{aligned}
    \right.
    \label{equ:eta_s}
\end{equation}

Last but not least, we state the property of \protect\TLSins{the} SRM algorithm:
\begin{itemize}
	\item the accuracy of SRM is mainly controlled by \protect\TLSdel{$\tau_{dur}$} \protect\TLSins{$\tau_{\textit{\textrm{dur}}}$} and \protect\TLSdel{$\tau_{win}$,} \protect\TLSins{$\tau_{\textit{\textrm{win}}}$,} where longer spike trains and a longer STDP window expresses the rate more \protect\TLSdel{reliable.} \protect\TLSins{reliably.}
	...
	Thus \protect\TLSins{a} negative product is applied with weight decrease, $-\eta_s$. 
	\item the multiplier and the multiplicand are interchangeable due to the independence of the spike trains. 
	\item the accuracy is independent of \protect\TLSins{the} neural and synaptic \protect\TLSdel{model} \protect\TLSins{models} of \protect\TLSdel{a} \protect\TLSins{the} spiking neuron because the calculation relies on the firing rate only.
\end{itemize}
...
In Sections~\ref{sec:AE} and \ref{sec:rbm}, we \protect\TLSdel{have} derived the training algorithms of AEs and RBMs in detail, whose learning rules are of \protect\TLSins{the} same structure as Widrow-Hoff~(Equation~\ref{equ:widrow-hoff}) for ADALINE, see Equations~\ref{equ:ae_widrow_hoff} and~\ref{equ:rbm_train}.
Consequently, we can easily apply SRM in training spiking AEs and RBMs and conduct experiments to compare conventional \protect\TLSdel{deep learning} \protect\TLSins{Deep Learning} modules with the spiking versions.
...
\begin{figure}
	...
	\label{fig:sym_conn}
\end{figure}
Initially, AEs and RBMs were trained with clean numerical values, then with noisy values generated by counting \protect\TLSins{the} spikes \protect\TLSdel{of} \protect\TLSins{in} Poisson spike trains.
...
Exp1 shows how the network responded to the same input value and reconstructed it, while Exp2 demonstrated the influence of ranging input values and more importantly how different firing \protect\TLSdel{rate} \protect\TLSins{rates} may affect the corresponding SNN.
These experiments were run as baselines to compare with spike-based training on the features of \protect\TLSdel{weights'} \protect\TLSins{weight} convergence, reconstruction error and \protect\TLSdel{neurons'} \protect\TLSins{neural} activities.
...
The input vector, seen as an image, repeatedly fed into the network $5,000$ times and Figure~\ref{fig:ae_orig} shows the dynamics of the AE as more repeating images \protect\TLSdel{present} \protect\TLSins{are presented} through training.
...
\begin{figure}
	...
	\caption{Changes of weights, output of visible and hidden units, and mean squared error (loss) during the AE training of the reconstruction tests. 
		Experiments 1) 10 visible units fully connected to 10 hidden units with input data of all 1s; 2) \protect\TLSins{the} same network fed with 10 values \protect\TLSdel{distribute} \protect\TLSins{distributed} linearly from 0.1 to 1.}
	\label{fig:ae_orig}
\end{figure}

To compare all the following experiments, we present the results in the same template of figures:
among such a set of figures, (a) and (b) depict the weight \protect\TLSdel{change} \protect\TLSins{changes} of the two experiment set-ups (Exp1 and Exp2) which \protect\TLSdel{is} \protect\TLSins{are} the most important output of \protect\TLSdel{a} \protect\TLSins{the} training method;
...
\begin{figure}
	...
	\caption{Changes of weights, output of visible and hidden units, and mean squared error (loss) during the AE training of the noisy reconstruction tests. 
		Experiments 1) 10 visible units fully connected to 10 hidden units with the count of Poisson spikes firing at 100~Hz which lasted 100~ms; 2) \protect\TLSins{the} same network fed with spike count of Poisson spikes at firing \protect\TLSdel{rate} \protect\TLSins{rates} ranging from 10~Hz to 100~Hz.}
	\label{fig:ae_noise}
\end{figure}

The same experiments were repeated with noisy training and testing data to provide a fair comparison with the spike-based \protect\TLSdel{deep learning} \protect\TLSins{Deep Learning} modules, see Figure~\ref{fig:ae_noise}.
...
The spike count \protect\TLSdel{$N_{\tau_{dur}}$} \protect\TLSins{$N_{\tau_{\textit{\textrm{dur}}}}$} of the generated Poisson spike train then transformed to the noisy input, \protect\TLSdel{$N_{\tau_{dur}}*1000/(\tau_{dur}} \protect\TLSins{$N_{\tau_{\textit{\textrm{dur}}}}*1000/(\tau_{\textit{\textrm{dur}}}} * K)$.
The scaling factor $1,000$ \protect\TLSdel{transfers} \protect\TLSins{converts} ms to s, and the length of spike trains \protect\TLSdel{$\tau_{dur}$} \protect\TLSins{$\tau_{\textit{\textrm{dur}}}$} was 100~ms when training, and $1,000$~ms for testing.
...
\protect\TLSdel{It}
\protect\TLSins{This} was mainly caused by the weaker noise level on the smaller input values which made the data of Exp2 less noisy \protect\TLSdel{in} \protect\TLSins{on} average.
...
\begin{figure}
	...
	\caption{Changes of weights, output of visible and hidden units, and mean squared error (loss) during the nRBM training of the noisy reconstruction tests. 
		Experiments 1) 10 visible units fully connected to 10 hidden units with the count of Poisson spikes firing at 100~Hz which lasted 100~ms; 2) \protect\TLSins{the} same network fed with spike count of Poisson spikes at firing rate ranging from 10~Hz to 100~Hz.}
	\label{fig:rbm_noise}
\end{figure}

\subsection{Noisy Restricted Boltzmann Machines (nRBMs)}
Instead of using binary units and sigmoid activations \protect\TLSdel{in} \protect\TLSins{the} RBM, we employed noisy ReLU (NReLU) units to construct \protect\TLSins{an} nRBM which was closer to biology and performed better in classification tasks~\citep{nair2010rectified}.
...
\label{equ:rbm}
\end{equation} 
where \protect\TLSdel{$h'_i$ and $v'_j$ is} a \protect\TLSdel{pair of} Gibbs \protect\TLSdel{sampling,} \protect\TLSins{sample comprises a pare of $h'_i$ and $v'_j$,} and the new sampling method NReLU is equivalent to generating multiple samples in the meantime and averaging them: $max(0, x+\mathcal{N}(0, \sigma(x))$.
...
Figure~\ref{fig:rbm_orig} demonstrates the training process and the \protect\TLSdel{testing result} \protect\TLSins{test results} of \protect\TLSins{the} nRBM; both experiments stabilised earlier than AE training: about 400 and $1,000$ steps respectively.
Due to \protect\TLSins{the} randomness of the sampling in \protect\TLSins{the} nRBM, there were noisy fluctuations adding on the weight change during training, the reconstructions and the output of hidden units in deterministic testing.
In addition, the loss stopped declining around $10^{-4}$ because of the same reason of randomness.

The same experiments were also carried out with the identical noisy input used in \protect\TLSins{the} SAE experiments, see Figure~\ref{fig:rbm_noise}.
The weight change was slightly noisier than the experiments on clean data, \protect\TLSdel{But} \protect\TLSins{but} the noise was more obvious on the output of the visible reconstruction and the hidden units.
The same effect of the noisy input data can be found in Figure~\ref{fig:ae_noise} where the noise \protect\TLSdel{on} \protect\TLSins{in} the reconstruction was much reduced compared to the input data and the loss remained about $10^{-2}$ and $10^{-2.5}$ after the stabilisation, although it took less time for \protect\TLSins{the} nRBM to converge than AE.
...
\end{equation}
and an IF neuron fires when \protect\TLSins{the} membrane potential $V$ surpasses the membrane threshold $V_{thresh}$, and $V$ resets to $V_{rest}$ after firing or when it reduces below \protect\TLSdel{than} $V_{rest}$.
The parameters used are listed in \protect\TLSdel{the} Table~\ref{tbl:srm}.
\begin{table}[th]
	\centering
...
\begin{figure}
	...
	\caption{Weights and firing rates of visible and hidden units change during training of the reconstruction tests of spiking AE. 
		Experiments 1) 10 visible units fully connected to 10 hidden units with Poisson spike trains of 100~Hz which lasted 100~ms; 2) \protect\TLSins{the} same network fed with 10 Poisson spike trains of firing rate ranging from 10~Hz to 100~Hz.}
	\label{fig:SAE_orig}
\end{figure}

Figure~\ref{fig:rSTDP} illustrates the learning mechanism of the SAE architecture by giving an example of one hidden neuron $h_i$ connected \protect\TLSdel{by} \protect\TLSins{to} an input neuron $v_j$ and connecting to a reconstruction neuron $v'_j$ with the same strength $w_{ij}$.
The weight $w_{ij}$ rises by $\eta_{s+}$ (marked with \protect\TLSins{an} up-arrow) when a spike comes within the period of \protect\TLSdel{$\tau_{win}$} \protect\TLSins{$\tau_{\textit{\textrm{win}}}$} after the hidden neuron fires;
and it decreases by $\eta_{s-}$ (marked with \protect\TLSins{a} down-arrow) if the reconstruction neuron generates a spike in the same time window.
If either $v_j$ or $v'_j$ spikes outside the effective STDP window, the weight will remain unchanged.
The learning continues as long as the neurons are active, however the weights may stay relatively stable when the input firing rate is the same as the \protect\TLSdel{reconstructions'.} \protect\TLSins{reconstruction's.}
...
The training was slower than the AE experiments and the loss took longer \protect\TLSdel{time} to decrease to \protect\TLSdel{a} \protect\TLSins{the} same level.
\protect\TLSdel{It}
\protect\TLSins{This} was mainly caused by the low firing rate of both input and hidden units at the beginning of the training.
Thus, few, even \protect\TLSdel{none,} \protect\TLSins{no,} spikes triggered STDP.
...
however, the SAE did not reconstruct the various input values of Exp2 as accurately as the AE did, since the parameters \protect\TLSdel{$\tau_{win}$} \protect\TLSins{$\tau_{\textit{\textrm{win}}}$} and \protect\TLSdel{$\tau_{dur}$} \protect\TLSins{$\tau_{\textit{\textrm{dur}}}$} were relatively short.
...
As illustrated in Equation~\ref{equ:rbm}, the positive weight change is generated from multiplication of visible and hidden units where the weight depression comes from the \protect\TLSdel{pair} \protect\TLSins{product} of \protect\TLSins{the} Gibbs \protect\TLSdel{sampling.} \protect\TLSins{sampled hidden and reconstruction values.}
...
meanwhile, \protect\TLSdel{down-arrow highlights} \protect\TLSins{down-arrows highlight}	 the weight depression when $v'_j$ generates a spike no later than \protect\TLSdel{$\tau_{win}$} \protect\TLSins{$\tau_{\textit{\textrm{win}}}$} after $h'_i$ fires.
...
\begin{figure}
	...
	\begin{subfigure}[t]{0.45\textwidth}
		...
		\caption{Loss of Exp2}
	\end{subfigure}
	\caption{Weights and firing rates of visible and hidden units change during training of the reconstruction tests of \protect\TLSins{a} spiking RBM. 
		Experiments 1) 10 visible units fully connected to 10 hidden units with Poisson spike trains of 100~Hz which lasted 100~ms; 2) \protect\TLSins{the} same network fed with 10 Poisson spike trains of firing rate ranging from 10~Hz to 100~Hz.}
\label{fig:srbm_orig}
\end{figure}

The weight divergence was a more severe problem in \protect\TLSins{the} SRBM, see Figure~\ref{fig:srbm_orig}.
Especially for Exp1, only one hidden neuron was active on the later half of the training, thus \protect\TLSdel{to make} \protect\TLSins{making the} firing rate of the reconstruction \protect\TLSdel{jumping} \protect\TLSins{jump} between two states.
...
\section[Problem of  Spike Correlations	]{Problem of \protect\TLSdel{Spikes Correlation} \protect\TLSins{Spike Correlations	}}
\label{sec:problem}
...
The diverging weights not only made the loss increase, for example \protect\TLSins{as} shown in Figure~\ref{fig:srbm_orig}(g), but also led the weights too far from the expected effective range of the AE or RBM training.

The diverging weight values were caused by \protect\TLSdel{the} correlations between spike trains.
...
Thus the correlation between $v_j$ and $h_i$ is stronger than $v'_j$ and $h_i$, making the positive weight update more \protect\TLSdel{frequent} \protect\TLSins{frequently} than it drops.
On the contrary, the decreasing weights appear in the opposite situation when $h_i$ correlates stronger with $v'_j$ than $v_j$.
Regarding \protect\TLSdel{to} \protect\TLSins{the} SRBM, the training is even more effected by the correlations where the values of the weights diverge at a faster pace.
 
 
This section proposes three solutions to reduce the correlation between \protect\TLSdel{layers of neurons, with the purpose} \protect\TLSins{spike trains} of \protect\TLSdel{training spiking deep learning models approximately to the process} \protect\TLSins{different layers} of \protect\TLSdel{their conventional training methods.
The aim is to modulate the weight updates close to} \protect\TLSins{neurons thus approximating} AE and RBM training \protect\TLSdel{under the premise of keeping a similar level} \protect\TLSins{with equivalent event-based on-line learning} of \protect\TLSdel{loss.} \protect\TLSins{SAEs and SRBMs using SRM.}
To highlight the performance of the solutions, we apply these methods \protect\TLSdel{on} \protect\TLSins{to} the same experiments \protect\TLSdel{above.
And} \protect\TLSins{above, and} the default parameters are the same as in Table\ref{tbl:srm}.
\begin{figure}
	\centering
	...
	\begin{subfigure}[c]{0.45\textwidth}\raggedleft
		...
		\caption{Exp2 Loss}
	\end{subfigure}\caption{Comparisons of weights and loss of solutions for training SAE on Exp2: [S1] longer STDP window, [S2] noisy threshold, [S3] teaching signal, and [S4] combined solutions. 10 visible units fully \protect\TLSdel{connected} \protect\TLSins{connects} to 10 hidden units with Poisson spike trains of firing rate ranging from 10~Hz to 100~Hz.}
	\label{fig:sols_ae}
\end{figure}
...
A stronger weight between a pair of pre- and post-synaptic neurons \protect\TLSdel{makes} \protect\TLSins{results in} a higher probability for the pre-synaptic neuron to trigger a post-synaptic spike in a shorter period; 
...
Therefore, we use a longer square STDP window to accumulate more evidence \protect\TLSdel{making} \protect\TLSins{to improve the accuracy of} SRM \protect\TLSdel{less bias on} \protect\TLSins{of spike trains with low frequency and} weak connections and \protect\TLSins{make SRM} more independent \protect\TLSdel{on} \protect\TLSins{of} the correlations between spike trains.
...
In the experiment, we doubled the window length to 20~ms and set $\eta_s$ to $\pm 5 \times 10^{-5}$, and recorded the complete results in \protect\TLSdel{Figure~\ref{fig:sol1_ae}} \protect\TLSins{Figures~\ref{fig:sol1_ae}} and~\ref{fig:sol1_rbm} in \protect\TLSdel{Appendices.} \protect\TLSins{Appendix~\ref{sec:app2}.}

To make the comparisons more straightforward, we list the weights and loss \protect\TLSdel{of Exp2} \protect\TLSins{recorded from Exp2, which is described in Section~\ref{sec:dSNN} where AEs and RBMs reconstruct 10 float numbers from 0.1 to 1.0,} for all the solutions in Figure~\ref{fig:sols_ae} (SAE) and Figure~\ref{fig:sols_rbm} (SRBM).
\protect\TLSdel{A}
\protect\TLSins{The} longer STDP window reduced the weight divergence that the bidirectional weight change slowed down and kept within a smaller range.
In addition, the loss dropped to a lower level compared to the original SAE test because the longer STDP window \protect\TLSdel{$\tau_{win}$} \protect\TLSins{$\tau_{\textit{\textrm{win}}}$} also made \protect\TLSins{the} SRM more accurate which reflected \protect\TLSdel{to} the first property of \protect\TLSins{the} SRM algorithm in Section~\ref{sec:SRM}.
...
The Gaussian noise \protect\TLSdel{adding} \protect\TLSins{added} to the membrane potential is randomly generated by $\mathcal{N}(0, \sigma)$~mV, where $\sigma$ is set to 0.2, 20\% of $V_{thresh} - V_{rest}.$
...
The complete \protect\TLSdel{testing} \protect\TLSins{test} results can be found in \protect\TLSdel{Appendices,} \protect\TLSins{Appendix~\ref{sec:app2},} see Figures~\ref{fig:sol2_ae} and~\ref{fig:sol2_rbm}.

Using the same STDP window \protect\TLSdel{$\tau_{win}$} \protect\TLSins{$\tau_{\textit{\textrm{win}}}$} of 20~ms, the noisy membrane threshold enhanced the decorrelation, thus the weight change remained more subtle than S1, see Figures~\ref{fig:sols_ae}(a) and~\ref{fig:sols_rbm}(a) .
...
A \protect\TLSdel{teaching signal which fires} \protect\TLSins{Poisson spike train firing} at the same rate as the input spikes \protect\TLSins{but generated with a different random seed} is \protect\TLSdel{completely} independent of \protect\TLSins{the equivalent input spike train and} all the spikes generated in the network.
\protect\TLSins{We call these spike trains teaching signals $\bf{s}$, because they are only used for weight updates but not conducted into the network.}
Thus it decorrelates the \protect\TLSins{spike trains used for} SRM in the positive part of the weight change $\Delta w_{ij} = SRM(v_j,h_i)$ in SAE and SRBM by using $\Delta w_{ij}=SRM(s_j,h_i)$ instead, where $s_j$ is the teaching spike train firing at the same rate of $v_j$.
...
The reconstruction bias can be observed in the firing rates of the reconstruction neurons recorded in the complete experimental result in Figures~\ref{fig:sol2_ae} and~\ref{fig:sol2_rbm} \protect\TLSdel{of Appendices.} \protect\TLSins{in Appendix~\ref{sec:app2}.}
...
All of the above experiments and solutions were also tested on Leaky Integrate-and-Fire~(LIF) neurons to \protect\TLSdel{validates} \protect\TLSins{validate} the features of \protect\TLSins{the} SRM that the accuracy was independent of neural models, see Figures~\ref{fig:LIF_sae} and~\ref{fig:LIF_srbm} in \protect\TLSdel{Appendices.} \protect\TLSins{Appendix~\ref{sec:app2}.}
...
Having discussed the problem of training \protect\TLSdel{of} SAEs and SRBMs, we finally applied the proposed methods to \protect\TLSins{the MNIST} image recognition \protect\TLSdel{tasks of MNIST;} \protect\TLSins{tasks;}
the network architecture \protect\TLSdel{was} \protect\TLSins{is} shown in Figure~\ref{fig:MNSIT}.
The training was conducted using a one-layer AE or RBM, which consisted of 794 visible units including 784 neurons representing the images, 10 label units marking the classification, and 500 neurons \protect\TLSdel{for} \protect\TLSins{are in} the hidden layer.
...
The forward path of the network \protect\TLSdel{recognised} \protect\TLSins{assigned} an image to a digit class, and the bidirectional connections reconstructed the input image.
...
\begin{figure}
	...
	\begin{subfigure}[t]{0.4\textwidth}
		...
		\caption{SAE-S4}
	\end{subfigure}\\
	\caption{Trained weights after 3 epochs of MNIST training using (a) AE, (b) AE-NI, Poisson spike trains as input data, (c) Original SAE, (d) SAE-S2, neurons \protect\TLSdel{of} \protect\TLSins{with} noisy \protect\TLSdel{threshold,} \protect\TLSins{thresholds,} (e) SAE-S3, extra teaching signal, and (f) SAE-S4, combined solutions of S2 and S3.}
...
\begin{figure}
	...
	\begin{subfigure}[t]{0.4\textwidth}
		...
		\caption{SRBM-S4}
	\end{subfigure}\\
	\caption{Trained weights after 3 epochs of MNIST training using (a) nRBM, (b) nRBM-NI, Poisson spike trains as input data, (c) Original SRBM, (d) SRBM-S2, neurons \protect\TLSdel{of} \protect\TLSins{with} noisy \protect\TLSdel{threshold,} \protect\TLSins{thresholds,} (e) SRBM-S3, extra teaching signal, and (f) SRBM-S4, combined solutions of S2 and S3.}
...
The trained weights of AE and AE-NI \protect\TLSdel{were of litter} \protect\TLSins{had little} difference.
...
Nevertheless, the other improved SAE trainings demonstrated a closer feature extraction to the conventional \protect\TLSdel{AE's.} \protect\TLSins{AEs.}



Regarding RBM training
the results in Figure~\ref{fig:weights_rbm} \protect\TLSdel{shows:} \protect\TLSins{show:}
extracted features of \protect\TLSins{the} nRBM \protect\TLSdel{were} consisted of continuous strokes although \protect\TLSins{they} were not \protect\TLSins{as} recognisable as those of \protect\TLSins{the} AE, due to the randomness introduced by the noisy sampling of NReLU;
\protect\TLSins{the} noise \protect\TLSdel{of} \protect\TLSins{in} the input data led to the noisy extracted features where discontinuity of strokes appeared;
few continuous strokes were generated in the original SRBM training;
but more noticeable extracted patterns turned up with the \protect\TLSdel{solution of} noisy threshold \protect\TLSins{solution} of SRBM-S2;
...
The classification neuron having \protect\TLSdel{a} \protect\TLSins{the} highest spike count indicated the predicted class of the given image.
...
\end{subfigure}
	\caption{Classification accuracy \protect\TLSdel{compared} \protect\TLSins{comparing conventional models} with \protect\TLSdel{traditional training method and} spike-based models. All the \protect\TLSdel{trainings} \protect\TLSins{training} started with same \protect\TLSdel{weights initialisation.} \protect\TLSins{initial weights.} Every 3K steps of training, the trained weights were recorded and tested on \protect\TLSins{the} MNIST testing dataset for classification accuracy. The Poisson spikes trains used for SNN simulations are identical for all the tests.}
	\label{fig:sdlm_ca}
\end{figure}
Shown in Figure~\ref{fig:sdlm_ca}(a), the CA of AE and AE-NI steadily increased until it reaches about 92\% with \protect\TLSins{the} first 20K images trained in the network.
The noise in the training images of AE-NI made the peak performance of 93.29\%, a bit lower than AE (93.76\%).
\protect\TLSdel{While the}
\protect\TLSins{The} training on SAEs was slower than \protect\TLSins{on} AE at the beginning, due to the reason (explained in Section~\ref{subsec:SAE}) of low spiking rates.
The classification performance of SAEs reached the AEs' \protect\TLSins{after} about 50K training steps, and started to diverge:
the original SAE kept a \protect\TLSdel{close} CA \protect\TLSins{close to that} of \protect\TLSdel{AE's} \protect\TLSins{AEs} until it reached the best performance of 92.71\% at 87K steps, and then the CA began to decay because of the correlation problem;
while both SAE-S2 and SAE-S3 had similar performance \protect\TLSdel{with} \protect\TLSins{to} AEs and \protect\TLSdel{achieved} even overtook the best CA of AEs at 93.75\% and 94.38\% respectively;
...
The quantitative testing results of SRBMs drew a consistent conclusion with the trained weights described qualitatively above: \protect\TLSins{the} original SRBM worked poorly, SRBM-S2 performed closer to nRBM-NI, SRBM-S3 approximated to the training of nRBM and the performance of the combined solution was between the two solutions.
...
The shorter the period \protect\TLSdel{of} \protect\TLSins{that} each image \protect\TLSins{is} presented to the network, the \protect\TLSins{lower the} energy required \protect\TLSdel{is reduced} when implemented in hardware.
Thus to have a closer look at the CA performance over the testing time, we tested \protect\TLSins{the} CA on every ms for the entire duration of 1~s, and the best models of SAEs and SRBMs showed the CA in Figure~\ref{fig:latency}.
The SAE-S4 model can achieve a CA close to their best performance \protect\TLSdel{only} using \protect\TLSins{only} 200~ms, which may save 80\% of energy use.
...
If a close classification performance can be achieved with fewer spikes or \protect\TLSins{a} lower firing rate, then the neuromorphic implementation will be more energy efficient.
Figure~\ref{fig:sdlm_freq} shows how the firing rates of the input spike trains \protect\TLSdel{affects} \protect\TLSins{affect} the classification performance over testing time of 1~s.
...
Thus, for instance, the CA can achieve 93.5\% using only half of the firing events in the network and 200~ms for testing period, which may cost 10\% of energy use but achieve merely 0.5\% loss on CA \protect\TLSdel{comparing} \protect\TLSins{compared} to \protect\TLSins{the} baseline of $k=100$~Hz.
The CA of the SRBM-S3 was more sensitive \protect\TLSdel{on} \protect\TLSins{to} the input rate, where the model with $K=100$~Hz \protect\TLSdel{held} \protect\TLSins{achieved} the highest accuracy.
However, similar to SAEs, with half \protect\TLSins{of the} events used in the network and 400~ms testing time, the CA was also about 0.5\% lower than the model \protect\TLSdel{feeding with} \protect\TLSins{using the} preferred input rates.
...
\begin{figure}
	...
	\begin{subfigure}[t]{0.8\textwidth}
		...
		\caption{nRBMs and SRBMs.}
	\end{subfigure}
	\caption{Loss (mean squared error) pixel-wise of \protect\TLSins{the} traditional training method and spike-based models. All the trainings started with same \protect\TLSdel{weights initialisation.} \protect\TLSins{initial weights.} Every 3K \protect\TLSdel{steps of training,} \protect\TLSins{training steps,} the trained weights were recorded and tested on \protect\TLSins{the} MNIST testing dataset for reconstruction loss. The Poisson spikes trains used for SNN simulations are identical for all the tests.}
	\label{fig:sdlm_loss}
\end{figure}
...
\begin{figure}
	...
	\begin{subfigure}[t]{0.32\textwidth}
		...
		\caption{SRBM-S4}
	\end{subfigure}\\
	\caption{Reconstruct of the same digit '2' with trained weights using \protect\TLSdel{of} conventional AEs and RBMs to compare with their spike-based models.
...
Conventional RBMs demonstrate a better reconstruction capability than AEs in general, see Figure~\ref{fig:sdlm_loss}, thus RBMs are usually used for pre-training \protect\TLSdel{of} AEs or Deep Belief Networks.
...
SAE-S2 converged slowly to the reconstruction loss level of AE-NI, while in SRBM \protect\TLSdel{training, although the noise made the loss oscillated} \protect\TLSins{training} SRBM-S2 \protect\TLSdel{converged} \protect\TLSins{reached its best loss level} earlier than RBM and RBM-NI.
...
whereas in SRBMs, the teaching signal smoothed the loss curve, and both \protect\TLSdel{of} SRBM-S3 and SRBM-S4 converged to lower \protect\TLSdel{level} \protect\TLSins{levels} of loss than the conventional nRBMs.

Therefore, \protect\TLSdel{it depends on requirements to choose} \protect\TLSins{the choice of} a proper spiking model for \protect\TLSdel{reconstruction.
Original} \protect\TLSins{reconstruction depends on the requirements.
The original} SRBM is outstanding in \protect\TLSins{the} reconstruction task alone taking no account of the training resources and its poor classification performance.
Conversely the SAE-S2 wins in energy-efficient training and testing on classification, and is stable in reconstruction with similar performance \protect\TLSdel{of} \protect\TLSins{to} AE-NI.


\section{Summary}
SNNs can be trained with biologically-plausible synaptic plasticity and demonstrate \protect\TLSdel{competent} learning capability as \protect\TLSins{competent as} ANNs, even as deep architectures: AEs and RBMs.
...
This is the first attempt \protect\TLSdel{of formalising} \protect\TLSins{to formalise} the biological-plausible learning rule to compete with \protect\TLSdel{the} unsupervised \protect\TLSdel{deep learning} \protect\TLSins{Deep Learning} models.
