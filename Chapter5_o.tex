...
This chapter continues the discussion of training deep SNNs and takes extra step forward to the second research problem of biologically inspired, on-line, spike-based SNN training.
...
The first on-line training algorithm of unsupervised learning is event-based contrastive divergence (evtCD)~\citep{neil2013online} for SRBM, which established the feasibility of applying STDP rule to approximate CD algorithm.
...
%1. Neftci
Neftci et al.~\citep{neftci2013event} derived the division of potentiating and depression to their event-based CD algorithm, but conducted both parts on the same neural populations which was more biologically plausible.
...
Another similar work on SRBM training~\citep{burbank2015mirrored} applied STDP rule to train SAEs in a very much biologically-plausible manner.
...
%4. remote (ReSuMe)
The work we propose was mostly inspired from the supervised time-based spiking learning rule, ReSuMe~\citep{ponulak2010supervised}, where the output spikes of a post-synaptic neuron were trained to fire at expected time points as the teaching spikes do, see Figure~\ref{fig:resume}.
...
Instead of using binary units and sigmoid activations in RBM, we employed noisy ReLU (NReLU) units to construct nRBM which was closer to biology and performed better in classification tasks~\citep{nair2010rectified}.
...
\label{equ:rbm}
\end{equation} 
where $h'_i$ and $v'_j$ is a pair of Gibbs sampling, and the new sampling method NReLU is equivalent to generating multiple samples in the meantime and averaging them: $max(0, x+\mathcal{N}(0, \sigma(x))$.
...
%		\caption{Classification accuracy compared on RBM and SRBMs.}
	...
	\label{fig:sdlm_ca}
\end{figure}
Shown in Figure~\ref{fig:sdlm_ca}(a), the CA of AE and AE-NI steadily increased until it reaches about 92\% with first 20K images trained in the network.
