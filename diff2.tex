...
\label{cha:bkg}
The so-called third generation of artificial neural network, the \DIFdelbegin \DIFdel{spiking neural network }\DIFdelend \DIFaddbegin \DIFadd{Spiking Neural Network }\DIFaddend (SNN), is comprised of spiking neurons which mimic the dynamics of biological neural behaviour.
In this chapter we will demonstrate the special features of spiking neurons that differ from neurons of conventional \DIFdelbegin \DIFdel{artificial neural networks }\DIFdelend \DIFaddbegin \DIFadd{Artificial Neural Networks }\DIFaddend (ANNs);
these biologically-plausible neuronal operations are the root of the research problem raised in the thesis: how to operate SNNs to \DIFdelbegin \DIFdel{be capable in cognitive tasks and }\DIFdelend \DIFaddbegin \DIFadd{equip them with cognitive capabilities }\DIFaddend as competent as ANNs.
...
\DIFdelendFL \DIFaddbeginFL \caption[Two neurons connected by synapses~\mbox{%DIFAUXCMD
\citep{reece2011campbell,hodgkin1939action}
}%DIFAUXCMD
.]{\DIFaddendFL Two neurons connected by synapses. 
...
This minimal time difference between two spikes of a single neuron is the absolute \DIFdelbegin \DIFdel{refractory period }\DIFdelend \DIFaddbegin \textbf{\DIFadd{refractory period}} \DIFaddend during which no spike can be generated.
...
\begin{figure}[bt]
	...
	\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Example of rate coding~\mbox{%DIFAUXCMD
\citep{hubel1962receptive}
}%DIFAUXCMD
.]{\DIFaddendFL Example of rate coding: spike trains for different stimulus orientation (left) of a V1 simple cell of a cat, and the tuning curve (firing rate against stimulus orientation) of the neuron (right)~\citep{hubel1962receptive}.
...
\begin{figure}[bt]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.9\textwidth]{pics_snn/phaselocking.png}
%DIFDELCMD < 	\caption{%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.8\textwidth]{pics_snn/phaselocking.png}
	\caption[Example of temporal coding~\mbox{%DIFAUXCMD
\citep{liu2013modeling}
}%DIFAUXCMD
.]{\DIFaddendFL Example of temporal coding: phase-locked spike trains generated by \DIFaddbeginFL \DIFaddFL{simulated }\DIFaddendFL Inner Hair Cells in the cochlea~\citep{liu2013modeling}.
	...
	Sound localisation \DIFdelbeginFL \DIFdelFL{should }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{can }\DIFaddendFL be resolved by calculating the time difference and\DIFaddbeginFL \DIFaddFL{/or }\DIFaddendFL level difference of these sound waves which are encoded in the spike trains.
    }
...
\begin{figure}[tb!]
	...
	\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Post-synaptic potential.]{\DIFaddendFL Post-synaptic potential driven by a spike, where the red arrow represents a spike arriving at the neuron~\citep{marieb2007human}.}
	\label{Fig:psp}
\end{figure}
...
The state of the membrane potential change caused by a single spike is called the \DIFdelbegin \DIFdel{post-synaptic potential }\DIFdelend \DIFaddbegin \DIFadd{Post-Synaptic Potential }\DIFaddend (\textbf{PSP}). 
Thus, a spike transmitted by an excitatory synapse triggers a positive PSP, called an \DIFdelbegin \DIFdel{excitatory post-synaptic potential }\DIFdelend \DIFaddbegin \DIFadd{Excitatory Post-Synaptic Potential }\DIFaddend (\textbf{EPSP}), see Figure~\ref{Fig:psp}(a);
a negative change, an \DIFdelbegin \DIFdel{inhibitory post-synaptic potential }\DIFdelend \DIFaddbegin \DIFadd{Inhibitory Post-Synaptic Potential }\DIFaddend (\textbf{IPSP}), is driven by an inhibitory synaptic event and is shown in Figure~\ref{Fig:psp}(b).
...
\DIFdelend \DIFaddbegin \begin{figure}[b!]
	...
	\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Summation of post-synaptic potentials~\mbox{%DIFAUXCMD
\citep{reece2011campbell}
}%DIFAUXCMD
.]{\DIFaddendFL Summation of post-synaptic potentials~\citep{reece2011campbell}. 
...
\DIFdelbegin \DIFdel{Multiples }\DIFdelend \DIFaddbegin \DIFadd{Multiple }\DIFaddend PSPs have an accumulative effect on the membrane \DIFdelbegin \DIFdel{potential }\DIFdelend \DIFaddbegin \DIFadd{potentials }\DIFaddend both in temporal and spatial terms.
...
\begin{figure}[tb!]
	...
	\caption[The cell membrane acts like a RC circuit~\mbox{%DIFAUXCMD
\citep{gerstner2014neuronal}
}%DIFAUXCMD
.]
	{\DIFaddFL{The cell membrane acts like a Resistor-Capacitor (RC) circuit~\mbox{%DIFAUXCMD
\citep{gerstner2014neuronal}
}%DIFAUXCMD
.
...
\DIFaddend The evolution of membrane potential \DIFdelbegin \DIFdel{$v$ }\DIFdelend \DIFaddbegin \DIFadd{$V$ }\DIFaddend can be simplified to a \DIFdelbegin \DIFdel{resistor-capacitor }\DIFdelend \DIFaddbegin \DIFadd{Resistor-Capacitor }\DIFaddend (RC) circuit which consists of a membrane capacitor $C_m$ and a membrane resistance $R_m$, both driven by an input current flow $I$, see Figure~\ref{Fig:rc}.
...
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\includegraphics[width=0.3\textwidth]{pics_snn/RC.png}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Current flowing into a neuron functions like a resistor-capacitor (RC) circuit~\mbox{%DIFAUXCMD
\citep{gerstner2014neuronal}
}%DIFAUXCMD
.}}
	%DIFAUXCMD
%DIFDELCMD < \label{Fig:rc}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend The simple LIF model uses: (1) a linear differential equation to describe the evolution of membrane potential;
...
The Hodgkin-Huxley model\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep{hodgkin1952quantitative}
}%DIFAUXCMD
}\DIFaddend is the Nobel Prize winning model that explains the ionic mechanisms generating and transmitting action potentials in the squid giant axon\DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{hodgkin1952quantitative}
}%DIFAUXCMD
}\DIFdelend .
...
\textrm{~~~~when~} v \geq 30.
\end{equation}  
\DIFaddbegin \DIFadd{Parameters }\DIFaddend $a$, $b$, $c$, and $d$ are constant\DIFdelbegin \DIFdel{parameters that }\DIFdelend \DIFaddbegin \DIFadd{, which }\DIFaddend can be configured to reproduce various neural dynamics of real biological neurons~\citep{izhikevich2004model}.
...
\begin{figure}[bt!]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.5\textwidth]{pics_snn/stdp.jpeg}
%DIFDELCMD < 	\caption{%%%
\DIFdelFL{spike-time-dependent-plasticity~}%DIFDELCMD < [%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.7\textwidth]{pics_snn/stdp.jpeg}
	\caption[Spike-Timing-Dependent Plasticity (STDP)~\mbox{%DIFAUXCMD
\citep{bi2001synaptic}
}%DIFAUXCMD
.]{\DIFaddFL{Spike-Timing-Dependent Plasticity (}\DIFaddendFL STDP\DIFdelbeginFL %DIFDELCMD < ]%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{)}\DIFaddendFL ~\citep{bi2001synaptic}.
...
\right.
\end{equation}
where $A_+$ and $A_-$ in Equation~\ref{equ:stdp} become functions of variables $w$, $\eta_+$ and $\eta_-$ \DIFdelbegin \DIFdel{, and }\DIFdelend define the learning rate of the weight potentiation and depression, and $w_{max}$ is the maximum synaptic efficacy while 0 is the minimum.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %DIF > The weights are usually restricted within hard bounds.
%DIF > http://www.scholarpedia.org/article/Spike-timing_dependent_plasticity
We use the multiplicative STDP \DIFdelbegin \DIFdel{model }\DIFdelend for SNN training in Section~\ref{sec:test}\DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{.
It performs better than the addictive STDP when used in such a classification task, since the weight are much more distributed in the working range.
}

\DIFadd{In Chapter~\ref{cha:sdlm} we exploit }\DIFaddend a much simplified version of asymmetric rectangular STDP \DIFdelbegin \DIFdel{in Chapter~\ref{cha:sdlm} where the STDP window }\DIFdelend \DIFaddbegin \DIFadd{where the weight change }\DIFaddend is just a constant \DIFdelbegin \DIFdel{:
}\DIFdelend \DIFaddbegin \DIFadd{within an STDP window, $\tau_{win}$:
...
\right.
\end{equation}
\DIFaddbegin \DIFadd{We choose this simple algorithm for the straightforward linear conversion of the number of coincident spikes to the weight update.
 }

%DIF > The parameter $\tau_{win}$ is the effective range of the STDP window.
\DIFaddend 

\section{Simulating Networks of Spiking Neurons}
\label{sec:snn_sim}
In the previous section, we described neural dynamics as \DIFdelbegin \DIFdel{abstracted by }\DIFdelend mathematical models at the neuronal level.
...
Addressing this problem, existing solutions vary from software simulators to \DIFdelbegin \DIFdel{hardware neuromorphic systems}\DIFdelend \DIFaddbegin \DIFadd{neuromorphic hardware}\DIFaddend .
...
The synchronous `clock-driven' method uses numerical integration for solving the \DIFdelbegin \DIFdel{ordinary differential equations }\DIFdelend \DIFaddbegin \DIFadd{Ordinary Differential Equations }\DIFaddend (ODEs), that describe the evolution functions of the membrane potential with respect to time.
...
The sensor is capable of capturing very fast moving objects (e.g., up to \DIFdelbegin \DIFdel{10 K }\DIFdelend \DIFaddbegin \DIFadd{10K }\DIFaddend rotations per second), which is equivalent to \DIFdelbegin \DIFdel{100 K }\DIFdelend \DIFaddbegin \DIFadd{100K }\DIFaddend conventional frames per second~\citep{lenero20113}.
...
The binaural silicon cochlea~\citep{5537164} models the elementary functions of the cochlea including the basilar membrane, the \DIFdelbegin \DIFdel{inner hair cells}\DIFdelend \DIFaddbegin \DIFadd{Inner Hair Cells}\DIFaddend ~(IHCs), and the Spiral Ganglion Cells (SGCs).
...
However, these hardware SNN simulators either run on FPGAs~\citep{neil2014minitaur, kiselev2016event} or analogue circuits~\citep{qiao2015reconfigurable} \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{thus }\DIFaddend are limited in scale.
	...
	\end{subfigure}
	\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Neuromorphic hardware systems using SpiNNaker.]{\DIFaddendFL Three set-ups of neuromorphic sensory and processing hardware platform using SpiNNaker\DIFdelbeginFL \DIFdelFL{~\mbox{%DIFAUXCMD
...
The difference between biologically-plausible spiking neuronal operations and rate-based artificial activations holds the key to the research question: \DIFdelbegin \DIFdel{How }\DIFdelend \DIFaddbegin \DIFadd{how }\DIFaddend to equip SNNs with cognitive capabilities equivalent to ANNs.
