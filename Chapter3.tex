...
Again, the weight update is also dependant on the layer of neurons, where a convolution of the input vector $\mathbf{x}$ with the error gradient  $\mathbf{\delta}$ is needed in Conv layer.
...
Rectified Linear Unit (ReLU) is proposed to tackle the problem of vanishing gradient in deep network architectures.
...
\section{Autoencoder}
\label{sec:AE}
Figure~\ref{fig:AE} shows a simplest architecture of an autoencoder, which is the same as a MLP.
