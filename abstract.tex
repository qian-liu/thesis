...
Neuromorphic Engineering~(NE) has led to the development of biologically-inspired computer architectures which may provide an alternative to the conventional Von Neumann architecture and achieve the performance of human brain in terms of energy efficiency and cognitive capabilities.
...
Thus it raises the main research problem of how to operate and train biologically-plausible SNNs to close the gap of cognitive capabilities between SNNs and ANNs?
...
However, previous work on such off-line training methods have struggled in poor modelling accuracy of spiking neurons and high computation complexity.
In this thesis we propose a simple and novel activation function, Noisy Softplus~(NSP), to closely model the
response firing activity of biologically-plausible
spiking neurons;
...
We then take the step forward to `on-line' training methods, where Deep Learning modules are trained purely on SNNs in an event-driven manner.
The existing works have failed to provide SNNs with equivalent recognition accuracy of ANNs due to the lack of mathematical analysis. 
Thus we propose a formalised Spike-based Rate Multiplication method~(SRM) which transfers numerical multiplications to possibility of some specific firing events of a pair of rate-coded spike trains.
...
The promising results of training spiking Autoencoders~(AEs) and Restricted Boltzmann Machines~(RBMs) exhibit equivalent, sometimes, even superior classification and reconstruction capabilities compared to their non-spiking methods.

To provide meaningful comparisons between theses proposed SNN models and other existing methods within this rapidly advancing field of NE, we propose a large dataset of spike-based visual stimuli and a corresponding evaluation methodology to overall estimate the performance of SNN models and their hardware implementations.
