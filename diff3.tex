...
Deep Learning \DIFdelbegin \DIFdel{seems to have }\DIFdelend \DIFaddbegin \DIFadd{has }\DIFaddend become the answer to \DIFdelbegin \DIFdel{all }\DIFdelend \DIFaddbegin \DIFadd{increasing number of }\DIFaddend artificial intelligence problems \DIFdelbegin \DIFdel{overnight sinceGeoffrey Hinton }\DIFdelend \DIFaddbegin \DIFadd{since~\mbox{%DIFAUXCMD
\cite{hinton2006fast} }%DIFAUXCMD
firstly }\DIFaddend proposed the training method of \DIFdelbegin \DIFdel{a type of ANN, }\DIFdelend the Deep Belief Network\DIFdelbegin \DIFdel{, in 2006~\mbox{%DIFAUXCMD
\citep{hinton2006fast}}%DIFAUXCMD
}\DIFdelend .
However, Deep Learning is not new `magic', but rather has a history over a few decades\DIFdelbegin \DIFdel{and its }\DIFdelend \DIFaddbegin \DIFadd{.
Its }\DIFaddend sudden success is the result of the availability of an increasing amount of training data, the growing size of network models and greater computational power.
...
\DIFaddbegin \DIFadd{In this section, we briefly introduce some of the influencing techniques.
}\DIFaddend %However, at that time, deep networks could hardly be trained, mainly because of the limits on computational power.
...
The current trend in Deep Learning is to combine machine learning algorithms towards more complex objectives such as \DIFaddbegin \DIFadd{sequential }\DIFaddend decision making and data generation.
...
\DIFdelbegin \DIFdel{BP }\DIFdelend \DIFaddbegin \DIFadd{Backpropagation~(BP) }\DIFaddend propagates the gradient of the loss with respect to the weights backwards to each connection in the network.
...
In addition, \DIFdelbegin \DIFdel{the weights used in BP }\DIFdelend \DIFaddbegin \DIFadd{when BP is implemented, the weights }\DIFaddend are either transposed in the FC layer or rotated in the Conv layer compared to the forward path.

After error propagation, the BP algorithm updates weights using the optimisation method, gradient descent, to \DIFdelbegin \DIFdel{minimize }\DIFdelend \DIFaddbegin \DIFadd{minimise }\DIFaddend the objective function.
...
The Rectified Linear Unit (ReLU) is proposed to tackle the problem of vanishing gradients in deep networks\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep{nair2010rectified}}%DIFAUXCMD
}\DIFaddend .
...
\section{Autoencoders \DIFaddbegin \DIFadd{(AEs)}\DIFaddend }
\label{sec:AE}
Autoencoders \DIFdelbegin \DIFdel{~(AE) }\DIFdelend are categorised as unsupervised learning systems, since AE aims to reconstruct its original input as closely as possible.
...
\section{Restricted Boltzmann Machines \DIFaddbegin \DIFadd{(RBMs)}\DIFaddend }
\label{sec:rbm}
...
Despite \DIFdelbegin \DIFdel{the various statistical models involved in RBMs and their training method}\DIFdelend \DIFaddbegin \DIFadd{there are several statistical models and algorithms needed to explain how RBMs work}\DIFaddend , the actual training \DIFdelbegin \DIFdel{algorithm }\DIFdelend \DIFaddbegin \DIFadd{method }\DIFaddend is rather simple thanks to the use of Contrastive Divergence~(CD)\DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{hinton2002training}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{, proposed by~\mbox{%DIFAUXCMD
\cite{hinton2002training}}%DIFAUXCMD
}\DIFaddend .
