...
Merging \DIFdelbegin \DIFdel{deep learning techniques into SNNs }\DIFdelend \DIFaddbegin \DIFadd{Deep Learning techniques into Spiking Neural Networks~(SNNs), introduced in the previous chapter, }\DIFaddend may provide an answer to the problem of how to operate SNNs to perform as well as ANNs in cognitive tasks. 
...
Therefore, all the classical \DIFdelbegin \DIFdel{deep learning }\DIFdelend \DIFaddbegin \DIFadd{Deep Learning }\DIFaddend architectures date back to the last century \DIFdelbegin \DIFdel{when deep learning un-named}\DIFdelend \DIFaddbegin \DIFadd{before the `Deep Learning' name was coined}\DIFaddend .
...
As mentioned above, the first break-through \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend training deep, \DIFdelbegin \DIFdel{not }\DIFdelend \DIFaddbegin \DIFadd{as opposed to }\DIFaddend shallow ($\le 3$ layers), networks was the greedy layer-wise strategy~\citep{hinton2006fast} proposed to train stacked RBMs, which will be described in more detail in Section~\ref{sec:rbm}.
Shortly after\DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend this method was proved \DIFaddbegin \DIFadd{also }\DIFaddend to be efficient for training other kinds of deep networks including stacked AEs~\citep{bengio2007greedy} (stated in Section~\ref{sec:AE}).
...
In 2012, using such \DIFdelbegin \DIFdel{a unsupervised deep learning architecture}\DIFdelend \DIFaddbegin \DIFadd{an unsupervised Deep Learning architecture, }\DIFaddend the Google Brain team achieved a milestone in the \DIFdelbegin \DIFdel{deep learning }\DIFdelend \DIFaddbegin \DIFadd{Deep Learning }\DIFaddend era; the neural network learned to recognise cats by `watching' 10 million images generated from random frames of YouTube videos~\citep{le2013building}.
...
\DIFaddbegin \DIFadd{The }\DIFaddend NeoCognitron~\citep{fukushima1982neocognitron} was the first \DIFaddbegin \DIFadd{network }\DIFaddend to mimic the functions of V1 simple and complex neurons in an \DIFdelbegin \DIFdel{artificial neural network}\DIFdelend \DIFaddbegin \DIFadd{ANN}\DIFaddend , and later this feature detection of single cells was improved by sharing weights among receptive fields in LeNet-5~\citep{lecun1998gradient}, the typical ConvNet used today.
...
Despite the powerful capabilities of these feed-forward deep networks, sequence processing is a challenge for them since the \DIFaddbegin \DIFadd{size of the }\DIFaddend input and output vectors are \DIFdelbegin \DIFdel{of static size}\DIFdelend \DIFaddbegin \DIFadd{constrained to the number of neurons}\DIFaddend .
...
Exciting achievements have been reported \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend generating complex data such as realistic image generation based on \DIFdelbegin \DIFdel{description }\DIFdelend \DIFaddbegin \DIFadd{descriptions }\DIFaddend in text~\citep{radford2015unsupervised}.
...
Convolving a whole input image with a kernel composes a feature map in the Conv layer, thus \DIFaddbegin \DIFadd{in this LeNet example }\DIFaddend there are 6 feature maps in the first Conv layer.
...
The example demonstrates a common set-up using \DIFdelbegin \DIFdel{the whole }\DIFdelend \DIFaddbegin \DIFadd{all the feature maps of the }\DIFaddend 3D feature vectors; usually \DIFdelbegin \DIFdel{the feature maps can be selected to be involved in different convolutions }\DIFdelend \DIFaddbegin \DIFadd{it is feasible to select a subset of the input feature maps to involve in the convolutions with each kernel}\DIFaddend .
Then a \DIFdelbegin \DIFdel{pooling }\DIFdelend \DIFaddbegin \DIFadd{Pool }\DIFaddend layer follows the convolution.
Repeating these \DIFdelbegin \DIFdel{alternative presentation }\DIFdelend \DIFaddbegin \DIFadd{alternating presentations }\DIFaddend of a Conv layer and a \DIFdelbegin \DIFdel{pooling }\DIFdelend \DIFaddbegin \DIFadd{Pool }\DIFaddend layer builds up \DIFaddbegin \DIFadd{a }\DIFaddend deeper ConvNet.
...
In a ConvNet an FC layer is usually located \DIFdelbegin \DIFdel{on the top }\DIFdelend \DIFaddbegin \DIFadd{at the last layer }\DIFaddend (right in Figure~\ref{Fig:ConvNet})\DIFdelbegin \DIFdel{of a network, }\DIFdelend \DIFaddbegin \DIFadd{, and }\DIFaddend connects the last \DIFdelbegin \DIFdel{pooling }\DIFdelend \DIFaddbegin \DIFadd{Pool }\DIFaddend layer to the output neurons with all-to-all connections\DIFdelbegin \DIFdel{and predicts the classification of the input image.
The strongest output among the label neurons addresses }\DIFdelend \DIFaddbegin \DIFadd{.
%DIF >  to predict the classification of the input image.
The strongest response among the output neurons indicates }\DIFaddend to which class the input image belongs, as illustrated in Figure~\ref{Fig:ConvNet} where the red neuron represents the first digit out of ten, `0'.
There can be more than one FC \DIFdelbegin \DIFdel{layers consisting a MLP on the top }\DIFdelend \DIFaddbegin \DIFadd{layer at the end }\DIFaddend of a ConvNet\DIFdelbegin \DIFdel{, in }\DIFdelend \DIFaddbegin \DIFadd{.
...
In this section we }\DIFaddend will demonstrate the training of a network by back-propagating errors \DIFdelbegin \DIFdel{and weightsmodification}\DIFdelend \DIFaddbegin \DIFadd{to tune the connection weights}\DIFaddend .
...
Given a set of training data with desired labels $\mathbf{T}=\{\mathbf{t}^1, \mathbf{t}^2, ..., \mathbf{t}^K\}$ and the network output $\mathbf{Y}=\{\mathbf{y}^1, \mathbf{y}^2, ..., \mathbf{y}^K\}$ the \DIFdelbegin \DIFdel{mean squared error }\DIFdelend \DIFaddbegin \DIFadd{Mean Squared Error~(MSE) }\DIFaddend can be seen as the objective function:  
\begin{equation}
L=\DIFaddbegin {\DIFaddend MSE\DIFaddbegin }\DIFaddend (\mathbf{T}, \mathbf{Y}) =\frac{1}{2K}\sum_{k=1}^K \sum_{m=1}^M (y^{k}_{m}-t^{k}_{m})^{2}~~.
\label{equ:loss_all}
\end{equation}
...
\DIFdelbegin \DIFdel{\frac{\partial net_j}{\partial w_{ij}} }%DIFDELCMD < &%%%
\DIFdel{= }\DIFdelend x_i \DIFaddbegin &\DIFadd{= \frac{\partial net_j}{\partial w_{ij}}}\DIFaddend ~~.
\end{aligned}
\end{equation}
...
&(\sum_l^L \delta_l w\DIFdelbegin \DIFdel{_{jl}}\DIFdelend \DIFaddbegin \DIFadd{_{lj}}\DIFaddend ) f'(net_j)  \DIFdelbegin \textrm{\DIFdel{~,~~if otherwise}}
%DIFAUXCMD
...
\end{equation}
It is easy to obtain the first term, $ \frac{\partial L}{\partial y_j}  $, of $ \delta_j $ \DIFdelbegin \DIFdel{if }\DIFdelend \DIFaddbegin \DIFadd{when }\DIFaddend $j$ is an output neuron\DIFaddbegin \DIFadd{, }\DIFaddend since it only involves \DIFaddbegin \DIFadd{a }\DIFaddend single dimension of the output vector.
...
A detailed description of BP training on \DIFdelbegin \DIFdel{ConvNet }\DIFdelend \DIFaddbegin \DIFadd{ConvNets }\DIFaddend can be found \DIFdelbegin \DIFdel{in}\DIFdelend \DIFaddbegin \DIFadd{elsewhere}\DIFaddend ~\citep{bouvrie2006notes}.

Moreover, applying stochastic gradient descent (SGD), the gradient over the full training set can be approximated using only a \DIFdelbegin \DIFdel{single or a few}\DIFdelend \DIFaddbegin \DIFadd{few, even a single, }\DIFaddend training data.
...
Hence, the error gradient is able to be back-propagated in deeper \DIFdelbegin \DIFdel{network }\DIFdelend \DIFaddbegin \DIFadd{networks }\DIFaddend by multiplying ReLU derivatives.
...
\end{equation}
where $\eta$ is the learning rate, $h_i$ is the hidden neuron and the term \DIFdelbegin \DIFdel{$\delta^k_j$ }\DIFdelend \DIFaddbegin \DIFadd{$\delta_j$ }\DIFaddend is the error gradient with the respect to $net_j$, the weighted summation.
...
Hence, \DIFdelbegin \DIFdel{$\delta^k_j$  }\DIFdelend \DIFaddbegin \DIFadd{$\delta_j$  }\DIFaddend can be calculated according to Equation~\ref{equ:derivative}:
\begin{equation}
...
\section{Restricted Boltzmann Machine}
\label{sec:rbm}
RBMs share similar ideas of unsupervised learning with AEs, but aim to present a data distribution of some training dataset, rather than reconstructing each \DIFdelbegin \DIFdel{element }\DIFdelend \DIFaddbegin \DIFadd{sample }\DIFaddend of the dataset.
...
We can only illustrate some of the statistical models and sampling methods involved in RBM training, a detailed description can be found \DIFdelbegin \DIFdel{in}\DIFdelend \DIFaddbegin \DIFadd{elsewhere}\DIFaddend ~\citep{fischer2012introduction}. 
Stacked RBMs are also trained by layer-wise greedy algorithms, \DIFdelbegin \DIFdel{like stacked AEs, }\DIFdelend thus this section will focus on training a single RBM block.
...
%\end{aligned}
\end{equation} 
The energy function~\citep{hopfield1982neural} of \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend RBM is defined as follows:
\begin{equation}
...
Given a set of \DIFdelbegin \DIFdel{outcomes }\DIFdelend data $ \mathbf{D}=\{\mathbf{v}^1, \mathbf{v}^2, ..., \mathbf{v}^K\} $, the likelihood function of model parameters $\Theta$ defines the probability of the \DIFaddbegin \DIFadd{observed }\DIFaddend outcomes $ \mathbf{D}$ given $\Theta$:
\begin{equation}
...
The \DIFdelbegin \DIFdel{loss derivative }\DIFdelend \DIFaddbegin \DIFadd{derivative of the log-likelihood }\DIFaddend with respect to a parameter $\theta$ is as follows:
\begin{equation}
\label{equ:part}
\begin{aligned}
\dfrac{\partial \hat{l} (\Theta \mid \mathbf{D})}{\partial \theta} 
...
%These generative samples is called ``fantasy data'' and can be generated using Monte Carlo Markov Chain (MCMC) sampling, see section~\ref{sec:mcmc}.
The detailed derivation process can be found in \DIFdelbegin \DIFdel{Appendix~\ref{app:part}}\DIFdelend \DIFaddbegin \DIFadd{the Appendix (Equation~\ref{app_equ:part})}\DIFaddend .
%Although in this example the integration of product of normal distribution is still tractable, it is also helpful to use numerical integration.
...
Applying the \DIFdelbegin \DIFdel{model function of RBMs }\DIFdelend \DIFaddbegin \DIFadd{RBM model function }\DIFaddend in Equation~\ref{equ:rbm_prob} to the loss derivative (Equation~\ref{equ:part}), \DIFdelbegin \DIFdel{Appendix~\ref{app:RBM} }\DIFdelend \DIFaddbegin \DIFadd{Equation~\ref{app_equ:RBM} }\DIFaddend derives the loss function of RBMs in detail:
\begin{equation}
\label{equ:RBM}
\begin{aligned}
\dfrac{\partial \hat{l} (\Theta \mid \mathbf{D})}{\partial \theta} 
& = \left \langle \DIFdelbegin %DIFDELCMD < \dfrac{\partial -E(\mathbf{v}, \mathbf{h} \mid \Theta)}{\partial \theta} %%%
\DIFdelend \DIFaddbegin \dfrac{\partial {-E}(\mathbf{v}, \mathbf{h} \mid \Theta)}{\partial \theta} \DIFaddend \right \rangle_{\{\mathbf{D}, \mathbf{D}_h\} \sim p( \mathbf{h} \mid \mathbf{v}, \Theta)} 
- \left \langle \DIFdelbegin %DIFDELCMD < \dfrac{\partial -E(\mathbf{v}, \mathbf{h} \mid \Theta)}{\partial \theta} %%%
\DIFdelend \DIFaddbegin \dfrac{\partial {-E}(\mathbf{v}, \mathbf{h} \mid \Theta)}{\partial \theta} \DIFaddend \right \rangle_{\{\mathbf{C_v}, \mathbf{C_h}\} \sim p( \mathbf{v}, \mathbf{h} \mid  \Theta)},  \\
\end{aligned}
\end{equation}
...
However, if we \DIFdelbegin \DIFdel{just take }\DIFdelend \DIFaddbegin \DIFadd{take just }\DIFaddend a few steps, the KL divergence can be seen as a k-step contrastive convergence (\DIFdelbegin \DIFdel{$ CD_{k}) $.
Even $ CD_1 $ has performed }\DIFdelend \DIFaddbegin \DIFadd{$ {CD}_{k}) $.
Even $ {CD}_1 $ performs }\DIFaddend surprisingly well in RBM training~\citep{hinton2002training}.
