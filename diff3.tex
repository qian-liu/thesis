...
Merging Deep Learning techniques into Spiking Neural Networks~(SNNs), introduced in the previous chapter, may provide an answer to the problem of how to operate SNNs to perform \DIFdelbegin \DIFdel{as well }\DIFdelend \DIFaddbegin \DIFadd{equivalently }\DIFaddend as ANNs in cognitive tasks. 
...
A typical ConvNet\DIFdelbegin \DIFdel{(LeNet ) }\DIFdelend \DIFaddbegin \DIFadd{, LeNet shown in Figure~\ref{Fig:ConvNet},  }\DIFaddend consists of four types of neural layers: an input layer, convolutional~(Conv) layers of simple cells, pooling~(Pool) layers of complex cells and fully-connected~(FC) layers\DIFdelbegin \DIFdel{, as shown in Figure~\ref{Fig:ConvNet}}\DIFdelend .
...
However, a \DIFdelbegin \DIFdel{multilayer perceptron }\DIFdelend \DIFaddbegin \DIFadd{Multilayer Perceptron }\DIFaddend (MLP) network possesses only FC layers, where each neural layer fully connects to the next one.
...
Moreover, applying \DIFdelbegin \DIFdel{stochastic gradient descent }\DIFdelend \DIFaddbegin \DIFadd{Stochastic Gradient Descent }\DIFaddend (SGD), the gradient over the full training set can be approximated using only a few, even a single, training data.
...
We illustrated the structure and training procedure for three Deep Learning models: Convolutional \DIFdelbegin \DIFdel{networks}\DIFdelend \DIFaddbegin \DIFadd{Networks}\DIFaddend , Autoencoders and Restricted Boltzmann Machines, which will be applied in spiking deep networks in the following chapters.