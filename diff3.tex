...
\label{cha:dnn}
Deep Learning research in the field of artificial neural networks~(ANNs) has dominated state-of-the-art solutions on cognitive tasks, e.g. the performance exceeding human-level  on image classification tasks~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{he2015delving}
}%DIFAUXCMD
...
Merging \DIFdelbegin \DIFdel{deep learning }\DIFdelend \DIFaddbegin \DIFadd{Deep Learning }\DIFaddend techniques into SNNs may provide an answer to the problem of how to operate SNNs to perform as well as ANNs in cognitive tasks. 
In this chapter, we will give an overview of popular architectures and models of \DIFdelbegin \DIFdel{deep learning }\DIFdelend \DIFaddbegin \DIFadd{Deep Learning }\DIFaddend in Section~\ref{sec:dl_history}, and the rest of this chapter will illustrate the mechanisms of convolutional networks (ConvNets), autoencoders (AEs), and restricted Boltzmann machines~(RBMs) in detail which will be used in following chapters.

\section{Brief Overview}
\label{sec:dl_history}
Deep \DIFdelbegin \DIFdel{learning }\DIFdelend \DIFaddbegin \DIFadd{Learning }\DIFaddend seems to have become the answer to all artificial intelligence problems overnight since Geoffrey Hinton proposed the training method of a type of ANN, \DIFdelbegin \DIFdel{deep belief network}\DIFdelend \DIFaddbegin \DIFadd{the Deep Belief Network}\DIFaddend , in 2006~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{hinton2006fast}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
However, \DIFdelbegin \DIFdel{deep learning }\DIFdelend \DIFaddbegin \DIFadd{Deep Learning }\DIFaddend is not a new `magic', but rather has a history over a few decades and its sudden success is the result of the availability of an increasing amount of training data, \DIFaddbegin \DIFadd{the }\DIFaddend growing size of network models and greater computational power.
Therefore, all the classical \DIFdelbegin \DIFdel{deep learning }\DIFdelend \DIFaddbegin \DIFadd{Deep Learning }\DIFaddend architectures date back to the last century \DIFdelbegin \DIFdel{when deep learning un-named}\DIFdelend \DIFaddbegin \DIFadd{before the `Deep Learning' named was coined}\DIFaddend .
However, at that time, deep networks \DIFdelbegin \DIFdel{were hardly to be trained}\DIFdelend \DIFaddbegin \DIFadd{could hardly be trained, }\DIFaddend mainly because of the \DIFdelbegin \DIFdel{limit }\DIFdelend \DIFaddbegin \DIFadd{limits }\DIFaddend on computational power.


\subsection{Classical Models}
We call the well-known and widely-used \DIFdelbegin \DIFdel{deep learning }\DIFdelend \DIFaddbegin \DIFadd{Deep Learning }\DIFaddend models `classical' and give a brief introduction to those models in this section. 
As mentioned above, the first break-through \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend training deep, \DIFdelbegin \DIFdel{not }\DIFdelend \DIFaddbegin \DIFadd{as opposed to }\DIFaddend shallow ($\le 3$ layers), networks was the greedy layer-wise strategy~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{hinton2006fast}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{hinton2006fast}
}%DIFAUXCMD
}\DIFaddend proposed to train stacked RBMs, which will be described in more detail in Section~\ref{sec:rbm}.
Shortly after that this method was proved to be efficient for training other kinds of deep networks including stacked AEs~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{bengio2007greedy}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{bengio2007greedy}
}%DIFAUXCMD
}\DIFaddend (stated in Section~\ref{sec:AE}).
...
In 2012, using such \DIFdelbegin \DIFdel{a unsupervised deep learning architecture}\DIFdelend \DIFaddbegin \DIFadd{an unsupervised Deep Learning architecture, }\DIFaddend the Google Brain team achieved a milestone in the \DIFdelbegin \DIFdel{deep learning }\DIFdelend \DIFaddbegin \DIFadd{Deep Learning }\DIFaddend era; the neural network learned to recognise cats by `watching' 10 million images generated from random frames of YouTube videos~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{le2013building}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{le2013building}
}%DIFAUXCMD
}\DIFaddend .

ConvNets are biologically inspired from the significant discovery of Hubel and Wiesel that the orientation selectivity (simple cells) and pooling mechanism (complex cells) represent the basic functions in the primary visual cortex in cats~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{NeoCognitron~\mbox{%DIFAUXCMD
\cite{fukushima1982neocognitron}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{The NeoCognitron~\mbox{%DIFAUXCMD
\citep{fukushima1982neocognitron}
}%DIFAUXCMD
}\DIFaddend was the first \DIFaddbegin \DIFadd{network }\DIFaddend to mimic the functions of V1 simple and complex neurons in an artificial neural network, and later this feature detection of single cells was improved by sharing weights among receptive fields in LeNet-5~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{lecun1998gradient}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{lecun1998gradient}
}%DIFAUXCMD
}\DIFaddend , the typical ConvNet used today.
...
The most significant milestones produced by deep ConvNet dominated the best performances in the annual ImageNet Challenge~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{russakovsky2015imagenet}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{russakovsky2015imagenet}
}%DIFAUXCMD
}\DIFaddend : AlexNet~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{krizhevsky2012imagenet}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{krizhevsky2012imagenet}
}%DIFAUXCMD
}\DIFaddend , VGG Net~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{simonyan2014very}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{simonyan2014very}
}%DIFAUXCMD
}\DIFaddend , GoogLeNet~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{szegedy2015going}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{szegedy2015going}
}%DIFAUXCMD
}\DIFaddend and ResNet~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{he2016deep}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{he2016deep}
...
Thus recurrent neural networks~(RNNs), containing feed-back connections, are ideal solutions for dealing with sequential information since their current output is always \DIFdelbegin \DIFdel{dependant }\DIFdelend \DIFaddbegin \DIFadd{dependent }\DIFaddend on the previous `memory'.
As training mechanisms have become more mature, for example using the long short-term memory or (LSTM)~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{hochreiter1997long}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{hochreiter1997long}
}%DIFAUXCMD
}\DIFaddend , RNNs have shown great success in many natural language processing tasks: language modelling~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{mikolov2010recurrent}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{mikolov2010recurrent}
}%DIFAUXCMD
}\DIFaddend , machine translation~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{sutskever2014sequence}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{sutskever2014sequence}
}%DIFAUXCMD
}\DIFaddend , speech recognition~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{graves2014towards}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{graves2014towards}
}%DIFAUXCMD
}\DIFaddend and image caption generation~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{karpathy2015deep}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{karpathy2015deep}
}%DIFAUXCMD
}\DIFaddend .

\subsection{Combined Approaches}
\DIFdelbegin \DIFdel{Current trend of deep learning }\DIFdelend \DIFaddbegin \DIFadd{The current trend in Deep Learning }\DIFaddend is to combine machine learning algorithms towards more complex objectives \DIFdelbegin \DIFdel{: }\DIFdelend such as decision making and data generation.
...
To address complex decision making problems in practical life, RL requires a sufficiently abstract representation of the high-dimensional environment\DIFdelbegin \DIFdel{, where deep learning is exactly clever }\DIFdelend \DIFaddbegin \DIFadd{.
Here Deep Learning is very effective }\DIFaddend at dimensionality reduction and feature extraction.
The milestone achieved by \DIFaddbegin \DIFadd{the }\DIFaddend integration of RL and deep networks, \DIFdelbegin \DIFdel{the deep reinforcement learning}\DIFdelend \DIFaddbegin \DIFadd{deep RL}\DIFaddend , drew everyone's attention \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend artificial intelligence when AlphaGo beat a professional human player \DIFdelbegin \DIFdel{in Go~\mbox{%DIFAUXCMD
\cite{silver2016mastering}
}%DIFAUXCMD
.
%DIF < which has been successfully applied to various tasks: such as games~\cite{mnih2015human}, robotics~\cite{schulman2015trust} and question-and-answer systems.
}\DIFdelend \DIFaddbegin \DIFadd{at Go~\mbox{%DIFAUXCMD
\citep{silver2016mastering}
}%DIFAUXCMD
.
%DIF > which has been successfully applied to various tasks: such as games~\citep{mnih2015human}, robotics~\citep{schulman2015trust} and question-and-answer systems.
}\DIFaddend %Potential in real-time neuromorphic system
%DIF < These real-time decision making systems, especially used for robotics~\cite{schulman2015trust}, are ideal for meromorphic hardware systems which takes advantages in event-based processing and energy efficiency.
%DIF > These real-time decision making systems, especially used for robotics~\citep{schulman2015trust}, are ideal for meromorphic hardware systems which takes advantages in event-based processing and energy efficiency.
%http://blog.aylien.com/introduction-generative-adversarial-networks-code-tensorflow/

Generative Adversarial Networks (GANs)~~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{goodfellow2014generative}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{goodfellow2014generative}
}%DIFAUXCMD
}\DIFaddend are proposed for training generative models of complex data.
...
Thus, the generator learns to produce more realistic data to \DIFdelbegin \DIFdel{cheat }\DIFdelend \DIFaddbegin \DIFadd{fool }\DIFaddend the discriminator; meanwhile the discriminator learns to become better at distinguishing generated from real data.
Exciting achievements have been reported \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend generating complex data such as realistic image generation based on \DIFdelbegin \DIFdel{description }\DIFdelend \DIFaddbegin \DIFadd{descriptions }\DIFaddend in text~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{radford2015unsupervised}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{radford2015unsupervised}
}%DIFAUXCMD
}\DIFaddend .

\section{Convolutional Networks}
\label{sec:convnet}
In Chapter~\ref{cha:Conv}, we will use a convolutional network to demonstrate a training method \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend spiking deep architectures.
This section aims to give a detailed introduction to ConvNet architectures and \DIFdelbegin \DIFdel{its }\DIFdelend \DIFaddbegin \DIFadd{their }\DIFaddend training with back propagation.
...
A typical ConvNet (LeNet) consists of four types of neural layers: an input layer, convolutional~(Conv) layers of simple cells, pooling\DIFaddbegin \DIFadd{~(Pool) }\DIFaddend layers of complex cells and fully-connected~(FC) layers, as shown in Figure~\ref{Fig:ConvNet}.
From the left to right of Figure~\ref{Fig:ConvNet}, we illustrate the mechanism of \DIFaddbegin \DIFadd{the }\DIFaddend ConvNet in detail.
...
Stride controls the output volume of a convolution such that setting the stride to $step$ causes the kernel \DIFdelbegin \DIFdel{moving }\DIFdelend \DIFaddbegin \DIFadd{to move }\DIFaddend $step$ pixels at a time.
...
The special characteristics of Conv layers lie in the local connectivity and the parameter sharing, such that a neuron connects only to a spatially local input volume, its receptive field; and the weights (\DIFaddbegin \DIFadd{the }\DIFaddend convolutional kernel) are shared among all the receptive fields.
Consequently, the number of parameters (trainable weights) hugely decreases compared to all-to-all connections of \DIFaddbegin \DIFadd{the }\DIFaddend same network size. 

The complex neurons of \DIFdelbegin \DIFdel{pooling }\DIFdelend \DIFaddbegin \DIFadd{the Pool }\DIFaddend layers either output the maximum input within their receptive fields (max pooling) or the \DIFdelbegin \DIFdel{averaging }\DIFdelend \DIFaddbegin \DIFadd{average }\DIFaddend (average pooling), \DIFdelbegin \DIFdel{as }\DIFdelend \DIFaddbegin \DIFadd{by }\DIFaddend applying a max/average filter to non-overlapping receptive fields.
The pooling process reduces the spatial size of the feature maps but keeps the number of features.
In Chapter~\ref{cha:Conv}, we will use average pooling of $2\times2$ for all the \DIFdelbegin \DIFdel{pooling layers, }\DIFdelend \DIFaddbegin \DIFadd{Pool layers, as }\DIFaddend shown in Figure~\ref{Fig:ConvNet}.
The average filter traverses \DIFdelbegin \DIFdel{over }\DIFdelend an entire feature map with a stride of 2, and outputs the averaged element of each receptive field to the next layer.
...
The example demonstrates a common set-up using \DIFdelbegin \DIFdel{the whole }\DIFdelend \DIFaddbegin \DIFadd{all the feature maps of the }\DIFaddend 3D feature vectors; usually the \DIFaddbegin \DIFadd{individual }\DIFaddend feature maps can be selected to \DIFaddbegin \DIFadd{form a subset of the 3D vectors to }\DIFaddend be involved in \DIFdelbegin \DIFdel{different convolutions }\DIFdelend \DIFaddbegin \DIFadd{convolutions with different kernels}\DIFaddend .
Then a \DIFdelbegin \DIFdel{pooling }\DIFdelend \DIFaddbegin \DIFadd{Pool }\DIFaddend layer follows the convolution.
Repeating these alternative \DIFdelbegin \DIFdel{presentation }\DIFdelend \DIFaddbegin \DIFadd{presentations }\DIFaddend of a Conv layer and a \DIFdelbegin \DIFdel{pooling }\DIFdelend \DIFaddbegin \DIFadd{Pool }\DIFaddend layer builds up \DIFaddbegin \DIFadd{a }\DIFaddend deeper ConvNet.

The trainable shared weights used in the Conv layer hugely \DIFdelbegin \DIFdel{reduces }\DIFdelend \DIFaddbegin \DIFadd{reduce }\DIFaddend the number of weight parameters in a ConvNet, while \DIFdelbegin \DIFdel{pooling }\DIFdelend \DIFaddbegin \DIFadd{Pool }\DIFaddend layers use static convolutional kernels to shrink the size of \DIFdelbegin \DIFdel{networks}\DIFdelend \DIFaddbegin \DIFadd{the network}\DIFaddend .
These convolutional connections can be described as 6c5-2s-12c5-2s, where $\alpha c \beta$ indicates $\alpha$ kernels of side length $\beta$ used in \DIFaddbegin \DIFadd{the }\DIFaddend Conv layer, and $2s$ specifies the side length and the stride of a pooling kernel.
...
In a ConvNet an FC layer is usually located \DIFdelbegin \DIFdel{on the top }\DIFdelend \DIFaddbegin \DIFadd{at the last layer }\DIFaddend (right in Figure~\ref{Fig:ConvNet}) of a network, \DIFdelbegin \DIFdel{connects the last pooling }\DIFdelend \DIFaddbegin \DIFadd{to connect the last Pool }\DIFaddend layer to the output neurons with all-to-all connections \DIFdelbegin \DIFdel{and predicts }\DIFdelend \DIFaddbegin \DIFadd{to predict }\DIFaddend the classification of the input image.
The strongest output among the label neurons \DIFdelbegin \DIFdel{addresses }\DIFdelend \DIFaddbegin \DIFadd{indicates }\DIFaddend to which class the input image belongs, as illustrated in Figure~\ref{Fig:ConvNet} where the red neuron represents the first digit out of ten, `0'.
There can be more than one FC \DIFdelbegin \DIFdel{layers consisting a MLP on the top }\DIFdelend \DIFaddbegin \DIFadd{layer comprising an MLP at the end }\DIFaddend of a ConvNet\DIFdelbegin \DIFdel{, in }\DIFdelend \DIFaddbegin \DIFadd{.
In }\DIFaddend this thesis, we will use only a single FC layer.


%Chain rule, reduced parameter amounts. 

\subsection{Backpropagation}
We have described the feed-forward path of a ConvNet to classify an input image\DIFdelbegin \DIFdel{, this section }\DIFdelend \DIFaddbegin \DIFadd{.
In this section we }\DIFaddend will demonstrate the training of a network by back-propagating errors \DIFdelbegin \DIFdel{and weightsmodification}\DIFdelend \DIFaddbegin \DIFadd{to modify weights}\DIFaddend .
The objective function (or loss function) estimates an error \DIFaddbegin \DIFadd{by }\DIFaddend comparing the output vector of a network, $\mathbf{y}=(y_1,y_2,...,y_M)$, to the desired label vector, $\mathbf{t}=(t_1,t_2,...,t_M)$, and the error is to be minimised during training.
...
Backpropagation~(BP) \DIFdelbegin \DIFdel{, }\DIFdelend propagates the gradient of the loss with respect to the weights backwards to each connection in the network.
...
\begin{figure}[bt]
	...
	\caption{ $N$ neurons connect to the $j_{th}$ neuron of the next layer. As illustrated in Figure~\ref{Fig:compare_as}(a), we use \DIFaddbeginFL \DIFaddFL{a }\DIFaddendFL simplified artificial neuron without bias.}
...
\right. \DIFdelbegin \textrm{\DIFdel{~,~~where}}%DIFAUXCMD
\DIFdelend \\
...
\end{equation}
It is easy to obtain the first term, $ \frac{\partial L}{\partial y_j}  $, of $ \delta_j $ if $j$ is an output neuron since it only involves \DIFaddbegin \DIFadd{a }\DIFaddend single dimension of the output vector.
However, when $j$ is in an inner layer of the network, which connects \DIFaddbegin \DIFadd{to }\DIFaddend $L$ neurons on the next layer, we have to take \DIFaddbegin \DIFadd{the }\DIFaddend total derivative with respect to $y_j$: $\sum_l^L \delta_l w_{jl}$.
The error propagation applies to any form of connection in a network, such as FC and Conv layers, and the difference only appears in the summation where \DIFaddbegin \DIFadd{the }\DIFaddend matrix product is used for the FC layer and convolution for \DIFaddbegin \DIFadd{the }\DIFaddend Conv layer.
In addition, the weights used in BP are either transposed in \DIFaddbegin \DIFadd{the }\DIFaddend FC layer or rotated in \DIFaddbegin \DIFadd{the }\DIFaddend Conv layer compared to the forward path.
...
\label{equ:delta_w}
\end{equation}
where $\eta$ defines the length of these updating steps which is also called \DIFaddbegin \DIFadd{the }\DIFaddend learning rate.
Again, the weight update is also \DIFdelbegin \DIFdel{dependant }\DIFdelend \DIFaddbegin \DIFadd{dependent }\DIFaddend on the types of \DIFdelbegin \DIFdel{layers}\DIFdelend \DIFaddbegin \DIFadd{layer}\DIFaddend , where a convolution of the input vector $\mathbf{x}$ with the error gradient  $\mathbf{\delta}$ is needed in Conv layers.
A detailed description of BP training on \DIFdelbegin \DIFdel{ConvNet }\DIFdelend \DIFaddbegin \DIFadd{ConvNets }\DIFaddend can be found \DIFdelbegin \DIFdel{in~\mbox{%DIFAUXCMD
\cite{bouvrie2006notes}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{elsewhere~\mbox{%DIFAUXCMD
\citep{bouvrie2006notes}
}%DIFAUXCMD
}\DIFaddend .
...
\begin{figure}[tbp!]
	...
	\label{fig:af}
\end{figure}
Each error gradient propagated through layers of \DIFaddbegin \DIFadd{the }\DIFaddend network consists of a chain of multiplied factors as illustrated in Equation~\ref{equ:derivative}.
...
For the sigmoid function example, shown in Figure~\ref{fig:af}(a), \DIFdelbegin \DIFdel{its derivative keeps }\DIFdelend \DIFaddbegin \DIFadd{the derivative is }\DIFaddend the maximum (around~0.25) only when the input is close to 0, and decays to its minimum 0 as the input either increases or decreases, see Figure~\ref{fig:af}(b).

\DIFaddbegin \DIFadd{The }\DIFaddend Rectified Linear Unit (ReLU) is proposed to tackle the problem of vanishing \DIFdelbegin \DIFdel{gradient }\DIFdelend \DIFaddbegin \DIFadd{gradients }\DIFaddend in deep networks.
...
\label{sec:AE}
Autoencoders\DIFaddbegin \DIFadd{~(AE) }\DIFaddend are categorised as unsupervised learning \DIFaddbegin \DIFadd{systems}\DIFaddend , since AE aims to reconstruct its original input as closely as possible.
Thus the output and input vectors have the same dimensionality\DIFdelbegin \DIFdel{.
But }\DIFdelend \DIFaddbegin \DIFadd{, but }\DIFaddend the hidden middle layer is allowed to have either more, or fewer, neurons for the purpose of compressing the inputs or mapping them to higher dimensions.
Hence \DIFaddbegin \DIFadd{the }\DIFaddend AE is suitable \DIFdelbegin \DIFdel{to learn }\DIFdelend \DIFaddbegin \DIFadd{for learning an }\DIFaddend effective representation of the original inputs at the hidden layer, which can be seen as the encoding phase of \DIFaddbegin \DIFadd{the }\DIFaddend AE, and \DIFdelbegin \DIFdel{reconstruct }\DIFdelend \DIFaddbegin \DIFadd{reconstructing }\DIFaddend them on the output layer where the hidden vectors are decoded. 

Multiple AEs can be stacked to build deep AEs where the hidden layer of a lower AE represents the input of the upper block, and each AE block can be trained individually with greedy layer-wise training~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{hinton2006fast}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{hinton2006fast}
...
Due to \DIFaddbegin \DIFadd{the }\DIFaddend AE's simple network architecture and training algorithm, spiking AEs will be built and trained with \DIFaddbegin \DIFadd{a }\DIFaddend biologically-plausible training mechanism in Chapter~\ref{cha:sdlm}.

\subsection{Structure}
Figure~\ref{fig:AE} shows the architecture of an AE, which is an MLP consisting of three layers of neurons: \DIFaddbegin \DIFadd{the }\DIFaddend visible ($\mathbf{v}$), hidden ($\mathbf{h}$) and reconstruction ($\mathbf{v'}$) layers.
Both visible and reconstruction \DIFdelbegin \DIFdel{layer }\DIFdelend \DIFaddbegin \DIFadd{layers }\DIFaddend have $N$ dimensions, and the hidden layer has $M$.
...
Therefore the loss function generated by \DIFaddbegin \DIFadd{a }\DIFaddend single data sample $\mathbf{v}$, described in Equation~\ref{equ:error_non} for supervised learning, is as follow:
\begin{equation}
...
The weight tuning needs only to be applied in the reconstruction layer, since study~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{xu1993least}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{xu1993least}
}%DIFAUXCMD
}\DIFaddend has proved that updating the transposed weights $\mathbf{w}^T$ at the hidden layer does not improve the learning and the error gradients are usually small.
...
%DIF < \subsection{Contrastive Divergence\cite{hinton2002training,woodfordnotes}}
%DIF > \subsection{Contrastive Divergence\citep{hinton2002training,woodfordnotes}}
...
%\right.
%\end{equation}
%DIF < $\hat{l} (\Theta \mid D)$ here is the function of two dimensional parameters $\mu$ and $\theta$, and searching the highest point in the parameter space ``is equivalent to being in the field on a clear, sunny day,''~\cite{woodfordnotes} seeing the point straight away.
%DIF > $\hat{l} (\Theta \mid D)$ here is the function of two dimensional parameters $\mu$ and $\theta$, and searching the highest point in the parameter space ``is equivalent to being in the field on a clear, sunny day,''~\citep{woodfordnotes} seeing the point straight away.
%
...
%
%DIF < The gradient descent method is equivalent to ``being in the field at night with a torch.''~\cite{woodfordnotes}.
%DIF > The gradient descent method is equivalent to ``being in the field at night with a torch.''~\citep{woodfordnotes}.
...
%DIF < The computation of Equation~(\ref{equ:part}) is to ``feel the gradient of the field under our feet''.~\cite{woodfordnotes}.
%DIF > The computation of Equation~(\ref{equ:part}) is to ``feel the gradient of the field under our feet''.~\citep{woodfordnotes}.
...
%DIF < However, if we just take a small number of data, even $ k = 1 $ per iteration~\cite{hinton2002training}, the KL divergence can be seen as a ``k-step contrastive convergence ($ CD_{k}) $'':
%DIF > However, if we just take a small number of data, even $ k = 1 $ per iteration~\citep{hinton2002training}, the KL divergence can be seen as a ``k-step contrastive convergence ($ CD_{k}) $'':
%\begin{equation}
%\label{equ:cdk}
...
%DIF < Although there is practical explanation on using $CD_1$ for parameter space searching, arguments exist over whether the search converge to the same maxima/minima as $CD_\infty$~\cite{wu2015bias}.
%DIF > Although there is practical explanation on using $CD_1$ for parameter space searching, arguments exist over whether the search converge to the same maxima/minima as $CD_\infty$~\citep{wu2015bias}.
...
%%	\end{equation}
%DIF < %\subsection{RBM\cite{zhang2013rbm}}
%DIF > %\subsection{RBM\citep{zhang2013rbm}}
\section{Restricted Boltzmann Machine}
\label{sec:rbm}
\DIFdelbegin \DIFdel{RBMs }\DIFdelend \DIFaddbegin \DIFadd{The Restricted Boltzmann Machine~}{\DIFadd{RBMs}} \DIFaddend share similar ideas of unsupervised learning with AEs, but aim to present a data distribution of some training dataset, rather than reconstructing each element of the dataset.
Thus, \DIFaddbegin \DIFadd{the }\DIFaddend RBM is an energy-based model and uses a stochastic approach.
Besides that, the structure of \DIFdelbegin \DIFdel{RBMs}\DIFdelend \DIFaddbegin \DIFadd{an RBM}\DIFaddend , see Figure~\ref{fig:RBM}, is also similar to \DIFdelbegin \DIFdel{AEs' }\DIFdelend \DIFaddbegin \DIFadd{an AE }\DIFaddend where the visible units $\mathbf{v}$ bidirectionally connect to the hidden units $\mathbf{h}$ with strength $\mathbf{w}$.
...
Despite the various statistical models involved in RBMs and their training method, the actual training algorithm is rather simple thanks to the use of Contrastive Divergence~(CD)~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
We can only illustrate some of the statistical models and sampling methods involved in RBM training, a detailed description can be found \DIFdelbegin \DIFdel{in~\mbox{%DIFAUXCMD
\cite{fischer2012introduction}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{elsewhere~\mbox{%DIFAUXCMD
\citep{fischer2012introduction}
}%DIFAUXCMD
}\DIFaddend . 
...
%\end{aligned}
\end{equation} 
The energy function~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{hopfield1982neural}
}%DIFAUXCMD
of a }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{hopfield1982neural}
}%DIFAUXCMD
of an }\DIFaddend RBM is defined as follows:
\begin{equation}
...
\end{equation}
where the visible layer has $ N $ units, \DIFaddbegin \DIFadd{the }\DIFaddend hidden layer has $ M $, and $ \Theta$ are the model parameters used in RBMs $ \Theta =\{\mathbf{a}, \mathbf{b}, \mathbf{w}\} $ including biases on \DIFaddbegin \DIFadd{the }\DIFaddend visible layer, $\mathbf{a}$, \DIFdelbegin \DIFdel{hidden layer, }\DIFdelend \DIFaddbegin \DIFadd{the hidden layer }\DIFaddend $\mathbf{b}$ \DIFdelbegin \DIFdel{, }\DIFdelend and weights between the layers $\mathbf{w}$.
...
Thus the joint probability of the visible vector (input) $\mathbf{v}$ and the output of \DIFaddbegin \DIFadd{the }\DIFaddend hidden units $\mathbf{h}$ is defined as follows: 
\begin{equation}
\begin{aligned}
...
Given a set of \DIFdelbegin \DIFdel{outcomes }\DIFdelend \DIFaddbegin \DIFadd{outcome }\DIFaddend data $ \mathbf{D}=\{\mathbf{v}^1, \mathbf{v}^2, ..., \mathbf{v}^K\} $, the likelihood function of model parameters $\Theta$ defines the probability of the outcomes $ \mathbf{D}$ given $\Theta$:
\begin{equation}
...
\end{equation}
Thus the objective is to maximise the likelihood $L (\Theta \mid \mathbf{D})$\DIFaddbegin \DIFadd{; }\DIFaddend that is to say, the model parameters $ \Theta $ \DIFdelbegin \DIFdel{best defines }\DIFdelend \DIFaddbegin \DIFadd{that best define }\DIFaddend the given data $\mathbf{D}$.
...
Applying the \DIFdelbegin \DIFdel{model function of RBMs }\DIFdelend \DIFaddbegin \DIFadd{RBM model function }\DIFaddend in Equation~\ref{equ:rbm_prob} to the loss derivative (Equation~\ref{equ:part}), Appendix~\ref{app:RBM} derives the loss function of RBMs in detail:
\begin{equation}
\label{equ:RBM}
\begin{aligned}
\dfrac{\partial \hat{l} (\Theta \mid \mathbf{D})}{\partial \theta} 
...
\end{aligned}
\end{equation}
Then we plug the \DIFdelbegin \DIFdel{energy function of RBMs }\DIFdelend \DIFaddbegin \DIFadd{RBM energy function }\DIFaddend (Equation~\ref{equ:rbm_energy}) into the equation above, the loss derivative with respect to the weight is:
\begin{equation}
\label{equ:RBM_2}
%\begin{aligned}
...
\label{sec:cd}
Generating data samples for the negative term of Equation~\ref{equ:RBM_2} requires Gibbs sampling on a Markov chain with \DIFdelbegin \DIFdel{infinite }\DIFdelend \DIFaddbegin \DIFadd{an infinite number of }\DIFaddend steps to convergence.
Gibbs sampling approximates the joint probability $p( \mathbf{v}, \mathbf{h} \mid  \Theta)$ with \DIFaddbegin \DIFadd{the }\DIFaddend conditionally probability defined in RBMs:
\begin{equation}
\begin{aligned}
...
\label{equ:con_prob}
\end{equation} 
where $\sigma$ represents \DIFaddbegin \DIFadd{the }\DIFaddend Sigmoid activation function for Bernoulli-Bernoulli RBMs.
Figure~\ref{fig:gibbs} illustrates \DIFdelbegin \DIFdel{the }\DIFdelend Gibbs sampling on an RBM by $k$ steps where each pair \DIFdelbegin \DIFdel{of }\DIFdelend $(\mathbf{v}, \mathbf{h})$ composes a state of the Markov chain, and the joint probability $p( \mathbf{v}, \mathbf{h} \mid  \Theta)$ is approximated by conditional probabilities embedded in the weights (Equation~\ref{equ:con_prob}).

\begin{figure}[hbt]
	\centering
	\includegraphics[width=0.98\textwidth]{pics_sdlm/gibbs_sampling.pdf}
	\caption{Gibbs sampling on \DIFdelbeginFL \DIFdelFL{a }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{an }\DIFaddendFL RBM.}
	\label{fig:gibbs}
\end{figure}

If $ k \to \infty $, the Markov chain will converge to an equilibrium which represents the distribution of the model-generated data $\{\mathbf{C_v}, \mathbf{C_h}\}$ described by the RBM, and Equation~\ref{equ:RBM} will represent the derivative of \DIFaddbegin \DIFadd{the }\DIFaddend Kullback-Leibler~(KL) divergence with respect to parameter $\theta$.
...
However, if we \DIFdelbegin \DIFdel{just take }\DIFdelend \DIFaddbegin \DIFadd{take just }\DIFaddend a few steps, the KL divergence can be seen as a k-step contrastive convergence ($ CD_{k}) $.
Even $ CD_1 $ \DIFdelbegin \DIFdel{has performed }\DIFdelend \DIFaddbegin \DIFadd{performs }\DIFaddend surprisingly well in RBM training~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{hinton2002training}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{hinton2002training}
...
\section{Summary}
This chapter briefly introduced the most popular \DIFdelbegin \DIFdel{deep learning }\DIFdelend \DIFaddbegin \DIFadd{Deep Learning }\DIFaddend techniques and models for potential use in spiking neural networks.
We illustrated the structure and training procedure for three \DIFdelbegin \DIFdel{deep learning }\DIFdelend \DIFaddbegin \DIFadd{Deep Learning }\DIFaddend models: Convolutional networks, Autoencoders and Restricted Boltzmann Machines, which will be applied in spiking deep networks in the following chapters.