...
\label{cha:dnn}
Deep Learning research in the field of \DIFdelbegin \DIFdel{artificial neural networks}\DIFdelend \DIFaddbegin \DIFadd{Artificial Neural Networks}\DIFaddend ~(ANNs) has dominated state-of-the-art solutions on cognitive tasks, e.g. the performance exceeding human-level  on image classification tasks~\citep{he2015delving} and playing GO~\citep{silver2016mastering}\DIFdelbegin \DIFdel{)}\DIFdelend .
Merging Deep Learning techniques into Spiking Neural Networks~(SNNs), introduced in the previous chapter, may provide an answer to the problem of how to operate SNNs to perform \DIFdelbegin \DIFdel{as well as }\DIFdelend \DIFaddbegin \DIFadd{equivalently to }\DIFaddend ANNs in cognitive tasks. 
In this chapter, we will give an overview of popular architectures and models of Deep Learning in Section~\ref{sec:dl_history}, and the rest of this chapter will illustrate the mechanisms of \DIFdelbegin \DIFdel{convolutional networks }\DIFdelend \DIFaddbegin \DIFadd{Convolutional Networks }\DIFaddend (ConvNets), \DIFdelbegin \DIFdel{autoencoders }\DIFdelend \DIFaddbegin \DIFadd{Autoencoders }\DIFaddend (AEs), and \DIFdelbegin \DIFdel{restricted Boltzmann machines}\DIFdelend \DIFaddbegin \DIFadd{Restricted Boltzmann Machines}\DIFaddend ~(RBMs) in detail which will be used in following chapters.
...
However, Deep Learning is not \DIFdelbegin \DIFdel{a }\DIFdelend new `magic', but rather has a history over a few decades and its sudden success is the result of the availability of an increasing amount of training data, the growing size of network models and greater computational power.
Therefore, all the classical Deep Learning architectures date back to the last century\DIFaddbegin \DIFadd{, even }\DIFaddend before the `Deep Learning' name was coined.
\DIFdelbegin \DIFdel{However, at that time, deep networks could hardly be trained, mainly because of the limits on computational power.
}\DIFdelend %DIF > However, at that time, deep networks could hardly be trained, mainly because of the limits on computational power.
...
Thus \DIFdelbegin \DIFdel{recurrent neural networks}\DIFdelend \DIFaddbegin \DIFadd{Recurrent Neural Networks}\DIFaddend ~(RNNs), containing feed-back connections, are ideal solutions for dealing with sequential information since their current output is always dependent on the previous `memory'.
As training mechanisms have become more mature, for example using the \DIFdelbegin \DIFdel{long short-term memory}\DIFdelend \DIFaddbegin \DIFadd{Long Short-Term Memory}\DIFaddend ~(LSTM)~\citep{hochreiter1997long}, RNNs have shown great success in many natural language processing tasks: language modelling~\citep{mikolov2010recurrent}, machine translation~\citep{sutskever2014sequence}, speech recognition~\citep{graves2014towards} and image caption generation~\citep{karpathy2015deep}.
...
Reinforcement \DIFdelbegin \DIFdel{learning }\DIFdelend \DIFaddbegin \DIFadd{Learning }\DIFaddend (RL) is inspired from animal behaviour when agents learn to make sequential optimised decisions to control an environment.
...
This section aims to give a detailed introduction to ConvNet architectures and their training with \DIFdelbegin \DIFdel{back propagation}\DIFdelend \DIFaddbegin \DIFadd{Backpropagation}\DIFaddend .
...
A typical ConvNet\DIFdelbegin \DIFdel{(LeNet) }\DIFdelend \DIFaddbegin \DIFadd{, LeNet~\ref{Fig:ConvNet}, shown in Figure, }\DIFaddend consists of four types of neural layers: an input layer, convolutional~(Conv) layers of simple cells, pooling~(Pool) layers of complex cells and fully-connected~(FC) layers\DIFdelbegin \DIFdel{, as shown in Figure~\ref{Fig:ConvNet}}\DIFdelend .
...
Suppose an input image and a kernel are both squares\DIFaddbegin \DIFadd{, }\DIFaddend with side lengths of $l_{in}$ and $l_k$, then the side length of the convolved feature map is $(l_{in} - l_k + 1)/stride$.
...
\DIFdelbegin \DIFdel{However, a multilayer perceptron (MLP) network possesses only FC layers, where each neural layer fully connects to the next one.
}\DIFdelend In a ConvNet an FC layer is usually located at the last layer (right in Figure~\ref{Fig:ConvNet}), and connects the last Pool layer to the output neurons with all-to-all connections.
%DIF <  to predict the classification of the input image.
%DIF > However, a Multilayer Perceptron (MLP) network possesses only FC layers, where each neural layer fully connects to the next one.
...
There can be more than one FC layer at the end of a ConvNet\DIFdelbegin \DIFdel{.
In }\DIFdelend \DIFaddbegin \DIFadd{, thereby building a Multilayer Perceptron (MLP) network.
However, in }\DIFaddend this thesis, we will use only a single FC layer.
...
The objective function (or loss function) estimates an error by comparing the output vector of \DIFdelbegin \DIFdel{a network}\DIFdelend \DIFaddbegin \DIFadd{$M$ dimension}\DIFaddend , $\mathbf{y}=(y_1,y_2,...,y_M)$, to the desired label vector, $\mathbf{t}=(t_1,t_2,...,t_M)$, and the error is to be minimised during training.
Given a set of training data \DIFaddbegin \DIFadd{of $K$ elements $\mathbf{X}=\{\mathbf{x}^1, \mathbf{x}^2, ..., \mathbf{x}^K\}$ }\DIFaddend with desired labels $\mathbf{T}=\{\mathbf{t}^1, \mathbf{t}^2, ..., \mathbf{t}^K\}$ and the network output $\mathbf{Y}=\{\mathbf{y}^1, \mathbf{y}^2, ..., \mathbf{y}^K\}$ the Mean Squared Error~(MSE) can be seen as the objective function:  
\begin{equation}
L={MSE}(\mathbf{T}, \mathbf{Y}) =\frac{1}{2K}\sum_{k=1}^K \sum_{m=1}^M (y^{k}_{m}-t^{k}_{m})^{2}~~.
\label{equ:loss_all}
\end{equation}

\DIFdelbegin \DIFdel{Backpropagation~(BP ) }\DIFdelend \DIFaddbegin \DIFadd{BP }\DIFaddend propagates the gradient of the loss with respect to the weights backwards to each connection in the network.
The computation requires a closer look into the structure of a neuron, see Figure~\ref{Fig:neuron_net}\DIFaddbegin \DIFadd{, }\DIFaddend where $N$ neurons connect to the $j^{th}$ neuron of the next layer, and the neuron converts the weighted summation of the input, $net_j$, to the output $y_j$ according to its activation function.
\begin{figure}[bt]
	\centering
	\includegraphics[width=0.7\textwidth]{pics_iconip/neuron.pdf}
	\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[An artificial neuron.]{\DIFaddendFL $N$ neurons connect to the $j_{th}$ neuron of the next layer. As illustrated in Figure~\ref{Fig:compare_as}(a), we use a simplified artificial neuron without bias.}
	\label{Fig:neuron_net}
\end{figure}
\DIFaddbegin 

\DIFaddend Thus the gradient of the loss $L$ with respect to a weight $w_{ij}$ is as follows:
\begin{equation}
\begin{aligned}
...
Moreover, applying \DIFdelbegin \DIFdel{stochastic gradient descent }\DIFdelend \DIFaddbegin \DIFadd{Stochastic Gradient Descent }\DIFaddend (SGD), the gradient over the full training set can be approximated using only a few, even a single, training data \DIFaddbegin \DIFadd{per weight update}\DIFaddend .
...
\begin{figure}[tbp!]
	...
	\end{subfigure}
	\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Activation functions]{\DIFaddendFL Activation functions: (a) sigmoid and ReLU, and (b) their derivatives.}
	\label{fig:af}
\end{figure}
...
For the sigmoid function example, shown in Figure~\ref{fig:af}(a), the derivative \DIFdelbegin \DIFdel{is the maximum (}\DIFdelend \DIFaddbegin \DIFadd{only peaks to }\DIFaddend around~0.25 \DIFdelbegin \DIFdel{) only }\DIFdelend when the input is close to 0, and decays to its minimum \DIFdelbegin \DIFdel{0 }\DIFdelend as the input either increases or decreases, see Figure~\ref{fig:af}(b).
...
\section{Restricted Boltzmann \DIFdelbegin \DIFdel{Machine}\DIFdelend \DIFaddbegin \DIFadd{Machines}\DIFaddend }
\label{sec:rbm}
...
\end{equation}
where the visible layer has $ N $ units, the hidden layer has $ M $, and $ \Theta$ are the model parameters used in RBMs $ \Theta =\{\mathbf{a}, \mathbf{b}, \mathbf{w}\} $ including biases \DIFaddbegin \DIFadd{$\mathbf{a}$ and $\mathbf{b}$ }\DIFaddend on the visible \DIFdelbegin \DIFdel{layer, $\mathbf{a}$, }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend the hidden layer \DIFdelbegin \DIFdel{$\mathbf{b}$ and weights }\DIFdelend \DIFaddbegin \DIFadd{respectively, and the weights $\mathbf{w}$ }\DIFaddend between the layers\DIFdelbegin \DIFdel{$\mathbf{w}$}\DIFdelend .
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Applying the RBM model function (Equation~\ref{equ:rbm_prob}) to the Equation~\ref{equ:part}, we derive the simplified representation of the derivative of the log likelihood:
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{where the first term of the right-hand side can be gathered from the given data, $ \{\mathbf{D}, \mathbf{D}_h\}$, and the second term will be approximated by sampling $ \{\mathbf{C_v}, \mathbf{C_h}\} $.
...
= \left \langle v_i h_j \right \rangle\DIFdelbegin \DIFdel{_{\{\mathbf{D}, \mathbf{D}_h\} \sim p( \mathbf{h} \mid \mathbf{v}, \Theta)}
}\DIFdelend \DIFaddbegin \DIFadd{_{\{\mathbf{D}, \mathbf{D}_h \sim p( \mathbf{h} \mid \mathbf{v}, \Theta) \}}
}\DIFaddend - \left \langle  v_i h_j \right \rangle_{\{\mathbf{C_v}, \mathbf{C_h}\} \sim p( \mathbf{v}, \mathbf{h} \mid  \Theta)}~~. %  \\
...
\subsection{\DIFdelbegin \DIFdel{Weight update with }\DIFdelend Contrastive Divergence}
\label{sec:cd}
...
\begin{figure}[hbt]
	...
	\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Gibbs sampling on a RBM.]{\DIFaddendFL Gibbs sampling on \DIFdelbeginFL \DIFdelFL{an }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{a }\DIFaddendFL RBM. \DIFaddbeginFL \DIFaddFL{The method approximates the joint probability $p( \mathbf{v}, \mathbf{h} \mid  \Theta)$ by sampling from conditional probabilities $p( \mathbf{h} \mid  \mathbf{v}, \Theta)$ and $p( \mathbf{v} \mid  \mathbf{h}, \Theta)$. Each sampled data is a pair of $ \mathbf{v}^k$ and $\mathbf{h}^k$.}\DIFaddendFL }
	\label{fig:gibbs}
\end{figure}
...
However, if we take just a few steps, the KL divergence can be seen as a k-step contrastive convergence (\DIFdelbegin \DIFdel{$ {CD}_{k}) $.
Even $ {CD}_1 $ }\DIFdelend \DIFaddbegin \DIFadd{$ \textit{\textrm{CD}}_{k}) $.
Even $ \textit{\textrm{CD}}_1 $ }\DIFaddend performs surprisingly well in RBM training~\citep{hinton2002training}\DIFdelbegin \DIFdel{.
Hence }\DIFdelend \DIFaddbegin \DIFadd{, which means for every weight update there is only one sample pair $(v^1, h^1)$ generated from Gibbs sampling.
Hence, replacing the mean expectation with 1-step Gibbs sampled data in Equation~\ref{equ:RBM_2}, }\DIFaddend the RBM training using SGD \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{can be written }\DIFaddend as follows:
\begin{equation}
\Delta w_{ij} = \eta (v^0_i h^0_j-v^1_i h^1_j)~~\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend \label{equ:rbm_train}
\end{equation}
\DIFaddbegin \DIFadd{and $v^0_i$ is a given training data, $h^0_j$ is produced with the probability $p( \mathbf{h} \mid \mathbf{v^0_i}, \mathbf{w})$, and the model data pair $(v^1_i, h^1_j)$ is generated by 1-step Gibbs sampling.
}\DIFaddend 
...
\section{Summary}
This chapter briefly introduced the most popular Deep Learning techniques and models for potential use in \DIFdelbegin \DIFdel{spiking neural networks}\DIFdelend \DIFaddbegin \DIFadd{SNNs}\DIFaddend .
We illustrated the structure and training procedure for three Deep Learning models: \DIFdelbegin \DIFdel{Convolutional networks, Autoencoders and Restricted Boltzmann Machines}\DIFdelend \DIFaddbegin \DIFadd{ConvNets, AEs and RBMs}\DIFaddend , which will be applied in spiking deep networks in the following chapters.